...
template<typename T> class Factory final {
	...
	Mat<T> auxiliary_stiffness; // for constraints using multiplier method
...
public:
	...
	const bool lagrange_multiplier = false;
	...
	const uvec& get_reference_dof() const;
	...
	const Mat<T>& get_auxiliary_stiffness() const;
	...
	template<typename T1> friend Mat<T1>& get_auxiliary_stiffness(const shared_ptr<Factory<T1>>&);
	...
	template<typename T1> friend MetaMat<T1>& get_geometry(const shared_ptr<Factory<T1>>&);
...
template<typename T> void Factory<T>::set_nlgeom(const bool B) { nlgeom = B; }
...
template<typename T> void Factory<T>::initialize_geometry() {
	...
	case StorageScheme::FULL:
		...
		break;
	case StorageScheme::BAND:
		global_geometry = make_shared<BandMat<T>>(n_size, n_lobw, n_upbw);
		break;
	case StorageScheme::BANDSYMM:
	...
	case StorageScheme::SYMMPACK:
		...
		break;
	case StorageScheme::SPARSE:
		global_geometry = make_shared<SparseMat<T>>(n_size, n_size);
		break;
	case StorageScheme::SPARSESYMM:
		global_geometry = make_shared<SparseMat<T>>(n_size, n_size);
		break;
...
template<typename T> const Mat<T>& Factory<T>::get_auxiliary_stiffness() const { return auxiliary_stiffness; }
...
template<typename T> void Factory<T>::assemble_mass(const Mat<T>& EM, const uvec& EI) {
	...
	if(storage_type == StorageScheme::SPARSE || storage_type == StorageScheme::SPARSESYMM) for(unsigned I = 0; I < EI.n_elem; ++I) for(unsigned J = 0; J < EI.n_elem; ++J) global_mass->at(EI(J), EI(I)) = EM(J, I);
...
template<typename T> void Factory<T>::assemble_damping(const Mat<T>& EC, const uvec& EI) {
	...
	if(storage_type == StorageScheme::SPARSE || storage_type == StorageScheme::SPARSESYMM) for(unsigned I = 0; I < EI.n_elem; ++I) for(unsigned J = 0; J < EI.n_elem; ++J) global_damping->at(EI(J), EI(I)) = EC(J, I);
...
template<typename T> void Factory<T>::assemble_stiffness(const Mat<T>& EK, const uvec& EI) {
	...
	if(storage_type == StorageScheme::SPARSE || storage_type == StorageScheme::SPARSESYMM) for(unsigned I = 0; I < EI.n_elem; ++I) for(unsigned J = 0; J < EI.n_elem; ++J) global_stiffness->at(EI(J), EI(I)) = EK(J, I);
...
template<typename T> void Factory<T>::assemble_geometry(const Mat<T>& EG, const uvec& EI) {
	...
	if(storage_type == StorageScheme::SPARSE || storage_type == StorageScheme::SPARSESYMM) for(unsigned I = 0; I < EI.n_elem; ++I) for(unsigned J = 0; J < EI.n_elem; ++J) global_geometry->at(EI(J), EI(I)) = EG(J, I);
...
template<typename T> Mat<T>& get_auxiliary_stiffness(const shared_ptr<Factory<T>>& W) { return W->auxiliary_stiffness; }
...
template<typename T> MetaMat<T>& get_geometry(const shared_ptr<Factory<T>>& W) { return *W->global_geometry; }
