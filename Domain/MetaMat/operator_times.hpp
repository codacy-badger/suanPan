...
template<typename T> bool is_dense(const MetaMat<T>& M) {
	if(typeid(M) == typeid(FullMat<T>)) return true;
	if(typeid(M) == typeid(BandMat<T>)) return true;
	if(typeid(M) == typeid(BandSymmMat<T>)) return true;
	if(typeid(M) == typeid(SymmPackMat<T>)) return true;
	return false;
}

template<typename T> bool is_sparse(const MetaMat<T>& M) {
	if(typeid(M) == typeid(SparseMat<T>)) return true;
	return false;
}

template<typename T> bool is_sparse(const shared_ptr<MetaMat<T>>& M) {
	if(typeid(*M) == typeid(SparseMat<T>)) return true;
	return false;
}
...
template<typename T> MetaMat<T> operator*(const T& value, const MetaMat<T>& M) {
	auto N = M;
	if(is_sparse(M)) N.triplet_mat *= value;
	else arrayops::inplace_mul(N.memptr(), value, N.n_elem);
	return N;
}

template<typename T> MetaMat<T> operator*(const T& value, const shared_ptr<MetaMat<T>>& M) {
	auto N = MetaMat<T>(*M);
	if(is_sparse(M)) N.triplet_mat *= value;
	else arrayops::inplace_mul(N.memptr(), value, N.n_elem);
	return N;
