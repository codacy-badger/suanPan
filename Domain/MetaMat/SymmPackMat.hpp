...
template<typename T> void SymmPackMat<T>::unify(const uword idx) {
	for(uword I = 0; I < idx; ++I) at(I, idx) = 0.;
	for(auto I = idx + 1; I < n_rows; ++I) at(idx, I) = 0.;
	at(idx, idx) = 1.;
}
...
template<typename T> T& SymmPackMat<T>::at(const uword in_row, const uword in_col) { return in_col < in_row ? bin : access::rw(memory[(in_col * in_col + in_col) / 2 + in_row]); }
