...
NodalLoad::NodalLoad(const unsigned T, const unsigned S, const double L, uvec&& N, const unsigned D, const unsigned AT)
	: Load(T, S, AT, std::move(N), uvec{D}, L) {}

NodalLoad::NodalLoad(const unsigned T, const unsigned S, const double L, uvec&& N, uvec&& D, const unsigned AT)
	: Load(T, S, AT, std::move(N), std::move(D), L) {}

int NodalLoad::process(const shared_ptr<DomainBase>& D) {
