...
int Load::initialize(const shared_ptr<DomainBase>& D) {
	if(magnitude != nullptr) { if(!magnitude->is_active()) magnitude = nullptr; } else if(amplitude_tag != 0 && D->find_amplitude(amplitude_tag)) magnitude = D->get_amplitude(amplitude_tag);
