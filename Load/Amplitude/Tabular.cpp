...
Tabular::Tabular(const unsigned T, vec&& TI, vec&& M, const unsigned ST)
	...
	, magnitude(std::forward<vec>(M)) { if(time.n_elem != magnitude.n_elem) throw logic_error("Tabular requires two vectors of the same size.\n"); }

Tabular::Tabular(const unsigned T, const char* P, const unsigned ST)
	: Amplitude(T, ST) {
	mat ext_data;
	struct stat buffer{};
	if(stat(P, &buffer) == 0 && ext_data.load(P, auto_detect)) {
