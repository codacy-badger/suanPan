...
void Concrete22::initialize(const shared_ptr<DomainBase>&) {
	...
	trial_history = current_history.zeros(3);
}
...
int Concrete22::update_trial_status(const vec& t_strain) {
	...
	if(max_t_strain < get_parameter(ParameterType::CRACKSTRAIN) && max_c_strain > get_parameter(ParameterType::PEAKSTRAIN)) principal_angle = transform::strain::angle(trial_strain);
