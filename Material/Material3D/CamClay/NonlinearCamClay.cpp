...
const unsigned NonlinearCamClay::max_iteration = 20;
...
void NonlinearCamClay::initialize(const shared_ptr<DomainBase>&) {
	inv_stiffness = tensor::isotropic_flexibility(elastic_modulus, poissons_ratio);
...
int NonlinearCamClay::update_trial_status(const vec& t_strain) {
	...
	auto& current_alpha = current_history(0);
	...
	const auto p = tensor::mean(trial_stress);
	...
	while(true) {
		if(max_iteration == ++counter) {
			suanpan_debug("NonlinearCamClay cannot converge within %u iterations.\n", max_iteration);
			return SUANPAN_FAIL;
		}
