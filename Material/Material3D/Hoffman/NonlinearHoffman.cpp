...
const double NonlinearHoffman::four_third = 4. / 3.;
const double NonlinearHoffman::root_two_third = sqrt(.5 * four_third);
const unsigned NonlinearHoffman::max_iteration = 20;
...
void NonlinearHoffman::initialize(const shared_ptr<DomainBase>&) {
	trial_stiffness = current_stiffness = initial_stiffness = tensor::orthotropic_stiffness(modulus, ratio);

	inv_stiffess = inv(initial_stiffness);
...
int NonlinearHoffman::update_trial_status(const vec& t_strain) {
	trial_stress = current_stress + (trial_stiffness = initial_stiffness) * ((trial_strain = t_strain) - current_strain);

	trial_history = current_history;
	auto& plastic_strain = trial_history(0);
	auto& current_plastic_strain = current_history(0);
