...
podarray<double> CDPT::compute_tension_backbone(const double kappa) const {
...
podarray<double> CDPT::compute_compression_backbone(const double kappa) const {
	podarray<double> out(6);
...
CDPT::CDPT(const unsigned T, const double E, const double V, const double ST, const double SC, const double GT, const double GC, const double AT, const double AC, const double DT, const double DC, const double AP, const double BC, const double S, const double R)
	...
	if(ratio_t >= DT) {
		suanpan_warning("CDPT model requires a minimum tension degradation of %.2f, now reset it.\n", ratio_t);
	...
	if(ratio_c >= DC) {
		suanpan_warning("CDPT model requires a minimum compression degradation of %.2f, now reset it.\n", ratio_c);
		access::rw(cb_c) = log(ratio_c >= .9 ? ratio_c : ratio_c + .05) / log(.5 + .5 / a_c);
	}
}

unique_ptr<Material> CDPT::get_copy() { return make_unique<CDPT>(*this); }
