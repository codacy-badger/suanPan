...
void Rebar3D::initialize(const shared_ptr<DomainBase>& D) {
	...
	initial_stiffness(0, 0) = rebar_x->get_initial_stiffness().at(0);
	initial_stiffness(1, 1) = rebar_y->get_initial_stiffness().at(0);
	initial_stiffness(2, 2) = rebar_z->get_initial_stiffness().at(0);
