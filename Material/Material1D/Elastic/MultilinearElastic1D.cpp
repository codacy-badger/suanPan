...
void MultilinearElastic1D::initialize(const shared_ptr<DomainBase>&) { trial_stiffness = current_stiffness = initial_stiffness = backbone(0, 1) / backbone(0, 0); }
...
void MultilinearElastic1D::print() {
	suanpan_info("A mulilinear elastic material model.\n");
