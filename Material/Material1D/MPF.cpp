...
int MPF::update_trial_status(const vec& t_strain) {
	...
	const auto normal_strain = (trial_strain(0) - reverse_strain) / gap_strain;
	const auto factor_a = 1. + pow(normal_strain, radius);
	const auto factor_b = (1. - hardening_ratio) * pow(factor_a, -1. / radius);

	const auto t_stress = (hardening_ratio + factor_b) * normal_strain * gap_stress + reverse_stress;
	const auto t_stiffness = gap_stress / gap_strain * (hardening_ratio + factor_b / factor_a);

	if(std::isfinite(t_stress) && std::isfinite(t_stiffness)) {
		trial_stress = t_stress;
		trial_stiffness = t_stiffness;
	} else suanpan_debug([]() { throw out_of_range("not finite stress or stiffness"); });
