...
const unsigned NonlinearMises1D::max_iteration = 20;

NonlinearMises1D::NonlinearMises1D(const unsigned T, const double E, const double R)
	: DataMises1D{fabs(E)}
	, Material1D(T, R) {}

void NonlinearMises1D::initialize(const shared_ptr<DomainBase>&) {
...
int NonlinearMises1D::update_trial_status(const vec& t_strain) {
	...
	if(yield_func >= 0.) {
		...
		while(++counter < max_iteration) {
			...
			suanpan_extra_debug("local iterative loop error: %.5E.\n", abs_error);
...
void NonlinearMises1D::print() {
	suanpan_info("1D hardening material using J2 plasticity and associated flow rule.\n");
