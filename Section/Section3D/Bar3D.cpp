...
void Bar3D::initialize(const shared_ptr<DomainBase>& D) {
	s_material = D->get_material(material_tag)->get_copy();

	linear_density = area * s_material->get_parameter(ParameterType::DENSITY);
...
void Bar3D::print() {
	...
	suanpan_info("current strain: %.3E\tcurrent stress: %.3E\n", s_material->get_current_strain().at(0), s_material->get_current_stress().at(0));
}
