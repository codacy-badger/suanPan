...
Fibre1D::Fibre1D(const unsigned T, const podarray<unsigned>& ST)
	: Section1D(T, 0, 0.)
	, fibre_tag(ST) {}
...
Fibre1D::Fibre1D(const Fibre1D& old_obj)
	...
	fibre.clear(), fibre.reserve(old_obj.fibre.size());
...
void Fibre1D::initialize(const shared_ptr<DomainBase>& D) {
	...
	fibre.clear(), fibre.reserve(fibre_tag.n_elem);
...
double Fibre1D::get_parameter(const ParameterType P) {
	switch(P) {
	case ParameterType::AREA:
		return area;
	case ParameterType::DENSITY:
		return get_parameter(ParameterType::LINEARDENSITY) / area;
	case ParameterType::LINEARDENSITY: {
		auto linear_density = 0.;
		for(const auto& I : fibre) linear_density += I->get_parameter(ParameterType::LINEARDENSITY);
