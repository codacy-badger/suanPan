...
Recorder::Recorder(const unsigned T, uvec&& B, const OutputType L, const unsigned I, const bool R, const bool H)
	...
	, record_time(R)
	, use_hdf5(H)
	, interval(I) { suanpan_debug("NodeRecorder %u ctor() called.\n", T); }
...
void Recorder::save() {
	...
	if(use_hdf5) {
		...
		if(file_name.width() > 40) file_name.str(origin_name);
    ...
    for(const auto& s_data_pool : data_pool) {
        ...
        for(size_t I = 0; I < time_pool.size(); ++I) {
            ...
            for(const auto& J : s_data_pool[I])
                for(unsigned K = 0; K < J.n_elem; ++K)
                    data_to_write(idx1++, I + 1) = J[K];
        }
