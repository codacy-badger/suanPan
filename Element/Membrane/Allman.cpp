...
const unsigned Allman::m_node = 3;
const unsigned Allman::m_dof = 3;
const unsigned Allman::m_size = m_dof * m_node;
...
Allman::Allman(const unsigned T, uvec&& NT, const unsigned MT, const double TH)
	: MaterialElement(T, m_node, m_dof, std::forward<uvec>(NT), uvec{MT}, false)
	, thickness(TH) {}

void Allman::initialize(const shared_ptr<DomainBase>& D) {
...
void Allman::GetDisplacement(vtkSmartPointer<vtkDoubleArray>& arrays) {
