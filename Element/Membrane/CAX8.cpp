...
const unsigned CAX8::m_node = 8;
const unsigned CAX8::m_dof = 2;
const unsigned CAX8::m_size = m_dof * m_node;
...
void CAX8::initialize(const shared_ptr<DomainBase>& D) {
	...
	for(unsigned I = 0; I < plan.n_rows; ++I) {
		...
		for(unsigned J = 0, K = 0; J < m_node; ++J, K += m_dof) {
			c_int_pt.strain_mat(0, K) = c_int_pt.strain_mat(3, K + 1) = pn_pxy(0, J);
			c_int_pt.strain_mat(3, K) = c_int_pt.strain_mat(1, K + 1) = pn_pxy(1, J);
...
void CAX8::GetDisplacement(vtkSmartPointer<vtkDoubleArray>& arrays) {
