...
void QE2::initialize(const shared_ptr<DomainBase>& D) {
	...
	for(unsigned I = 0; I < plan.n_rows; ++I) {
		...
		for(unsigned K = 0; K < m_node; ++K) {
			c_pt.B(0, m_dof * K) = c_pt.B(2, m_dof * K + 1) = pn_pxy(0, K);
			c_pt.B(2, m_dof * K) = c_pt.B(1, m_dof * K + 1) = pn_pxy(1, K);
		}
	...
	const auto tmp_density = mat_proto->get_parameter();
	if(tmp_density != 0.) {
...
void QE2::GetDisplacement(vtkSmartPointer<vtkDoubleArray>& arrays) {
