...
RebarLayer::RebarLayer(const unsigned T, uvec&& N, vec&& C, const double TH, const char DR)
	: MaterialElement(T, m_node, m_dof, std::forward<uvec>(N), conv_to<uvec>::from(C(span(6, 8))), false)
	, thickness(TH)
