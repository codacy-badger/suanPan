...
void RCP4::initialize(const shared_ptr<DomainBase>& D) {
	...
	if(reduced_scheme) {
		...
		for(unsigned I = 0; I < m_node; ++I) for(unsigned J = 0; J < m_node; ++J) hourglassing(m_dof * I + 1, m_dof * J + 1) = hourglassing(m_dof * I, m_dof * J) = t_hourglassing(I, J);
	}
	...
	for(unsigned I = 0; I < plan.n_rows; ++I) {
		...
		for(unsigned J = 0; J < m_node; ++J) {
			...
			c_pt.B(0, K) = c_pt.B(2, K + 1) = c_pt.pn_pxy(0, J);
			c_pt.B(2, K) = c_pt.B(1, K + 1) = c_pt.pn_pxy(1, J);
		}
		initial_stiffness += c_pt.jacob_det * c_pt.weight * thickness * c_pt.B.t() * ini_stiffness * c_pt.B;
	}
