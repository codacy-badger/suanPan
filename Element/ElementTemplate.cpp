 ...
 * Domain object.
 *
 * Please note that **we do not have to check the existance of any objects**
 * which are used in the element. The validity of the connected node objects and
 * the material models is checked before calling initialize(). The excution of
 * this initialize() method means this is a valid Element object.
 *
...
void ElementTemplate::initialize(const shared_ptr<DomainBase>& D) {
	...
	for(unsigned i = 0; i < m_node; ++i) {
		...
		for(unsigned j = 0; j < 2; ++j) ele_coor(i, j + 1) = tmp_coor(j);
	}
	...
	//! A standerd way to construct the strain mat is to derive from the partial
	//! direvative of the shape function N. For CP3, as it is a constant
	//! stress/strain element, the direvatives are constants which can be directly
 ...
 * pointers and pass trial strain to the material model. Then get updated
 * stiffness and stress back to form element stiffness and resistance.
 *
 * The pointers of related node objects are stored in a base member `node_ptr`,
 * which is a `std::vector` of `weak_ptr`. (Why `weak_ptr`? To avoid potential
 * wrong deallocation.) To access any methods in Node class, we use
 * ```cpp
 *     auto& my_ptr = node_ptr[i];
 *     my_ptr.lock()->call_any_valid_method();
 * ```
 * Please note that the `weak_ptr` has to be locked to generate a valid
 * shared_ptr before calling any methods.
 *
...
int ElementTemplate::update_status() {
	...
	trial_stiffness = t_factor * m_material->get_trial_stiffness() * strain_mat;
	trial_resistance = t_factor * m_material->get_trial_stress();
