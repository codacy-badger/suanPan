...
const unsigned DKT4::p_node = 4;
const unsigned DKT4::p_dof = 3;
const unsigned DKT4::p_size = p_dof * p_node;
...
void DKT4::initialize(const shared_ptr<DomainBase>& D) {
	...
	for(unsigned I = 0; I < plan.n_rows; ++I) {
		...
		for(unsigned J = 0; J < num_section_ip; ++J) {
			...
			c_ip.emplace_back(t_eccentricity, t_weight * sec_plan(J, 1), material_proto->get_copy());
...
void DKT4::GetDisplacement(vtkSmartPointer<vtkDoubleArray>& arrays) {
