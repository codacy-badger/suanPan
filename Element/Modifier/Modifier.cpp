...
void Modifier::initialize(const shared_ptr<DomainBase>& D) {
	...
	if(element_tag.empty()) for(const auto& I : D->get_element_pool()) element_pool.emplace_back(I);
	else {
		element_pool.reserve(element_tag.size());
		for(const auto I : element_tag) {
			const auto t_tag = unsigned(I);
			if(D->find_element(t_tag) && D->get_element(t_tag)->is_active()) element_pool.emplace_back(D->get_element(t_tag));
		}
	}
}
