...
#include <Material/Material1D/Maxwell.h>

unsigned Damper02::d_node = 2;
unsigned Damper02::d_dof = 2;
unsigned Damper02::d_size = d_dof * d_node;
uvec Damper02::IS{0, 1};
uvec Damper02::JS{2, 3};

Damper02::Damper02(const unsigned T, uvec&& NT, const unsigned DT, const unsigned ST, const bool UM, const unsigned PC)
	: Element(T, d_node, d_dof, std::forward<uvec>(NT))
	, device(make_unique<Maxwell>(0, DT, ST, UM, PC)) {}
...
int Damper02::update_status() {
	// use current state for quadratic convergence
	// const auto t_disp = get_current_displacement();
	// const auto t_vec = get_current_velocity();
