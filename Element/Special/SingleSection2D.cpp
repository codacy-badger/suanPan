...
const unsigned SingleSection2D::s_node = 1;
const unsigned SingleSection2D::s_dof = 2;
...
void SingleSection2D::initialize(const shared_ptr<DomainBase>& D) {
	const auto s_tag = unsigned(section_tag(0));

	if(!D->find_section(s_tag) || D->get_section(s_tag)->get_section_type() != SectionType::D2) {
