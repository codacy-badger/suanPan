...
const unsigned T3D2S::t_node = 2;
const unsigned T3D2S::t_dof = 3;
const unsigned T3D2S::t_size = t_dof * t_node;
...
void T3D2S::initialize(const shared_ptr<DomainBase>& D) {
	...
	trial_mass = current_mass = initial_mass = t_trans->to_global_mass_mat(t_section->get_parameter());
}
...
int T3D2S::commit_status() { return t_section->commit_status(); }

int T3D2S::clear_status() { return t_section->clear_status(); }

int T3D2S::reset_status() { return t_section->reset_status(); }
