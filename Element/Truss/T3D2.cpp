...
void T3D2::GetDisplacement(vtkSmartPointer<vtkDoubleArray>& arrays) {
	mat t_disp(6, t_node, fill::zeros);
	t_disp.rows(0, 2) = reshape(get_current_displacement(), t_dof, t_node);
...
void T3D2::SetDeformation(vtkSmartPointer<vtkPoints>& nodes, const double amplifier) {
	...
	for(unsigned I = 0; I < t_node; ++I) nodes->SetPoint(node_encoding(I), ele_disp(I, 0), ele_disp(I, 1), ele_disp(I, 2));
}
