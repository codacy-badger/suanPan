...
const unsigned F21H::b_size = b_node * b_dof;
const unsigned F21H::max_iteration = 20;
const double F21H::tolerance = 1E-11;
...
int F21H::update_status() {
	...
	while(++counter < max_iteration) {
		...
		for(const auto& I : int_pt) {
			...
			// update status
			I.b_section->update_trial_status(I.b_section->get_trial_deformation() + incre_deformation);
		...
		// quit if converged
		const auto abs_error = norm(residual_deformation);
		suanpan_extra_debug("F21H lcoal iteration error: %.4E.\n", abs_error);
...
void F21H::print() {
	suanpan_info("A 2D forced based beam element with lumped plasticity%sReference: https://doi.org/10.1016/0045-7949(95)00103-N \n", nlgeom ? " and corotational formulation.\n" : ".\n");
