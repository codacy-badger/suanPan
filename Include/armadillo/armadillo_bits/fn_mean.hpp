...
template<typename T1> arma_warn_unused
inline
typename T1::elem_type mean(const Op<T1, op_mean>& in) {
	arma_extra_debug_sigprint();
...
template<typename T1> arma_warn_unused
arma_inline
const Op<Op<T1, op_mean>, op_mean> mean(const Op<T1, op_mean>& in, const uword dim) {
	arma_extra_debug_sigprint();
...
arma_inline
const typename arma_scalar_only<T>::result& mean(const T& x) { return x; }

template<typename T1> arma_warn_unused
arma_inline
const OpCube<T1, op_mean> mean
(
...
template<typename T1> arma_warn_unused
inline
typename T1::elem_type mean(const SpOp<T1, spop_mean>& in) {
	arma_extra_debug_sigprint();
	arma_extra_debug_print("mean(): two consecutive mean() calls detected");
