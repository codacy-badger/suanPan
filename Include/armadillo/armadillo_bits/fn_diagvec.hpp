...
arma_inline
const Op<T1, op_diagvec> diagvec(const Base<typename T1::elem_type, T1>& X, const sword diag_id = 0) {
...
template<typename T1> arma_warn_unused
arma_inline
const SpOp<T1, spop_diagvec> diagvec(const SpBase<typename T1::elem_type, T1>& X, const sword diag_id = 0) {
	arma_extra_debug_sigprint();
