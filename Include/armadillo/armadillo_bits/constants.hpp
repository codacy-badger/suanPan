...
namespace priv {
	...
	public:
		...
		template<typename eT> static
		typename arma_real_only<eT>::result nan(typename arma_real_only<eT>::result* junk = 0) {
			arma_ignore(junk);

			if(std::numeric_limits<eT>::has_quiet_NaN) { return std::numeric_limits<eT>::quiet_NaN(); } else { return eT(0); }
		}

		template<typename eT> static
		typename arma_cx_only<eT>::result nan(typename arma_cx_only<eT>::result* junk = 0) {
			arma_ignore(junk);
		...
		template<typename eT> static
		typename arma_integral_only<eT>::result nan(typename arma_integral_only<eT>::result* junk = 0) {
			arma_ignore(junk);
		...
		template<typename eT> static
		typename arma_real_only<eT>::result inf(typename arma_real_only<eT>::result* junk = 0) {
			arma_ignore(junk);

			if(std::numeric_limits<eT>::has_infinity) { return std::numeric_limits<eT>::infinity(); } else { return std::numeric_limits<eT>::max(); }
		}

		template<typename eT> static
		typename arma_cx_only<eT>::result inf(typename arma_cx_only<eT>::result* junk = 0) {
			arma_ignore(junk);
		...
		template<typename eT> static
		typename arma_integral_only<eT>::result inf(typename arma_integral_only<eT>::result* junk = 0) {
			arma_ignore(junk);
...
template<typename eT> class Datum {
public:
...
namespace priv {
	...
	template<typename eT> static
	arma_inline
	typename arma_real_only<eT>::result most_neg(typename arma_real_only<eT>::result* junk = 0) {
		arma_ignore(junk);
	...
	template<typename eT> static
	arma_inline
	typename arma_integral_only<eT>::result most_neg(typename arma_integral_only<eT>::result* junk = 0) {
		arma_ignore(junk);
	...
	template<typename eT> static
	arma_inline
	typename arma_real_only<eT>::result most_pos(typename arma_real_only<eT>::result* junk = 0) {
		arma_ignore(junk);
	...
	template<typename eT> static
	arma_inline
	typename arma_integral_only<eT>::result most_pos(typename arma_integral_only<eT>::result* junk = 0) {
		arma_ignore(junk);
