 ...
 */
template<typename eT> inline SpMat<eT>::SpMat()
	: n_rows(0)
	, n_cols(0)
	, n_elem(0)
	, n_nonzero(0)
 ...
 */
template<typename eT> inline SpMat<eT>::SpMat(const uword in_rows, const uword in_cols)
	: n_rows(0)
	, n_cols(0)
	, n_elem(0)
	, n_nonzero(0)
...
template<typename eT> inline SpMat<eT>::SpMat(const SizeMat& s)
	: n_rows(0)
	, n_cols(0)
	, n_elem(0)
	, n_nonzero(0)
...
template<typename eT> inline SpMat<eT>::SpMat(const arma_reserve_indicator&, const uword in_rows, const uword in_cols, const uword new_n_nonzero)
	: n_rows(0)
	, n_cols(0)
	, n_elem(0)
	, n_nonzero(0)
...
template<typename eT> template<typename eT2> inline SpMat<eT>::SpMat(const arma_layout_indicator&, const SpMat<eT2>& x)
	: n_rows(0)
	, n_cols(0)
	, n_elem(0)
	, n_nonzero(0)
 ...
 * Assemble from text.
 */
template<typename eT> inline SpMat<eT>::SpMat(const char* text)
	: n_rows(0)
	, n_cols(0)
	, n_elem(0)
	, n_nonzero(0)
...
SpMat<eT>& SpMat<eT>::operator=(const char* text) {
...
template<typename eT> inline SpMat<eT>::SpMat(const std::string& text)
	: n_rows(0)
	, n_cols(0)
	, n_elem(0)
	, n_nonzero(0)
...
template<typename eT> inline
SpMat<eT>& SpMat<eT>::operator=(const std::string& text) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline SpMat<eT>::SpMat(const SpMat<eT>& x)
	: n_rows(0)
	, n_cols(0)
	, n_elem(0)
	, n_nonzero(0)
...
template<typename eT> inline SpMat<eT>::SpMat(SpMat<eT>&& in_mat)
	: n_rows(0)
	, n_cols(0)
	, n_elem(0)
	, n_nonzero(0)
...
template<typename eT> inline
SpMat<eT>& SpMat<eT>::operator=(SpMat<eT>&& in_mat) {
	arma_extra_debug_sigprint(arma_str::format("this = %x   in_mat = %x") % this % &in_mat);
...
template<typename eT> inline SpMat<eT>::SpMat(const MapMat<eT>& x)
	: n_rows(0)
	, n_cols(0)
	, n_elem(0)
	, n_nonzero(0)
...
template<typename eT> inline
SpMat<eT>& SpMat<eT>::operator=(const MapMat<eT>& x) {
	arma_extra_debug_sigprint();
...
template<typename eT> template<typename T1, typename T2> inline SpMat<eT>::SpMat(const Base<uword, T1>& locations_expr, const Base<eT, T2>& vals_expr, const bool sort_locations)
	: n_rows(0)
	, n_cols(0)
	, n_elem(0)
	, n_nonzero(0)
	...
	for(uword i = 0; i < N_old; ++i) { if(vals[i] != eT(0)) { ++N_new; } }
	...
	} else { init_batch_std(locs, vals, sort_locations); }
}
...
template<typename eT> template<typename T1, typename T2> inline SpMat<eT>::SpMat(const Base<uword, T1>& locations_expr, const Base<eT, T2>& vals_expr, const uword in_n_rows, const uword in_n_cols, const bool sort_locations, const bool check_for_zeros)
	: n_rows(0)
	, n_cols(0)
	, n_elem(0)
	, n_nonzero(0)
...
template<typename eT> template<typename T1, typename T2> inline SpMat<eT>::SpMat(const bool add_values, const Base<uword, T1>& locations_expr, const Base<eT, T2>& vals_expr, const uword in_n_rows, const uword in_n_cols, const bool sort_locations, const bool check_for_zeros)
	: n_rows(0)
	, n_cols(0)
	, n_elem(0)
	, n_nonzero(0)
	...
	if(check_for_zeros) {
		...
		} else { add_values ? init_batch_add(locs, vals, sort_locations) : init_batch_std(locs, vals, sort_locations); }
	} else { add_values ? init_batch_add(locs, vals, sort_locations) : init_batch_std(locs, vals, sort_locations); }
}
...
template<typename eT> template<typename T1, typename T2, typename T3> inline SpMat<eT>::SpMat
(
	...
	const Mat<uword>& rowind_t = rowind_tmp.M;
	const Mat<uword>& colptr_t = colptr_tmp.M;
	const Mat<eT>& vals = vals_tmp.M;

	arma_debug_check((rowind_t.is_vec() == false), "SpMat::SpMat(): given 'rowind' object is not a vector");
	arma_debug_check((colptr_t.is_vec() == false), "SpMat::SpMat(): given 'colptr' object is not a vector");
	arma_debug_check((vals.is_vec() == false), "SpMat::SpMat(): given 'values' object is not a vector");

	// Resize to correct number of elements (this also sets n_nonzero)
	init_cold(in_n_rows, in_n_cols, vals.n_elem);

	arma_debug_check((rowind_t.n_elem != vals.n_elem), "SpMat::SpMat(): number of row indices is not equal to number of values");
	arma_debug_check((colptr_t.n_elem != (n_cols + 1)), "SpMat::SpMat(): number of column pointers is not equal to n_cols+1");

	// copy supplied values into sparse matrix -- not checked for consistency
	arrayops::copy(access::rwp(row_indices), rowind_t.memptr(), rowind_t.n_elem);
	arrayops::copy(access::rwp(col_ptrs), colptr_t.memptr(), colptr_t.n_elem);
...
template<typename eT> inline
SpMat<eT>& SpMat<eT>::operator=(const eT val) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
SpMat<eT>& SpMat<eT>::operator=(const SpMat<eT>& x) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
SpMat<eT>& SpMat<eT>::operator*=(const SpMat<eT>& y) {
	arma_extra_debug_sigprint();
...
SpMat<eT>& SpMat<eT>::operator/=(const SpMat<eT>& x) {
	...
	for(uword c = 0; c < n_cols; ++c) { for(uword r = 0; r < n_rows; ++r) { at(r, c) /= x.at(r, c); } }

	return *this;
}

// Construct a complex matrix out of two non-complex matrices
template<typename eT> template<typename T1, typename T2> inline SpMat<eT>::SpMat
(
	...
	for(uword c = 1; c <= n_cols; ++c) { access::rw(col_ptrs[c]) += col_ptrs[c - 1]; }

}

template<typename eT> template<typename T1> inline SpMat<eT>::SpMat(const Base<eT, T1>& x)
	: n_rows(0)
	, n_cols(0)
	, n_elem(0)
	, n_nonzero(0)
...
SpMat<eT>& SpMat<eT>::operator=(const Base<eT, T1>& expr) {
	arma_extra_debug_sigprint();
...
template<typename eT> template<typename T1> inline
SpMat<eT>& SpMat<eT>::operator+=(const Base<eT, T1>& x) {
	arma_extra_debug_sigprint();
...
template<typename eT> template<typename T1> inline
SpMat<eT>& SpMat<eT>::operator*=(const Base<eT, T1>& y) {
	arma_extra_debug_sigprint();
	...
	for(uword c = 1; c <= z.n_cols; ++c) { access::rw(z.col_ptrs[c]) += z.col_ptrs[c - 1]; }
...
SpMat<eT>& SpMat<eT>::operator%=(const Base<eT, T1>& x) {
	...
	while(it != it_end) {
		...
		if(((*it) * p.at(it.row(), it.col())) != eT(0)) { ++new_n_nonzero; }
		++it;
	}
	...
	// Fix column pointers.
	for(uword c = 1; c <= n_cols; ++c) { access::rw(tmp.col_ptrs[c]) += tmp.col_ptrs[c - 1]; }
...
template<typename eT> template<typename T1> inline SpMat<eT>::SpMat(const Op<T1, op_diagmat>& expr)
	: n_rows(0)
	, n_cols(0)
	, n_elem(0)
	, n_nonzero(0)
...
template<typename eT> template<typename T1> inline
SpMat<eT>& SpMat<eT>::operator=(const Op<T1, op_diagmat>& expr) {
	arma_extra_debug_sigprint();
	...
	if(P_is_vec) // generate diagonal sparse matrix from dense vector
	{
		...
		} else { if(P_n_rows == 1) { for(uword i = 0; i < N; ++i) { this_values[i] = P.at(0, i); } } else { for(uword i = 0; i < N; ++i) { this_values[i] = P.at(i, 0); } } }
 ...
 */
template<typename eT> inline SpMat<eT>::SpMat(const SpSubview<eT>& X)
	: n_rows(0)
	, n_cols(0)
	, n_elem(0)
	, n_nonzero(0)
...
template<typename eT> inline
SpMat<eT>& SpMat<eT>::operator=(const SpSubview<eT>& X) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
SpMat<eT>& SpMat<eT>::operator*=(const SpSubview<eT>& y) {
	arma_extra_debug_sigprint();
...
SpMat<eT>& SpMat<eT>::operator/=(const SpSubview<eT>& x) {
	...
	for(uword elem = 0; elem < n_elem; elem++) { at(elem) /= x(elem); }

	return *this;
}

template<typename eT> inline SpMat<eT>::SpMat(const spdiagview<eT>& X)
	: n_rows(0)
	, n_cols(0)
	, n_elem(0)
	, n_nonzero(0)
...
template<typename eT> inline
SpMat<eT>& SpMat<eT>::operator=(const spdiagview<eT>& X) {
	arma_extra_debug_sigprint();
...
template<typename eT> template<typename T1, typename spop_type> inline SpMat<eT>::SpMat(const SpOp<T1, spop_type>& X)
	: n_rows(0)
	, n_cols(0)
	, n_elem(0)
	, n_nonzero(0)
...
template<typename eT> template<typename T1, typename spop_type> inline
SpMat<eT>& SpMat<eT>::operator=(const SpOp<T1, spop_type>& X) {
	arma_extra_debug_sigprint();
...
template<typename eT> template<typename T1, typename T2, typename spglue_type> inline SpMat<eT>::SpMat(const SpGlue<T1, T2, spglue_type>& X)
	: n_rows(0)
	, n_cols(0)
	, n_elem(0)
	, n_nonzero(0)
...
template<typename eT> template<typename T1, typename spop_type> inline SpMat<eT>::SpMat(const mtSpOp<eT, T1, spop_type>& X)
	: n_rows(0)
	, n_cols(0)
	, n_elem(0)
	, n_nonzero(0)
...
template<typename eT> template<typename T1, typename spop_type> inline
SpMat<eT>& SpMat<eT>::operator=(const mtSpOp<eT, T1, spop_type>& X) {
	arma_extra_debug_sigprint();
...
template<typename eT> template<typename T1, typename T2, typename spglue_type> inline
SpMat<eT>& SpMat<eT>::operator=(const SpGlue<T1, T2, spglue_type>& X) {
	arma_extra_debug_sigprint();
...
SpSubview<eT> SpMat<eT>::row(const uword row_num) {
	arma_extra_debug_sigprint();
...
template<typename eT> arma_inline
const SpSubview<eT> SpMat<eT>::row(const uword row_num) const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
SpSubview<eT> SpMat<eT>::operator()(const uword row_num, const span& col_span) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
const SpSubview<eT> SpMat<eT>::operator()(const uword row_num, const span& col_span) const {
	arma_extra_debug_sigprint();
...
template<typename eT> arma_inline
SpSubview<eT> SpMat<eT>::col(const uword col_num) {
	arma_extra_debug_sigprint();
...
template<typename eT> arma_inline
const SpSubview<eT> SpMat<eT>::col(const uword col_num) const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
SpSubview<eT> SpMat<eT>::operator()(const span& row_span, const uword col_num) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
const SpSubview<eT> SpMat<eT>::operator()(const span& row_span, const uword col_num) const {
	arma_extra_debug_sigprint();
...
template<typename eT> arma_inline
SpSubview<eT> SpMat<eT>::rows(const uword in_row1, const uword in_row2) {
	arma_extra_debug_sigprint();
...
template<typename eT> arma_inline
const SpSubview<eT> SpMat<eT>::rows(const uword in_row1, const uword in_row2) const {
	arma_extra_debug_sigprint();
...
template<typename eT> arma_inline
SpSubview<eT> SpMat<eT>::cols(const uword in_col1, const uword in_col2) {
	arma_extra_debug_sigprint();
...
template<typename eT> arma_inline
const SpSubview<eT> SpMat<eT>::cols(const uword in_col1, const uword in_col2) const {
	arma_extra_debug_sigprint();
...
template<typename eT> arma_inline
SpSubview<eT> SpMat<eT>::submat(const uword in_row1, const uword in_col1, const uword in_row2, const uword in_col2) {
	arma_extra_debug_sigprint();
...
template<typename eT> arma_inline
const SpSubview<eT> SpMat<eT>::submat(const uword in_row1, const uword in_col1, const uword in_row2, const uword in_col2) const {
	arma_extra_debug_sigprint();
...
template<typename eT> arma_inline
SpSubview<eT> SpMat<eT>::submat(const uword in_row1, const uword in_col1, const SizeMat& s) {
	arma_extra_debug_sigprint();
...
template<typename eT> arma_inline
const SpSubview<eT> SpMat<eT>::submat(const uword in_row1, const uword in_col1, const SizeMat& s) const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
SpSubview<eT> SpMat<eT>::submat(const span& row_span, const span& col_span) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
const SpSubview<eT> SpMat<eT>::submat(const span& row_span, const span& col_span) const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
SpSubview<eT> SpMat<eT>::operator()(const span& row_span, const span& col_span) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
const SpSubview<eT> SpMat<eT>::operator()(const span& row_span, const span& col_span) const {
	arma_extra_debug_sigprint();
...
template<typename eT> arma_inline
SpSubview<eT> SpMat<eT>::operator()(const uword in_row1, const uword in_col1, const SizeMat& s) {
	arma_extra_debug_sigprint();
...
template<typename eT> arma_inline
const SpSubview<eT> SpMat<eT>::operator()(const uword in_row1, const uword in_col1, const SizeMat& s) const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
SpSubview<eT> SpMat<eT>::head_rows(const uword N) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
const SpSubview<eT> SpMat<eT>::head_rows(const uword N) const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
SpSubview<eT> SpMat<eT>::tail_rows(const uword N) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
const SpSubview<eT> SpMat<eT>::tail_rows(const uword N) const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
SpSubview<eT> SpMat<eT>::head_cols(const uword N) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
const SpSubview<eT> SpMat<eT>::head_cols(const uword N) const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
SpSubview<eT> SpMat<eT>::tail_cols(const uword N) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
const SpSubview<eT> SpMat<eT>::tail_cols(const uword N) const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
spdiagview<eT> SpMat<eT>::diag(const sword in_id) {
	arma_extra_debug_sigprint();
...
//! creation of spdiagview (diagonal)
template<typename eT> inline
const spdiagview<eT> SpMat<eT>::diag(const sword in_id) const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
void SpMat<eT>::swap_rows(const uword in_row1, const uword in_row2) {
	arma_extra_debug_sigprint();
	...
	for(uword lcol = 0; lcol < n_cols; lcol++) {
		...
		if(col_ptrs[lcol] == col_ptrs[lcol + 1]) { continue; }
		...
		for(uword search_pos = col_ptrs[lcol]; search_pos < col_ptrs[lcol + 1]; search_pos++) {
			if(row_indices[search_pos] == col1) { loc1 = search_pos; }
...
template<typename eT> inline
void SpMat<eT>::swap_cols(const uword in_col1, const uword in_col2) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
void SpMat<eT>::shed_row(const uword row_num) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
void SpMat<eT>::shed_col(const uword col_num) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
void SpMat<eT>::shed_rows(const uword in_row1, const uword in_row2) {
	arma_extra_debug_sigprint();
	...
	for(uword i = 0; i < n_nonzero; ++i) {
		...
		if(lrow >= in_row1 && lrow <= in_row2) { ++removing; }
	}

	// Obtain counts of the number of points in each column and store them as the
	// (invalid) column pointers of the new matrix.
	for(uword i = 1; i < n_cols + 1; ++i) { access::rw(newmat.col_ptrs[i]) = col_ptrs[i] - col_ptrs[i - 1]; }
	...
	while(it != it_end) {
		...
		if(lrow >= in_row1 && lrow <= in_row2) {
			...
			if(lrow > in_row2) { access::rw(newmat.row_indices[j]) = lrow - (in_row2 - in_row1 + 1); } else { access::rw(newmat.row_indices[j]) = lrow; }
	...
	for(uword i = 1; i < n_cols + 1; ++i) { access::rw(newmat.col_ptrs[i]) += newmat.col_ptrs[i - 1]; }
...
template<typename eT> inline
void SpMat<eT>::shed_cols(const uword in_col1, const uword in_col2) {
	arma_extra_debug_sigprint();
	...
	if(in_col1 != 0) { arrayops::copy(new_col_ptrs, col_ptrs, in_col1); }

	// Copy second set of columns (manipulation required).
	uword cur_col = in_col1;
	for(uword i = in_col2 + 1; i <= n_cols; ++i, ++cur_col) { new_col_ptrs[cur_col] = col_ptrs[i] - diff; }
...
template<typename eT> arma_inline
arma_warn_unused
SpMat_MapMat_val<eT> SpMat<eT>::operator[](const uword i) {
...
template<typename eT> arma_inline
arma_warn_unused
eT SpMat<eT>::operator[](const uword i) const { return get_value(i); }

template<typename eT> arma_inline
arma_warn_unused
SpMat_MapMat_val<eT> SpMat<eT>::at(const uword i) {
	const uword in_col = i / n_rows;
	const uword in_row = i % n_rows;
...
template<typename eT> arma_inline
arma_warn_unused
eT SpMat<eT>::at(const uword i) const { return get_value(i); }
...
template<typename eT> arma_inline
arma_warn_unused
eT SpMat<eT>::operator()(const uword i) const {
	arma_debug_check((i >= n_elem), "SpMat::operator(): out of bounds");
...
template<typename eT> arma_inline
arma_warn_unused
SpMat_MapMat_val<eT> SpMat<eT>::at(const uword in_row, const uword in_col) { return SpMat_MapMat_val<eT>((*this), cache, in_row, in_col); }

template<typename eT> arma_inline
arma_warn_unused
eT SpMat<eT>::at(const uword in_row, const uword in_col) const { return get_value(in_row, in_col); }

template<typename eT> arma_inline
arma_warn_unused
SpMat_MapMat_val<eT> SpMat<eT>::operator()(const uword in_row, const uword in_col) {
...
template<typename eT> arma_inline
arma_warn_unused
eT SpMat<eT>::operator()(const uword in_row, const uword in_col) const {
	arma_debug_check(((in_row >= n_rows) || (in_col >= n_cols)), "SpMat::operator(): out of bounds");
...
template<typename eT> arma_inline
arma_warn_unused
bool SpMat<eT>::is_empty() const { return (n_elem == 0); }

//! returns true if the object can be interpreted as a column or row vector
template<typename eT> arma_inline
arma_warn_unused
bool SpMat<eT>::is_vec() const { return ((n_rows == 1) || (n_cols == 1)); }

//! returns true if the object can be interpreted as a row vector
template<typename eT> arma_inline
arma_warn_unused
bool SpMat<eT>::is_rowvec() const { return (n_rows == 1); }

//! returns true if the object can be interpreted as a column vector
template<typename eT> arma_inline
arma_warn_unused
bool SpMat<eT>::is_colvec() const { return (n_cols == 1); }

//! returns true if the object has the same number of non-zero rows and columnns
template<typename eT> arma_inline
arma_warn_unused
bool SpMat<eT>::is_square() const { return (n_rows == n_cols); }

//! returns true if all of the elements are finite
template<typename eT> inline
arma_warn_unused
bool SpMat<eT>::is_finite() const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
arma_warn_unused
bool SpMat<eT>::is_symmetric() const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
arma_warn_unused
bool SpMat<eT>::is_symmetric(const typename get_pod_type<elem_type>::result tol) const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
arma_warn_unused
bool SpMat<eT>::is_hermitian() const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
arma_warn_unused
bool SpMat<eT>::is_hermitian(const typename get_pod_type<elem_type>::result tol) const {
	arma_extra_debug_sigprint();
...
template<typename eT> arma_inline
arma_warn_unused
bool SpMat<eT>::in_range(const uword i) const { return (i < n_elem); }

//! returns true if the given start and end indices are currently in range
template<typename eT> arma_inline
arma_warn_unused
bool SpMat<eT>::in_range(const span& x) const {
	arma_extra_debug_sigprint();
...
template<typename eT> arma_inline
arma_warn_unused
bool SpMat<eT>::in_range(const uword in_row, const uword in_col) const { return ((in_row < n_rows) && (in_col < n_cols)); }

template<typename eT> arma_inline
arma_warn_unused
bool SpMat<eT>::in_range(const span& row_span, const uword in_col) const {
	arma_extra_debug_sigprint();

	if(row_span.whole == true) { return (in_col < n_cols); } else {
...
template<typename eT> arma_inline
arma_warn_unused
bool SpMat<eT>::in_range(const uword in_row, const span& col_span) const {
	arma_extra_debug_sigprint();

	if(col_span.whole == true) { return (in_row < n_rows); } else {
...
template<typename eT> arma_inline
arma_warn_unused
bool SpMat<eT>::in_range(const span& row_span, const span& col_span) const {
	arma_extra_debug_sigprint();
...
template<typename eT> arma_inline
arma_warn_unused
bool SpMat<eT>::in_range(const uword in_row, const uword in_col, const SizeMat& s) const {
	const uword l_n_rows = n_rows;
	const uword l_n_cols = n_cols;
...
inline
void SpMat<eT>::impl_print(const std::string& extra_text) const {
	arma_extra_debug_sigprint();
...
template<typename eT> arma_cold
inline
void SpMat<eT>::impl_print(std::ostream& user_stream, const std::string& extra_text) const {
	arma_extra_debug_sigprint();
...
template<typename eT> arma_cold
inline
void SpMat<eT>::impl_raw_print(const std::string& extra_text) const {
	arma_extra_debug_sigprint();
...
template<typename eT> arma_cold
inline
void SpMat<eT>::impl_raw_print(std::ostream& user_stream, const std::string& extra_text) const {
	arma_extra_debug_sigprint();
...
template<typename eT> arma_cold
inline
void SpMat<eT>::impl_print_dense(const std::string& extra_text) const {
	arma_extra_debug_sigprint();
...
template<typename eT> arma_cold
inline
void SpMat<eT>::impl_print_dense(std::ostream& user_stream, const std::string& extra_text) const {
	arma_extra_debug_sigprint();
...
template<typename eT> arma_cold
inline
void SpMat<eT>::impl_raw_print_dense(const std::string& extra_text) const {
	arma_extra_debug_sigprint();
...
template<typename eT> arma_cold
inline
void SpMat<eT>::impl_raw_print_dense(std::ostream& user_stream, const std::string& extra_text) const {
	arma_extra_debug_sigprint();
...
void SpMat<eT>::copy_size(const SpMat<eT2>& m) {
	arma_extra_debug_sigprint();
...
template<typename eT> template<typename eT2> inline
void SpMat<eT>::copy_size(const Mat<eT2>& m) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
void SpMat<eT>::set_size(const uword in_elem) {
	arma_extra_debug_sigprint();

	// If this is a row vector, we resize to a row vector.
	if(vec_state == 2) { set_size(1, in_elem); } else { set_size(in_elem, 1); }
}

template<typename eT> inline
void SpMat<eT>::set_size(const uword in_rows, const uword in_cols) {
	arma_extra_debug_sigprint();

	invalidate_cache(); // placed here, as set_size() is used during matrix modification

	if((n_rows == in_rows) && (n_cols == in_cols)) { return; } else { init(in_rows, in_cols); }
}

template<typename eT> inline
void SpMat<eT>::set_size(const SizeMat& s) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
void SpMat<eT>::resize(const uword in_rows, const uword in_cols) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
void SpMat<eT>::resize(const SizeMat& s) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
void SpMat<eT>::reshape(const uword in_rows, const uword in_cols) {
	arma_extra_debug_sigprint();
	...
	for(uword i = 1; i <= in_cols; i++) { access::rw(new_col_ptrs[i]) += new_col_ptrs[i - 1]; }
...
template<typename eT> inline
void SpMat<eT>::reshape(const SizeMat& s) {
	arma_extra_debug_sigprint();
...
inline
void SpMat<eT>::reshape(const uword in_rows, const uword in_cols, const uword dim) {
	arma_extra_debug_sigprint();

	arma_debug_check((dim > 1), "SpMat::reshape(): parameter 'dim' must be 0 or 1");

	if(dim == 0) { (*this).reshape(in_rows, in_cols); } else if(dim == 1) {
		arma_check(((in_rows * in_cols) != n_elem), "SpMat::reshape(): changing the number of elements in a sparse matrix is currently not supported");
...
const SpMat<eT>& SpMat<eT>::for_each(functor F) {
	arma_extra_debug_sigprint();
...
template<typename eT> template<typename functor> inline
const SpMat<eT>& SpMat<eT>::for_each(functor F) const {
	arma_extra_debug_sigprint();
...
template<typename eT> template<typename functor> inline
const SpMat<eT>& SpMat<eT>::transform(functor F) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
const SpMat<eT>& SpMat<eT>::replace(const eT old_val, const eT new_val) {
	arma_extra_debug_sigprint();

	if(old_val == eT(0)) { arma_debug_warn("SpMat::replace(): replacement not done, as old_val = 0"); } else {
		sync_csc();
		invalidate_cache();
...
template<typename eT> inline
const SpMat<eT>& SpMat<eT>::zeros() {
	arma_extra_debug_sigprint();

	if(n_nonzero != 0) { init(n_rows, n_cols); }

	return *this;
}

template<typename eT> inline
const SpMat<eT>& SpMat<eT>::zeros(const uword in_elem) {
	arma_extra_debug_sigprint();

	if(vec_state == 2) {
		zeros(1, in_elem); // Row vector
	} else { zeros(in_elem, 1); }

	return *this;
}

template<typename eT> inline
const SpMat<eT>& SpMat<eT>::zeros(const uword in_rows, const uword in_cols) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
const SpMat<eT>& SpMat<eT>::zeros(const SizeMat& s) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
const SpMat<eT>& SpMat<eT>::eye(const uword in_rows, const uword in_cols) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
const SpMat<eT>& SpMat<eT>::eye(const SizeMat& s) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
const SpMat<eT>& SpMat<eT>::speye(const uword in_n_rows, const uword in_n_cols) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
const SpMat<eT>& SpMat<eT>::speye(const SizeMat& s) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
const SpMat<eT>& SpMat<eT>::sprandu(const uword in_rows, const uword in_cols, const double density) {
	arma_extra_debug_sigprint();
	...
	for(uword i = 1; i < new_n_nonzero - 1; ++i) {
		...
		if((index_left < index_new) && (index_new < index_right)) { indices[i] = index_new; }
	}
	...
	// Sum column pointers.
	for(uword lcol = 1; lcol <= in_cols; ++lcol) { access::rw(col_ptrs[lcol]) += col_ptrs[lcol - 1]; }

	return *this;
}

template<typename eT> inline
const SpMat<eT>& SpMat<eT>::sprandu(const SizeMat& s, const double density) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
const SpMat<eT>& SpMat<eT>::sprandn(const uword in_rows, const uword in_cols, const double density) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
const SpMat<eT>& SpMat<eT>::sprandn(const SizeMat& s, const double density) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
void SpMat<eT>::reserve(const uword in_rows, const uword in_cols, const uword new_n_nonzero) {
	arma_extra_debug_sigprint();
...
template<typename eT> template<typename T1> inline
void SpMat<eT>::set_real(const SpBase<typename SpMat<eT>::pod_type, T1>& X) {
	arma_extra_debug_sigprint();
...
template<typename eT> template<typename T1> inline
void SpMat<eT>::set_imag(const SpBase<typename SpMat<eT>::pod_type, T1>& X) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
arma_cold
bool SpMat<eT>::save(const std::string name, const file_type type, const bool print_status) const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
arma_cold
bool SpMat<eT>::save(std::ostream& os, const file_type type, const bool print_status) const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
arma_cold
bool SpMat<eT>::load(const std::string name, const file_type type, const bool print_status) {
	arma_extra_debug_sigprint();
	...
	if(print_status && (load_okay == false)) { if(err_msg.length() > 0) { arma_debug_warn("SpMat::load(): ", err_msg, name); } else { arma_debug_warn("SpMat::load(): couldn't read ", name); } }
...
template<typename eT> inline
arma_cold
bool SpMat<eT>::load(std::istream& is, const file_type type, const bool print_status) {
	arma_extra_debug_sigprint();
	...
	if(print_status && (load_okay == false)) { if(err_msg.length() > 0) { arma_debug_warn("SpMat::load(): ", err_msg, "the given stream"); } else { arma_debug_warn("SpMat::load(): couldn't load from the given stream"); } }
...
template<typename eT> inline
arma_cold
bool SpMat<eT>::quiet_save(const std::string name, const file_type type) const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
arma_cold
bool SpMat<eT>::quiet_save(std::ostream& os, const file_type type) const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
arma_cold
bool SpMat<eT>::quiet_load(const std::string name, const file_type type) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
arma_cold
bool SpMat<eT>::quiet_load(std::istream& is, const file_type type) {
	arma_extra_debug_sigprint();
 ...
 */
template<typename eT> inline
void SpMat<eT>::init(uword in_rows, uword in_cols, const uword new_n_nonzero) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
void SpMat<eT>::init(const std::string& text) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
void SpMat<eT>::init(const SpMat<eT>& x) {
	arma_extra_debug_sigprint();
	...
	if(init_done == false) { (*this).init_simple(x); }
}

template<typename eT> inline
void SpMat<eT>::init(const MapMat<eT>& x) {
	arma_extra_debug_sigprint();
	...
	for(uword i = 0; i < x_n_cols; ++i) { access::rw(col_ptrs[i + 1]) += col_ptrs[i]; }
...
template<typename eT> inline
void SpMat<eT>::init_simple(const SpMat<eT>& x) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
void SpMat<eT>::init_batch_std(const Mat<uword>& locs, const Mat<eT>& vals, const bool sort_locations) {
	arma_extra_debug_sigprint();
	...
	if(sort_locations == true) {
		...
		if(actually_sorted == false) {
			...
			for(uword i = 0; i < locs_n_cols; ++i) {
				const uword row = (*locs_mem);
				locs_mem++;
				const uword col = (*locs_mem);
				locs_mem++;
	...
	// Now fix the column pointers.
	for(uword i = 0; i < n_cols; ++i) { access::rw(col_ptrs[i + 1]) += col_ptrs[i]; }
}

template<typename eT> inline
void SpMat<eT>::init_batch_add(const Mat<uword>& locs, const Mat<eT>& vals, const bool sort_locations) {
	arma_extra_debug_sigprint();
	...
	if(sort_locations == true) {
		...
		if(actually_sorted == false) {
			...
			for(uword i = 1; i < sorted_indices.n_elem; ++i) {
				...
				if((locs_i[1] == locs_im1[1]) && (locs_i[0] == locs_im1[0])) { access::rw(values[count]) += vals[sorted_indices[i]]; } else {
					count++;
					access::rw(values[count]) = vals[sorted_indices[i]];
					access::rw(row_indices[count]) = locs_i[0];
	...
	if((sort_locations == false) || (actually_sorted == true)) {
		...
		for(uword i = 1; i < locs.n_cols; ++i) {
			...
			if((locs_i[1] == locs_im1[1]) && (locs_i[0] == locs_im1[0])) { access::rw(values[count]) += vals[i]; } else {
				count++;
...
template<typename eT> inline SpMat<eT>::SpMat(const arma_vec_indicator&, const uword in_vec_state)
	: n_rows(0)
	, n_cols(0)
	, n_elem(0)
	, n_nonzero(0)
...
//! constructor used by SpRow and SpCol classes
template<typename eT> inline SpMat<eT>::SpMat(const arma_vec_indicator&, const uword in_n_rows, const uword in_n_cols, const uword in_vec_state)
	: n_rows(0)
	, n_cols(0)
	, n_elem(0)
	, n_nonzero(0)
...
template<typename eT> inline
void SpMat<eT>::mem_resize(const uword new_n_nonzero) {
	arma_extra_debug_sigprint();
...
void SpMat<eT>::remove_zeros() {
	...
	for(uword i = 0; i < old_n_nonzero; ++i) { new_n_nonzero += (old_values[i] != eT(0)) ? uword(1) : uword(0); }
	...
	if(new_n_nonzero != old_n_nonzero) {
		...
		for(uword i = 0; i < n_cols; ++i) { access::rw(tmp.col_ptrs[i + 1]) += tmp.col_ptrs[i]; }
...
template<typename eT> inline
void SpMat<eT>::steal_mem(SpMat<eT>& x) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
void SpMat<eT>::steal_mem_simple(SpMat<eT>& x) {
	arma_extra_debug_sigprint();
...
template<typename eT> template<typename T1, typename Functor> arma_hot
inline
void SpMat<eT>::init_xform(const SpBase<eT, T1>& A, const Functor& func) {
	arma_extra_debug_sigprint();
	...
	if(SpProxy<T1>::Q_is_generated) {
		...
		if(has_zero) { remove_zeros(); }
	} else { init_xform_mt(A.get_ref(), func); }
}

template<typename eT> template<typename eT2, typename T1, typename Functor> arma_hot
inline
void SpMat<eT>::init_xform_mt(const SpBase<eT2, T1>& A, const Functor& func) {
	arma_extra_debug_sigprint();
...
template<typename eT> arma_inline
bool SpMat<eT>::is_alias(const SpMat<eT>& X) const { return (&X == this); }

template<typename eT> inline
typename SpMat<eT>::iterator SpMat<eT>::begin() {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
typename SpMat<eT>::const_iterator SpMat<eT>::begin() const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
typename SpMat<eT>::const_iterator SpMat<eT>::cbegin() const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
typename SpMat<eT>::iterator SpMat<eT>::end() {
	sync_csc();
...
template<typename eT> inline
typename SpMat<eT>::const_iterator SpMat<eT>::end() const {
	sync_csc();
...
template<typename eT> inline
typename SpMat<eT>::const_iterator SpMat<eT>::cend() const {
	sync_csc();
...
template<typename eT> inline
typename SpMat<eT>::col_iterator SpMat<eT>::begin_col(const uword col_num) {
	sync_csc();
...
template<typename eT> inline
typename SpMat<eT>::const_col_iterator SpMat<eT>::begin_col(const uword col_num) const {
	sync_csc();
...
template<typename eT> inline
typename SpMat<eT>::col_iterator SpMat<eT>::begin_col_no_sync(const uword col_num) { return col_iterator(*this, 0, col_num); }

template<typename eT> inline
typename SpMat<eT>::const_col_iterator SpMat<eT>::begin_col_no_sync(const uword col_num) const { return const_col_iterator(*this, 0, col_num); }

template<typename eT> inline
typename SpMat<eT>::col_iterator SpMat<eT>::end_col(const uword col_num) {
	sync_csc();
...
template<typename eT> inline
typename SpMat<eT>::const_col_iterator SpMat<eT>::end_col(const uword col_num) const {
	sync_csc();
...
template<typename eT> inline
typename SpMat<eT>::col_iterator SpMat<eT>::end_col_no_sync(const uword col_num) { return col_iterator(*this, 0, col_num + 1); }

template<typename eT> inline
typename SpMat<eT>::const_col_iterator SpMat<eT>::end_col_no_sync(const uword col_num) const { return const_col_iterator(*this, 0, col_num + 1); }

template<typename eT> inline
typename SpMat<eT>::row_iterator SpMat<eT>::begin_row(const uword row_num) {
	sync_csc();
...
template<typename eT> inline
typename SpMat<eT>::const_row_iterator SpMat<eT>::begin_row(const uword row_num) const {
	sync_csc();
...
template<typename eT> inline
typename SpMat<eT>::row_iterator SpMat<eT>::end_row() {
	sync_csc();
...
template<typename eT> inline
typename SpMat<eT>::const_row_iterator SpMat<eT>::end_row() const {
	sync_csc();
...
template<typename eT> inline
typename SpMat<eT>::row_iterator SpMat<eT>::end_row(const uword row_num) {
	sync_csc();
...
template<typename eT> inline
typename SpMat<eT>::const_row_iterator SpMat<eT>::end_row(const uword row_num) const {
	sync_csc();
...
template<typename eT> inline
typename SpMat<eT>::row_col_iterator SpMat<eT>::begin_row_col() {
	sync_csc();
...
template<typename eT> inline
typename SpMat<eT>::const_row_col_iterator SpMat<eT>::begin_row_col() const {
	sync_csc();

	return begin();
}

template<typename eT> inline typename SpMat<eT>::row_col_iterator SpMat<eT>::end_row_col() {
	sync_csc();
...
template<typename eT> inline
typename SpMat<eT>::const_row_col_iterator SpMat<eT>::end_row_col() const {
	sync_csc();
...
template<typename eT> inline
void SpMat<eT>::clear() { (*this).reset(); }

template<typename eT> inline
bool SpMat<eT>::empty() const { return (n_elem == 0); }

template<typename eT> inline
uword SpMat<eT>::size() const { return n_elem; }

template<typename eT> inline
arma_hot
arma_warn_unused
eT SpMat<eT>::get_value(const uword i) const {
...
arma_hot
arma_warn_unused
eT SpMat<eT>::get_value(const uword in_row, const uword in_col) const {
	const MapMat<eT>& const_cache = cache; // declare as const for clarity of intent
...
arma_hot
arma_warn_unused
eT SpMat<eT>::get_value_csc(const uword i) const {
...
arma_hot
arma_warn_unused
eT SpMat<eT>::get_value_csc(const uword in_row, const uword in_col) const {
...
template<typename eT> inline
arma_warn_unused
eT& SpMat<eT>::insert_element(const uword in_row, const uword in_col, const eT val) {
	arma_extra_debug_sigprint();
	...
	if(colptr != next_colptr) {
		...
		while(pos < next_colptr && in_row > row_indices[pos]) { pos++; }
 ...
 */
template<typename eT> inline
void SpMat<eT>::delete_element(const uword in_row, const uword in_col) {
	arma_extra_debug_sigprint();
...
template<typename eT> arma_inline
void SpMat<eT>::invalidate_cache() const {
	arma_extra_debug_sigprint();
...
template<typename eT> arma_inline
void SpMat<eT>::invalidate_csc() const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
void SpMat<eT>::sync_cache() const {
	arma_extra_debug_sigprint();
...
void SpMat_aux::set_real(SpMat<eT>& out, const SpBase<eT, T1>& X) {
	arma_extra_debug_sigprint();
...
template<typename eT, typename T1> inline
void SpMat_aux::set_imag(SpMat<eT>&, const SpBase<eT, T1>&) {
	arma_extra_debug_sigprint();
}

template<typename T, typename T1> inline
void SpMat_aux::set_real(SpMat<std::complex<T>>& out, const SpBase<T, T1>& X) {
	arma_extra_debug_sigprint();
...
template<typename T, typename T1> inline
void SpMat_aux::set_imag(SpMat<std::complex<T>>& out, const SpBase<T, T1>& X) {
	arma_extra_debug_sigprint();
