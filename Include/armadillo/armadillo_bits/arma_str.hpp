...
#define arma_snprintf std::snprintf
	...
	basic_format<format, T2> operator%(const format& X, const T2& arg) { return basic_format<format, T2>(X, arg); }

	template<typename T1, typename T2, typename T3> inline
	basic_format<basic_format<T1, T2>, T3> operator%(const basic_format<T1, T2>& X, const T3& arg) { return basic_format<basic_format<T1, T2>, T3>(X, arg); }
	...
	std::string str(const basic_format<format, T2>& X) {
		...
		do {
			if(using_local_buffer == false) { buffer = new char[size_t(buffer_size)]; }
	...
	template<typename T2, typename T3> inline
	...
	template<typename T2, typename T3, typename T4> inline
	...
	template<typename T2, typename T3, typename T4, typename T5> inline
	...
	template<typename T2, typename T3, typename T4, typename T5, typename T6> inline
	...
	template<typename T2, typename T3, typename T4, typename T5, typename T6, typename T7> inline
	...
	template<typename T1> struct format_metaprog {
		...
		inline
		static
		const std::string& get_fmt(const T1& X) { return X.A; }
	};
	...
	template<typename T1, typename T2> struct format_metaprog<basic_format<T1, T2>> {
		...
		inline
		static
		const std::string& get_fmt(const T1& X) { return format_metaprog<T1>::get_fmt(X.A); }

	};

	template<typename T1, typename T2> inline
	std::string str(const basic_format<T1, T2>& X) { return format_metaprog<basic_format<T1, T2>>::get_fmt(X.A); }
	...
	template<> struct string_only<std::string> {
		typedef std::string result;
	};

	template<typename T> struct char_only { };

	template<> struct char_only<char> {
		typedef char result;
	};
	...
	static
	const T1& str_wrapper(const T1& x, const typename string_only<T1>::result* junk = 0) {
		arma_ignore(junk);
	...
	template<typename T1> inline
	static
	const T1* str_wrapper(const T1* x, const typename char_only<T1>::result* junk = 0) {
		arma_ignore(junk);
	...
	template<typename T1> inline
	static
	std::string str_wrapper(const T1& x, const typename basic_format_only<T1>::result* junk = 0) {
		arma_ignore(junk);
