...
void op_reshape::apply_unwrap(Mat<eT>& out, const Mat<eT>& A, const uword in_n_rows, const uword in_n_cols, const uword in_dim) {
	...
	if(A.n_elem == in_n_elem) {
		...
		if(in_dim == 0) { arrayops::copy(out_mem, B.memptr(), n_elem_to_copy); } else {
			uword row = 0;
			uword col = 0;
...
void op_reshape::apply_proxy(Mat<typename T1::elem_type>& out, const Proxy<T1>& P, const uword in_n_rows, const uword in_n_cols) {
	arma_extra_debug_sigprint();
...
template<typename T1> inline
void op_reshape::apply(Mat<typename T1::elem_type>& out, const Op<T1, op_reshape>& in) {
	arma_extra_debug_sigprint();
...
template<typename T1> inline
void op_reshape_ext::apply(Mat<typename T1::elem_type>& out, const Op<T1, op_reshape_ext>& in) {
	arma_extra_debug_sigprint();
...
template<typename T1> inline
void op_reshape_ext::apply(Cube<typename T1::elem_type>& out, const OpCube<T1, op_reshape_ext>& in) {
	arma_extra_debug_sigprint();
