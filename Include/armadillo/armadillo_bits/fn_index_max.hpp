...
arma_inline
const mtOp<uword, T1, op_index_max> index_max
(
	const T1& X,
	const uword dim = 0,
...
template<typename T1> arma_warn_unused
arma_inline
const mtOpCube<uword, T1, op_index_max> index_max
(
...
<
	...
	Mat<uword>
>::result index_max(const T1& X, const uword dim = 0) {
	arma_extra_debug_sigprint();
...
inline
uword index_max(const SizeMat& s) { return (s.n_rows >= s.n_cols) ? uword(0) : uword(1); }

arma_warn_unused

inline
uword index_max(const SizeCube& s) {
