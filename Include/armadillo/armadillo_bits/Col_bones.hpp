...
public:
	...
	template<typename T1, typename T2> inline explicit Col(const Base<pod_type, T1>& A, const Base<pod_type, T2>& B);
...
template<typename eT> template<uword fixed_n_elem> const uword Col<eT>::fixed<fixed_n_elem>::n_rows = fixed_n_elem;

template<typename eT> template<uword fixed_n_elem> const uword Col<eT>::fixed<fixed_n_elem>::n_cols = 1u;

template<typename eT> template<uword fixed_n_elem> const uword Col<eT>::fixed<fixed_n_elem>::n_elem = fixed_n_elem;
