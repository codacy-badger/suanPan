...
bool arma_isfinite(eT val) {
	arma_ignore(val);

	return true;
}

template<> arma_inline
bool arma_isfinite(float x) {
...
bool arma_isfinite(const std::complex<T>& x) { if((arma_isfinite(x.real()) == false) || (arma_isfinite(x.imag()) == false)) { return false; } else { return true; } }

//
// wrappers for isinf

template<typename eT> arma_inline
bool arma_isinf(eT val) {
	arma_ignore(val);
...
template<typename T> arma_inline
bool arma_isinf(const std::complex<T>& x) { return (arma_isinf(x.real()) || arma_isinf(x.imag())); }

//
// wrappers for isnan

template<typename eT> arma_inline
bool arma_isnan(eT val) {
	arma_ignore(val);
...
template<typename T> arma_inline
bool arma_isnan(const std::complex<T>& x) { return (arma_isnan(x.real()) || arma_isnan(x.imag())); }

// rudimentary wrappers for log1p()

arma_inline
float arma_log1p(const float x) {
#if defined(ARMA_USE_CXX11)
	{
		return std::log1p(x);
	}
...
arma_inline
double arma_log1p(const double x) {
#if defined(ARMA_USE_CXX11)
	{
		return std::log1p(x);
	}
...
template<typename eT> arma_inline
typename arma_unsigned_integral_only<eT>::result arma_sign(const eT x) { return (x > eT(0)) ? eT(+1) : eT(0); }
...
template<typename T> arma_inline
std::complex<T> arma_acos(const std::complex<T>& x) {
#if defined(ARMA_USE_CXX11)
	{
		return std::acos(x);
	}
...
template<typename T> arma_inline
std::complex<T> arma_asin(const std::complex<T>& x) {
#if defined(ARMA_USE_CXX11)
	{
		return std::asin(x);
	}
...
template<typename T> arma_inline
std::complex<T> arma_atan(const std::complex<T>& x) {
#if defined(ARMA_USE_CXX11)
	{
		return std::atan(x);
	}
...
template<typename T> arma_inline
std::complex<T> arma_acosh(const std::complex<T>& x) {
#if defined(ARMA_USE_CXX11)
	{
		return std::acosh(x);
	}
...
template<typename T> arma_inline
std::complex<T> arma_asinh(const std::complex<T>& x) {
#if defined(ARMA_USE_CXX11)
	{
		return std::asinh(x);
	}
...
template<typename T> arma_inline
std::complex<T> arma_atanh(const std::complex<T>& x) {
#if defined(ARMA_USE_CXX11)
	{
		return std::atanh(x);
	}
...
eT arma_hypot_generic(const eT x, const eT y) {
#if defined(ARMA_USE_CXX11)
	{
...
template<typename eT> inline
eT arma_hypot(const eT x, const eT y) {
	arma_ignore(x);
	arma_ignore(y);
...
template<> arma_inline
float arma_hypot(const float x, const float y) { return arma_hypot_generic(x, y); }

template<> arma_inline
double arma_hypot(const double x, const double y) { return arma_hypot_generic(x, y); }

//
// implementation of arma_sinc()

template<typename eT> arma_inline
eT arma_sinc_generic(const eT x) {
...
template<typename eT> arma_inline
eT arma_sinc(const eT x) { return eT(arma_sinc_generic(double(x))); }

template<> arma_inline
float arma_sinc(const float x) { return arma_sinc_generic(x); }

template<> arma_inline
double arma_sinc(const double x) { return arma_sinc_generic(x); }

template<typename T> arma_inline
std::complex<T> arma_sinc(const std::complex<T>& x) { return arma_sinc_generic(x); }

//
// wrappers for arg()

template<typename eT> struct arma_arg {
	static
	inline
	eT eval(const eT x) {
#if defined(ARMA_USE_CXX11)
...
template<> struct arma_arg<float> {
	static
	arma_inline
...
template<> struct arma_arg<double> {
	static
	arma_inline
...
template<> struct arma_arg<std::complex<float>> {
	static
	arma_inline
	float eval(const std::complex<float>& x) { return std::arg(x); }
};

template<> struct arma_arg<std::complex<double>> {
	static
	arma_inline
	double eval(const std::complex<double>& x) { return std::arg(x); }
};
