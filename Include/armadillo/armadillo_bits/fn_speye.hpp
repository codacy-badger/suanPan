...
inline
obj_type speye(const uword n_rows, const uword n_cols, const typename arma_SpMat_SpCol_SpRow_only<obj_type>::result* junk = NULL) {
...
template<typename obj_type> arma_warn_unused
inline
obj_type speye(const SizeMat& s, const typename arma_SpMat_SpCol_SpRow_only<obj_type>::result* junk = NULL) {
	arma_extra_debug_sigprint();
	arma_ignore(junk);
...
inline
sp_mat speye(const uword n_rows, const uword n_cols) {
	arma_extra_debug_sigprint();
