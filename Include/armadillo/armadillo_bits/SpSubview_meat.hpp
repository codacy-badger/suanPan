...
template<typename eT> arma_inline SpSubview<eT>::SpSubview(const SpMat<eT>& in_m, const uword in_row1, const uword in_col1, const uword in_n_rows, const uword in_n_cols)
	: m(in_m)
	, aux_row1(in_row1)
	, aux_col1(in_col1)
	, n_rows(in_n_rows)
...
template<typename eT> arma_inline SpSubview<eT>::SpSubview(SpMat<eT>& in_m, const uword in_row1, const uword in_col1, const uword in_n_rows, const uword in_n_cols)
	: m(in_m)
	, aux_row1(in_row1)
	, aux_col1(in_col1)
	, n_rows(in_n_rows)
...
template<typename eT> inline SpSubview<eT>::~SpSubview() {
	arma_extra_debug_sigprint();
}

template<typename eT> inline
const SpSubview<eT>& SpSubview<eT>::operator+=(const eT val) {
	arma_extra_debug_sigprint();
...
const SpSubview<eT>& SpSubview<eT>::operator*=(const eT val) {
	...
	if(has_zero) {
		...
		if(m.n_nonzero != old_m_n_nonzero) { access::rw(n_nonzero) = n_nonzero - (old_m_n_nonzero - m.n_nonzero); }
	}
...
const SpSubview<eT>& SpSubview<eT>::operator=(const Base<eT, T1>& in) {
	arma_extra_debug_sigprint();
	...
	for(uword i = 0; i < b_n_elem; ++i) { box_count += (b_mem[i] != eT(0)) ? uword(1) : uword(0); }
...
template<typename eT> template<typename T1> inline
const SpSubview<eT>& SpSubview<eT>::operator+=(const Base<eT, T1>& x) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
const SpSubview<eT>& SpSubview<eT>::operator=(const SpSubview<eT>& x) {
	arma_extra_debug_sigprint();
...
template<typename eT> template<typename T1> inline
const SpSubview<eT>& SpSubview<eT>::operator=(const SpBase<eT, T1>& x) {
	arma_extra_debug_sigprint();
...
template<typename eT> template<typename T1> inline
const SpSubview<eT>& SpSubview<eT>::operator_equ_common(const SpBase<eT, T1>& in) {
	arma_extra_debug_sigprint();
...
const SpSubview<eT>& SpSubview<eT>::operator/=(const SpBase<eT, T1>& x) {
	...
	if(p.is_alias(m) == false) {
		for(uword lcol = 0; lcol < n_cols; ++lcol)
			for(uword lrow = 0; lrow < n_rows; ++lrow) { at(lrow, lcol) /= p.at(lrow, lcol); }
	} else {
...
void SpSubview<eT>::for_each(functor F) {
	arma_extra_debug_sigprint();
...
template<typename eT> template<typename functor> inline
void SpSubview<eT>::for_each(functor F) const {
	arma_extra_debug_sigprint();
	...
	for(uword c = lstart_col; c < lend_col; ++c) {
		...
		for(uword r = r_start; r < r_end; ++r) {
			...
			if((m_row_indices_r >= lstart_row) && (m_row_indices_r < lend_row)) { F(m.values[r]); }
		}
	}
}
...
template<typename eT> template<typename functor> inline
void SpSubview<eT>::transform(functor F) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
void SpSubview<eT>::replace(const eT old_val, const eT new_val) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
void SpSubview<eT>::fill(const eT val) {
	arma_extra_debug_sigprint();
...
inline
SpSubview_MapMat_val<eT> SpSubview<eT>::operator[](const uword i) {
...
template<typename eT> arma_hot
inline
eT SpSubview<eT>::operator[](const uword i) const {
	const uword lrow = i % n_rows;
	const uword lcol = i / n_rows;
...
template<typename eT> arma_hot
inline
SpSubview_MapMat_val<eT> SpSubview<eT>::operator()(const uword in_row, const uword in_col) {
...
template<typename eT> arma_hot
inline
eT SpSubview<eT>::operator()(const uword in_row, const uword in_col) const {
	arma_debug_check((in_row >= n_rows) || (in_col >= n_cols), "SpSubview::operator(): index out of bounds");
...
template<typename eT> arma_hot
inline
SpSubview_MapMat_val<eT> SpSubview<eT>::at(const uword i) {
	const uword lrow = i % n_rows;
...
template<typename eT> arma_hot
inline
eT SpSubview<eT>::at(const uword i) const {
	const uword lrow = i % n_rows;
	const uword lcol = i / n_cols;
...
template<typename eT> arma_hot
inline
SpSubview_MapMat_val<eT> SpSubview<eT>::at(const uword in_row, const uword in_col) { return SpSubview_MapMat_val<eT>((*this), m.cache, aux_row1 + in_row, aux_col1 + in_col); }

template<typename eT> arma_hot
inline
eT SpSubview<eT>::at(const uword in_row, const uword in_col) const { return m.at(aux_row1 + in_row, aux_col1 + in_col); }

template<typename eT> inline
bool SpSubview<eT>::check_overlap(const SpSubview<eT>& x) const {
...
template<typename eT> inline
bool SpSubview<eT>::is_vec() const { return ((n_rows == 1) || (n_cols == 1)); }

template<typename eT> inline
SpSubview<eT> SpSubview<eT>::row(const uword row_num) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
const SpSubview<eT> SpSubview<eT>::row(const uword row_num) const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
SpSubview<eT> SpSubview<eT>::col(const uword col_num) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
const SpSubview<eT> SpSubview<eT>::col(const uword col_num) const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
SpSubview<eT> SpSubview<eT>::rows(const uword in_row1, const uword in_row2) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
const SpSubview<eT> SpSubview<eT>::rows(const uword in_row1, const uword in_row2) const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
SpSubview<eT> SpSubview<eT>::cols(const uword in_col1, const uword in_col2) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
const SpSubview<eT> SpSubview<eT>::cols(const uword in_col1, const uword in_col2) const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
SpSubview<eT> SpSubview<eT>::submat(const uword in_row1, const uword in_col1, const uword in_row2, const uword in_col2) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
const SpSubview<eT> SpSubview<eT>::submat(const uword in_row1, const uword in_col1, const uword in_row2, const uword in_col2) const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
SpSubview<eT> SpSubview<eT>::submat(const span& row_span, const span& col_span) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
const SpSubview<eT> SpSubview<eT>::submat(const span& row_span, const span& col_span) const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
SpSubview<eT> SpSubview<eT>::operator()(const uword row_num, const span& col_span) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
const SpSubview<eT> SpSubview<eT>::operator()(const uword row_num, const span& col_span) const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
SpSubview<eT> SpSubview<eT>::operator()(const span& row_span, const uword col_num) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
const SpSubview<eT> SpSubview<eT>::operator()(const span& row_span, const uword col_num) const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
SpSubview<eT> SpSubview<eT>::operator()(const span& row_span, const span& col_span) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
const SpSubview<eT> SpSubview<eT>::operator()(const span& row_span, const span& col_span) const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
void SpSubview<eT>::swap_cols(const uword in_col1, const uword in_col2) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
typename SpSubview<eT>::iterator SpSubview<eT>::begin() { return iterator(*this); }

template<typename eT> inline
typename SpSubview<eT>::const_iterator SpSubview<eT>::begin() const {
	m.sync_csc();
...
template<typename eT> inline
typename SpSubview<eT>::const_iterator SpSubview<eT>::cbegin() const {
	m.sync_csc();
...
template<typename eT> inline
typename SpSubview<eT>::iterator SpSubview<eT>::begin_col(const uword col_num) {
	m.sync_csc();
...
template<typename eT> inline
typename SpSubview<eT>::const_iterator SpSubview<eT>::begin_col(const uword col_num) const {
	m.sync_csc();
...
template<typename eT> inline
typename SpSubview<eT>::row_iterator SpSubview<eT>::begin_row(const uword row_num) {
	m.sync_csc();
...
template<typename eT> inline
typename SpSubview<eT>::const_row_iterator SpSubview<eT>::begin_row(const uword row_num) const {
	m.sync_csc();
...
template<typename eT> inline
typename SpSubview<eT>::iterator SpSubview<eT>::end() {
	m.sync_csc();
...
template<typename eT> inline
typename SpSubview<eT>::const_iterator SpSubview<eT>::end() const {
	m.sync_csc();
...
template<typename eT> inline
typename SpSubview<eT>::const_iterator SpSubview<eT>::cend() const {
	m.sync_csc();
...
template<typename eT> inline
typename SpSubview<eT>::row_iterator SpSubview<eT>::end_row() {
	m.sync_csc();
...
template<typename eT> inline
typename SpSubview<eT>::const_row_iterator SpSubview<eT>::end_row() const {
	m.sync_csc();
...
template<typename eT> inline
typename SpSubview<eT>::row_iterator SpSubview<eT>::end_row(const uword row_num) {
	m.sync_csc();
...
template<typename eT> inline
typename SpSubview<eT>::const_row_iterator SpSubview<eT>::end_row(const uword row_num) const {
	m.sync_csc();
...
bool SpSubview<eT>::is_alias(const SpMat<eT>& X) const { return m.is_alias(X); }

template<typename eT> inline
arma_warn_unused
eT& SpSubview<eT>::insert_element(const uword in_row, const uword in_col, const eT in_val) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
void SpSubview<eT>::delete_element(const uword in_row, const uword in_col) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
void SpSubview<eT>::invalidate_cache() const {
	arma_extra_debug_sigprint();
