...
template<typename eT> inline SpRow<eT>::SpRow()
	: SpMat<eT>(arma_vec_indicator(), 2) {
	arma_extra_debug_sigprint();
}

template<typename eT> inline SpRow<eT>::SpRow(const uword in_n_elem)
	: SpMat<eT>(arma_vec_indicator(), 1, in_n_elem, 2) {
	arma_extra_debug_sigprint();
}

template<typename eT> inline SpRow<eT>::SpRow(const uword in_n_rows, const uword in_n_cols)
	: SpMat<eT>(arma_vec_indicator(), in_n_rows, in_n_cols, 2) {
	arma_extra_debug_sigprint();
}

template<typename eT> inline SpRow<eT>::SpRow(const SizeMat& s)
...
template<typename eT> inline SpRow<eT>::SpRow(const char* text)
	: SpMat<eT>(arma_vec_indicator(), 2) {
	arma_extra_debug_sigprint();
...
SpRow<eT>& SpRow<eT>::operator=(const char* text) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline SpRow<eT>::SpRow(const std::string& text)
	: SpMat<eT>(arma_vec_indicator(), 2) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
SpRow<eT>& SpRow<eT>::operator=(const std::string& text) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
SpRow<eT>& SpRow<eT>::operator=(const eT val) {
	arma_extra_debug_sigprint();
...
SpRow<eT>& SpRow<eT>::operator=(const Base<eT, T1>& X) {
	arma_extra_debug_sigprint();
...
template<typename eT> template<typename T1> inline
SpRow<eT>& SpRow<eT>::operator=(const SpBase<eT, T1>& X) {
	arma_extra_debug_sigprint();
...
template<typename eT> template<typename T1, typename T2> inline SpRow<eT>::SpRow
(
...
template<typename eT> inline
void SpRow<eT>::shed_col(const uword col_num) {
	arma_extra_debug_sigprint();
...
//! remove specified columns
template<typename eT> inline
void SpRow<eT>::shed_cols(const uword in_col1, const uword in_col2) {
	arma_extra_debug_sigprint();
	...
	if(in_col1 > 0) { arrayops::copy(new_col_ptrs, SpMat<eT>::col_ptrs, in_col1); }
...
template<typename eT> inline
typename SpRow<eT>::row_iterator SpRow<eT>::begin_row(const uword row_num) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
typename SpRow<eT>::const_row_iterator SpRow<eT>::begin_row(const uword row_num) const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
typename SpRow<eT>::row_iterator SpRow<eT>::end_row(const uword row_num) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
typename SpRow<eT>::const_row_iterator SpRow<eT>::end_row(const uword row_num) const {
	arma_extra_debug_sigprint();
