...
namespace newarp {
	...
	template<typename eT, int SelectionRule, typename OpType> inline
	void GenEigsSolver<eT, SelectionRule, OpType>::factorise_from(uword from_k, uword to_m, const Col<eT>& fk) {
	...
	template<typename eT, int SelectionRule, typename OpType> inline
	void GenEigsSolver<eT, SelectionRule, OpType>::restart(uword k) {
		arma_extra_debug_sigprint();
	...
	template<typename eT, int SelectionRule, typename OpType> inline
	uword GenEigsSolver<eT, SelectionRule, OpType>::num_converged(eT tol) {
		arma_extra_debug_sigprint();
	...
	template<typename eT, int SelectionRule, typename OpType> inline
	uword GenEigsSolver<eT, SelectionRule, OpType>::nev_adjusted(uword nconv) {
		arma_extra_debug_sigprint();

		uword nev_new = nev;

		for(uword i = nev; i < ncv; i++) { if(std::abs(ritz_est(i)) < eps) { nev_new++; } }
		// Adjust nev_new again, according to dnaup2.f line 660~674 in ARPACK
		nev_new += std::min(nconv, (ncv - nev_new) / 2);
		if(nev_new == 1 && ncv >= 6) { nev_new = ncv / 2; } else if(nev_new == 1 && ncv > 3) { nev_new = 2; }

		if(nev_new > ncv - 2) { nev_new = ncv - 2; }

		// Increase nev by one if ritz_val[nev - 1] and
		// ritz_val[nev] are conjugate pairs
		if(cx_attrib::is_complex(ritz_val(nev_new - 1), eps) && cx_attrib::is_conj(ritz_val(nev_new - 1), ritz_val(nev_new), eps)) { nev_new++; }

		return nev_new;
	}

	template<typename eT, int SelectionRule, typename OpType> inline
	void GenEigsSolver<eT, SelectionRule, OpType>::retrieve_ritzpair() {
		arma_extra_debug_sigprint();
	...
	template<typename eT, int SelectionRule, typename OpType> inline
	void GenEigsSolver<eT, SelectionRule, OpType>::sort_ritzpair() {
		arma_extra_debug_sigprint();
	...
	template<typename eT, int SelectionRule, typename OpType> inline GenEigsSolver<eT, SelectionRule, OpType>::GenEigsSolver(const OpType& op_, uword nev_, uword ncv_)
		: op(op_)
		, nev(nev_)
		, dim_n(op.n_rows)
	...
	template<typename eT, int SelectionRule, typename OpType> inline
	void GenEigsSolver<eT, SelectionRule, OpType>::init(eT* init_resid) {
		arma_extra_debug_sigprint();
	...
	template<typename eT, int SelectionRule, typename OpType> inline
	void GenEigsSolver<eT, SelectionRule, OpType>::init() {
		arma_extra_debug_sigprint();
	...
	template<typename eT, int SelectionRule, typename OpType> inline
	uword GenEigsSolver<eT, SelectionRule, OpType>::compute(uword maxit, eT tol) {
		arma_extra_debug_sigprint();
	...
	template<typename eT, int SelectionRule, typename OpType> inline
	Col<std::complex<eT>> GenEigsSolver<eT, SelectionRule, OpType>::eigenvalues() {
		arma_extra_debug_sigprint();
	...
	template<typename eT, int SelectionRule, typename OpType> inline
	Mat<std::complex<eT>> GenEigsSolver<eT, SelectionRule, OpType>::eigenvectors(uword nvec) {
		arma_extra_debug_sigprint();
