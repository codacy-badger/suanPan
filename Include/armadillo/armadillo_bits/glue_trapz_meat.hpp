...
void glue_trapz::apply(Mat<typename T1::elem_type>& out, const Glue<T1, T2, glue_trapz>& in) {
	...
	if((&out == &X) || (&out == &Y)) {
		...
		out.steal_mem(tmp);
	} else { glue_trapz::apply_noalias(out, X, Y, dim); }
}

template<typename eT> inline
void glue_trapz::apply_noalias(Mat<eT>& out, const Mat<eT>& X, const Mat<eT>& Y, const uword dim) {
	arma_extra_debug_sigprint();
...
void op_trapz::apply(Mat<typename T1::elem_type>& out, const Op<T1, op_trapz>& in) {
	arma_extra_debug_sigprint();
	...
	if(&out == &Y) {
		...
		out.steal_mem(tmp);
	} else { op_trapz::apply_noalias(out, Y, dim); }
}

template<typename eT> inline
void op_trapz::apply_noalias(Mat<eT>& out, const Mat<eT>& Y, const uword dim) {
	arma_extra_debug_sigprint();
