...
void op_trimat::fill_zeros(Mat<eT>& out, const bool upper) {
...
void op_trimat::apply(Mat<typename T1::elem_type>& out, const Op<T1, op_trimat>& in) {
	arma_extra_debug_sigprint();
...
template<typename T1> inline
void op_trimat::apply(Mat<typename T1::elem_type>& out, const Op<Op<T1, op_htrans>, op_trimat>& in) {
	arma_extra_debug_sigprint();
...
(
	...
	if(upper) {
		...
		for(uword row = 0; row < N; ++row) { for(uword col = row; col < N; ++col) { out.at(col, row) = A.at(row, col); } }
	}
...
(
	...
	if(upper) {
		...
		// Lower triangular: but since we're transposing, we're taking the upper
		// triangular and putting it in the lower half.
		for(uword row = 0; row < N; ++row) { for(uword col = row; col < N; ++col) { out.at(col, row) = std::conj(A.at(row, col)); } }
	}
...
template<typename T1> inline
void op_trimatu_ext::apply(Mat<typename T1::elem_type>& out, const Op<T1, op_trimatu_ext>& in) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
void op_trimatu_ext::fill_zeros(Mat<eT>& out, const uword row_offset, const uword col_offset) {
	arma_extra_debug_sigprint();
	...
	for(uword col = 0; col < col_offset; ++col) { arrayops::fill_zeros(out.colptr(col), n_rows); }

	for(uword i = 0; i < N; ++i) {
		const uword start_row = i + row_offset + 1;
		const uword col = i + col_offset;

		for(uword row = start_row; row < n_rows; ++row) { out.at(row, col) = eT(0); }
	}
}
...
template<typename T1> inline
void op_trimatl_ext::apply(Mat<typename T1::elem_type>& out, const Op<T1, op_trimatl_ext>& in) {
	arma_extra_debug_sigprint();
	...
	if(&out != &A) {
		...
		for(uword col = 0; col < col_offset; ++col) { arrayops::copy(out.colptr(col), A.colptr(col), n_rows); }

		for(uword i = 0; i < N; ++i) {
			const uword start_row = i + row_offset;
			const uword col = i + col_offset;

			for(uword row = start_row; row < n_rows; ++row) { out.at(row, col) = A.at(row, col); }
		}
	}
...
template<typename eT> inline
void op_trimatl_ext::fill_zeros(Mat<eT>& out, const uword row_offset, const uword col_offset) {
	arma_extra_debug_sigprint();
