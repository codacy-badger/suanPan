...
inline
double chi2rnd(const double df) {
...
inline
typename arma_real_only<eT>::result chi2rnd(const eT df) {
	arma_extra_debug_sigprint();
...
<
	(is_Mat<obj_type>::value && is_real<typename obj_type::elem_type>::value),
	obj_type
>::result chi2rnd(const typename obj_type::elem_type df, const SizeMat& s) {
	arma_extra_debug_sigprint();
...
inline
mat chi2rnd(const double df, const uword n_rows, const uword n_cols) {
	arma_extra_debug_sigprint();
...
inline
mat chi2rnd(const double df, const SizeMat& s) {
	arma_extra_debug_sigprint();
...
inline
vec chi2rnd(const double df, const uword n_elem) {
	arma_extra_debug_sigprint();
