...
public:
	...
	static
	void inplace_conj_copy_upper_tri_to_lower_tri(Mat<eT>& C) {
	...
	arma_hot
	inline
	eT dot_conj_row(const uword n_elem, const eT* const A, const Mat<eT>& B, const uword row) {
...
public:
	...
	template<typename T, typename TA> arma_hot
	inline
	static
	void apply
	(
...
public:
	...
	template<typename T, typename TA> inline
	static
	void apply_blas_type(Mat<std::complex<T>>& C, const TA& A, const T alpha = T(1), const T beta = T(0)) {
		arma_extra_debug_sigprint();
	...
	static
	void apply(Mat<eT>& C, const TA& A, const eT alpha = eT(1), const eT beta = eT(0), const typename arma_not_cx<eT>::result* junk = 0) {
		arma_ignore(C);
		arma_ignore(A);
		arma_ignore(alpha);
		arma_ignore(beta);
