...
template<typename eT> inline subview<eT>::~subview() {
	arma_extra_debug_sigprint();
}

template<typename eT> inline subview<eT>::subview(const Mat<eT>& in_m, const uword in_row1, const uword in_col1, const uword in_n_rows, const uword in_n_cols)
	: m(in_m)
	, aux_row1(in_row1)
	, aux_col1(in_col1)
	, n_rows(in_n_rows)
...
void subview<eT>::operator=(const eT val) {
	...
	if(n_elem != 1) { arma_debug_assert_same_size(n_rows, n_cols, 1, 1, "copy into submatrix"); }
...
template<typename eT> template<typename op_type> inline
void subview<eT>::inplace_op(const eT val) {
	arma_extra_debug_sigprint();
	...
	if(s_n_rows == 1) {
		...
		for(jj = 1; jj < s_n_cols; jj += 2) {
			...
			if(is_same_type<op_type, op_internal_minus>::yes) {
				(*Aptr) -= val;
				Aptr += A_n_rows;
				(*Aptr) -= val;
				Aptr += A_n_rows;
			}
			if(is_same_type<op_type, op_internal_schur>::yes) {
				(*Aptr) *= val;
				Aptr += A_n_rows;
				(*Aptr) *= val;
				Aptr += A_n_rows;
			}
			if(is_same_type<op_type, op_internal_div>::yes) {
				(*Aptr) /= val;
				Aptr += A_n_rows;
				(*Aptr) /= val;
				Aptr += A_n_rows;
...
template<typename eT> template<typename op_type, typename T1> inline
void subview<eT>::inplace_op(const Base<eT, T1>& in, const char* identifier) {
	arma_extra_debug_sigprint();
		...
		} else // not a row vector
		{
			if(Proxy<T1>::use_at) {
				...
				for(uword ucol = 0; ucol < s_n_cols; ++ucol) {
					...
					uword jj;
					for(jj = 1; jj < s_n_rows; jj += 2) {
						const eT tmp1 = Pea[count];
						count++;
						const eT tmp2 = Pea[count];
						count++;
...
template<typename eT> template<typename op_type> inline
void subview<eT>::inplace_op(const subview<eT>& x, const char* identifier) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
void subview<eT>::operator=(const subview<eT>& x) {
	arma_extra_debug_sigprint();
...
void subview<eT>::operator=(const Base<eT, T1>& in) {
	arma_extra_debug_sigprint();
...
template<typename eT> template<typename T1> inline
void subview<eT>::operator=(const SpBase<eT, T1>& x) {
	arma_extra_debug_sigprint();
...
typename enable_if2<is_same_type<typename T1::elem_type, eT>::value, void>::result subview<eT>::operator=(const Gen<T1, gen_type>& in) {
	arma_extra_debug_sigprint();
...
void subview<eT>::for_each(functor F) {
	arma_extra_debug_sigprint();
	...
	if(n_rows == 1) {
		...
		for(uword ucol = start_col; ucol < end_col_plus1; ++ucol) { F(X.at(urow, ucol)); }
	} else {
		const uword start_col = aux_col1;
		...
		for(uword ucol = start_col; ucol < end_col_plus1; ++ucol)
			for(uword urow = start_row; urow < end_row_plus1; ++urow) { F(X.at(urow, ucol)); }
	}
}

template<typename eT> template<typename functor> inline
void subview<eT>::for_each(functor F) const {
	arma_extra_debug_sigprint();
...
template<typename eT> template<typename functor> inline
void subview<eT>::transform(functor F) {
	arma_extra_debug_sigprint();
	...
	if(n_rows == 1) {
		...
		for(uword ucol = start_col; ucol < end_col_plus1; ++ucol) { X.at(urow, ucol) = eT(F(X.at(urow, ucol))); }
	} else {
		const uword start_col = aux_col1;
		const uword start_row = aux_row1;
		...
		for(uword ucol = start_col; ucol < end_col_plus1; ++ucol)
			for(uword urow = start_row; urow < end_row_plus1; ++urow) { X.at(urow, ucol) = eT(F(X.at(urow, ucol))); }
	}
}

//! imbue (fill) the subview with values provided by a functor
template<typename eT> template<typename functor> inline
void subview<eT>::imbue(functor F) {
	arma_extra_debug_sigprint();
	...
	if(n_rows == 1) {
		...
		for(uword ucol = start_col; ucol < end_col_plus1; ++ucol) { X.at(urow, ucol) = eT(F()); }
	} else {
		const uword start_col = aux_col1;
		const uword start_row = aux_row1;
		...
		for(uword ucol = start_col; ucol < end_col_plus1; ++ucol)
			for(uword urow = start_row; urow < end_row_plus1; ++urow) { X.at(urow, ucol) = eT(F()); }
	}
}

template<typename eT> inline
void subview<eT>::replace(const eT old_val, const eT new_val) {
	arma_extra_debug_sigprint();
	...
	} else { for(uword ucol = 0; ucol < s_n_cols; ++ucol) { arrayops::replace(s.colptr(ucol), s_n_rows, old_val, new_val); } }
}
...
void subview<eT>::fill(const eT val) {
	...
	if(s_n_rows == 1) {
		...
		if((jj - 1) < s_n_cols) { (*Aptr) = val; }
	} else { for(uword ucol = 0; ucol < s_n_cols; ++ucol) { arrayops::inplace_set(s.colptr(ucol), val, s_n_rows); } }
}
...
template<typename eT> inline
void subview<eT>::randu() {
	arma_extra_debug_sigprint();

	const uword local_n_rows = n_rows;
	const uword local_n_cols = n_cols;

	if(local_n_rows == 1) { for(uword ii = 0; ii < local_n_cols; ++ii) { at(0, ii) = eT(arma_rng::randu<eT>()); } } else { for(uword ii = 0; ii < local_n_cols; ++ii) { arma_rng::randu<eT>::fill(colptr(ii), local_n_rows); } }
}

template<typename eT> inline
void subview<eT>::randn() {
	arma_extra_debug_sigprint();

	const uword local_n_rows = n_rows;
	const uword local_n_cols = n_cols;

	if(local_n_rows == 1) { for(uword ii = 0; ii < local_n_cols; ++ii) { at(0, ii) = eT(arma_rng::randn<eT>()); } } else { for(uword ii = 0; ii < local_n_cols; ++ii) { arma_rng::randn<eT>::fill(colptr(ii), local_n_rows); } }
}

template<typename eT> inline
eT subview<eT>::at_alt(const uword ii) const { return operator[](ii); }

template<typename eT> inline
eT& subview<eT>::operator[](const uword ii) {
...
template<typename eT> inline
eT subview<eT>::operator[](const uword ii) const {
	const uword in_col = ii / n_rows;
	const uword in_row = ii % n_rows;
...
eT* subview<eT>::colptr(const uword in_col) { return & access::rw((const_cast<Mat<eT>&>(m)).mem[(in_col + aux_col1) * m.n_rows + aux_row1]); }

template<typename eT> arma_inline
const eT* subview<eT>::colptr(const uword in_col) const { return & m.mem[(in_col + aux_col1) * m.n_rows + aux_row1]; }

template<typename eT> template<typename eT2> inline
bool subview<eT>::check_overlap(const subview<eT2>& x) const {
...
template<typename eT> inline
arma_warn_unused
bool subview<eT>::is_vec() const { return ((n_rows == 1) || (n_cols == 1)); }

template<typename eT> inline
arma_warn_unused
bool subview<eT>::is_finite() const {
	arma_extra_debug_sigprint();

	const uword local_n_rows = n_rows;
	const uword local_n_cols = n_cols;

	for(uword ii = 0; ii < local_n_cols; ++ii) { if(arrayops::is_finite(colptr(ii), local_n_rows) == false) { return false; } }
...
template<typename eT> inline
arma_warn_unused
bool subview<eT>::has_inf() const {
	arma_extra_debug_sigprint();

	const uword local_n_rows = n_rows;
	const uword local_n_cols = n_cols;

	for(uword ii = 0; ii < local_n_cols; ++ii) { if(arrayops::has_inf(colptr(ii), local_n_rows)) { return true; } }
...
template<typename eT> inline
arma_warn_unused
bool subview<eT>::has_nan() const {
	arma_extra_debug_sigprint();

	const uword local_n_rows = n_rows;
	const uword local_n_cols = n_cols;

	for(uword ii = 0; ii < local_n_cols; ++ii) { if(arrayops::has_nan(colptr(ii), local_n_rows)) { return true; } }
...
//! X = Y.submat(...)
template<typename eT> inline
void subview<eT>::extract(Mat<eT>& out, const subview<eT>& in) {
	arma_extra_debug_sigprint();
	...
	if(in.is_vec() == true) {
		...
		} else // a row vector (possibly empty)
		{
			...
			if((j - 1) < n_cols) { (*out_mem) = (*Xptr); }
		}
	} else // general submatrix
	{
		arma_extra_debug_print("subview::extract(): general submatrix");

		for(uword col = 0; col < n_cols; ++col) { arrayops::copy(out.colptr(col), in.colptr(col), n_rows); }
	}
}

//! X += Y.submat(...)
template<typename eT> inline
void subview<eT>::plus_inplace(Mat<eT>& out, const subview<eT>& in) {
	arma_extra_debug_sigprint();
	...
	if(n_rows == 1) {
		...
		if(i < n_cols) { out_mem[i] += X.at(row, start_col + i); }
	} else { for(uword col = 0; col < n_cols; ++col) { arrayops::inplace_plus(out.colptr(col), in.colptr(col), n_rows); } }
}

//! X -= Y.submat(...)
template<typename eT> inline
void subview<eT>::minus_inplace(Mat<eT>& out, const subview<eT>& in) {
	arma_extra_debug_sigprint();
	...
	if(n_rows == 1) {
		...
		if(i < n_cols) { out_mem[i] -= X.at(row, start_col + i); }
	} else { for(uword col = 0; col < n_cols; ++col) { arrayops::inplace_minus(out.colptr(col), in.colptr(col), n_rows); } }
}

//! X %= Y.submat(...)
template<typename eT> inline
void subview<eT>::schur_inplace(Mat<eT>& out, const subview<eT>& in) {
	arma_extra_debug_sigprint();
	...
	if(n_rows == 1) {
		...
		if(i < n_cols) { out_mem[i] *= X.at(row, start_col + i); }
	} else { for(uword col = 0; col < n_cols; ++col) { arrayops::inplace_mul(out.colptr(col), in.colptr(col), n_rows); } }
}

//! X /= Y.submat(...)
template<typename eT> inline
void subview<eT>::div_inplace(Mat<eT>& out, const subview<eT>& in) {
	arma_extra_debug_sigprint();
	...
	if(n_rows == 1) {
		...
		if(i < n_cols) { out_mem[i] /= X.at(row, start_col + i); }
	} else { for(uword col = 0; col < n_cols; ++col) { arrayops::inplace_div(out.colptr(col), in.colptr(col), n_rows); } }
}
...
template<typename eT> inline
subview_each1<subview<eT>, 0> subview<eT>::each_col() {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
subview_each1<subview<eT>, 1> subview<eT>::each_row() {
	arma_extra_debug_sigprint();
...
template<typename eT> template<typename T1> inline
subview_each2<subview<eT>, 0, T1> subview<eT>::each_col(const Base<uword, T1>& indices) {
	arma_extra_debug_sigprint();
...
template<typename eT> template<typename T1> inline
subview_each2<subview<eT>, 1, T1> subview<eT>::each_row(const Base<uword, T1>& indices) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
void subview<eT>::each_col(const std::function<void(Col<eT>&)>& F) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
void subview<eT>::each_col(const std::function<void(const Col<eT>&)>& F) const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
void subview<eT>::each_row(const std::function<void(Row<eT>&)>& F) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
void subview<eT>::each_row(const std::function<void(const Row<eT>&)>& F) const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
diagview<eT> subview<eT>::diag(const sword in_id) {
	arma_extra_debug_sigprint();
...
//! creation of diagview (diagonal)
template<typename eT> inline
const diagview<eT> subview<eT>::diag(const sword in_id) const {
	arma_extra_debug_sigprint();
...
void subview<eT>::swap_cols(const uword in_col1, const uword in_col2) {
	...
	if(n_elem > 0) {
		...
		for(uword urow = 0; urow < n_rows; ++urow) { std::swap(ptr1[urow], ptr2[urow]); }
	}
}

template<typename eT> inline
typename subview<eT>::iterator subview<eT>::begin() { return iterator(*this, aux_row1, aux_col1); }

template<typename eT> inline
typename subview<eT>::const_iterator subview<eT>::begin() const { return const_iterator(*this, aux_row1, aux_col1); }

template<typename eT> inline
typename subview<eT>::const_iterator subview<eT>::cbegin() const { return const_iterator(*this, aux_row1, aux_col1); }

template<typename eT> inline
typename subview<eT>::iterator subview<eT>::end() { return iterator(*this, aux_row1, aux_col1 + n_cols); }

template<typename eT> inline
typename subview<eT>::const_iterator subview<eT>::end() const { return const_iterator(*this, aux_row1, aux_col1 + n_cols); }

template<typename eT> inline
typename subview<eT>::const_iterator subview<eT>::cend() const { return const_iterator(*this, aux_row1, aux_col1 + n_cols); }

//
//
//

template<typename eT> inline subview<eT>::iterator::iterator()
	: M(NULL)
	, current_ptr(NULL)
	, current_row(0)
	, current_col(0)
...
template<typename eT> inline subview<eT>::iterator::iterator(const iterator& X)
	: M(X.M)
...
template<typename eT> inline subview<eT>::iterator::iterator(subview<eT>& in_sv, const uword in_row, const uword in_col)
...
template<typename eT> inline
arma_warn_unused
eT& subview<eT>::iterator::operator*() { return (*current_ptr); }

template<typename eT> inline
typename subview<eT>::iterator& subview<eT>::iterator::operator++() {
	current_row++;
...
template<typename eT> inline
arma_warn_unused
typename subview<eT>::iterator subview<eT>::iterator::operator++(int) {
...
template<typename eT> inline
arma_warn_unused
bool subview<eT>::iterator::operator==(const iterator& rhs) const { return (current_ptr == rhs.current_ptr); }
...
template<typename eT> inline
arma_warn_unused
bool subview<eT>::iterator::operator==(const const_iterator& rhs) const { return (current_ptr == rhs.current_ptr); }
...
template<typename eT> inline subview<eT>::const_iterator::const_iterator()
	: M(NULL)
	, current_ptr(NULL)
	, current_row(0)
	, current_col(0)
...
template<typename eT> inline subview<eT>::const_iterator::const_iterator(const iterator& X)
	: M(X.M)
	, current_ptr(X.current_ptr)
	, current_row(X.current_row)
	, current_col(X.current_col)
...
template<typename eT> inline subview<eT>::const_iterator::const_iterator(const const_iterator& X)
	: M(X.M)
	, current_ptr(X.current_ptr)
	, current_row(X.current_row)
	, current_col(X.current_col)
...
template<typename eT> inline subview<eT>::const_iterator::const_iterator(const subview<eT>& in_sv, const uword in_row, const uword in_col)
	: M(&(in_sv.m))
	, current_ptr(&(M->at(in_row, in_col)))
	, current_row(in_row)
	, current_col(in_col)
...
template<typename eT> inline
arma_warn_unused
const eT& subview<eT>::const_iterator::operator*() { return (*current_ptr); }

template<typename eT> inline
typename subview<eT>::const_iterator& subview<eT>::const_iterator::operator++() {
	current_row++;
...
template<typename eT> inline
arma_warn_unused
typename subview<eT>::const_iterator subview<eT>::const_iterator::operator++(int) {
...
template<typename eT> inline
arma_warn_unused
bool subview<eT>::const_iterator::operator==(const iterator& rhs) const { return (current_ptr == rhs.current_ptr); }
...
template<typename eT> inline
arma_warn_unused
bool subview<eT>::const_iterator::operator==(const const_iterator& rhs) const { return (current_ptr == rhs.current_ptr); }
...
template<typename eT> inline subview<eT>::row_iterator::row_iterator()
	: M(NULL)
	, current_ptr(NULL)
	, current_row(0)
	, current_col(0)
...
template<typename eT> inline subview<eT>::row_iterator::row_iterator(const row_iterator& X)
	: M(X.M)
	, current_ptr(X.current_ptr)
	, current_row(X.current_row)
	, current_col(X.current_col)
...
template<typename eT> inline subview<eT>::row_iterator::row_iterator(subview<eT>& in_sv, const uword in_row, const uword in_col)
	: M(&(const_cast<Mat<eT>&>(in_sv.m)))
	, current_ptr(&(M->at(in_row, in_col)))
	, current_row(in_row)
	, current_col(in_col)
...
template<typename eT> inline
arma_warn_unused
eT& subview<eT>::row_iterator::operator*() { return (*current_ptr); }

template<typename eT> inline
typename subview<eT>::row_iterator& subview<eT>::row_iterator::operator++() {
	current_col++;
...
template<typename eT> inline
arma_warn_unused
typename subview<eT>::row_iterator subview<eT>::row_iterator::operator++(int) {
...
template<typename eT> inline
arma_warn_unused
bool subview<eT>::row_iterator::operator==(const row_iterator& rhs) const { return (current_ptr == rhs.current_ptr); }
...
template<typename eT> inline
arma_warn_unused
bool subview<eT>::row_iterator::operator==(const const_row_iterator& rhs) const { return (current_ptr == rhs.current_ptr); }
...
template<typename eT> inline subview<eT>::const_row_iterator::const_row_iterator()
	: M(NULL)
	, current_ptr(NULL)
	, current_row(0)
	, current_col(0)
...
template<typename eT> inline subview<eT>::const_row_iterator::const_row_iterator(const row_iterator& X)
	: M(X.M)
	, current_ptr(X.current_ptr)
	, current_row(X.current_row)
	, current_col(X.current_col)
...
template<typename eT> inline subview<eT>::const_row_iterator::const_row_iterator(const const_row_iterator& X)
	: M(X.M)
	, current_ptr(X.current_ptr)
	, current_row(X.current_row)
	, current_col(X.current_col)
...
template<typename eT> inline subview<eT>::const_row_iterator::const_row_iterator(const subview<eT>& in_sv, const uword in_row, const uword in_col)
	: M(&(in_sv.m))
	, current_ptr(&(M->at(in_row, in_col)))
	, current_row(in_row)
	, current_col(in_col)
...
template<typename eT> inline
arma_warn_unused
const eT& subview<eT>::const_row_iterator::operator*() const { return (*current_ptr); }

template<typename eT> inline
typename subview<eT>::const_row_iterator& subview<eT>::const_row_iterator::operator++() {
	current_col++;
...
template<typename eT> inline
arma_warn_unused
typename subview<eT>::const_row_iterator subview<eT>::const_row_iterator::operator++(int) {
...
template<typename eT> inline
arma_warn_unused
bool subview<eT>::const_row_iterator::operator==(const row_iterator& rhs) const { return (current_ptr == rhs.current_ptr); }
...
template<typename eT> inline
arma_warn_unused
bool subview<eT>::const_row_iterator::operator==(const const_row_iterator& rhs) const { return (current_ptr == rhs.current_ptr); }
...
template<typename eT> inline subview_col<eT>::subview_col(const Mat<eT>& in_m, const uword in_col)
	: subview<eT>(in_m, 0, in_col, in_m.n_rows, 1)
	, colmem(subview<eT>::colptr(0)) {
	arma_extra_debug_sigprint();
}

template<typename eT> inline subview_col<eT>::subview_col(const Mat<eT>& in_m, const uword in_col, const uword in_row1, const uword in_n_rows)
...
template<typename eT> inline
void subview_col<eT>::operator=(const subview_col<eT>& X) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
void subview_col<eT>::operator=(const eT val) {
	arma_extra_debug_sigprint();

	if(subview<eT>::n_elem != 1) { arma_debug_assert_same_size(subview<eT>::n_rows, subview<eT>::n_cols, 1, 1, "copy into submatrix"); }
...
template<typename eT> template<typename T1, typename gen_type> inline
typename enable_if2<is_same_type<typename T1::elem_type, eT>::value, void>::result subview_col<eT>::operator=(const Gen<T1, gen_type>& in) {
	arma_extra_debug_sigprint();
...
template<typename eT> arma_inline
const Op<subview_col<eT>, op_htrans> subview_col<eT>::t() const { return Op<subview_col<eT>, op_htrans>(*this); }

template<typename eT> arma_inline
const Op<subview_col<eT>, op_htrans> subview_col<eT>::ht() const { return Op<subview_col<eT>, op_htrans>(*this); }

template<typename eT> arma_inline
const Op<subview_col<eT>, op_strans> subview_col<eT>::st() const { return Op<subview_col<eT>, op_strans>(*this); }

template<typename eT> inline
void subview_col<eT>::fill(const eT val) {
	arma_extra_debug_sigprint();
...
template<typename eT> arma_inline
eT subview_col<eT>::at_alt(const uword ii) const {
...
template<typename eT> arma_inline
eT& subview_col<eT>::operator[](const uword ii) { return access::rw(colmem[ii]); }

template<typename eT> arma_inline
eT subview_col<eT>::operator[](const uword ii) const { return colmem[ii]; }

template<typename eT> inline
eT& subview_col<eT>::operator()(const uword ii) {
...
template<typename eT> inline
eT subview_col<eT>::operator()(const uword ii) const {
	arma_debug_check((ii >= subview<eT>::n_elem), "subview::operator(): index out of bounds");
...
template<typename eT> inline
eT& subview_col<eT>::operator()(const uword in_row, const uword in_col) {
...
template<typename eT> inline
eT subview_col<eT>::operator()(const uword in_row, const uword in_col) const {
	arma_debug_check(((in_row >= subview<eT>::n_rows) || (in_col > 0)), "subview::operator(): index out of bounds");
...
template<typename eT> inline
eT& subview_col<eT>::at(const uword in_row, const uword) { return access::rw(colmem[in_row]); }

template<typename eT> inline
eT subview_col<eT>::at(const uword in_row, const uword) const { return colmem[in_row]; }

template<typename eT> arma_inline
eT* subview_col<eT>::colptr(const uword) { return const_cast<eT*>(colmem); }

template<typename eT> arma_inline
const eT* subview_col<eT>::colptr(const uword) const { return colmem; }
...
template<typename eT> inline
subview_col<eT> subview_col<eT>::subvec(const uword start_row, const SizeMat& s) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
const subview_col<eT> subview_col<eT>::subvec(const uword start_row, const SizeMat& s) const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
subview_col<eT> subview_col<eT>::head(const uword N) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
const subview_col<eT> subview_col<eT>::head(const uword N) const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
subview_col<eT> subview_col<eT>::tail(const uword N) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
const subview_col<eT> subview_col<eT>::tail(const uword N) const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
eT subview_col<eT>::min(uword& index_of_min_val) const {
	arma_extra_debug_sigprint();
	...
	if(subview<eT>::n_elem == 0) {
		...
		return Datum<eT>::nan;
	} else { return op_min::direct_min(colmem, subview<eT>::n_elem, index_of_min_val); }
}

template<typename eT> inline
eT subview_col<eT>::max(uword& index_of_max_val) const {
	arma_extra_debug_sigprint();
	...
	if(subview<eT>::n_elem == 0) {
		...
		return Datum<eT>::nan;
	} else { return op_max::direct_max(colmem, subview<eT>::n_elem, index_of_max_val); }
}

template<typename eT> inline
arma_warn_unused
uword subview_col<eT>::index_min() const {
	arma_extra_debug_sigprint();

	uword index = 0;

	if(subview<eT>::n_elem == 0) { arma_debug_check(true, "index_min(): object has no elements"); } else { op_min::direct_min(colmem, subview<eT>::n_elem, index); }
...
template<typename eT> inline
arma_warn_unused
uword subview_col<eT>::index_max() const {
	arma_extra_debug_sigprint();

	uword index = 0;

	if(subview<eT>::n_elem == 0) { arma_debug_check(true, "index_max(): object has no elements"); } else { op_max::direct_max(colmem, subview<eT>::n_elem, index); }
...
template<typename eT> inline subview_row<eT>::subview_row(const Mat<eT>& in_m, const uword in_row)
	: subview<eT>(in_m, in_row, 0, 1, in_m.n_cols) {
	arma_extra_debug_sigprint();
}

template<typename eT> inline subview_row<eT>::subview_row(const Mat<eT>& in_m, const uword in_row, const uword in_col1, const uword in_n_cols)
...
template<typename eT> inline
void subview_row<eT>::operator=(const subview_row<eT>& X) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
void subview_row<eT>::operator=(const eT val) {
	arma_extra_debug_sigprint();
...
template<typename eT> template<typename T1, typename gen_type> inline
typename enable_if2<is_same_type<typename T1::elem_type, eT>::value, void>::result subview_row<eT>::operator=(const Gen<T1, gen_type>& in) {
	arma_extra_debug_sigprint();
...
template<typename eT> arma_inline
const Op<subview_row<eT>, op_htrans> subview_row<eT>::t() const { return Op<subview_row<eT>, op_htrans>(*this); }

template<typename eT> arma_inline
const Op<subview_row<eT>, op_htrans> subview_row<eT>::ht() const { return Op<subview_row<eT>, op_htrans>(*this); }

template<typename eT> arma_inline
const Op<subview_row<eT>, op_strans> subview_row<eT>::st() const { return Op<subview_row<eT>, op_strans>(*this); }

template<typename eT> inline
eT subview_row<eT>::at_alt(const uword ii) const {
...
template<typename eT> inline
eT& subview_row<eT>::operator[](const uword ii) {
	const uword index = (ii + (subview<eT>::aux_col1)) * (subview<eT>::m).n_rows + (subview<eT>::aux_row1);
...
template<typename eT> inline
eT subview_row<eT>::operator[](const uword ii) const {
	const uword index = (ii + (subview<eT>::aux_col1)) * (subview<eT>::m).n_rows + (subview<eT>::aux_row1);
...
template<typename eT> inline
eT& subview_row<eT>::operator()(const uword in_row, const uword in_col) {
...
template<typename eT> inline
eT subview_row<eT>::operator()(const uword in_row, const uword in_col) const {
	arma_debug_check(((in_row > 0) || (in_col >= subview<eT>::n_cols)), "subview::operator(): index out of bounds");
...
template<typename eT> inline
eT& subview_row<eT>::at(const uword, const uword in_col) {
	const uword index = (in_col + (subview<eT>::aux_col1)) * (subview<eT>::m).n_rows + (subview<eT>::aux_row1);
...
template<typename eT> inline
eT subview_row<eT>::at(const uword, const uword in_col) const {
	const uword index = (in_col + (subview<eT>::aux_col1)) * (subview<eT>::m).n_rows + (subview<eT>::aux_row1);
...
template<typename eT> inline
subview_row<eT> subview_row<eT>::subvec(const uword start_col, const SizeMat& s) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
const subview_row<eT> subview_row<eT>::subvec(const uword start_col, const SizeMat& s) const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
subview_row<eT> subview_row<eT>::head(const uword N) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
const subview_row<eT> subview_row<eT>::head(const uword N) const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
subview_row<eT> subview_row<eT>::tail(const uword N) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
const subview_row<eT> subview_row<eT>::tail(const uword N) const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
arma_warn_unused
uword subview_row<eT>::index_min() const {
...
template<typename eT> inline
arma_warn_unused
uword subview_row<eT>::index_max() const {
	const Proxy<subview_row<eT>> P(*this);
...
template<typename eT> inline
typename subview<eT>::row_iterator subview_row<eT>::begin() { return typename subview<eT>::row_iterator(*this, subview<eT>::aux_row1, subview<eT>::aux_col1); }

template<typename eT> inline
typename subview<eT>::const_row_iterator subview_row<eT>::begin() const { return typename subview<eT>::const_row_iterator(*this, subview<eT>::aux_row1, subview<eT>::aux_col1); }

template<typename eT> inline
typename subview<eT>::const_row_iterator subview_row<eT>::cbegin() const { return typename subview<eT>::const_row_iterator(*this, subview<eT>::aux_row1, subview<eT>::aux_col1); }

template<typename eT> inline
typename subview<eT>::row_iterator subview_row<eT>::end() { return typename subview<eT>::row_iterator(*this, subview<eT>::aux_row1 + subview<eT>::n_rows, subview<eT>::aux_col1); }

template<typename eT> inline
typename subview<eT>::const_row_iterator subview_row<eT>::end() const { return typename subview<eT>::const_row_iterator(*this, subview<eT>::aux_row1 + subview<eT>::n_rows, subview<eT>::aux_col1); }

template<typename eT> inline
typename subview<eT>::const_row_iterator subview_row<eT>::cend() const { return typename subview<eT>::const_row_iterator(*this, subview<eT>::aux_row1 + subview<eT>::n_rows, subview<eT>::aux_col1); }

//
//
//

template<typename eT> inline subview_row_strans<eT>::subview_row_strans(const subview_row<eT>& in_sv_row)
	: sv_row(in_sv_row)
...
template<typename eT> inline
void subview_row_strans<eT>::extract(Mat<eT>& out) const {
	arma_extra_debug_sigprint();
	...
	if(ii < sv_row_n_cols) { out_mem[ii] = X.at(row, start_col + ii); }
}

template<typename eT> inline
eT subview_row_strans<eT>::at_alt(const uword ii) const { return sv_row[ii]; }

template<typename eT> inline
eT subview_row_strans<eT>::operator[](const uword ii) const { return sv_row[ii]; }

template<typename eT> inline
eT subview_row_strans<eT>::operator()(const uword ii) const { return sv_row(ii); }

template<typename eT> inline
eT subview_row_strans<eT>::operator()(const uword in_row, const uword in_col) const {
	return sv_row(in_col, in_row); // deliberately swapped
}

template<typename eT> inline
eT subview_row_strans<eT>::at(const uword in_row, const uword) const {
...
template<typename eT> inline subview_row_htrans<eT>::subview_row_htrans(const subview_row<eT>& in_sv_row)
	: sv_row(in_sv_row)
	, n_rows(in_sv_row.n_cols)
	, n_elem(in_sv_row.n_elem) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
void subview_row_htrans<eT>::extract(Mat<eT>& out) const {
	arma_extra_debug_sigprint();
	...
	for(uword ii = 0; ii < sv_row_n_cols; ++ii) { out_mem[ii] = access::alt_conj(X.at(row, start_col + ii)); }
}

template<typename eT> inline
eT subview_row_htrans<eT>::at_alt(const uword ii) const { return access::alt_conj(sv_row[ii]); }

template<typename eT> inline
eT subview_row_htrans<eT>::operator[](const uword ii) const { return access::alt_conj(sv_row[ii]); }

template<typename eT> inline
eT subview_row_htrans<eT>::operator()(const uword ii) const { return access::alt_conj(sv_row(ii)); }

template<typename eT> inline
eT subview_row_htrans<eT>::operator()(const uword in_row, const uword in_col) const {
	return access::alt_conj(sv_row(in_col, in_row)); // deliberately swapped
}

template<typename eT> inline
eT subview_row_htrans<eT>::at(const uword in_row, const uword) const {
