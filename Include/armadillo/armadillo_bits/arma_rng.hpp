...
inline
void arma_rng::set_seed(const arma_rng::seed_type val) {
#if   defined(ARMA_RNG_ALT)
    {
...
template<typename eT> struct arma_rng::randi {
	...
	inline
	static
	void fill(eT* mem, const uword N, const int a, const int b) {
#if   defined(ARMA_RNG_ALT)
      {
...
template<typename eT> struct arma_rng::randu {
	...
	inline
	static
	void fill(eT* mem, const uword N) {
		uword j;
...
template<typename T> struct arma_rng::randu<std::complex<T>> {
	...
	inline
	static
	void fill(std::complex<T>* mem, const uword N) {
		for(uword i = 0; i < N; ++i) {
			const T a = T(arma_rng::randu<T>());
			const T b = T(arma_rng::randu<T>());
...
template<typename eT> struct arma_rng::randn {
	...
	inline
	static
	void fill_simple(eT* mem, const uword N) {
		uword i, j;
...
template<typename T> struct arma_rng::randn<std::complex<T>> {
	...
	inline
	static
	void fill_simple(std::complex<T>* mem, const uword N) { for(uword i = 0; i < N; ++i) { mem[i] = std::complex<T>(arma_rng::randn<std::complex<T>>()); } }
