...
inline
void spglue_minus::apply(SpMat<typename T1::elem_type>& out, const SpGlue<T1, T2, spglue_minus>& X) {
...
template<typename eT, typename T1, typename T2> arma_hot
inline
void spglue_minus::apply_noalias(SpMat<eT>& out, const SpProxy<T1>& pa, const SpProxy<T2>& pb) {
	arma_extra_debug_sigprint();
...
inline
void spglue_minus::apply_noalias(SpMat<eT>& out, const SpMat<eT>& A, const SpMat<eT>& B) {
	arma_extra_debug_sigprint();
...
void spglue_minus_mixed::sparse_minus_sparse(SpMat<typename promote_type<typename T1::elem_type, typename T2::elem_type>::result>& out, const T1& X, const T2& Y) {
	arma_extra_debug_sigprint();
...
template<typename T1, typename T2> inline
void spglue_minus_mixed::sparse_minus_dense(Mat<typename promote_type<typename T1::elem_type, typename T2::elem_type>::result>& out, const T1& X, const T2& Y) {
	arma_extra_debug_sigprint();
	...
	for(uword i = 0; i < B_n_elem; ++i) { out_mem[i] = out_eT(-B_mem[i]); }
...
template<typename T1, typename T2> inline
void spglue_minus_mixed::dense_minus_sparse(Mat<typename promote_type<typename T1::elem_type, typename T2::elem_type>::result>& out, const T1& X, const T2& Y) {
	arma_extra_debug_sigprint();
