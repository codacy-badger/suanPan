...
typename enable_if2<is_arma_type<T1>::value, const eOp<T1, eop_scalar_times>>::result operator*
...
//! scalar * trans(T1)
template<typename T1> arma_inline
const Op<T1, op_htrans2> operator*
...
Mat<typename promote_type<typename T1::elem_type, typename T2::elem_type>::result> operator*
