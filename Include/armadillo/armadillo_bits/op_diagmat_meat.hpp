...
void op_diagmat::apply(Mat<typename T1::elem_type>& out, const Op<T1, op_diagmat>& X) {
	...
	if(P.is_alias(out) == false) {
		if(P_is_vec) // generate a diagonal matrix out of a vector
		{
			...
			} else { if(n_rows == 1) { for(uword i = 0; i < N; ++i) { out.at(i, i) = P.at(0, i); } } else { for(uword i = 0; i < N; ++i) { out.at(i, i) = P.at(i, 0); } } }
	...
	} else // we have aliasing
	{
		if(P_is_vec) // generate a diagonal matrix out of a vector
		{
			...
			} else { if(n_rows == 1) { for(uword i = 0; i < N; ++i) { tmp_mem[i] = P.at(0, i); } } else { for(uword i = 0; i < N; ++i) { tmp_mem[i] = P.at(i, 0); } } }
...
template<typename T1> inline
void op_diagmat2::apply(Mat<typename T1::elem_type>& out, const Proxy<T1>& P, const uword row_offset, const uword col_offset) {
	arma_extra_debug_sigprint();
...
template<typename T1> inline
void op_diagmat2::apply(Mat<typename T1::elem_type>& out, const Op<T1, op_diagmat2>& X) {
	arma_extra_debug_sigprint();
