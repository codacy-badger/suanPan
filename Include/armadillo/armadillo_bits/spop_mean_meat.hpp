...
void spop_mean::apply(SpMat<typename T1::elem_type>& out, const SpOp<T1, spop_mean>& in) {
...
(
	...
	if(out.is_finite() == false) { spop_mean::apply_noalias_slow(out, p, dim); }
}

template<typename T1> inline
void spop_mean::apply_noalias_slow
(
	SpMat<typename T1::elem_type>& out,
	const SpProxy<T1>& p,
	const uword dim
...
template<typename eT> inline
eT spop_mean::direct_mean_robust
(
	const eT* const X,
	const uword length,
	const uword N
...
template<typename T1> inline
typename T1::elem_type spop_mean::mean_all(const SpBase<typename T1::elem_type, T1>& X) {
	arma_extra_debug_sigprint();
...
eT spop_mean::iterator_mean(T1& it, const T1& end, const uword n_zero, const eT junk) {
	arma_extra_debug_sigprint();
	arma_ignore(junk);
...
template<typename T1, typename eT> inline
eT spop_mean::iterator_mean_robust(T1& it, const T1& end, const uword n_zero, const eT junk) {
	arma_extra_debug_sigprint();
	arma_ignore(junk);
