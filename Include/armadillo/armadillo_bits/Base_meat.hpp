...
const derived& Base<elem_type, derived>::get_ref() const { return static_cast<const derived&>(*this); }
...
arma_warn_unused
elem_type Base<elem_type, derived>::min() const { return op_min::min((*this).get_ref()); }

template<typename elem_type, typename derived> inline
arma_warn_unused
elem_type Base<elem_type, derived>::max() const { return op_max::max((*this).get_ref()); }

template<typename elem_type, typename derived> inline
elem_type Base<elem_type, derived>::min(uword& index_of_min_val) const {
	const Proxy<derived> P((*this).get_ref());
...
template<typename elem_type, typename derived> inline
elem_type Base<elem_type, derived>::max(uword& index_of_max_val) const {
	const Proxy<derived> P((*this).get_ref());
...
template<typename elem_type, typename derived> inline
elem_type Base<elem_type, derived>::min(uword& row_of_min_val, uword& col_of_min_val) const {
	const Proxy<derived> P((*this).get_ref());
...
template<typename elem_type, typename derived> inline
elem_type Base<elem_type, derived>::max(uword& row_of_max_val, uword& col_of_max_val) const {
	const Proxy<derived> P((*this).get_ref());
...
template<typename elem_type, typename derived> inline
arma_warn_unused
uword Base<elem_type, derived>::index_min() const {
	const Proxy<derived> P((*this).get_ref());
...
template<typename elem_type, typename derived> inline
arma_warn_unused
uword Base<elem_type, derived>::index_max() const {
	const Proxy<derived> P((*this).get_ref());
...
template<typename elem_type, typename derived> inline
arma_warn_unused
bool Base<elem_type, derived>::is_symmetric() const {
...
template<typename elem_type, typename derived> inline
arma_warn_unused
bool Base<elem_type, derived>::is_symmetric(const typename get_pod_type<elem_type>::result tol) const {
	arma_extra_debug_sigprint();
...
template<typename elem_type, typename derived> inline
arma_warn_unused
bool Base<elem_type, derived>::is_hermitian() const {
	arma_extra_debug_sigprint();
...
template<typename elem_type, typename derived> inline
arma_warn_unused
bool Base<elem_type, derived>::is_hermitian(const typename get_pod_type<elem_type>::result tol) const {
	arma_extra_debug_sigprint();
...
template<typename elem_type, typename derived> inline
arma_warn_unused
bool Base<elem_type, derived>::is_empty() const {
	arma_extra_debug_sigprint();
...
template<typename elem_type, typename derived> inline
arma_warn_unused
bool Base<elem_type, derived>::is_square() const {
	arma_extra_debug_sigprint();
...
template<typename elem_type, typename derived> inline
arma_warn_unused
bool Base<elem_type, derived>::is_vec() const {
	arma_extra_debug_sigprint();
...
template<typename elem_type, typename derived> inline
arma_warn_unused
bool Base<elem_type, derived>::is_colvec() const {
	arma_extra_debug_sigprint();
...
template<typename elem_type, typename derived> inline
arma_warn_unused
bool Base<elem_type, derived>::is_rowvec() const {
	arma_extra_debug_sigprint();
...
template<typename derived> arma_inline
const Op<derived, op_inv> Base_inv_yes<derived>::i() const { return Op<derived, op_inv>(static_cast<const derived&>(*this)); }

template<typename derived> arma_deprecated
inline
const Op<derived, op_inv> Base_inv_yes<derived>::i(const bool) const // argument kept only for compatibility with old user code
{
...
template<typename derived> arma_deprecated
inline
const Op<derived, op_inv> Base_inv_yes<derived>::i(const char*) const // argument kept only for compatibility with old user code
{
...
template<typename elem_type, typename derived> arma_inline
const derived& Base_eval_Mat<elem_type, derived>::eval() const {
	arma_extra_debug_sigprint();
...
template<typename elem_type, typename derived> arma_inline
Mat<elem_type> Base_eval_expr<elem_type, derived>::eval() const {
	arma_extra_debug_sigprint();
...
template<typename derived> arma_inline
const Op<derived, op_htrans> Base_trans_cx<derived>::t() const { return Op<derived, op_htrans>(static_cast<const derived&>(*this)); }

template<typename derived> arma_inline
const Op<derived, op_htrans> Base_trans_cx<derived>::ht() const { return Op<derived, op_htrans>(static_cast<const derived&>(*this)); }

template<typename derived> arma_inline
const Op<derived, op_strans> Base_trans_cx<derived>::st() const { return Op<derived, op_strans>(static_cast<const derived&>(*this)); }

//
// extra functions defined in Base_trans_default

template<typename derived> arma_inline
const Op<derived, op_htrans> Base_trans_default<derived>::t() const { return Op<derived, op_htrans>(static_cast<const derived&>(*this)); }

template<typename derived> arma_inline
const Op<derived, op_htrans> Base_trans_default<derived>::ht() const { return Op<derived, op_htrans>(static_cast<const derived&>(*this)); }

template<typename derived> arma_inline
const Op<derived, op_htrans> Base_trans_default<derived>::st() const { return Op<derived, op_htrans>(static_cast<const derived&>(*this)); }
