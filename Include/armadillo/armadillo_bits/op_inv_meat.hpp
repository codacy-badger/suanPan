...
void op_inv::apply(Mat<typename T1::elem_type>& out, const Op<T1, op_inv>& X) {
	...
	if(strip.do_diagmat) { op_inv::apply_diagmat(out, strip.M); } else {
		...
		if(U.is_alias(out)) {
			...
			out.steal_mem(tmp);
		} else { op_inv::apply_noalias(out, U.M); }
	}
}

template<typename eT> inline
void op_inv::apply_noalias(Mat<eT>& out, const Mat<eT>& A) {
	arma_extra_debug_sigprint();
	...
	if(A.n_rows <= 4) { status = auxlib::inv_tiny(out, A); } else if(sympd_helper::guess_sympd(A)) { status = auxlib::inv_sympd(out, A); }
...
template<typename T1> inline
void op_inv::apply_diagmat(Mat<typename T1::elem_type>& out, const T1& X) {
	arma_extra_debug_sigprint();
...
template<typename T1> inline
void op_inv_tr::apply(Mat<typename T1::elem_type>& out, const Op<T1, op_inv_tr>& X) {
	arma_extra_debug_sigprint();
...
template<typename T1> inline
void op_inv_sympd::apply(Mat<typename T1::elem_type>& out, const Op<T1, op_inv_sympd>& X) {
	arma_extra_debug_sigprint();
