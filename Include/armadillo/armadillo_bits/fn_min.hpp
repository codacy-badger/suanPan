...
template<typename T1> arma_warn_unused
inline
typename T1::elem_type min(const Op<T1, op_min>& in) {
	arma_extra_debug_sigprint();
...
arma_inline
const typename arma_scalar_only<T>::result& min(const T& x) { return x; }
...
template<typename T1, typename T2> arma_warn_unused
arma_inline
const GlueCube<T1, T2, glue_min> min
(
	const BaseCube<typename T1::elem_type, T1>& X,
...
>::result min(const T1& X, const uword dim = 0) {
	arma_extra_debug_sigprint();
...
template<typename T1> arma_warn_unused
inline
typename T1::elem_type min(const SpOp<T1, spop_min>& X) {
	arma_extra_debug_sigprint();
	arma_extra_debug_print("min(): two consecutive min() calls detected");
...
template<typename T1> arma_warn_unused
inline
const SpOp<SpOp<T1, spop_min>, spop_min> min(const SpOp<T1, spop_min>& in, const uword dim) {
	arma_extra_debug_sigprint();
...
inline
uword min(const SizeMat& s) { return (std::min)(s.n_rows, s.n_cols); }

arma_warn_unused

inline
uword min(const SizeCube& s) { return (std::min)((std::min)(s.n_rows, s.n_cols), s.n_slices); }
