...
<
	is_arma_type<T1>::value,
	Mat<std::complex<typename T1::pod_type>>
>::result fft2(const T1& A, const uword n_rows, const uword n_cols) {
	arma_extra_debug_sigprint();
...
<
	(is_arma_type<T1>::value && is_complex_strict<typename T1::elem_type>::value),
	Mat<std::complex<typename T1::pod_type>>
>::result ifft2(const T1& A, const uword n_rows, const uword n_cols) {
	arma_extra_debug_sigprint();
