...
template<uword N> struct as_scalar_redirect {
	template<typename T1> inline static typename T1::elem_type apply(const T1& X);
};

template<> struct as_scalar_redirect<2> {
	template<typename T1, typename T2> inline static typename T1::elem_type apply(const Glue<T1, T2, glue_times>& X);
};

template<> struct as_scalar_redirect<3> {
	template<typename T1, typename T2, typename T3> inline static typename T1::elem_type apply(const Glue<Glue<T1, T2, glue_times>, T3, glue_times>& X);
};

template<uword N> template<typename T1> inline
typename T1::elem_type as_scalar_redirect<N>::apply(const T1& X) {
...
typename T1::elem_type as_scalar_redirect<2>::apply(const Glue<T1, T2, glue_times>& X) {
	arma_extra_debug_sigprint();
...
typename T1::elem_type as_scalar_redirect<3>::apply(const Glue<Glue<T1, T2, glue_times>, T3, glue_times>& X) {
	arma_extra_debug_sigprint();
...
template<typename T1, typename T2, typename T3> inline
typename T1::elem_type as_scalar_diag(const Glue<Glue<T1, T2, glue_times_diag>, T3, glue_times>& X) {
	arma_extra_debug_sigprint();
	...
	if(B_is_vec) { return val * op_dot::direct_dot(A.n_elem, A.mem, B.mem, C.mem); } else { return val * op_dotext::direct_rowvec_diagmat_colvec(A.mem, B, C.mem); }
}

template<typename T1, typename T2> arma_warn_unused
arma_inline
typename T1::elem_type as_scalar(const Glue<T1, T2, glue_times>& X, const typename arma_not_cx<typename T1::elem_type>::result* junk = 0) {
	arma_extra_debug_sigprint();
	arma_ignore(junk);
	...
	} else { return as_scalar_diag(X); }
}

template<typename T1> arma_warn_unused
inline
typename T1::elem_type as_scalar(const Base<typename T1::elem_type, T1>& X) {
	arma_extra_debug_sigprint();
...
template<typename T1, typename eop_type> arma_warn_unused
inline
typename T1::elem_type as_scalar(const eOp<T1, eop_type>& X) {
	arma_extra_debug_sigprint();
...
arma_warn_unused
typename T1::elem_type as_scalar(const eGlue<T1, T2, eglue_type>& X) {
	arma_extra_debug_sigprint();
	...
	if(is_same_type<eglue_type, eglue_plus>::yes) { return a + b; } else if(is_same_type<eglue_type, eglue_minus>::yes) { return a - b; } else if(is_same_type<eglue_type, eglue_div>::yes) { return a / b; } else if(is_same_type<eglue_type, eglue_schur>::yes) { return a * b; }
}

template<typename T1> arma_warn_unused
inline
typename T1::elem_type as_scalar(const Gen<T1, gen_randu>& X) {
	arma_extra_debug_sigprint();
...
template<typename T1> arma_warn_unused
inline
typename T1::elem_type as_scalar(const Gen<T1, gen_randn>& X) {
	arma_extra_debug_sigprint();
...
template<typename T1> arma_warn_unused
inline
typename T1::elem_type as_scalar(const BaseCube<typename T1::elem_type, T1>& X) {
	arma_extra_debug_sigprint();
...
arma_inline
const typename arma_scalar_only<T>::result& as_scalar(const T& x) { return x; }

template<typename T1> arma_warn_unused
inline
typename T1::elem_type as_scalar(const SpBase<typename T1::elem_type, T1>& X) {
	typedef typename T1::elem_type eT;
