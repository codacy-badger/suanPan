...
inline
arma_deprecated
uword memory::enlarge_to_mult_of_chunksize(const uword n_elem) { return n_elem; }

template<typename eT> inline
arma_malloc
eT* memory::acquire(const uword n_elem) {
...
template<typename eT> inline
arma_deprecated
eT* memory::acquire_chunked(const uword n_elem) { return memory::acquire<eT>(n_elem); }

template<typename eT> arma_inline
void memory::release(eT* mem) {
	if(mem == NULL) { return; }
...
template<typename eT> arma_inline
bool memory::is_aligned(const eT* mem) {
...
template<typename eT> arma_inline
void memory::mark_as_aligned(eT*& mem) {
...
template<typename eT> arma_inline
void memory::mark_as_aligned(const eT*& mem) {
#if defined(ARMA_HAVE_ICC_ASSUME_ALIGNED)
    {
    __assume_aligned(mem, 16);
    }
