...
namespace band_helper {
	...
	bool is_band(uword& out_KL, uword& out_KU, const Mat<eT>& A, const uword N_min) {
		...
		for(uword col = 0; col < N; ++col) {
			...
			for(uword row = 0; row < col; ++row) {
				if(A_colptr[row] != eT_zero) {
					first_nonzero_row = row;
					break;
				}
			}

			for(uword row = (col + 1); row < N; ++row) { last_nonzero_row = (A_colptr[row] != eT_zero) ? row : last_nonzero_row; }
	...
	template<typename eT> inline
	bool is_band_lower(uword& out_KD, const Mat<eT>& A, const uword N_min) {
		arma_extra_debug_sigprint();
	...
	template<typename eT> inline
	bool is_band_upper(uword& out_KD, const Mat<eT>& A, const uword N_min) {
		arma_extra_debug_sigprint();
	...
	template<typename eT> inline
	void compress(Mat<eT>& AB, const Mat<eT>& A, const uword KL, const uword KU, const bool use_offset) {
		arma_extra_debug_sigprint();
	...
	template<typename eT> inline
	void uncompress(Mat<eT>& A, const Mat<eT>& AB, const uword KL, const uword KU, const bool use_offset) {
		arma_extra_debug_sigprint();
	...
	template<typename eT> inline
	void extract_tridiag(Mat<eT>& out, const Mat<eT>& A) {
		arma_extra_debug_sigprint();
