...
namespace gmm_priv {
	...
	template<typename eT> inline gmm_diag<eT>::~gmm_diag() {
	...
	template<typename eT> inline gmm_diag<eT>::gmm_diag(const gmm_diag<eT>& x) {
		arma_extra_debug_sigprint_this(this);
	...
	gmm_diag<eT>& gmm_diag<eT>::operator=(const gmm_diag<eT>& x) {
	...
	template<typename eT> inline gmm_diag<eT>::gmm_diag(const gmm_full<eT>& x) {
		arma_extra_debug_sigprint_this(this);
	...
	template<typename eT> inline
	gmm_diag<eT>& gmm_diag<eT>::operator=(const gmm_full<eT>& x) {
		arma_extra_debug_sigprint();
	...
	template<typename eT> inline gmm_diag<eT>::gmm_diag(const uword in_n_dims, const uword in_n_gaus) {
		arma_extra_debug_sigprint_this(this);
	...
	template<typename eT> inline
	void gmm_diag<eT>::reset(const uword in_n_dims, const uword in_n_gaus) {
		arma_extra_debug_sigprint();

		init(in_n_dims, in_n_gaus);
	}

	template<typename eT> template<typename T1, typename T2, typename T3> inline
	void gmm_diag<eT>::set_params(const Base<eT, T1>& in_means_expr, const Base<eT, T2>& in_dcovs_expr, const Base<eT, T3>& in_hefts_expr) {
		arma_extra_debug_sigprint();
	...
	void gmm_diag<eT>::set_means(const Base<eT, T1>& in_means_expr) {
		arma_extra_debug_sigprint();
	...
	template<typename eT> template<typename T1> inline
	void gmm_diag<eT>::set_dcovs(const Base<eT, T1>& in_dcovs_expr) {
		arma_extra_debug_sigprint();
	...
	template<typename eT> template<typename T1> inline
	void gmm_diag<eT>::set_hefts(const Base<eT, T1>& in_hefts_expr) {
		arma_extra_debug_sigprint();
		...
		for(uword i = 0; i < hefts.n_elem; ++i) { hefts_mem[i] = (std::max)(in_hefts_mem[i], std::numeric_limits<eT>::min()); }
	...
	template<typename eT> inline
	uword gmm_diag<eT>::n_dims() const { return means.n_rows; }

	template<typename eT> inline
	uword gmm_diag<eT>::n_gaus() const { return means.n_cols; }

	template<typename eT> inline
	bool gmm_diag<eT>::load(const std::string name) {
		arma_extra_debug_sigprint();
	...
	template<typename eT> inline
	bool gmm_diag<eT>::save(const std::string name) const {
		arma_extra_debug_sigprint();
	...
	template<typename eT> inline
	Col<eT> gmm_diag<eT>::generate() const {
		arma_extra_debug_sigprint();
	...
	template<typename eT> inline
	Mat<eT> gmm_diag<eT>::generate(const uword N_vec) const {
		arma_extra_debug_sigprint();
	...
	template<typename eT> template<typename T1> inline
	eT gmm_diag<eT>::log_p(const T1& expr, const gmm_empty_arg& junk1, typename enable_if<((is_arma_type<T1>::value) && (resolves_to_colvector<T1>::value == true))>::result* junk2) const {
		arma_extra_debug_sigprint();
		arma_ignore(junk1);
		arma_ignore(junk2);
	...
	template<typename eT> template<typename T1> inline
	eT gmm_diag<eT>::log_p(const T1& expr, const uword gaus_id, typename enable_if<((is_arma_type<T1>::value) && (resolves_to_colvector<T1>::value == true))>::result* junk2) const {
		arma_extra_debug_sigprint();
		arma_ignore(junk2);
	...
	template<typename eT> template<typename T1> inline
	Row<eT> gmm_diag<eT>::log_p(const T1& expr, const gmm_empty_arg& junk1, typename enable_if<((is_arma_type<T1>::value) && (resolves_to_colvector<T1>::value == false))>::result* junk2) const {
		arma_extra_debug_sigprint();
		arma_ignore(junk1);
		arma_ignore(junk2);
	...
	template<typename eT> template<typename T1> inline
	Row<eT> gmm_diag<eT>::log_p(const T1& expr, const uword gaus_id, typename enable_if<((is_arma_type<T1>::value) && (resolves_to_colvector<T1>::value == false))>::result* junk2) const {
		arma_extra_debug_sigprint();
		arma_ignore(junk2);
	...
	template<typename eT> template<typename T1> inline
	eT gmm_diag<eT>::sum_log_p(const Base<eT, T1>& expr) const {
		arma_extra_debug_sigprint();
	...
	template<typename eT> template<typename T1> inline
	eT gmm_diag<eT>::sum_log_p(const Base<eT, T1>& expr, const uword gaus_id) const {
		arma_extra_debug_sigprint();
	...
	template<typename eT> template<typename T1> inline
	eT gmm_diag<eT>::avg_log_p(const Base<eT, T1>& expr) const {
		arma_extra_debug_sigprint();
	...
	template<typename eT> template<typename T1> inline
	eT gmm_diag<eT>::avg_log_p(const Base<eT, T1>& expr, const uword gaus_id) const {
		arma_extra_debug_sigprint();
	...
	template<typename eT> template<typename T1> inline
	uword gmm_diag<eT>::assign(const T1& expr, const gmm_dist_mode& dist, typename enable_if<((is_arma_type<T1>::value) && (resolves_to_colvector<T1>::value == true))>::result* junk) const {
		arma_extra_debug_sigprint();
		arma_ignore(junk);
	...
	template<typename eT> template<typename T1> inline
	urowvec gmm_diag<eT>::assign(const T1& expr, const gmm_dist_mode& dist, typename enable_if<((is_arma_type<T1>::value) && (resolves_to_colvector<T1>::value == false))>::result* junk) const {
		arma_extra_debug_sigprint();
		arma_ignore(junk);
	...
	template<typename eT> template<typename T1> inline
	urowvec gmm_diag<eT>::raw_hist(const Base<eT, T1>& expr, const gmm_dist_mode& dist_mode) const {
		arma_extra_debug_sigprint();
	...
	template<typename eT> template<typename T1> inline
	Row<eT> gmm_diag<eT>::norm_hist(const Base<eT, T1>& expr, const gmm_dist_mode& dist_mode) const {
		arma_extra_debug_sigprint();
	...
	(
		...
		if(X.is_empty()) {
			arma_debug_warn("gmm_diag::learn(): given matrix is empty");
			return false;
		}
		if(X.is_finite() == false) {
			arma_debug_warn("gmm_diag::learn(): given matrix has non-finite values");
			return false;
		}
		...
		if(seed_mode == keep_existing) {
			if(means.is_empty()) {
				arma_debug_warn("gmm_diag::learn(): no existing means");
				return false;
			}
			if(X.n_rows != means.n_rows) {
				arma_debug_warn("gmm_diag::learn(): dimensionality mismatch");
				return false;
			}
			...
			if(X.n_cols < N_gaus) {
				arma_debug_warn("gmm_diag::learn(): number of vectors is less than number of gaussians");
				return false;
			}
			...
			if(print_mode) {
				get_cout_stream() << "gmm_diag::learn(): generating initial means\n";
		...
		if(km_iter > 0) {
			...
			if(dist_mode == eucl_dist) { status = km_iterate<1>(X, km_iter, print_mode, "gmm_diag::learn(): k-means"); } else if(dist_mode == maha_dist) { status = km_iterate<2>(X, km_iter, print_mode, "gmm_diag::learn(): k-means"); }

			stream_state.restore(get_cout_stream());

			if(status == false) {
				arma_debug_warn("gmm_diag::learn(): k-means algorithm failed; not enough data, or too many gaussians requested");
				init(orig);
				return false;
			}
		}
		...
		if(seed_mode != keep_existing) {
			if(print_mode) {
				get_cout_stream() << "gmm_diag::learn(): generating initial covariances\n";
				get_cout_stream().flush();
			}
		...
		if(em_iter > 0) {
			...
			if(status == false) {
				arma_debug_warn("gmm_diag::learn(): EM algorithm failed");
				init(orig);
				return false;
			}
		}
	...
	template<typename eT> template<typename T1> inline
	bool gmm_diag<eT>::kmeans_wrapper
	(
		Mat<eT>& user_means,
		const Base<eT, T1>& data,
		const uword N_gaus,
		...
		if(X.is_empty()) {
			arma_debug_warn("kmeans(): given matrix is empty");
			return false;
		}
		if(X.is_finite() == false) {
			arma_debug_warn("kmeans(): given matrix has non-finite values");
			return false;
		}
		...
		if(seed_mode == keep_existing) {
			...
			if(means.is_empty()) {
				arma_debug_warn("kmeans(): no existing means");
				return false;
			}
			if(X.n_rows != means.n_rows) {
				arma_debug_warn("kmeans(): dimensionality mismatch");
				return false;
			}
			...
			if(X.n_cols < N_gaus) {
				arma_debug_warn("kmeans(): number of vectors is less than number of means");
				return false;
			}
		...
		if(km_iter > 0) {
			...
			if(status == false) {
				arma_debug_warn("kmeans(): clustering failed; not enough data, or too many means requested");
				return false;
			}
		}
	...
	template<typename eT> inline
	void gmm_diag<eT>::init(const gmm_diag<eT>& x) {
		arma_extra_debug_sigprint();
	...
	template<typename eT> inline
	void gmm_diag<eT>::init(const gmm_full<eT>& x) {
		arma_extra_debug_sigprint();
		...
		for(uword g = 0; g < N_gaus; ++g) {
			...
			for(uword d = 0; d < N_dims; ++d) { dcov_mem[d] = fcov.at(d, d); }
		}
	...
	template<typename eT> inline
	void gmm_diag<eT>::init(const uword in_n_dims, const uword in_n_gaus) {
		arma_extra_debug_sigprint();
	...
	template<typename eT> inline
	void gmm_diag<eT>::init_constants() {
		arma_extra_debug_sigprint();
		...
		for(uword i = 0; i < dcovs_n_elem; ++i) { inv_dcovs_mem[i] = eT(1) / (std::max)(dcovs_mem[i], std::numeric_limits<eT>::min()); }
		...
		for(uword g = 0; g < N_gaus; ++g) {
			...
			for(uword d = 0; d < N_dims; ++d) { log_det_val += std::log((std::max)(dcovs_colmem[d], std::numeric_limits<eT>::min())); }
		...
		for(uword g = 0; g < N_gaus; ++g) { hefts_mem[g] = (std::max)(hefts_mem[g], std::numeric_limits<eT>::min()); }

		log_hefts = log(hefts);
	}

	template<typename eT> inline
	umat gmm_diag<eT>::internal_gen_boundaries(const uword N) const {
		arma_extra_debug_sigprint();
	...
	inline
	eT gmm_diag<eT>::internal_scalar_log_p(const eT* x) const {
		arma_extra_debug_sigprint();
	...
	template<typename eT> arma_hot
	inline
	eT gmm_diag<eT>::internal_scalar_log_p(const eT* x, const uword g) const {
		arma_extra_debug_sigprint();
	...
	template<typename eT> template<typename T1> inline
	Row<eT> gmm_diag<eT>::internal_vec_log_p(const T1& X) const {
		arma_extra_debug_sigprint();
        ...
        for(uword i=start_index; i <= end_index; ++i)
          ...
          out_mem[i] = internal_scalar_log_p( X.colptr(i) );
...
#else
			{
				...
				for(uword i = 0; i < N; ++i) { out_mem[i] = internal_scalar_log_p(X.colptr(i)); }
			}
#endif
		}
	...
	template<typename eT> template<typename T1> inline
	Row<eT> gmm_diag<eT>::internal_vec_log_p(const T1& X, const uword gaus_id) const {
		arma_extra_debug_sigprint();
        ...
        for(uword i=start_index; i <= end_index; ++i)
          ...
          out_mem[i] = internal_scalar_log_p( X.colptr(i), gaus_id );
...
#else
			{
				...
				for(uword i = 0; i < N; ++i) { out_mem[i] = internal_scalar_log_p(X.colptr(i), gaus_id); }
			}
#endif
		}
	...
	template<typename eT> template<typename T1> inline
	eT gmm_diag<eT>::internal_sum_log_p(const T1& X) const {
		arma_extra_debug_sigprint();
    ...
    return eT(accu(t_accs));
...
#else
		{
			...
			for(uword i = 0; i < N; ++i) { acc += internal_scalar_log_p(X.colptr(i)); }
	...
	template<typename eT> template<typename T1> inline
	eT gmm_diag<eT>::internal_sum_log_p(const T1& X, const uword gaus_id) const {
		arma_extra_debug_sigprint();
    ...
    return eT(accu(t_accs));
...
#else
		{
			...
			for(uword i = 0; i < N; ++i) { acc += internal_scalar_log_p(X.colptr(i), gaus_id); }
	...
	template<typename eT> template<typename T1> inline
	eT gmm_diag<eT>::internal_avg_log_p(const T1& X) const {
		arma_extra_debug_sigprint();
    ...
    return avg;
...
#else
		{
			...
			for(uword i = 0; i < N; ++i) { running_mean(internal_scalar_log_p(X.colptr(i))); }
	...
	template<typename eT> template<typename T1> inline
	eT gmm_diag<eT>::internal_avg_log_p(const T1& X, const uword gaus_id) const {
		arma_extra_debug_sigprint();
    ...
    return avg;
...
#else
		{
			...
			for(uword i = 0; i < N; ++i) { running_mean(internal_scalar_log_p(X.colptr(i), gaus_id)); }
	...
	template<typename eT> template<typename T1> inline
	uword gmm_diag<eT>::internal_scalar_assign(const T1& X, const gmm_dist_mode& dist_mode) const {
		arma_extra_debug_sigprint();
		...
		if(dist_mode == eucl_dist) {
			...
			return best_g;
		} else { arma_debug_check(true, "gmm_diag::assign(): unsupported distance mode"); }

		return uword(0);
	}

	template<typename eT> template<typename T1> inline
	void gmm_diag<eT>::internal_vec_assign(urowvec& out, const T1& X, const gmm_dist_mode& dist_mode) const {
		arma_extra_debug_sigprint();
	...
	template<typename eT> inline
	void gmm_diag<eT>::internal_raw_hist(urowvec& hist, const Mat<eT>& X, const gmm_dist_mode& dist_mode) const {
		arma_extra_debug_sigprint();
	...
	template<typename eT> template<uword dist_id> inline
	void gmm_diag<eT>::generate_initial_means(const Mat<eT>& X, const gmm_seed_mode& seed_mode) {
		arma_extra_debug_sigprint();
		...
		if((seed_mode == static_subset) || (seed_mode == random_subset)) {
			...
			if(seed_mode == static_subset) { initial_indices = linspace<uvec>(0, X.n_cols - 1, N_gaus); } else if(seed_mode == random_subset) { initial_indices = uvec(sort_index(randu<vec>(X.n_cols))).rows(0, N_gaus - 1); }
	...
	template<typename eT> template<uword dist_id> inline
	void gmm_diag<eT>::generate_initial_params(const Mat<eT>& X, const eT var_floor) {
		arma_extra_debug_sigprint();
	...
	template<typename eT> template<uword dist_id> inline
	bool gmm_diag<eT>::km_iterate(const Mat<eT>& X, const uword max_iter, const bool verbose, const char* signature) {
		arma_extra_debug_sigprint();
    ...
    field< Row<uword> > t_last_indx(n_threads);
		...
		if(verbose) {
			get_cout_stream() << signature << ": n_threads: " << n_threads << '\n';
			get_cout_stream().flush();
		}
      ...
      for(uword t=0; t < n_threads; ++t)
        ...
        if( t_acc_hefts(t)(g) >= 1 )  { last_indx(g) = t_last_indx(t)(g); }
			...
			for(uword g = 0; g < N_gaus; ++g) {
				...
				for(uword d = 0; d < N_dims; ++d) { new_mean[d] = (acc_heft >= 1) ? (acc_mean[d] / eT(acc_heft)) : eT(0); }
			}
			...
			if(dead_gs.n_elem > 0) {
				if(verbose) {
					get_cout_stream() << signature << ": recovering from dead means\n";
					get_cout_stream().flush();
				}
				...
				for(uword dead_gs_count = 0; dead_gs_count < dead_gs.n_elem; ++dead_gs_count) {
					...
					if(live_gs_count < live_gs.n_elem) {
						const uword live_g_id = live_gs(live_gs_count);
						++live_gs_count;
			...
			for(uword g = 0; g < N_gaus; ++g) { rs_delta(distance<eT, dist_id>::eval(N_dims, old_means.colptr(g), new_means.colptr(g), mah_aux_mem)); }
	...
	template<typename eT> inline
	bool gmm_diag<eT>::em_iterate(const Mat<eT>& X, const uword max_iter, const eT var_floor, const bool verbose) {
		arma_extra_debug_sigprint();
		...
		if(verbose) { get_cout_stream() << "gmm_diag::learn(): EM: n_threads: " << n_threads << '\n'; }
	...
	template<typename eT> inline
	void gmm_diag<eT>::em_update_params
	(
		const Mat<eT>& X,
	...
	template<typename eT> inline
	void gmm_diag<eT>::em_generate_acc
	(
		const Mat<eT>& X,
	...
	const {
		...
		for(uword i = start_index; i <= end_index; i++) {
			...
			for(uword g = 0; g < N_gaus; ++g) { gaus_log_lhoods_mem[g] = internal_scalar_log_p(x, g) + log_hefts_mem[g]; }

			eT log_lhood_sum = gaus_log_lhoods_mem[0];

			for(uword g = 1; g < N_gaus; ++g) { log_lhood_sum = log_add_exp(log_lhood_sum, gaus_log_lhoods_mem[g]); }
	...
	template<typename eT> inline
	void gmm_diag<eT>::em_fix_params(const eT var_floor) {
		arma_extra_debug_sigprint();
