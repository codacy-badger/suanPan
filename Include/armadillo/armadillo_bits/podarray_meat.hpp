...
template<typename eT> inline podarray<eT>::podarray()
	: n_elem(0)
	, mem(0) {
	arma_extra_debug_sigprint_this(this);
}

template<typename eT> inline podarray<eT>::podarray(const podarray& x)
	: n_elem(x.n_elem) {
...
const podarray<eT>& podarray<eT>::operator=(const podarray& x) {
	arma_extra_debug_sigprint();
...
template<typename eT> arma_hot
arma_inline podarray<eT>::podarray(const uword new_n_elem)
	: n_elem(new_n_elem) {
	arma_extra_debug_sigprint_this(this);
...
template<typename eT> arma_inline podarray<eT>::podarray(const eT* X, const uword new_n_elem)
	: n_elem(new_n_elem) {
	arma_extra_debug_sigprint_this(this);
...
template<typename eT> template<typename T1> inline podarray<eT>::podarray(const Proxy<T1>& P)
	: n_elem(P.get_n_elem()) {
	arma_extra_debug_sigprint_this(this);
	...
	if(Proxy<T1>::use_at == false) {
		...
		} else { for(uword col = 0; col < P_n_cols; ++col) { out_mem[col] = P.at(0, col); } }
	}
}

template<typename eT> arma_inline
eT podarray<eT>::operator[](const uword i) const { return mem[i]; }

template<typename eT> arma_inline
eT& podarray<eT>::operator[](const uword i) { return access::rw(mem[i]); }
...
template<typename eT> inline
void podarray<eT>::set_min_size(const uword min_n_elem) {
	arma_extra_debug_sigprint();
...
template<typename eT> arma_inline
eT* podarray<eT>::memptr() { return mem; }
...
template<typename eT> arma_hot
inline
void podarray<eT>::init_cold(const uword new_n_elem) {
	arma_extra_debug_sigprint();

	if(new_n_elem <= podarray_prealloc_n_elem::val) { mem = mem_local; } else { mem = memory::acquire<eT>(new_n_elem); }
}

template<typename eT> inline
void podarray<eT>::init_warm(const uword new_n_elem) {
	arma_extra_debug_sigprint();
