...
template<typename eT> inline spdiagview<eT>::~spdiagview() {
	arma_extra_debug_sigprint();
}

template<typename eT> arma_inline spdiagview<eT>::spdiagview(const SpMat<eT>& in_m, const uword in_row_offset, const uword in_col_offset, const uword in_len)
	: m(in_m)
...
void spdiagview<eT>::operator=(const spdiagview<eT>& x) {
	arma_extra_debug_sigprint();
	...
	if(&d_m != &x_m) {
		...
		for(uword i = 0; i < d_n_elem; ++i) { d_m.at(i + d_row_offset, i + d_col_offset) = x_m.at(i + x_row_offset, i + x_col_offset); }
	} else {
		const Mat<eT> tmp = x;
...
template<typename eT> inline
void spdiagview<eT>::operator+=(const eT val) {
	arma_extra_debug_sigprint();
...
void spdiagview<eT>::operator=(const Base<eT, T1>& o) {
	arma_extra_debug_sigprint();
...
//! set a diagonal of our matrix using data from a foreign object
template<typename eT> template<typename T1> inline
void spdiagview<eT>::operator=(const SpBase<eT, T1>& o) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
void spdiagview<eT>::extract(SpMat<eT>& out, const spdiagview<eT>& d) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
void spdiagview<eT>::extract(Mat<eT>& out, const spdiagview<eT>& in) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
SpMat_MapMat_val<eT> spdiagview<eT>::operator[](const uword i) { return (const_cast<SpMat<eT>&>(m)).at(i + row_offset, i + col_offset); }

template<typename eT> inline
eT spdiagview<eT>::operator[](const uword i) const { return m.at(i + row_offset, i + col_offset); }

template<typename eT> inline
SpMat_MapMat_val<eT> spdiagview<eT>::at(const uword i) { return (const_cast<SpMat<eT>&>(m)).at(i + row_offset, i + col_offset); }

template<typename eT> inline
eT spdiagview<eT>::at(const uword i) const { return m.at(i + row_offset, i + col_offset); }
...
template<typename eT> inline
SpMat_MapMat_val<eT> spdiagview<eT>::at(const uword row, const uword) { return (const_cast<SpMat<eT>&>(m)).at(row + row_offset, row + col_offset); }

template<typename eT> inline
eT spdiagview<eT>::at(const uword row, const uword) const { return m.at(row + row_offset, row + col_offset); }
...
template<typename eT> inline
void spdiagview<eT>::fill(const eT val) {
	arma_extra_debug_sigprint();
...
void spdiagview<eT>::randn() {
	...
	for(uword i = 0; i < local_n_elem; ++i) { x.at(i + row_offset, i + col_offset) = eT(arma_rng::randn<eT>()); }
}
