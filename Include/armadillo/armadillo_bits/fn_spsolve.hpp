...
(
	...
	{
		if((opts.equilibrate == false) && (opts.refine == superlu_opts::REF_NONE)) { status = sp_auxlib::spsolve_simple(out, A.get_ref(), B.get_ref(), opts); } else { status = sp_auxlib::spsolve_refine(out, rcond, A.get_ref(), B.get_ref(), opts); }
	} else if(sig == 'l') // brutal LAPACK solver
	{
		if((settings.id != 0) && ((opts.symmetric) || (opts.pivot_thresh != double(1.0)))) { arma_debug_warn("spsolve(): ignoring settings not applicable to LAPACK based solver"); }
		...
		if(conversion_ok) {
			...
			if((opts.equilibrate == false) && (opts.refine == superlu_opts::REF_NONE)) { flags |= solve_opts::flag_fast; } else if(opts.equilibrate == true) { flags |= solve_opts::flag_equilibrate; }
	...
	if(status == false) {
		if(rcond > T(0)) { arma_debug_warn("spsolve(): system seems singular (rcond: ", rcond, ")"); } else { arma_debug_warn("spsolve(): system seems singular"); }

		out.soft_reset();
	}

	if((status == true) && (rcond > T(0)) && (rcond <= (T(0.5) * std::numeric_limits<T>::epsilon()))) { arma_debug_warn("solve(): solution computed, but system seems singular to working precision (rcond: ", rcond, ")"); }
...
inline
Mat<typename T1::elem_type> spsolve
(
	const SpBase<typename T1::elem_type, T1>& A,
	const Base<typename T1::elem_type, T2>& B,
	const char* solver = "superlu",
