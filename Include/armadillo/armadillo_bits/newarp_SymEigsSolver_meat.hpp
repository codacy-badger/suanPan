...
namespace newarp {
	...
	void SymEigsSolver<eT, SelectionRule, OpType>::factorise_from(uword from_k, uword to_m, const Col<eT>& fk) {
		...
		for(uword i = from_k; i <= to_m - 1; i++) {
			...
			if(restart) { fac_f = w - Hii * v; } else { fac_f = w - fac_H(i, i - 1) * fac_V.col(i - 1) - Hii * v; }
	...
	template<typename eT, int SelectionRule, typename OpType> inline
	void SymEigsSolver<eT, SelectionRule, OpType>::restart(uword k) {
		arma_extra_debug_sigprint();
	...
	template<typename eT, int SelectionRule, typename OpType> inline
	uword SymEigsSolver<eT, SelectionRule, OpType>::num_converged(eT tol) {
		arma_extra_debug_sigprint();
	...
	template<typename eT, int SelectionRule, typename OpType> inline
	uword SymEigsSolver<eT, SelectionRule, OpType>::nev_adjusted(uword nconv) {
		arma_extra_debug_sigprint();
		...
		if(nev_new == 1 && ncv >= 6) { nev_new = ncv / 2; } else if(nev_new == 1 && ncv > 2) { nev_new = 2; }

		return nev_new;
	}

	template<typename eT, int SelectionRule, typename OpType> inline
	void SymEigsSolver<eT, SelectionRule, OpType>::retrieve_ritzpair() {
		arma_extra_debug_sigprint();
	...
	template<typename eT, int SelectionRule, typename OpType> inline
	void SymEigsSolver<eT, SelectionRule, OpType>::sort_ritzpair() {
		arma_extra_debug_sigprint();
	...
	template<typename eT, int SelectionRule, typename OpType> inline SymEigsSolver<eT, SelectionRule, OpType>::SymEigsSolver(const OpType& op_, uword nev_, uword ncv_)
		: op(op_)
		, nev(nev_)
		, dim_n(op.n_rows)
	...
	template<typename eT, int SelectionRule, typename OpType> inline
	void SymEigsSolver<eT, SelectionRule, OpType>::init(eT* init_resid) {
		arma_extra_debug_sigprint();
	...
	template<typename eT, int SelectionRule, typename OpType> inline
	void SymEigsSolver<eT, SelectionRule, OpType>::init() {
		arma_extra_debug_sigprint();
	...
	template<typename eT, int SelectionRule, typename OpType> inline
	uword SymEigsSolver<eT, SelectionRule, OpType>::compute(uword maxit, eT tol) {
		arma_extra_debug_sigprint();
	...
	template<typename eT, int SelectionRule, typename OpType> inline
	Col<eT> SymEigsSolver<eT, SelectionRule, OpType>::eigenvalues() {
		arma_extra_debug_sigprint();
	...
	template<typename eT, int SelectionRule, typename OpType> inline
	Mat<eT> SymEigsSolver<eT, SelectionRule, OpType>::eigenvectors(uword nvec) {
		arma_extra_debug_sigprint();
