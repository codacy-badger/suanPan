...
void glue_hist::apply_noalias(Mat<uword>& out, const Mat<eT>& X, const Mat<eT>& C, const uword dim) {
...
void glue_hist::apply(Mat<uword>& out, const mtGlue<uword, T1, T2, glue_hist>& expr) {
	arma_extra_debug_sigprint();
...
template<typename T1, typename T2> inline
void glue_hist_default::apply(Mat<uword>& out, const mtGlue<uword, T1, T2, glue_hist_default>& expr) {
	arma_extra_debug_sigprint();
