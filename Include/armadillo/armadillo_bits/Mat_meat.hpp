...
template<typename eT> inline Mat<eT>::~Mat() {
	...
	if((mem_state == 0) && (n_elem > arma_config::mat_prealloc)) { memory::release(access::rw(mem)); }
...
template<typename eT> inline Mat<eT>::Mat(const uword in_n_rows, const uword in_n_cols)
	: n_rows(in_n_rows)
	, n_cols(in_n_cols)
...
template<typename eT> inline Mat<eT>::Mat(const SizeMat& s)
	: n_rows(s.n_rows)
	, n_cols(s.n_cols)
...
template<typename eT> template<typename fill_type> inline Mat<eT>::Mat(const uword in_n_rows, const uword in_n_cols, const fill::fill_class<fill_type>& f)
	: n_rows(in_n_rows)
	, n_cols(in_n_cols)
	, n_elem(in_n_rows * in_n_cols)
	, vec_state(0)
...
template<typename eT> template<typename fill_type> inline Mat<eT>::Mat(const SizeMat& s, const fill::fill_class<fill_type>& f)
	: n_rows(s.n_rows)
	, n_cols(s.n_cols)
	, n_elem(s.n_rows * s.n_cols)
	, vec_state(0)
...
template<typename eT> inline Mat<eT>::Mat(const arma_vec_indicator&, const uhword in_vec_state)
...
//! constructor used by Row and Col classes
template<typename eT> inline Mat<eT>::Mat(const arma_vec_indicator&, const uword in_n_rows, const uword in_n_cols, const uhword in_vec_state)
	: n_rows(in_n_rows)
	, n_cols(in_n_cols)
	, n_elem(in_n_rows * in_n_cols)
	, vec_state(in_vec_state)
...
template<typename eT> inline Mat<eT>::Mat(const arma_fixed_indicator&, const uword in_n_rows, const uword in_n_cols, const uhword in_vec_state, const eT* in_mem)
	: n_rows(in_n_rows)
	, n_cols(in_n_cols)
	, n_elem(in_n_rows * in_n_cols)
	, vec_state(in_vec_state)
...
void Mat<eT>::init_cold() {
	...
	if(n_elem <= arma_config::mat_prealloc) {
		if(n_elem == 0) { access::rw(mem) = NULL; } else {
...
template<typename eT> inline
void Mat<eT>::init_warm(uword in_n_rows, uword in_n_cols) {
	...
	} else // condition: old_n_elem != new_n_elem
	{
		...
		if(new_n_elem < old_n_elem) // reuse existing memory if possible
		{
			if((t_mem_state == 0) && (new_n_elem <= arma_config::mat_prealloc)) {
				...
				if(new_n_elem == 0) { access::rw(mem) = NULL; } else {
					arma_extra_debug_print("Mat::init(): using local memory");
					access::rw(mem) = mem_local;
				}
			} else { arma_extra_debug_print("Mat::init(): reusing memory"); }
...
template<typename eT> inline
arma_cold
Mat<eT>& Mat<eT>::operator=(const char* text) {
...
template<typename eT> inline
arma_cold
Mat<eT>& Mat<eT>::operator=(const std::string& text) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
arma_cold
void Mat<eT>::init(const std::string& text_orig) {
	arma_extra_debug_sigprint();
	...
	if(replace_commas) {
		text_mod = text_orig;
		std::replace(text_mod.begin(), text_mod.end(), ',', ' ');
	}
	...
	while(line_start < text.length()) {
		...
		if(t_n_rows == 0) { t_n_cols = line_n_cols; } else { if(has_semicolon || has_token) { arma_check((line_n_cols != t_n_cols), "Mat::init(): inconsistent number of columns in given string"); } }
...
template<typename eT> inline Mat<eT>::Mat(const std::vector<eT>& x)
	: n_rows(uword(x.size()))
	, n_cols(1)
	, n_elem(uword(x.size()))
	, vec_state(0)
...
//! create the matrix from std::vector
template<typename eT> inline
Mat<eT>& Mat<eT>::operator=(const std::vector<eT>& x) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline Mat<eT>::Mat(const std::initializer_list<eT>& list)
	: n_rows(0)
	, n_cols(0)
	, n_elem(0)
	, vec_state(0)
...
template<typename eT> inline
Mat<eT>& Mat<eT>::operator=(const std::initializer_list<eT>& list) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline Mat<eT>::Mat(const std::initializer_list<std::initializer_list<eT>>& list)
	: n_rows(0)
	, n_cols(0)
	, n_elem(0)
	, vec_state(0)
...
template<typename eT> inline
Mat<eT>& Mat<eT>::operator=(const std::initializer_list<std::initializer_list<eT>>& list) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline Mat<eT>::Mat(Mat<eT>&& X)
	: n_rows(X.n_rows)
	, n_cols(X.n_cols)
	, n_elem(X.n_elem)
	, vec_state(0)
...
template<typename eT> inline
Mat<eT>& Mat<eT>::operator=(Mat<eT>&& X) {
	arma_extra_debug_sigprint(arma_str::format("this = %x   X = %x") % this % &X);
...
template<typename eT> inline
Mat<eT>& Mat<eT>::operator=(const eT val) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline Mat<eT>::Mat(const Mat<eT>& in_mat)
...
//! construct a matrix from a given matrix
template<typename eT> inline
Mat<eT>& Mat<eT>::operator=(const Mat<eT>& in_mat) {
	arma_extra_debug_sigprint(arma_str::format("this = %x   in_mat = %x") % this % &in_mat);
...
template<typename eT> inline
void Mat<eT>::init(const std::initializer_list<eT>& list) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
void Mat<eT>::init(const std::initializer_list<std::initializer_list<eT>>& list) {
	arma_extra_debug_sigprint();
	...
	for(; it != it_end; ++it) {
		if(x_n_cols_found == false) {
			...
			x_n_cols_found = true;
		} else { arma_check((uword((*it).size()) != x_n_cols), "Mat::init(): inconsistent number of columns in initialiser list"); }
	}

	Mat<eT>& t = (*this);

	if(t.mem_state == 3) { arma_debug_check(((x_n_rows != t.n_rows) || (x_n_cols != t.n_cols)), "Mat::init(): size mismatch between fixed size matrix and initialiser list"); } else { t.set_size(x_n_rows, x_n_cols); }
...
template<typename eT> template<typename T1, typename T2> inline
void Mat<eT>::init
(
	...
	if(use_at == false) {
		...
		for(uword ii = 0; ii < N; ++ii) { out_mem[ii] = std::complex<T>(A[ii], B[ii]); }
	} else {
...
void Mat<eT>::swap(Mat<eT>& B) {
	...
	if(A.vec_state == B.vec_state) { layout_ok = true; } else {
	...
	if((A_mem_state == 0) && (B_mem_state == 0) && layout_ok) {
		...
		if((A_use_local_mem == false) && (B_use_local_mem == false)) { std::swap(access::rw(A.mem), access::rw(B.mem)); } else if((A_use_local_mem == true) && (B_use_local_mem == true)) {
		...
		} else if((A_use_local_mem == true) && (B_use_local_mem == false)) {
			eT* A_mem_local = &(A.mem_local[0]);
			eT* B_mem_local = &(B.mem_local[0]);
			...
			access::rw(B.mem) = B_mem_local;
		} else if((A_use_local_mem == false) && (B_use_local_mem == true)) {
			eT* A_mem_local = &(A.mem_local[0]);
			eT* B_mem_local = &(B.mem_local[0]);
	...
	} else if((A_mem_state <= 2) && (B_mem_state <= 2) && (A.n_elem == B.n_elem) && layout_ok) {
		std::swap(access::rw(A.n_rows), access::rw(B.n_rows));
		std::swap(access::rw(A.n_cols), access::rw(B.n_cols));
	...
	} else if((A.n_rows == B.n_rows) && (A.n_cols == B.n_cols)) {
		const uword N = A.n_elem;
...
void Mat<eT>::steal_mem(Mat<eT>& x) {
	...
	if(t_vec_state == x_vec_state) { layout_ok = true; } else {
	...
	if((t_mem_state <= 1) && (((x_mem_state == 0) && (x_n_elem > arma_config::mat_prealloc)) || (x_mem_state == 1)) && layout_ok) {
		...
		access::rw(x.mem_state) = 0;
		access::rw(x.mem) = 0;
	} else { (*this).operator=(x); }
}

template<typename eT> inline
void Mat<eT>::steal_mem_col(Mat<eT>& x, const uword max_n_rows) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline Mat<eT>::Mat(eT* aux_mem, const uword aux_n_rows, const uword aux_n_cols, const bool copy_aux_mem, const bool strict)
	: n_rows(aux_n_rows)
	, n_cols(aux_n_cols)
...
//! the array is copied.
template<typename eT> inline Mat<eT>::Mat(const eT* aux_mem, const uword aux_n_rows, const uword aux_n_cols)
	: n_rows(aux_n_rows)
	, n_cols(aux_n_cols)
	, n_elem(aux_n_rows * aux_n_cols)
	, vec_state(0)
...
template<typename eT> inline Mat<eT>::Mat(const char junk, const eT* aux_mem, const uword aux_n_rows, const uword aux_n_cols)
	: n_rows(aux_n_rows)
	, n_cols(aux_n_cols)
	, n_elem(aux_n_rows * aux_n_cols)
	, vec_state(0)
...
template<typename eT> inline
Mat<eT>& Mat<eT>::operator+=(const Mat<eT>& m) {
	arma_extra_debug_sigprint();
...
template<typename eT> template<typename T1> inline Mat<eT>::Mat(const BaseCube<eT, T1>& X)
	: n_rows(0)
	, n_cols(0)
	, n_elem(0)
	, vec_state(0)
...
Mat<eT>& Mat<eT>::operator=(const BaseCube<eT, T1>& X) {
	arma_extra_debug_sigprint();
	...
	if(in_n_slices == 1) {
		...
		for(uword ucol = 0; ucol < in_n_cols; ++ucol) { arrayops::copy(out.colptr(ucol), in.slice_colptr(0, ucol), in_n_rows); }
	} else {
		if(out_vec_state == 0) {
			if(in_n_cols == 1) {
				out.set_size(in_n_rows, in_n_slices);

				for(uword i = 0; i < in_n_slices; ++i) { arrayops::copy(out.colptr(i), in.slice_colptr(i, 0), in_n_rows); }
			} else if(in_n_rows == 1) {
				...
				for(uword slice = 0; slice < in_n_slices; ++slice) {
					...
					if(i < in_n_cols) { out_colptr[i] = in.at(0, i, slice); }
				}
			}
		} else {
			...
			for(uword i = 0; i < in_n_slices; ++i) { out_mem[i] = in.at(0, 0, i); }
		}
	}
...
Mat<eT>& Mat<eT>::operator+=(const BaseCube<eT, T1>& X) {
	...
	if(in_n_slices == 1) { for(uword ucol = 0; ucol < in_n_cols; ++ucol) { arrayops::inplace_plus(out.colptr(ucol), in.slice_colptr(0, ucol), in_n_rows); } } else {
		if(out_vec_state == 0) {
			if((in_n_rows == out_n_rows) && (in_n_cols == 1) && (in_n_slices == out_n_cols)) { for(uword i = 0; i < in_n_slices; ++i) { arrayops::inplace_plus(out.colptr(i), in.slice_colptr(i, 0), in_n_rows); } } else if((in_n_rows == 1) && (in_n_cols == out_n_rows) && (in_n_slices == out_n_cols)) {
				for(uword slice = 0; slice < in_n_slices; ++slice) {
					eT* out_colptr = out.colptr(slice);
...
Mat<eT>& Mat<eT>::operator-=(const BaseCube<eT, T1>& X) {
	...
	if(in_n_slices == 1) { for(uword ucol = 0; ucol < in_n_cols; ++ucol) { arrayops::inplace_minus(out.colptr(ucol), in.slice_colptr(0, ucol), in_n_rows); } } else {
		if(out_vec_state == 0) {
			if((in_n_rows == out_n_rows) && (in_n_cols == 1) && (in_n_slices == out_n_cols)) { for(uword i = 0; i < in_n_slices; ++i) { arrayops::inplace_minus(out.colptr(i), in.slice_colptr(i, 0), in_n_rows); } } else if((in_n_rows == 1) && (in_n_cols == out_n_rows) && (in_n_slices == out_n_cols)) {
				for(uword slice = 0; slice < in_n_slices; ++slice) {
					eT* out_colptr = out.colptr(slice);
...
Mat<eT>& Mat<eT>::operator%=(const BaseCube<eT, T1>& X) {
	...
	if(in_n_slices == 1) { for(uword ucol = 0; ucol < in_n_cols; ++ucol) { arrayops::inplace_mul(out.colptr(ucol), in.slice_colptr(0, ucol), in_n_rows); } } else {
		if(out_vec_state == 0) {
			if((in_n_rows == out_n_rows) && (in_n_cols == 1) && (in_n_slices == out_n_cols)) { for(uword i = 0; i < in_n_slices; ++i) { arrayops::inplace_mul(out.colptr(i), in.slice_colptr(i, 0), in_n_rows); } } else if((in_n_rows == 1) && (in_n_cols == out_n_rows) && (in_n_slices == out_n_cols)) {
				for(uword slice = 0; slice < in_n_slices; ++slice) {
					eT* out_colptr = out.colptr(slice);
...
Mat<eT>& Mat<eT>::operator/=(const BaseCube<eT, T1>& X) {
	...
	if(in_n_slices == 1) { for(uword ucol = 0; ucol < in_n_cols; ++ucol) { arrayops::inplace_div(out.colptr(ucol), in.slice_colptr(0, ucol), in_n_rows); } } else {
		if(out_vec_state == 0) {
			if((in_n_rows == out_n_rows) && (in_n_cols == 1) && (in_n_slices == out_n_cols)) { for(uword i = 0; i < in_n_slices; ++i) { arrayops::inplace_div(out.colptr(i), in.slice_colptr(i, 0), in_n_rows); } } else if((in_n_rows == 1) && (in_n_cols == out_n_rows) && (in_n_slices == out_n_cols)) {
				for(uword slice = 0; slice < in_n_slices; ++slice) {
					eT* out_colptr = out.colptr(slice);
...
//! for constructing a complex matrix out of two non-complex matrices
template<typename eT> template<typename T1, typename T2> inline Mat<eT>::Mat
(
...
template<typename eT> inline Mat<eT>::Mat(const subview<eT>& X, const bool use_colmem)
	: n_rows(X.n_rows)
	, n_cols(X.n_cols)
	, n_elem(X.n_elem)
	, vec_state(0)
	...
	if(use_colmem) { arma_extra_debug_print("Mat::Mat(): using existing memory in a submatrix"); } else {
		init_cold();
...
template<typename eT> inline Mat<eT>::Mat(const subview<eT>& X)
	: n_rows(X.n_rows)
	, n_cols(X.n_cols)
	, n_elem(X.n_elem)
	, vec_state(0)
...
//! construct a matrix from subview (e.g. construct a matrix from a delayed submatrix operation)
template<typename eT> inline
Mat<eT>& Mat<eT>::operator=(const subview<eT>& X) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline Mat<eT>::Mat(const subview_row_strans<eT>& X)
	: n_rows(X.n_rows)
	, n_cols(X.n_cols)
	, n_elem(X.n_elem)
	, vec_state(0)
...
template<typename eT> inline Mat<eT>::Mat(const subview_row_htrans<eT>& X)
	: n_rows(X.n_rows)
	, n_cols(X.n_cols)
	, n_elem(X.n_elem)
	, vec_state(0)
...
template<typename eT> inline Mat<eT>::Mat(const xvec_htrans<eT>& X)
	: n_rows(X.n_rows)
	, n_cols(X.n_cols)
	, n_elem(X.n_elem)
	, vec_state(0)
...
template<typename eT> template<bool do_conj> inline Mat<eT>::Mat(const xtrans_mat<eT, do_conj>& X)
	: n_rows(X.n_rows)
	, n_cols(X.n_cols)
	, n_elem(X.n_elem)
	, vec_state(0)
...
template<typename eT> inline Mat<eT>::Mat(const subview_cube<eT>& x)
	: n_rows(0)
	, n_cols(0)
	, n_elem(0)
	, vec_state(0)
...
//! construct a matrix from a subview_cube instance
template<typename eT> inline
Mat<eT>& Mat<eT>::operator=(const subview_cube<eT>& X) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline Mat<eT>::Mat(const diagview<eT>& X)
	: n_rows(X.n_rows)
	, n_cols(X.n_cols)
	, n_elem(X.n_elem)
	, vec_state(0)
...
//! construct a matrix from diagview (e.g. construct a matrix from a delayed diag operation)
template<typename eT> inline
Mat<eT>& Mat<eT>::operator=(const diagview<eT>& X) {
	arma_extra_debug_sigprint();
...
template<typename eT> template<typename T1> inline Mat<eT>::Mat(const subview_elem1<eT, T1>& X)
	: n_rows(0)
	, n_cols(0)
	, n_elem(0)
	, vec_state(0)
...
template<typename eT> template<typename T1> inline
Mat<eT>& Mat<eT>::operator=(const subview_elem1<eT, T1>& X) {
	arma_extra_debug_sigprint();
...
template<typename eT> template<typename T1, typename T2> inline Mat<eT>::Mat(const subview_elem2<eT, T1, T2>& X)
	: n_rows(0)
	, n_cols(0)
	, n_elem(0)
	, vec_state(0)
...
template<typename eT> template<typename T1, typename T2> inline
Mat<eT>& Mat<eT>::operator=(const subview_elem2<eT, T1, T2>& X) {
	arma_extra_debug_sigprint();
...
template<typename eT> template<typename T1> inline Mat<eT>::Mat(const SpBase<eT, T1>& m)
	: n_rows(0)
	, n_cols(0)
	, n_elem(0)
	, vec_state(0)
...
template<typename eT> template<typename T1> inline
Mat<eT>& Mat<eT>::operator=(const SpBase<eT, T1>& m) {
	arma_extra_debug_sigprint();
...
Mat<eT>& Mat<eT>::operator/=(const SpBase<eT, T1>& m) {
	...
	for(uword c = 0; c < n_cols; ++c) for(uword r = 0; r < n_rows; ++r) { at(r, c) /= p.at(r, c); }

	return *this;
}

template<typename eT> inline Mat<eT>::Mat(const spdiagview<eT>& X)
	: n_rows(X.n_rows)
	, n_cols(X.n_cols)
	, n_elem(X.n_elem)
	, vec_state(0)
...
template<typename eT> inline
Mat<eT>& Mat<eT>::operator=(const spdiagview<eT>& X) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
mat_injector<Mat<eT>> Mat<eT>::operator<<(const eT val) { return mat_injector<Mat<eT>>(*this, val); }

template<typename eT> inline
mat_injector<Mat<eT>> Mat<eT>::operator<<(const injector_end_of_row<>& x) { return mat_injector<Mat<eT>>(*this, x); }

//! creation of subview (row vector)
template<typename eT> arma_inline
subview_row<eT> Mat<eT>::row(const uword row_num) {
	arma_extra_debug_sigprint();
...
//! creation of subview (row vector)
template<typename eT> arma_inline
const subview_row<eT> Mat<eT>::row(const uword row_num) const {
	arma_extra_debug_sigprint();
...
template<typename eT> arma_inline
subview_col<eT> Mat<eT>::col(const uword col_num) {
	arma_extra_debug_sigprint();
...
//! creation of subview (column vector)
template<typename eT> arma_inline
const subview_col<eT> Mat<eT>::col(const uword col_num) const {
	arma_extra_debug_sigprint();
...
template<typename eT> arma_inline
subview<eT> Mat<eT>::rows(const uword in_row1, const uword in_row2) {
	arma_extra_debug_sigprint();
...
//! creation of subview (submatrix comprised of specified row vectors)
template<typename eT> arma_inline
const subview<eT> Mat<eT>::rows(const uword in_row1, const uword in_row2) const {
	arma_extra_debug_sigprint();
...
template<typename eT> arma_inline
subview<eT> Mat<eT>::cols(const uword in_col1, const uword in_col2) {
	arma_extra_debug_sigprint();
...
//! creation of subview (submatrix comprised of specified column vectors)
template<typename eT> arma_inline
const subview<eT> Mat<eT>::cols(const uword in_col1, const uword in_col2) const {
	arma_extra_debug_sigprint();
...
//! creation of subview (submatrix comprised of specified row vectors)
template<typename eT> inline
subview<eT> Mat<eT>::rows(const span& row_span) {
	arma_extra_debug_sigprint();
...
//! creation of subview (submatrix comprised of specified row vectors)
template<typename eT> inline
const subview<eT> Mat<eT>::rows(const span& row_span) const {
	arma_extra_debug_sigprint();
...
//! creation of subview (submatrix comprised of specified column vectors)
template<typename eT> arma_inline
subview<eT> Mat<eT>::cols(const span& col_span) {
	arma_extra_debug_sigprint();
...
//! creation of subview (submatrix comprised of specified column vectors)
template<typename eT> arma_inline
const subview<eT> Mat<eT>::cols(const span& col_span) const {
	arma_extra_debug_sigprint();
...
template<typename eT> arma_inline
subview<eT> Mat<eT>::submat(const uword in_row1, const uword in_col1, const uword in_row2, const uword in_col2) {
	arma_extra_debug_sigprint();
...
template<typename eT> arma_inline
const subview<eT> Mat<eT>::submat(const uword in_row1, const uword in_col1, const uword in_row2, const uword in_col2) const {
	arma_extra_debug_sigprint();
...
//! creation of subview (submatrix)
template<typename eT> arma_inline
subview<eT> Mat<eT>::submat(const uword in_row1, const uword in_col1, const SizeMat& s) {
	arma_extra_debug_sigprint();
...
//! creation of subview (submatrix)
template<typename eT> arma_inline
const subview<eT> Mat<eT>::submat(const uword in_row1, const uword in_col1, const SizeMat& s) const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
subview<eT> Mat<eT>::operator()(const uword in_row1, const uword in_col1, const SizeMat& s) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
const subview<eT> Mat<eT>::operator()(const uword in_row1, const uword in_col1, const SizeMat& s) const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
subview<eT> Mat<eT>::head_rows(const uword N) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
const subview<eT> Mat<eT>::head_rows(const uword N) const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
subview<eT> Mat<eT>::tail_rows(const uword N) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
const subview<eT> Mat<eT>::tail_rows(const uword N) const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
subview<eT> Mat<eT>::head_cols(const uword N) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
const subview<eT> Mat<eT>::head_cols(const uword N) const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
subview<eT> Mat<eT>::tail_cols(const uword N) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
const subview<eT> Mat<eT>::tail_cols(const uword N) const {
	arma_extra_debug_sigprint();
...
template<typename eT> template<typename T1> arma_inline
subview_elem1<eT, T1> Mat<eT>::elem(const Base<uword, T1>& a) {
	arma_extra_debug_sigprint();
...
template<typename eT> template<typename T1> arma_inline
const subview_elem1<eT, T1> Mat<eT>::elem(const Base<uword, T1>& a) const {
	arma_extra_debug_sigprint();
...
template<typename eT> template<typename T1> arma_inline
subview_elem1<eT, T1> Mat<eT>::operator()(const Base<uword, T1>& a) {
	arma_extra_debug_sigprint();
...
template<typename eT> template<typename T1> arma_inline
const subview_elem1<eT, T1> Mat<eT>::operator()(const Base<uword, T1>& a) const {
	arma_extra_debug_sigprint();

	return subview_elem1<eT, T1>(*this, a);
}

template<typename eT> template<typename T1, typename T2> arma_inline
subview_elem2<eT, T1, T2> Mat<eT>::elem(const Base<uword, T1>& ri, const Base<uword, T2>& ci) {
	arma_extra_debug_sigprint();
...
template<typename eT> template<typename T1, typename T2> arma_inline
const subview_elem2<eT, T1, T2> Mat<eT>::elem(const Base<uword, T1>& ri, const Base<uword, T2>& ci) const {
	arma_extra_debug_sigprint();
...
template<typename eT> template<typename T1, typename T2> arma_inline
subview_elem2<eT, T1, T2> Mat<eT>::submat(const Base<uword, T1>& ri, const Base<uword, T2>& ci) {
	arma_extra_debug_sigprint();
...
template<typename eT> template<typename T1, typename T2> arma_inline
const subview_elem2<eT, T1, T2> Mat<eT>::submat(const Base<uword, T1>& ri, const Base<uword, T2>& ci) const {
	arma_extra_debug_sigprint();
...
template<typename eT> template<typename T1, typename T2> arma_inline
subview_elem2<eT, T1, T2> Mat<eT>::operator()(const Base<uword, T1>& ri, const Base<uword, T2>& ci) {
	arma_extra_debug_sigprint();
...
template<typename eT> template<typename T1, typename T2> arma_inline
const subview_elem2<eT, T1, T2> Mat<eT>::operator()(const Base<uword, T1>& ri, const Base<uword, T2>& ci) const {
	arma_extra_debug_sigprint();
...
template<typename eT> template<typename T1> arma_inline
subview_elem2<eT, T1, T1> Mat<eT>::rows(const Base<uword, T1>& ri) {
	arma_extra_debug_sigprint();
...
template<typename eT> template<typename T1> arma_inline
const subview_elem2<eT, T1, T1> Mat<eT>::rows(const Base<uword, T1>& ri) const {
	arma_extra_debug_sigprint();

	return subview_elem2<eT, T1, T1>(*this, ri, ri, false, true);
}

template<typename eT> template<typename T2> arma_inline
subview_elem2<eT, T2, T2> Mat<eT>::cols(const Base<uword, T2>& ci) {
	arma_extra_debug_sigprint();
...
template<typename eT> template<typename T2> arma_inline
const subview_elem2<eT, T2, T2> Mat<eT>::cols(const Base<uword, T2>& ci) const {
	arma_extra_debug_sigprint();
...
template<typename eT> arma_inline
subview_each1<Mat<eT>, 0> Mat<eT>::each_col() {
	arma_extra_debug_sigprint();
...
template<typename eT> arma_inline
subview_each1<Mat<eT>, 1> Mat<eT>::each_row() {
	arma_extra_debug_sigprint();
...
template<typename eT> arma_inline
const subview_each1<Mat<eT>, 0> Mat<eT>::each_col() const {
	arma_extra_debug_sigprint();
...
template<typename eT> arma_inline
const subview_each1<Mat<eT>, 1> Mat<eT>::each_row() const {
	arma_extra_debug_sigprint();
...
template<typename eT> template<typename T1> inline
subview_each2<Mat<eT>, 0, T1> Mat<eT>::each_col(const Base<uword, T1>& indices) {
	arma_extra_debug_sigprint();
...
template<typename eT> template<typename T1> inline
subview_each2<Mat<eT>, 1, T1> Mat<eT>::each_row(const Base<uword, T1>& indices) {
	arma_extra_debug_sigprint();
...
template<typename eT> template<typename T1> inline
const subview_each2<Mat<eT>, 0, T1> Mat<eT>::each_col(const Base<uword, T1>& indices) const {
	arma_extra_debug_sigprint();
...
template<typename eT> template<typename T1> inline
const subview_each2<Mat<eT>, 1, T1> Mat<eT>::each_row(const Base<uword, T1>& indices) const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
const Mat<eT>& Mat<eT>::each_col(const std::function<void(Col<eT>&)>& F) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
const Mat<eT>& Mat<eT>::each_col(const std::function<void(const Col<eT>&)>& F) const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
const Mat<eT>& Mat<eT>::each_row(const std::function<void(Row<eT>&)>& F) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
const Mat<eT>& Mat<eT>::each_row(const std::function<void(const Row<eT>&)>& F) const {
	arma_extra_debug_sigprint();
...
template<typename eT> arma_inline
diagview<eT> Mat<eT>::diag(const sword in_id) {
	arma_extra_debug_sigprint();
...
//! creation of diagview (diagonal)
template<typename eT> arma_inline
const diagview<eT> Mat<eT>::diag(const sword in_id) const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
void Mat<eT>::swap_cols(const uword in_colA, const uword in_colB) {
	arma_extra_debug_sigprint();
	...
	if(n_elem > 0) {
		...
		if(iq < local_n_rows) { std::swap(ptrA[iq], ptrB[iq]); }
	}
}
...
void Mat<eT>::shed_rows(const uword in_row1, const uword in_row2) {
	...
	if(n_keep_front > 0) { X.rows(0, (n_keep_front - 1)) = rows(0, (in_row1 - 1)); }

	if(n_keep_back > 0) { X.rows(n_keep_front, (n_keep_front + n_keep_back - 1)) = rows((in_row2 + 1), (n_rows - 1)); }
...
void Mat<eT>::shed_cols(const uword in_col1, const uword in_col2) {
	...
	if(n_keep_front > 0) { X.cols(0, (n_keep_front - 1)) = cols(0, (in_col1 - 1)); }

	if(n_keep_back > 0) { X.cols(n_keep_front, (n_keep_front + n_keep_back - 1)) = cols((in_col2 + 1), (n_cols - 1)); }
...
template<typename eT> inline
void Mat<eT>::insert_rows(const uword row_num, const uword N, const bool set_to_zero) {
	arma_extra_debug_sigprint();
	...
	if(N > 0) {
		...
		if(A_n_rows > 0) { out.rows(0, A_n_rows - 1) = rows(0, A_n_rows - 1); }

		if(B_n_rows > 0) { out.rows(row_num + N, t_n_rows + N - 1) = rows(row_num, t_n_rows - 1); }

		if(set_to_zero) { out.rows(row_num, row_num + N - 1).zeros(); }
...
template<typename eT> inline
void Mat<eT>::insert_cols(const uword col_num, const uword N, const bool set_to_zero) {
	arma_extra_debug_sigprint();
	...
	if(N > 0) {
		...
		if(A_n_cols > 0) { out.cols(0, A_n_cols - 1) = cols(0, A_n_cols - 1); }

		if(B_n_cols > 0) { out.cols(col_num + N, t_n_cols + N - 1) = cols(col_num, t_n_cols - 1); }

		if(set_to_zero) { out.cols(col_num, col_num + N - 1).zeros(); }
...
void Mat<eT>::insert_rows(const uword row_num, const Base<eT, T1>& X) {
	...
	if(C_n_rows > 0) {
		...
		if(t_n_cols > 0) {
			...
			if((t_n_cols > 0) && (B_n_rows > 0)) { out.rows(row_num + C_n_rows, t_n_rows + C_n_rows - 1) = rows(row_num, t_n_rows - 1); }
		}

		if(C_n_cols > 0) { out.rows(row_num, row_num + C_n_rows - 1) = C; }
...
void Mat<eT>::insert_cols(const uword col_num, const Base<eT, T1>& X) {
	...
	if(C_n_cols > 0) {
		...
		if(t_n_rows > 0) {
			...
			if(B_n_cols > 0) { out.cols(col_num + C_n_cols, t_n_cols + C_n_cols - 1) = cols(col_num, t_n_cols - 1); }
		}

		if(C_n_rows > 0) { out.cols(col_num, col_num + C_n_cols - 1) = C; }

		steal_mem(out);
	}
}

template<typename eT> template<typename T1, typename gen_type> inline Mat<eT>::Mat(const Gen<T1, gen_type>& X)
	: n_rows(X.n_rows)
	, n_cols(X.n_cols)
...
template<typename eT> template<typename T1, typename gen_type> inline
Mat<eT>& Mat<eT>::operator=(const Gen<T1, gen_type>& X) {
	arma_extra_debug_sigprint();
...
template<typename eT> template<typename T1, typename op_type> inline Mat<eT>::Mat(const Op<T1, op_type>& X)
	: n_rows(0)
	, n_cols(0)
	, n_elem(0)
	, vec_state(0)
...
//! create a matrix from Op, i.e. run the previously delayed unary operations
template<typename eT> template<typename T1, typename op_type> inline
Mat<eT>& Mat<eT>::operator=(const Op<T1, op_type>& X) {
	arma_extra_debug_sigprint();
...
template<typename eT> template<typename T1, typename eop_type> inline Mat<eT>::Mat(const eOp<T1, eop_type>& X)
	: n_rows(X.get_n_rows())
	, n_cols(X.get_n_cols())
	, n_elem(X.get_n_elem())
	, vec_state(0)
...
//! create a matrix from eOp, i.e. run the previously delayed unary operations
template<typename eT> template<typename T1, typename eop_type> inline
Mat<eT>& Mat<eT>::operator=(const eOp<T1, eop_type>& X) {
	arma_extra_debug_sigprint();
...
template<typename eT> template<typename T1, typename op_type> inline Mat<eT>::Mat(const mtOp<eT, T1, op_type>& X)
	: n_rows(0)
	, n_cols(0)
	, n_elem(0)
	, vec_state(0)
...
template<typename eT> template<typename T1, typename op_type> inline
Mat<eT>& Mat<eT>::operator=(const mtOp<eT, T1, op_type>& X) {
	arma_extra_debug_sigprint();
...
template<typename eT> template<typename T1, typename T2, typename glue_type> inline Mat<eT>::Mat(const Glue<T1, T2, glue_type>& X)
	: n_rows(0)
	, n_cols(0)
	, n_elem(0)
	, vec_state(0)
...
//! create a matrix from Glue, i.e. run the previously delayed binary operations
template<typename eT> template<typename T1, typename T2, typename glue_type> inline
Mat<eT>& Mat<eT>::operator=(const Glue<T1, T2, glue_type>& X) {
	arma_extra_debug_sigprint();
...
template<typename eT> template<typename T1, typename T2> inline
Mat<eT>& Mat<eT>::operator+=(const Glue<T1, T2, glue_times>& X) {
	arma_extra_debug_sigprint();
...
template<typename eT> template<typename T1, typename T2, typename eglue_type> inline Mat<eT>::Mat(const eGlue<T1, T2, eglue_type>& X)
	: n_rows(X.get_n_rows())
	, n_cols(X.get_n_cols())
	, n_elem(X.get_n_elem())
	, vec_state(0)
...
//! create a matrix from eGlue, i.e. run the previously delayed binary operations
template<typename eT> template<typename T1, typename T2, typename eglue_type> inline
Mat<eT>& Mat<eT>::operator=(const eGlue<T1, T2, eglue_type>& X) {
	arma_extra_debug_sigprint();
...
template<typename eT> template<typename T1, typename T2, typename glue_type> inline Mat<eT>::Mat(const mtGlue<eT, T1, T2, glue_type>& X)
	: n_rows(0)
	, n_cols(0)
	, n_elem(0)
	, vec_state(0)
...
template<typename eT> template<typename T1, typename T2, typename glue_type> inline
Mat<eT>& Mat<eT>::operator=(const mtGlue<eT, T1, T2, glue_type>& X) {
	arma_extra_debug_sigprint();
...
template<typename eT> arma_inline
arma_warn_unused
const eT& Mat<eT>::at_alt(const uword ii) const {
...
template<typename eT> arma_inline
arma_warn_unused
eT& Mat<eT>::operator()(const uword ii) {
...
template<typename eT> arma_inline
arma_warn_unused
const eT& Mat<eT>::operator()(const uword ii) const {
	arma_debug_check((ii >= n_elem), "Mat::operator(): index out of bounds");
	return mem[ii];
}
...
template<typename eT> arma_inline
arma_warn_unused
eT& Mat<eT>::operator[](const uword ii) { return access::rw(mem[ii]); }

//! linear element accessor (treats the matrix as a vector); no bounds check
template<typename eT> arma_inline
arma_warn_unused
const eT& Mat<eT>::operator[](const uword ii) const { return mem[ii]; }

//! linear element accessor (treats the matrix as a vector); no bounds check.  
template<typename eT> arma_inline
arma_warn_unused
eT& Mat<eT>::at(const uword ii) { return access::rw(mem[ii]); }

//! linear element accessor (treats the matrix as a vector); no bounds check
template<typename eT> arma_inline
arma_warn_unused
const eT& Mat<eT>::at(const uword ii) const { return mem[ii]; }

//! element accessor; bounds checking not done when ARMA_NO_DEBUG is defined
template<typename eT> arma_inline
arma_warn_unused
eT& Mat<eT>::operator()(const uword in_row, const uword in_col) {
...
template<typename eT> arma_inline
arma_warn_unused
const eT& Mat<eT>::operator()(const uword in_row, const uword in_col) const {
	arma_debug_check(((in_row >= n_rows) || (in_col >= n_cols)), "Mat::operator(): index out of bounds");
...
template<typename eT> arma_inline
arma_warn_unused
eT& Mat<eT>::at(const uword in_row, const uword in_col) { return access::rw(mem[in_row + in_col * n_rows]); }

//! element accessor; no bounds check
template<typename eT> arma_inline
arma_warn_unused
const eT& Mat<eT>::at(const uword in_row, const uword in_col) const { return mem[in_row + in_col * n_rows]; }

//! prefix ++
template<typename eT> arma_inline
const Mat<eT>& Mat<eT>::operator++() {
...
template<typename eT> arma_inline
void Mat<eT>::operator++(int) { Mat_aux::postfix_pp(*this); }
...
//! postfix --  (must not return the object by reference)
template<typename eT> arma_inline
void Mat<eT>::operator--(int) { Mat_aux::postfix_mm(*this); }

//! returns true if the matrix has no elements
template<typename eT> arma_inline
arma_warn_unused
bool Mat<eT>::is_empty() const { return (n_elem == 0); }

//! returns true if the object can be interpreted as a column or row vector
template<typename eT> arma_inline
arma_warn_unused
bool Mat<eT>::is_vec() const { return ((n_rows == 1) || (n_cols == 1)); }

//! returns true if the object can be interpreted as a row vector
template<typename eT> arma_inline
arma_warn_unused
bool Mat<eT>::is_rowvec() const { return (n_rows == 1); }

//! returns true if the object can be interpreted as a column vector
template<typename eT> arma_inline
arma_warn_unused
bool Mat<eT>::is_colvec() const { return (n_cols == 1); }

//! returns true if the object has the same number of non-zero rows and columnns
template<typename eT> arma_inline
arma_warn_unused
bool Mat<eT>::is_square() const { return (n_rows == n_cols); }

//! returns true if all of the elements are finite
template<typename eT> inline
arma_warn_unused
bool Mat<eT>::is_finite() const { return arrayops::is_finite(memptr(), n_elem); }

template<typename eT> inline
arma_warn_unused
bool Mat<eT>::has_inf() const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
arma_warn_unused
bool Mat<eT>::is_sorted(const char* direction) const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
arma_warn_unused
bool Mat<eT>::is_sorted(const char* direction, const uword dim) const {
	arma_extra_debug_sigprint();
...
template<typename eT> arma_inline
arma_warn_unused
bool Mat<eT>::in_range(const uword ii) const { return (ii < n_elem); }

//! returns true if the given start and end indices are currently in range
template<typename eT> arma_inline
arma_warn_unused
bool Mat<eT>::in_range(const span& x) const {
	arma_extra_debug_sigprint();

	if(x.whole) { return true; } else {
		const uword a = x.a;
		const uword b = x.b;
...
template<typename eT> arma_inline
arma_warn_unused
bool Mat<eT>::in_range(const uword in_row, const uword in_col) const { return ((in_row < n_rows) && (in_col < n_cols)); }

template<typename eT> arma_inline
arma_warn_unused
bool Mat<eT>::in_range(const span& row_span, const uword in_col) const {
	arma_extra_debug_sigprint();

	if(row_span.whole) { return (in_col < n_cols); } else {
...
template<typename eT> arma_inline
arma_warn_unused
bool Mat<eT>::in_range(const uword in_row, const span& col_span) const {
	arma_extra_debug_sigprint();

	if(col_span.whole) { return (in_row < n_rows); } else {
...
template<typename eT> arma_inline
arma_warn_unused
bool Mat<eT>::in_range(const span& row_span, const span& col_span) const {
	arma_extra_debug_sigprint();
...
template<typename eT> arma_inline
arma_warn_unused
bool Mat<eT>::in_range(const uword in_row, const uword in_col, const SizeMat& s) const {
	const uword l_n_rows = n_rows;
	const uword l_n_cols = n_cols;

	if((in_row >= l_n_rows) || (in_col >= l_n_cols) || ((in_row + s.n_rows) > l_n_rows) || ((in_col + s.n_cols) > l_n_cols)) { return false; } else { return true; }
}

//! returns a pointer to array of eTs for a specified column; no bounds check
template<typename eT> arma_inline
arma_warn_unused
eT* Mat<eT>::colptr(const uword in_col) { return & access::rw(mem[in_col * n_rows]); }

//! returns a pointer to array of eTs for a specified column; no bounds check
template<typename eT> arma_inline
arma_warn_unused
const eT* Mat<eT>::colptr(const uword in_col) const { return & mem[in_col * n_rows]; }

//! returns a pointer to array of eTs used by the matrix
template<typename eT> arma_inline
arma_warn_unused
eT* Mat<eT>::memptr() { return const_cast<eT*>(mem); }

//! returns a pointer to array of eTs used by the matrix
template<typename eT> arma_inline
arma_warn_unused
const eT* Mat<eT>::memptr() const { return mem; }
...
template<typename eT> inline
void Mat<eT>::set_size(const SizeMat& s) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
void Mat<eT>::resize(const SizeMat& s) {
	arma_extra_debug_sigprint();
...
//! change the matrix to have user specified dimensions (data is preserved)
template<typename eT> inline
void Mat<eT>::reshape(const uword in_rows, const uword in_cols) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
void Mat<eT>::reshape(const SizeMat& s) {
	arma_extra_debug_sigprint();
...
template<typename eT> arma_deprecated
inline
void Mat<eT>::reshape(const uword in_rows, const uword in_cols, const uword dim) {
	arma_extra_debug_sigprint();
...
template<typename eT> template<typename eT2, typename expr> inline
void Mat<eT>::copy_size(const Base<eT2, expr>& X) {
	arma_extra_debug_sigprint();
...
template<typename eT> template<typename functor> inline
const Mat<eT>& Mat<eT>::for_each(functor F) {
	arma_extra_debug_sigprint();
...
template<typename eT> template<typename functor> inline
const Mat<eT>& Mat<eT>::for_each(functor F) const {
	arma_extra_debug_sigprint();
...
template<typename eT> template<typename functor> inline
const Mat<eT>& Mat<eT>::transform(functor F) {
	arma_extra_debug_sigprint();
	...
	if(ii < N) { out_mem[ii] = eT(F(out_mem[ii])); }
...
template<typename eT> template<typename functor> inline
const Mat<eT>& Mat<eT>::imbue(functor F) {
	arma_extra_debug_sigprint();
	...
	if(ii < N) { out_mem[ii] = eT(F()); }

	return *this;
}

template<typename eT> inline
const Mat<eT>& Mat<eT>::replace(const eT old_val, const eT new_val) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
const Mat<eT>& Mat<eT>::fill(const eT val) {
	arma_extra_debug_sigprint();
...
//! fill the matrix with the specified value
template<typename eT> template<typename fill_type> inline
const Mat<eT>& Mat<eT>::fill(const fill::fill_class<fill_type>&) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
const Mat<eT>& Mat<eT>::zeros(const uword in_n_rows, const uword in_n_cols) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
const Mat<eT>& Mat<eT>::zeros(const SizeMat& s) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
const Mat<eT>& Mat<eT>::ones(const uword in_rows, const uword in_cols) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
const Mat<eT>& Mat<eT>::ones(const SizeMat& s) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
const Mat<eT>& Mat<eT>::randu(const uword in_rows, const uword in_cols) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
const Mat<eT>& Mat<eT>::randu(const SizeMat& s) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
const Mat<eT>& Mat<eT>::randn(const uword in_rows, const uword in_cols) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
const Mat<eT>& Mat<eT>::randn(const SizeMat& s) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
const Mat<eT>& Mat<eT>::eye(const uword in_rows, const uword in_cols) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
const Mat<eT>& Mat<eT>::eye(const SizeMat& s) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
arma_cold
bool Mat<eT>::save(const std::string name, const file_type type, const bool print_status) const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
arma_cold
bool Mat<eT>::save(const hdf5_name& spec, const file_type type, const bool print_status) const {
	arma_extra_debug_sigprint();
	...
	} else { save_okay = diskio::save_hdf5_binary(*this, spec, err_msg); }

	if((print_status == true) && (save_okay == false)) { if(err_msg.length() > 0) { arma_debug_warn("Mat::save(): ", err_msg, spec.filename); } else { arma_debug_warn("Mat::save(): couldn't write to ", spec.filename); } }
...
template<typename eT> inline
arma_cold
bool Mat<eT>::save(std::ostream& os, const file_type type, const bool print_status) const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
arma_cold
bool Mat<eT>::load(const std::string name, const file_type type, const bool print_status) {
	arma_extra_debug_sigprint();
	...
	if((print_status == true) && (load_okay == false)) { if(err_msg.length() > 0) { arma_debug_warn("Mat::load(): ", err_msg, name); } else { arma_debug_warn("Mat::load(): couldn't read ", name); } }
...
template<typename eT> inline
arma_cold
bool Mat<eT>::load(const hdf5_name& spec, const file_type type, const bool print_status) {
	arma_extra_debug_sigprint();
	...
	} else { load_okay = diskio::load_hdf5_binary(*this, spec, err_msg); }

	if((print_status == true) && (load_okay == false)) { if(err_msg.length() > 0) { arma_debug_warn("Mat::load(): ", err_msg, spec.filename); } else { arma_debug_warn("Mat::load(): couldn't read ", spec.filename); } }
...
template<typename eT> inline
arma_cold
bool Mat<eT>::load(std::istream& is, const file_type type, const bool print_status) {
	arma_extra_debug_sigprint();
	...
	if((print_status == true) && (load_okay == false)) { if(err_msg.length() > 0) { arma_debug_warn("Mat::load(): ", err_msg, "the given stream"); } else { arma_debug_warn("Mat::load(): couldn't load from the given stream"); } }
...
template<typename eT> inline
arma_cold
bool Mat<eT>::quiet_save(const std::string name, const file_type type) const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
arma_cold
bool Mat<eT>::quiet_save(const hdf5_name& spec, const file_type type) const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
arma_cold
bool Mat<eT>::quiet_save(std::ostream& os, const file_type type) const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
arma_cold
bool Mat<eT>::quiet_load(const std::string name, const file_type type) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
arma_cold
bool Mat<eT>::quiet_load(const hdf5_name& spec, const file_type type) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
arma_cold
bool Mat<eT>::quiet_load(std::istream& is, const file_type type) {
	arma_extra_debug_sigprint();

	return (*this).load(is, type, false);
}

template<typename eT> inline Mat<eT>::row_iterator::row_iterator()
	: M(NULL)
	, current_ptr(NULL)
	, current_row(0)
	, current_col(0) {
...
template<typename eT> inline Mat<eT>::row_iterator::row_iterator(const row_iterator& X)
	: M(X.M)
...
template<typename eT> inline Mat<eT>::row_iterator::row_iterator(Mat<eT>& in_M, const uword in_row)
	: M(&in_M)
...
template<typename eT> inline
arma_warn_unused
eT& Mat<eT>::row_iterator::operator*() { return (*current_ptr); }

template<typename eT> inline
typename Mat<eT>::row_iterator& Mat<eT>::row_iterator::operator++() {
	current_col++;
...
template<typename eT> inline
arma_warn_unused
typename Mat<eT>::row_iterator Mat<eT>::row_iterator::operator++(int) {
...
template<typename eT> inline
arma_warn_unused
bool Mat<eT>::row_iterator::operator!=(const typename Mat<eT>::row_iterator& X) const { return (current_ptr != X.current_ptr); }
...
template<typename eT> inline
arma_warn_unused
bool Mat<eT>::row_iterator::operator!=(const typename Mat<eT>::const_row_iterator& X) const { return (current_ptr != X.current_ptr); }

template<typename eT> inline
arma_warn_unused
bool Mat<eT>::row_iterator::operator==(const typename Mat<eT>::const_row_iterator& X) const { return (current_ptr == X.current_ptr); }

template<typename eT> inline Mat<eT>::const_row_iterator::const_row_iterator()
	: M(NULL)
	, current_ptr(NULL)
	, current_row(0)
	, current_col(0) {
...
template<typename eT> inline Mat<eT>::const_row_iterator::const_row_iterator(const typename Mat<eT>::row_iterator& X)
	: M(X.M)
	, current_ptr(X.current_ptr)
	, current_row(X.current_row)
	, current_col(X.current_col) {
...
template<typename eT> inline Mat<eT>::const_row_iterator::const_row_iterator(const typename Mat<eT>::const_row_iterator& X)
	: M(X.M)
	, current_ptr(X.current_ptr)
	, current_row(X.current_row)
	, current_col(X.current_col) {
...
template<typename eT> inline Mat<eT>::const_row_iterator::const_row_iterator(const Mat<eT>& in_M, const uword in_row)
	: M(&in_M)
	, current_ptr(&(in_M.at(in_row, 0)))
	, current_row(in_row)
	, current_col(0) {
...
template<typename eT> inline
arma_warn_unused
const eT& Mat<eT>::const_row_iterator::operator*() const { return (*current_ptr); }

template<typename eT> inline
typename Mat<eT>::const_row_iterator& Mat<eT>::const_row_iterator::operator++() {
	current_col++;
...
template<typename eT> inline
arma_warn_unused
typename Mat<eT>::const_row_iterator Mat<eT>::const_row_iterator::operator++(int) {
...
template<typename eT> inline
arma_warn_unused
bool Mat<eT>::const_row_iterator::operator!=(const typename Mat<eT>::row_iterator& X) const { return (current_ptr != X.current_ptr); }
...
template<typename eT> inline
arma_warn_unused
bool Mat<eT>::const_row_iterator::operator!=(const typename Mat<eT>::const_row_iterator& X) const { return (current_ptr != X.current_ptr); }

template<typename eT> inline
arma_warn_unused
bool Mat<eT>::const_row_iterator::operator==(const typename Mat<eT>::const_row_iterator& X) const { return (current_ptr == X.current_ptr); }

template<typename eT> inline Mat<eT>::row_col_iterator::row_col_iterator()
	: M(NULL)
	, current_ptr(NULL)
	, current_col(0)
	, current_row(0) {
...
template<typename eT> inline Mat<eT>::row_col_iterator::row_col_iterator(const row_col_iterator& in_it)
	: M(in_it.M)
...
template<typename eT> inline Mat<eT>::row_col_iterator::row_col_iterator(Mat<eT>& in_M, const uword in_row, const uword in_col)
	: M(&in_M)
...
template<typename eT> inline
arma_warn_unused
eT& Mat<eT>::row_col_iterator::operator*() { return *current_ptr; }

template<typename eT> inline
typename Mat<eT>::row_col_iterator& Mat<eT>::row_col_iterator::operator++() {
	current_ptr++;
	current_row++;
...
template<typename eT> inline
arma_warn_unused
typename Mat<eT>::row_col_iterator Mat<eT>::row_col_iterator::operator++(int) {
...
template<typename eT> inline typename Mat<eT>::row_col_iterator& Mat<eT>::row_col_iterator::operator--() {
	if(current_row > 0) {
		current_ptr--;
		current_row--;
...
template<typename eT> inline
arma_warn_unused
uword Mat<eT>::row_col_iterator::row() const { return current_row; }

template<typename eT> inline
arma_warn_unused
uword Mat<eT>::row_col_iterator::col() const { return current_col; }

template<typename eT> inline
arma_warn_unused
bool Mat<eT>::row_col_iterator::operator==(const row_col_iterator& rhs) const { return (current_ptr == rhs.current_ptr); }
...
template<typename eT> inline
arma_warn_unused
bool Mat<eT>::row_col_iterator::operator==(const const_row_col_iterator& rhs) const { return (current_ptr == rhs.current_ptr); }

template<typename eT> inline
arma_warn_unused
bool Mat<eT>::row_col_iterator::operator!=(const const_row_col_iterator& rhs) const { return (current_ptr != rhs.current_ptr); }

template<typename eT> inline Mat<eT>::const_row_col_iterator::const_row_col_iterator()
	: M(NULL)
	, current_ptr(NULL)
	, current_col(0)
	, current_row(0) {
...
template<typename eT> inline Mat<eT>::const_row_col_iterator::const_row_col_iterator(const row_col_iterator& in_it)
	: M(in_it.M)
	, current_ptr(in_it.current_ptr)
...
template<typename eT> inline Mat<eT>::const_row_col_iterator::const_row_col_iterator(const const_row_col_iterator& in_it)
	: M(in_it.M)
	, current_ptr(in_it.current_ptr)
	, current_col(in_it.col())
	, current_row(in_it.row()) {
...
template<typename eT> inline Mat<eT>::const_row_col_iterator::const_row_col_iterator(const Mat<eT>& in_M, const uword in_row, const uword in_col)
	: M(&in_M)
	, current_ptr(&in_M.at(in_row, in_col))
	, current_col(in_col)
	, current_row(in_row) {
...
template<typename eT> inline
arma_warn_unused
const eT& Mat<eT>::const_row_col_iterator::operator*() const { return *current_ptr; }
...
template<typename eT> inline
arma_warn_unused
typename Mat<eT>::const_row_col_iterator Mat<eT>::const_row_col_iterator::operator++(int) {
...
template<typename eT> inline
arma_warn_unused
uword Mat<eT>::const_row_col_iterator::row() const { return current_row; }

template<typename eT> inline
arma_warn_unused
uword Mat<eT>::const_row_col_iterator::col() const { return current_col; }

template<typename eT> inline
arma_warn_unused
bool Mat<eT>::const_row_col_iterator::operator==(const const_row_col_iterator& rhs) const { return (current_ptr == rhs.current_ptr); }
...
template<typename eT> inline
arma_warn_unused
bool Mat<eT>::const_row_col_iterator::operator==(const row_col_iterator& rhs) const { return (current_ptr == rhs.current_ptr); }
...
template<typename eT> inline
typename Mat<eT>::iterator Mat<eT>::begin() {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
typename Mat<eT>::const_iterator Mat<eT>::begin() const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
typename Mat<eT>::const_iterator Mat<eT>::cbegin() const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
typename Mat<eT>::iterator Mat<eT>::end() {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
typename Mat<eT>::const_iterator Mat<eT>::end() const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
typename Mat<eT>::const_iterator Mat<eT>::cend() const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
typename Mat<eT>::col_iterator Mat<eT>::begin_col(const uword col_num) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
typename Mat<eT>::const_col_iterator Mat<eT>::begin_col(const uword col_num) const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
typename Mat<eT>::col_iterator Mat<eT>::end_col(const uword col_num) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
typename Mat<eT>::const_col_iterator Mat<eT>::end_col(const uword col_num) const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
typename Mat<eT>::row_iterator Mat<eT>::begin_row(const uword row_num) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
typename Mat<eT>::const_row_iterator Mat<eT>::begin_row(const uword row_num) const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
typename Mat<eT>::row_iterator Mat<eT>::end_row(const uword row_num) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
typename Mat<eT>::const_row_iterator Mat<eT>::end_row(const uword row_num) const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
typename Mat<eT>::row_col_iterator Mat<eT>::begin_row_col() { return row_col_iterator(*this); }

template<typename eT> inline
typename Mat<eT>::const_row_col_iterator Mat<eT>::begin_row_col() const { return const_row_col_iterator(*this); }

template<typename eT> inline typename Mat<eT>::row_col_iterator Mat<eT>::end_row_col() { return row_col_iterator(*this, 0, n_cols); }

template<typename eT> inline typename Mat<eT>::const_row_col_iterator Mat<eT>::end_row_col() const { return const_row_col_iterator(*this, 0, n_cols); }

//! resets this matrix to an empty matrix
template<typename eT> inline
void Mat<eT>::clear() { reset(); }

//! returns true if the matrix has no elements
template<typename eT> inline
bool Mat<eT>::empty() const { return (n_elem == 0); }

//! returns the number of elements in this matrix
template<typename eT> inline
uword Mat<eT>::size() const { return n_elem; }

template<typename eT> template<uword fixed_n_rows, uword fixed_n_cols> arma_inline Mat<eT>::fixed<fixed_n_rows, fixed_n_cols>::fixed()
	: Mat<eT>(arma_fixed_indicator(), fixed_n_rows, fixed_n_cols, 0, ((use_extra) ? mem_local_extra : Mat<eT>::mem_local)) {
	arma_extra_debug_sigprint_this(this);
}

template<typename eT> template<uword fixed_n_rows, uword fixed_n_cols> arma_inline Mat<eT>::fixed<fixed_n_rows, fixed_n_cols>::fixed(const fixed<fixed_n_rows, fixed_n_cols>& X)
	: Mat<eT>(arma_fixed_indicator(), fixed_n_rows, fixed_n_cols, 0, ((use_extra) ? mem_local_extra : Mat<eT>::mem_local)) {
	arma_extra_debug_sigprint_this(this);
...
template<typename eT> template<uword fixed_n_rows, uword fixed_n_cols> template<typename fill_type> inline Mat<eT>::fixed<fixed_n_rows, fixed_n_cols>::fixed(const fill::fill_class<fill_type>&)
	: Mat<eT>(arma_fixed_indicator(), fixed_n_rows, fixed_n_cols, 0, ((use_extra) ? mem_local_extra : Mat<eT>::mem_local)) {
	arma_extra_debug_sigprint_this(this);
...
template<typename eT> template<uword fixed_n_rows, uword fixed_n_cols> template<typename T1> inline Mat<eT>::fixed<fixed_n_rows, fixed_n_cols>::fixed(const Base<eT, T1>& A)
	: Mat<eT>(arma_fixed_indicator(), fixed_n_rows, fixed_n_cols, 0, ((use_extra) ? mem_local_extra : Mat<eT>::mem_local)) {
	arma_extra_debug_sigprint_this(this);
...
template<typename eT> template<uword fixed_n_rows, uword fixed_n_cols> template<typename T1, typename T2> inline Mat<eT>::fixed<fixed_n_rows, fixed_n_cols>::fixed(const Base<pod_type, T1>& A, const Base<pod_type, T2>& B)
	: Mat<eT>(arma_fixed_indicator(), fixed_n_rows, fixed_n_cols, 0, ((use_extra) ? mem_local_extra : Mat<eT>::mem_local)) {
	arma_extra_debug_sigprint_this(this);
...
template<typename eT> template<uword fixed_n_rows, uword fixed_n_cols> inline Mat<eT>::fixed<fixed_n_rows, fixed_n_cols>::fixed(const eT* aux_mem)
	: Mat<eT>(arma_fixed_indicator(), fixed_n_rows, fixed_n_cols, 0, ((use_extra) ? mem_local_extra : Mat<eT>::mem_local)) {
	arma_extra_debug_sigprint_this(this);
...
template<typename eT> template<uword fixed_n_rows, uword fixed_n_cols> inline Mat<eT>::fixed<fixed_n_rows, fixed_n_cols>::fixed(const char* text)
	: Mat<eT>(arma_fixed_indicator(), fixed_n_rows, fixed_n_cols, 0, ((use_extra) ? mem_local_extra : Mat<eT>::mem_local)) {
	arma_extra_debug_sigprint_this(this);
...
template<typename eT> template<uword fixed_n_rows, uword fixed_n_cols> inline Mat<eT>::fixed<fixed_n_rows, fixed_n_cols>::fixed(const std::string& text)
	: Mat<eT>(arma_fixed_indicator(), fixed_n_rows, fixed_n_cols, 0, ((use_extra) ? mem_local_extra : Mat<eT>::mem_local)) {
	arma_extra_debug_sigprint_this(this);
...
template<typename eT> template<uword fixed_n_rows, uword fixed_n_cols> inline Mat<eT>::fixed<fixed_n_rows, fixed_n_cols>::fixed(const std::initializer_list<eT>& list)
	: Mat<eT>(arma_fixed_indicator(), fixed_n_rows, fixed_n_cols, 0, ((use_extra) ? mem_local_extra : Mat<eT>::mem_local)) {
	arma_extra_debug_sigprint_this(this);
...
template<typename eT> template<uword fixed_n_rows, uword fixed_n_cols> inline
Mat<eT>& Mat<eT>::fixed<fixed_n_rows, fixed_n_cols>::operator=(const std::initializer_list<eT>& list) {
	arma_extra_debug_sigprint();
...
template<typename eT> template<uword fixed_n_rows, uword fixed_n_cols> inline Mat<eT>::fixed<fixed_n_rows, fixed_n_cols>::fixed(const std::initializer_list<std::initializer_list<eT>>& list)
	: Mat<eT>(arma_fixed_indicator(), fixed_n_rows, fixed_n_cols, 0, ((use_extra) ? mem_local_extra : Mat<eT>::mem_local)) {
	arma_extra_debug_sigprint_this(this);
...
template<typename eT> template<uword fixed_n_rows, uword fixed_n_cols> inline
Mat<eT>& Mat<eT>::fixed<fixed_n_rows, fixed_n_cols>::operator=(const std::initializer_list<std::initializer_list<eT>>& list) {
	arma_extra_debug_sigprint();
...
template<typename eT> template<uword fixed_n_rows, uword fixed_n_cols> arma_inline
Mat<eT>& Mat<eT>::fixed<fixed_n_rows, fixed_n_cols>::operator=(const fixed<fixed_n_rows, fixed_n_cols>& X) {
	arma_extra_debug_sigprint();
...
template<typename eT> template<uword fixed_n_rows, uword fixed_n_cols> arma_inline
const Op<typename Mat<eT>::template fixed<fixed_n_rows, fixed_n_cols>::Mat_fixed_type, op_htrans> Mat<eT>::fixed<fixed_n_rows, fixed_n_cols>::t() const { return Op<typename Mat<eT>::template fixed<fixed_n_rows, fixed_n_cols>::Mat_fixed_type, op_htrans>(*this); }

template<typename eT> template<uword fixed_n_rows, uword fixed_n_cols> arma_inline
const Op<typename Mat<eT>::template fixed<fixed_n_rows, fixed_n_cols>::Mat_fixed_type, op_htrans> Mat<eT>::fixed<fixed_n_rows, fixed_n_cols>::ht() const { return Op<typename Mat<eT>::template fixed<fixed_n_rows, fixed_n_cols>::Mat_fixed_type, op_htrans>(*this); }

template<typename eT> template<uword fixed_n_rows, uword fixed_n_cols> arma_inline
const Op<typename Mat<eT>::template fixed<fixed_n_rows, fixed_n_cols>::Mat_fixed_type, op_strans> Mat<eT>::fixed<fixed_n_rows, fixed_n_cols>::st() const { return Op<typename Mat<eT>::template fixed<fixed_n_rows, fixed_n_cols>::Mat_fixed_type, op_strans>(*this); }

template<typename eT> template<uword fixed_n_rows, uword fixed_n_cols> arma_inline
arma_warn_unused
const eT& Mat<eT>::fixed<fixed_n_rows, fixed_n_cols>::at_alt(const uword ii) const {
...
template<typename eT> template<uword fixed_n_rows, uword fixed_n_cols> arma_inline
arma_warn_unused
eT& Mat<eT>::fixed<fixed_n_rows, fixed_n_cols>::operator[](const uword ii) { return (use_extra) ? mem_local_extra[ii] : mem_local[ii]; }

template<typename eT> template<uword fixed_n_rows, uword fixed_n_cols> arma_inline
arma_warn_unused
const eT& Mat<eT>::fixed<fixed_n_rows, fixed_n_cols>::operator[](const uword ii) const { return (use_extra) ? mem_local_extra[ii] : mem_local[ii]; }

template<typename eT> template<uword fixed_n_rows, uword fixed_n_cols> arma_inline
arma_warn_unused
eT& Mat<eT>::fixed<fixed_n_rows, fixed_n_cols>::at(const uword ii) { return (use_extra) ? mem_local_extra[ii] : mem_local[ii]; }

template<typename eT> template<uword fixed_n_rows, uword fixed_n_cols> arma_inline
arma_warn_unused
const eT& Mat<eT>::fixed<fixed_n_rows, fixed_n_cols>::at(const uword ii) const { return (use_extra) ? mem_local_extra[ii] : mem_local[ii]; }

template<typename eT> template<uword fixed_n_rows, uword fixed_n_cols> arma_inline
arma_warn_unused
eT& Mat<eT>::fixed<fixed_n_rows, fixed_n_cols>::operator()(const uword ii) {
...
template<typename eT> template<uword fixed_n_rows, uword fixed_n_cols> arma_inline
arma_warn_unused
const eT& Mat<eT>::fixed<fixed_n_rows, fixed_n_cols>::operator()(const uword ii) const {
	arma_debug_check((ii >= fixed_n_elem), "Mat::operator(): index out of bounds");
...
template<typename eT> template<uword fixed_n_rows, uword fixed_n_cols> arma_inline
arma_warn_unused
eT& Mat<eT>::fixed<fixed_n_rows, fixed_n_cols>::at(const uword in_row, const uword in_col) {
...
template<typename eT> template<uword fixed_n_rows, uword fixed_n_cols> arma_inline
arma_warn_unused
const eT& Mat<eT>::fixed<fixed_n_rows, fixed_n_cols>::at(const uword in_row, const uword in_col) const {
	const uword iq = in_row + in_col * fixed_n_rows;
...
template<typename eT> template<uword fixed_n_rows, uword fixed_n_cols> arma_inline
arma_warn_unused
eT& Mat<eT>::fixed<fixed_n_rows, fixed_n_cols>::operator()(const uword in_row, const uword in_col) {
...
template<typename eT> template<uword fixed_n_rows, uword fixed_n_cols> arma_inline
arma_warn_unused
const eT& Mat<eT>::fixed<fixed_n_rows, fixed_n_cols>::operator()(const uword in_row, const uword in_col) const {
	arma_debug_check(((in_row >= fixed_n_rows) || (in_col >= fixed_n_cols)), "Mat::operator(): index out of bounds");
...
template<typename eT> template<uword fixed_n_rows, uword fixed_n_cols> arma_inline
arma_warn_unused
eT* Mat<eT>::fixed<fixed_n_rows, fixed_n_cols>::colptr(const uword in_col) {
...
template<typename eT> template<uword fixed_n_rows, uword fixed_n_cols> arma_inline
arma_warn_unused
const eT* Mat<eT>::fixed<fixed_n_rows, fixed_n_cols>::colptr(const uword in_col) const {
...
template<typename eT> template<uword fixed_n_rows, uword fixed_n_cols> arma_inline
arma_warn_unused
eT* Mat<eT>::fixed<fixed_n_rows, fixed_n_cols>::memptr() { return (use_extra) ? mem_local_extra : mem_local; }

template<typename eT> template<uword fixed_n_rows, uword fixed_n_cols> arma_inline
arma_warn_unused
const eT* Mat<eT>::fixed<fixed_n_rows, fixed_n_cols>::memptr() const { return (use_extra) ? mem_local_extra : mem_local; }

template<typename eT> template<uword fixed_n_rows, uword fixed_n_cols> arma_inline
arma_warn_unused
bool Mat<eT>::fixed<fixed_n_rows, fixed_n_cols>::is_vec() const { return ((fixed_n_rows == 1) || (fixed_n_cols == 1)); }

template<typename eT> template<uword fixed_n_rows, uword fixed_n_cols> inline
const Mat<eT>& Mat<eT>::fixed<fixed_n_rows, fixed_n_cols>::fill(const eT val) {
	arma_extra_debug_sigprint();
...
template<typename eT> template<uword fixed_n_rows, uword fixed_n_cols> inline
const Mat<eT>& Mat<eT>::fixed<fixed_n_rows, fixed_n_cols>::zeros() {
	arma_extra_debug_sigprint();
...
template<typename eT> template<uword fixed_n_rows, uword fixed_n_cols> inline
const Mat<eT>& Mat<eT>::fixed<fixed_n_rows, fixed_n_cols>::ones() {
	arma_extra_debug_sigprint();
...
//! prefix ++
template<typename eT> inline
void Mat_aux::prefix_pp(Mat<eT>& x) {
	eT* memptr = x.memptr();
	...
	if(i < n_elem) { ++(memptr[i]); }
}

//! prefix ++ for complex numbers (work around for limitations of the std::complex class)
template<typename T> inline
void Mat_aux::prefix_pp(Mat<std::complex<T>>& x) { x += T(1); }

//! postfix ++
template<typename eT> inline
void Mat_aux::postfix_pp(Mat<eT>& x) {
	eT* memptr = x.memptr();
	const uword n_elem = x.n_elem;
...
template<typename T> inline
void Mat_aux::postfix_pp(Mat<std::complex<T>>& x) { x += T(1); }

//! prefix --
template<typename eT> inline
void Mat_aux::prefix_mm(Mat<eT>& x) {
	eT* memptr = x.memptr();
	const uword n_elem = x.n_elem;
...
//! prefix -- for complex numbers (work around for limitations of the std::complex class)
template<typename T> inline
void Mat_aux::prefix_mm(Mat<std::complex<T>>& x) { x -= T(1); }

//! postfix --
template<typename eT> inline
void Mat_aux::postfix_mm(Mat<eT>& x) {
	eT* memptr = x.memptr();
	const uword n_elem = x.n_elem;
...
//! postfix ++ for complex numbers (work around for limitations of the std::complex class)
template<typename T> inline
void Mat_aux::postfix_mm(Mat<std::complex<T>>& x) { x -= T(1); }

template<typename eT, typename T1> inline
void Mat_aux::set_real(Mat<eT>& out, const Base<eT, T1>& X) {
	arma_extra_debug_sigprint();
...
template<typename eT, typename T1> inline
void Mat_aux::set_imag(Mat<eT>&, const Base<eT, T1>&) {
	arma_extra_debug_sigprint();
}

template<typename T, typename T1> inline
void Mat_aux::set_real(Mat<std::complex<T>>& out, const Base<T, T1>& X) {
	arma_extra_debug_sigprint();
	...
	if(Proxy<T1>::use_at == false) {
		...
		for(uword i = 0; i < N; ++i) { out_mem[i] = std::complex<T>(A[i], out_mem[i].imag()); }
	} else {
...
template<typename T, typename T1> inline
void Mat_aux::set_imag(Mat<std::complex<T>>& out, const Base<T, T1>& X) {
	arma_extra_debug_sigprint();
	...
	if(Proxy<T1>::use_at == false) {
		...
		for(uword i = 0; i < N; ++i) { out_mem[i] = std::complex<T>(out_mem[i].real(), A[i]); }
	} else {
		for(uword col = 0; col < local_n_cols; ++col)
			for(uword row = 0; row < local_n_rows; ++row) {
