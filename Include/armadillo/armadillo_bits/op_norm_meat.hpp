...
inline
typename T1::pod_type op_norm::vec_norm_1(const Proxy<T1>& P, const typename arma_not_cx<typename T1::elem_type>::result* junk) {
...
template<typename T1> arma_hot
inline
typename T1::pod_type op_norm::vec_norm_1(const Proxy<T1>& P, const typename arma_cx_only<typename T1::elem_type>::result* junk) {
	arma_extra_debug_sigprint();
	arma_ignore(junk);
...
inline
eT op_norm::vec_norm_1_direct_std(const Mat<eT>& X) {
	arma_extra_debug_sigprint();
...
template<typename eT> arma_hot
inline
eT op_norm::vec_norm_1_direct_mem(const uword N, const eT* A) {
	arma_extra_debug_sigprint();
...
template<typename T1> arma_hot
inline
typename T1::pod_type op_norm::vec_norm_2(const Proxy<T1>& P, const typename arma_not_cx<typename T1::elem_type>::result* junk) {
	arma_extra_debug_sigprint();
	arma_ignore(junk);
...
template<typename T1> arma_hot
inline
typename T1::pod_type op_norm::vec_norm_2(const Proxy<T1>& P, const typename arma_cx_only<typename T1::elem_type>::result* junk) {
	arma_extra_debug_sigprint();
	arma_ignore(junk);
...
template<typename eT> arma_hot
inline
eT op_norm::vec_norm_2_direct_std(const Mat<eT>& X) {
	arma_extra_debug_sigprint();
...
template<typename eT> arma_hot
inline
eT op_norm::vec_norm_2_direct_mem(const uword N, const eT* A) {
	arma_extra_debug_sigprint();
...
template<typename eT> arma_hot
inline
eT op_norm::vec_norm_2_direct_robust(const Mat<eT>& X) {
	arma_extra_debug_sigprint();
...
template<typename T1> arma_hot
inline
typename T1::pod_type op_norm::vec_norm_k(const Proxy<T1>& P, const int k) {
	arma_extra_debug_sigprint();
	...
	if(Proxy<T1>::use_at == false) {
		...
		if(n_rows != 1) { for(uword col = 0; col < n_cols; ++col) for(uword row = 0; row < n_rows; ++row) { acc += std::pow(std::abs(P.at(row, col)), k); } } else { for(uword col = 0; col < n_cols; ++col) { acc += std::pow(std::abs(P.at(0, col)), k); } }
	}
...
template<typename T1> arma_hot
inline
typename T1::pod_type op_norm::vec_norm_max(const Proxy<T1>& P) {
	arma_extra_debug_sigprint();
...
template<typename T1> arma_hot
inline
typename T1::pod_type op_norm::vec_norm_min(const Proxy<T1>& P) {
	arma_extra_debug_sigprint();
