...
void spglue_times::apply(SpMat<typename T1::elem_type>& out, const SpGlue<T1, T2, spglue_times>& X) {
...
template<typename T1, typename T2> inline
void spglue_times::apply(SpMat<typename T1::elem_type>& out, const SpGlue<SpOp<T1, spop_scalar_times>, T2, spglue_times>& X) {
	arma_extra_debug_sigprint();
...
inline
void spglue_times::apply_noalias(SpMat<eT>& c, const SpMat<eT>& x, const SpMat<eT>& y) {
	arma_extra_debug_sigprint();
...
template<typename T1, typename T2> inline
void spglue_times_misc::sparse_times_dense(Mat<typename T1::elem_type>& out, const T1& x, const T2& y) {
	arma_extra_debug_sigprint();
	...
	if(is_op_diagmat<T2>::value) {
		...
		if(B_n_cols >= (B_n_rows / uword(100))) {
			...
			while(A_it != A_it_end) {
				...
				for(uword col = 0; col < B_n_cols; ++col) { out.at(A_it_row, col) += A_it_val * B.at(A_it_col, col); }
...
template<typename T1, typename T2> inline
void spglue_times_misc::dense_times_sparse(Mat<typename T1::elem_type>& out, const T1& x, const T2& y) {
	arma_extra_debug_sigprint();
          ...
          for(uword i=0; i < Y_n_cols; ++i)
            ...
            out.col(i) = X.cols(indices) * Y_col;
				...
				while(y_it != y_it_end) {
					...
					for(uword row = 0; row < out_n_rows; ++row) { out_col[row] += pa.at(row, y_it_row) * y_it_val; }
...
template<typename T1, typename T2> inline
void spglue_times_mixed::sparse_times_sparse(SpMat<typename promote_type<typename T1::elem_type, typename T2::elem_type>::result>& out, const T1& X, const T2& Y) {
	arma_extra_debug_sigprint();
...
template<typename T1, typename T2> inline
void spglue_times_mixed::sparse_times_dense(Mat<typename promote_type<typename T1::elem_type, typename T2::elem_type>::result>& out, const T1& X, const T2& Y) {
	arma_extra_debug_sigprint();
...
template<typename T1, typename T2> inline
void spglue_times_mixed::dense_times_sparse(Mat<typename promote_type<typename T1::elem_type, typename T2::elem_type>::result>& out, const T1& X, const T2& Y) {
	arma_extra_debug_sigprint();
