...
template<typename obj_type> arma_warn_unused
inline
obj_type sprandn(const SizeMat& s, const double density, const typename arma_SpMat_SpCol_SpRow_only<obj_type>::result* junk = 0) {
	arma_extra_debug_sigprint();
	arma_ignore(junk);
...
inline
sp_mat sprandn(const uword n_rows, const uword n_cols, const double density) {
	arma_extra_debug_sigprint();
...
inline
sp_mat sprandn(const SizeMat& s, const double density) {
	arma_extra_debug_sigprint();
...
inline
SpMat<typename T1::elem_type> sprandn(const SpBase<typename T1::elem_type, T1>& X) {
	arma_extra_debug_sigprint();
