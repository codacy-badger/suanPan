...
void op_diff::apply_noalias(Mat<eT>& out, const Mat<eT>& X, const uword k, const uword dim) {
...
void op_diff::apply(Mat<typename T1::elem_type>& out, const Op<T1, op_diff>& in) {
	arma_extra_debug_sigprint();
...
template<typename T1> inline
void op_diff_default::apply(Mat<typename T1::elem_type>& out, const Op<T1, op_diff_default>& in) {
	arma_extra_debug_sigprint();
