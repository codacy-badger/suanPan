...
(
	...
	if(is_SpCol<obj_type>::value) { arma_debug_check((n_cols != 1), "sprandu(): incompatible size"); } else if(is_SpRow<obj_type>::value) { arma_debug_check((n_rows != 1), "sprandu(): incompatible size"); }
...
template<typename obj_type> arma_warn_unused
inline
obj_type sprandu(const SizeMat& s, const double density, const typename arma_SpMat_SpCol_SpRow_only<obj_type>::result* junk = 0) {
	arma_extra_debug_sigprint();
	arma_ignore(junk);
...
inline
sp_mat sprandu(const uword n_rows, const uword n_cols, const double density) {
	arma_extra_debug_sigprint();
...
inline
sp_mat sprandu(const SizeMat& s, const double density) {
	arma_extra_debug_sigprint();
...
inline
SpMat<typename T1::elem_type> sprandu(const SpBase<typename T1::elem_type, T1>& X) {
	arma_extra_debug_sigprint();
