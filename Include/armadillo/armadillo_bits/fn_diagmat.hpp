...
>::result diagmat(const T1& X, const sword k) {
	arma_extra_debug_sigprint();
...
template<typename T1> arma_warn_unused
inline
const SpOp<T1, spop_diagmat> diagmat(const SpBase<typename T1::elem_type, T1>& X) {
	arma_extra_debug_sigprint();
...
template<typename T1> arma_warn_unused
inline
const SpOp<T1, spop_diagmat2> diagmat(const SpBase<typename T1::elem_type, T1>& X, const sword k) {
	arma_extra_debug_sigprint();
