...
template<typename T1> arma_warn_unused
inline
typename T1::elem_type max(const Op<T1, op_max>& in) {
	arma_extra_debug_sigprint();
...
template<typename T1> arma_warn_unused
arma_inline
const Op<Op<T1, op_max>, op_max> max(const Op<T1, op_max>& in, const uword dim) {
	arma_extra_debug_sigprint();
...
arma_inline
const typename arma_scalar_only<T>::result& max(const T& x) { return x; }
...
template<typename T1, typename T2> arma_warn_unused
arma_inline
const GlueCube<T1, T2, glue_max> max
(
	const BaseCube<typename T1::elem_type, T1>& X,
...
>::result max(const T1& X, const uword dim = 0) {
	arma_extra_debug_sigprint();
...
template<typename T1> arma_warn_unused
inline
typename T1::elem_type max(const SpOp<T1, spop_max>& X) {
	arma_extra_debug_sigprint();
	arma_extra_debug_print("max(): two consecutive max() calls detected");
...
template<typename T1> arma_warn_unused
inline
const SpOp<SpOp<T1, spop_max>, spop_max> max(const SpOp<T1, spop_max>& in, const uword dim) {
	arma_extra_debug_sigprint();
...
inline
uword max(const SizeMat& s) { return (std::max)(s.n_rows, s.n_cols); }

arma_warn_unused

inline
uword max(const SizeCube& s) { return (std::max)((std::max)(s.n_rows, s.n_cols), s.n_slices); }
