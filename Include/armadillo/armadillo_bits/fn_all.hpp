...
arma_inline
const mtOp<uword, T1, op_all> all
(
	const T1& X,
	const uword dim = 0,
...
template<typename T1> arma_warn_unused
inline
bool all(const mtOp<uword, T1, op_all>& in) {
	arma_extra_debug_sigprint();
...
template<typename T1> arma_warn_unused
arma_inline
const Op<mtOp<uword, T1, op_all>, op_all> all(const mtOp<uword, T1, op_all>& in, const uword dim) {
	arma_extra_debug_sigprint();
