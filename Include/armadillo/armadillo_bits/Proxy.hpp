...
template<typename T1> struct Proxy_fixed {
	...
	template<typename eT2> arma_inline bool has_overlap(const subview<eT2>& X) const { return is_alias(X.m); }
...
template<typename T1> struct Proxy_redirect<T1, false> {
	typedef Proxy_default<T1> result;
};

template<typename T1> struct Proxy_redirect<T1, true> {
...
public:
	...
	template<typename eT2> arma_inline bool is_alias(const Mat<eT2>& X) const { return (is_same_type<eT, eT2>::value) ? (void_ptr(&Q) == void_ptr(&X)) : false; }
...
public:
	...
	template<typename eT2> arma_inline bool has_overlap(const subview<eT2>&) const { return false; }
...
public:
	...
	template<typename eT2> arma_inline bool has_overlap(const subview<eT2>& X) const { return Q.P.has_overlap(X); }
...
public:
	...
	template<typename eT2> arma_inline bool has_overlap(const subview<eT2>& X) const { return (Q.P1.has_overlap(X) || Q.P2.has_overlap(X)); }
...
public:
	...
	template<typename eT2> arma_inline bool is_alias(const Mat<eT2>& X) const { return (is_same_type<eT, eT2>::value) ? (void_ptr(&(Q.m)) == void_ptr(&X)) : false; }

	template<typename eT2> arma_inline bool has_overlap(const subview<eT2>& X) const { return Q.check_overlap(X); }
...
public:
	...
	arma_inline elem_type operator[](const uword i) const {
		const uword ii = (Proxy<T1>::use_at) ? R.at(i, 0) : R[i];
		arma_debug_check((ii >= Q.m.n_elem), "Mat::elem(): index out of bounds");
		return Q.m[ii];
	}

	arma_inline elem_type at(const uword row, const uword) const {
		const uword ii = (Proxy<T1>::use_at) ? R.at(row, 0) : R[row];
		arma_debug_check((ii >= Q.m.n_elem), "Mat::elem(): index out of bounds");
		return Q.m[ii];
	}
...
public:
	...
	inline explicit Proxy_diagvec_mat(const Op<T1, op_diagvec>& A)
		: R(A.m)
		, Q(R.diag((A.aux_uword_b > 0) ? -sword(A.aux_uword_a) : sword(A.aux_uword_a))) {
		arma_extra_debug_sigprint();
	}
...
template<typename T1> class Proxy<Op<T1, op_diagvec>>
...
template<typename T1> struct Proxy_xtrans_redirect<T1, false> {
	typedef Proxy_xtrans_default<T1> result;
};

template<typename T1> struct Proxy_xtrans_redirect<T1, true> {
	typedef Proxy_xtrans_vector<T1> result;
};

template<typename T1> class Proxy<Op<T1, op_htrans>>
	: public
...
public:
	...
	template<typename eT2> arma_inline bool has_overlap(const subview<eT2>& X) const { return Proxy_xtrans::has_overlap(X); }

	arma_inline bool is_aligned() const { return Proxy_xtrans::is_aligned(); }
};

template<typename T1> class Proxy<Op<T1, op_strans>>
	: public
	Proxy_xtrans_redirect
	<
		Op<T1, op_strans>,
...
template<typename eT> struct Proxy_subview_row_htrans_redirect<eT, true> {
	typedef Proxy_subview_row_htrans_cx<eT> result;
};

template<typename eT> struct Proxy_subview_row_htrans_redirect<eT, false> {
	typedef Proxy_subview_row_htrans_non_cx<eT> result;
};

template<typename eT> class Proxy<Op<subview_row<eT>, op_htrans>>
	: public
...
public:
	...
	template<typename eT2> arma_inline bool has_overlap(const subview<eT2>& X) const { return Proxy_sv_row_ht::has_overlap(X); }
...
template<typename T1> struct Proxy_vectorise_col_redirect<Op<T1, op_vectorise_col>, true> {
	typedef Proxy_vectorise_col_mat<Op<T1, op_vectorise_col>> result;
};

template<typename T1> struct Proxy_vectorise_col_redirect<Op<T1, op_vectorise_col>, false> {
	typedef Proxy_vectorise_col_expr<Op<T1, op_vectorise_col>> result;
};

template<typename T1> class Proxy<Op<T1, op_vectorise_col>>
