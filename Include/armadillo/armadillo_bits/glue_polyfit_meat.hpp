...
bool glue_polyfit::apply_noalias(Mat<eT>& out, const Col<eT>& X, const Col<eT>& Y, const uword N) {
...
bool glue_polyfit::apply_direct(Mat<typename T1::elem_type>& out, const Base<typename T1::elem_type, T1>& X_expr, const Base<typename T1::elem_type, T2>& Y_expr, const uword N) {
	arma_extra_debug_sigprint();
...
template<typename T1, typename T2> inline
void glue_polyfit::apply(Mat<typename T1::elem_type>& out, const Glue<T1, T2, glue_polyfit>& expr) {
	arma_extra_debug_sigprint();
