...
template<typename eT, typename TA> arma_cold
inline
void op_strans::apply_mat_noalias_tinysq(Mat<eT>& out, const TA& A) {
...
inline
void op_strans::block_worker(eT* Y, const eT* X, const uword X_n_rows, const uword Y_n_rows, const uword n_rows, const uword n_cols) {
...
template<typename eT> arma_hot
inline
void op_strans::apply_mat_noalias_large(Mat<eT>& out, const Mat<eT>& A) {
...
template<typename eT, typename TA> arma_hot
inline
void op_strans::apply_mat_noalias(Mat<eT>& out, const TA& A) {
	arma_extra_debug_sigprint();
...
template<typename eT> arma_hot
inline
void op_strans::apply_mat_inplace(Mat<eT>& out) {
	arma_extra_debug_sigprint();
...
inline
void op_strans::apply_proxy(Mat<typename T1::elem_type>& out, const T1& X) {
	arma_extra_debug_sigprint();
...
template<typename T1> arma_hot
inline
void op_strans::apply(Mat<typename T1::elem_type>& out, const Op<T1, op_strans>& in) {
	arma_extra_debug_sigprint();
...
template<typename eT, typename TA> arma_cold
inline
void op_strans2::apply_noalias_tinysq(Mat<eT>& out, const TA& A, const eT val) {
	const eT* Am = A.memptr();
	eT* outm = out.memptr();

	switch(A.n_rows) {
	case 1: { outm[0] = val * Am[0]; }
	break;
...
template<typename eT, typename TA> arma_hot
inline
void op_strans2::apply_noalias(Mat<eT>& out, const TA& A, const eT val) {
	arma_extra_debug_sigprint();
	...
	if((TA::is_col) || (TA::is_row) || (A_n_cols == 1) || (A_n_rows == 1)) {
		...
		if(i < N) { out_mem[i] = val * A_mem[i]; }
	} else {
		if((A_n_rows <= 4) && (A_n_rows == A_n_cols)) { op_strans2::apply_noalias_tinysq(out, A, val); } else if((A_n_rows >= 512) && (A_n_cols >= 512)) {
...
template<typename eT, typename TA> arma_hot
inline
void op_strans2::apply(Mat<eT>& out, const TA& A, const eT val) {
	arma_extra_debug_sigprint();

	if(&out != &A) { op_strans2::apply_noalias(out, A, val); } else {
		const uword n_rows = out.n_rows;
		const uword n_cols = out.n_cols;
		...
		if(n_rows == n_cols) {
			...
			for(uword k = 0; k < N; ++k) {
				...
				if(i < N) { std::swap(out.at(k, i), colptr[i]); }
			}
...
template<typename T1> arma_hot
inline
void op_strans2::apply_proxy(Mat<typename T1::elem_type>& out, const T1& X, const typename T1::elem_type val) {
	arma_extra_debug_sigprint();
	...
	if((is_Mat<typename Proxy<T1>::stored_type>::value == true) && (Proxy<T1>::fake_mat == false)) {
		...
		if((resolves_to_vector<T1>::value == true) && (Proxy<T1>::use_at == false)) {
			if(is_alias == false) {
				...
				if(i < n_elem) { out_mem[i] = val * Pea[i]; }
			} else // aliasing
			{
				Mat<eT> out2(n_cols, n_rows);
...
void op_strans_cube::apply_noalias(Cube<eT>& out, const Cube<eT>& X) {
