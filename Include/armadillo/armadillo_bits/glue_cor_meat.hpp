...
void glue_cor::direct_cor(Mat<eT>& out, const Mat<eT>& A, const Mat<eT>& B, const uword norm_type) {
...
void glue_cor::direct_cor(Mat<std::complex<T>>& out, const Mat<std::complex<T>>& A, const Mat<std::complex<T>>& B, const uword norm_type) {
	arma_extra_debug_sigprint();
...
void glue_cor::apply(Mat<typename T1::elem_type>& out, const Glue<T1, T2, glue_cor>& X) {
	arma_extra_debug_sigprint();
	...
	if(&A != &B) { glue_cor::direct_cor(out, A, B, norm_type); } else { op_cor::direct_cor(out, A, norm_type); }
}
