...
void op_expmat::apply(Mat<typename T1::elem_type>& out, const Op<T1, op_expmat>& expr) {
...
template<typename T1> inline
bool op_expmat::apply_direct(Mat<typename T1::elem_type>& out, const Base<typename T1::elem_type, T1>& expr) {
	arma_extra_debug_sigprint();
	...
	if(is_op_diagmat<T1>::value) {
		...
		Mat<eT> E(A.n_rows, A.n_rows, fill::eye);
		E += c * A;
		Mat<eT> D(A.n_rows, A.n_rows, fill::eye);
		D -= c * A;
...
template<typename T1> inline
void op_expmat_sym::apply(Mat<typename T1::elem_type>& out, const Op<T1, op_expmat_sym>& in) {
	arma_extra_debug_sigprint();
...
template<typename T1> inline
bool op_expmat_sym::apply_direct(Mat<typename T1::elem_type>& out, const Base<typename T1::elem_type, T1>& expr) {
	arma_extra_debug_sigprint();
