...
void op_index_min::apply(Mat<uword>& out, const mtOp<uword, T1, op_index_min>& in) {
	...
	if(U.is_alias(out) == false) { op_index_min::apply_noalias(out, X, dim); } else {
		Mat<uword> tmp;
...
void op_index_min::apply_noalias(Mat<uword>& out, const Mat<eT>& X, const uword dim) {
	arma_extra_debug_sigprint();
...
template<typename T1> inline
void op_index_min::apply(Cube<uword>& out, const mtOpCube<uword, T1, op_index_min>& in) {
	arma_extra_debug_sigprint();
	...
	if(U.is_alias(out) == false) { op_index_min::apply_noalias(out, U.M, dim); } else {
		Cube<uword> tmp;
...
template<typename eT> inline
void op_index_min::apply_noalias(Cube<uword>& out, const Cube<eT>& X, const uword dim, const typename arma_not_cx<eT>::result* junk) {
	arma_extra_debug_sigprint();
	arma_ignore(junk);
	...
	if(dim == 0) {
		...
		for(uword slice = 0; slice < X_n_slices; ++slice) {
			...
			for(uword col = 0; col < X_n_cols; ++col) { op_min::direct_min(X.slice_colptr(slice, col), X_n_rows, out_mem[col]); }
		}
	} else if(dim == 1) {
		arma_extra_debug_print("op_index_min::apply(): dim = 1");
...
template<typename eT> inline
void op_index_min::apply_noalias(Cube<uword>& out, const Cube<eT>& X, const uword dim, const typename arma_cx_only<eT>::result* junk) {
	arma_extra_debug_sigprint();
	arma_ignore(junk);
	...
	if(dim == 0) {
		...
		for(uword slice = 0; slice < X_n_slices; ++slice) {
			...
			for(uword row = 0; row < X_n_rows; ++row) { tmp_mem[row] = std::abs(col0_mem[row]); }
		...
		for(uword i = 0; i < N; ++i) { tmp_mem[i] = std::abs(X_slice0_mem[i]); }
...
template<typename T1> inline
void op_index_min::apply(Mat<uword>& out, const SpBase<typename T1::elem_type, T1>& expr, const uword dim) {
	arma_extra_debug_sigprint();
