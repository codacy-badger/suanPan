...
(
	...
	if(H.n_rows > 2) { for(uword i = 0; i < H.n_rows - 2; ++i) { H(span(i + 2, H.n_rows - 1), i).zeros(); } }
...
inline
Mat<typename T1::elem_type> hess
(
	const Base<typename T1::elem_type, T1>& X,
	const typename arma_blas_type_only<typename T1::elem_type>::result* junk = 0
) {
...
(
	...
	if(H.n_rows == 0) { U.reset(); } else if(H.n_rows == 1) { U.ones(1, 1); } else if(H.n_rows == 2) { U.eye(2, 2); } else {
		...
		for(uword i = 0; i < H.n_rows - 2; ++i) { H(span(i + 2, H.n_rows - 1), i).zeros(); }
	}
