...
public:
	...
	template<bool do_conj> inline Mat(const xtrans_mat<eT, do_conj>& X); //        xtrans_mat can only be generated by the Proxy class
	...
	template<typename fill_type> inline const Mat& fill(const fill::fill_class<fill_type>& f);
...
protected:
	...
	template<typename T1, typename T2> inline void init(const Base<pod_type, T1>& A, const Base<pod_type, T2>& B);
...
template<typename eT> template<uword fixed_n_rows, uword fixed_n_cols> class Mat<eT>::fixed : public Mat<eT> {
private:
...
template<typename eT> template<uword fixed_n_rows, uword fixed_n_cols> const uword Mat<eT>::fixed<fixed_n_rows, fixed_n_cols>::n_rows = fixed_n_rows;

template<typename eT> template<uword fixed_n_rows, uword fixed_n_cols> const uword Mat<eT>::fixed<fixed_n_rows, fixed_n_cols>::n_cols = fixed_n_cols;

template<typename eT> template<uword fixed_n_rows, uword fixed_n_cols> const uword Mat<eT>::fixed<fixed_n_rows, fixed_n_cols>::n_elem = fixed_n_rows * fixed_n_cols;
