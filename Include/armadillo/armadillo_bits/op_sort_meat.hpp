...
void op_sort::direct_sort(eT* X, const uword n_elem, const uword sort_type) {
...
template<typename eT> inline
void op_sort::direct_sort_ascending(eT* X, const uword n_elem) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
void op_sort::copy_row(eT* X, const Mat<eT>& A, const uword row) {
...
template<typename eT> inline
void op_sort::copy_row(Mat<eT>& A, const eT* X, const uword row) {
	const uword N = A.n_cols;
...
template<typename eT> inline
void op_sort::apply_noalias(Mat<eT>& out, const Mat<eT>& X, const uword sort_type, const uword dim) {
	arma_extra_debug_sigprint();
...
void op_sort::apply(Mat<typename T1::elem_type>& out, const Op<T1, op_sort>& in) {
	arma_extra_debug_sigprint();
...
template<typename T1> inline
void op_sort_default::apply(Mat<typename T1::elem_type>& out, const Op<T1, op_sort_default>& in) {
	arma_extra_debug_sigprint();
