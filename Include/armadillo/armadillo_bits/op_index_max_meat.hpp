...
void op_index_max::apply(Mat<uword>& out, const mtOp<uword, T1, op_index_max>& in) {
...
void op_index_max::apply_noalias(Mat<uword>& out, const Mat<eT>& X, const uword dim) {
	arma_extra_debug_sigprint();
	...
	if(dim == 0) {
		...
		for(uword row = 0; row < X_n_rows; ++row) { out_mem[row] = X.row(row).index_max(); }
	}
}

template<typename T1> inline
void op_index_max::apply(Cube<uword>& out, const mtOpCube<uword, T1, op_index_max>& in) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
void op_index_max::apply_noalias(Cube<uword>& out, const Cube<eT>& X, const uword dim, const typename arma_not_cx<eT>::result* junk) {
	arma_extra_debug_sigprint();
	arma_ignore(junk);
...
template<typename eT> inline
void op_index_max::apply_noalias(Cube<uword>& out, const Cube<eT>& X, const uword dim, const typename arma_cx_only<eT>::result* junk) {
	arma_extra_debug_sigprint();
	arma_ignore(junk);
...
template<typename T1> inline
void op_index_max::apply(Mat<uword>& out, const SpBase<typename T1::elem_type, T1>& expr, const uword dim) {
	arma_extra_debug_sigprint();
