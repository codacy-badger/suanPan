...
namespace gmm_priv {
	...
	gmm_full<eT>& gmm_full<eT>::operator=(const gmm_full<eT>& x) {
	...
	template<typename eT> inline
	gmm_full<eT>& gmm_full<eT>::operator=(const gmm_diag<eT>& x) {
		arma_extra_debug_sigprint();
	...
	template<typename eT> inline
	void gmm_full<eT>::reset(const uword in_n_dims, const uword in_n_gaus) {
		arma_extra_debug_sigprint();
	...
	void gmm_full<eT>::set_params(const Base<eT, T1>& in_means_expr, const BaseCube<eT, T2>& in_fcovs_expr, const Base<eT, T3>& in_hefts_expr) {
		arma_extra_debug_sigprint();
		...
		for(uword g = 0; g < in_fcovs.n_slices; ++g) { arma_debug_check((any(diagvec(in_fcovs.slice(g)) <= eT(0))), "gmm_full::set_params(): given fcovs have negative or zero values on diagonals"); }
	...
	void gmm_full<eT>::set_means(const Base<eT, T1>& in_means_expr) {
		arma_extra_debug_sigprint();
	...
	template<typename eT> template<typename T1> inline
	void gmm_full<eT>::set_fcovs(const BaseCube<eT, T1>& in_fcovs_expr) {
		arma_extra_debug_sigprint();
		...
		for(uword i = 0; i < in_fcovs.n_slices; ++i) { arma_debug_check((any(diagvec(in_fcovs.slice(i)) <= eT(0))), "gmm_full::set_fcovs(): given fcovs have negative or zero values on diagonals"); }
	...
	template<typename eT> template<typename T1> inline
	void gmm_full<eT>::set_hefts(const Base<eT, T1>& in_hefts_expr) {
		arma_extra_debug_sigprint();
	...
	template<typename eT> inline
	uword gmm_full<eT>::n_dims() const { return means.n_rows; }

	template<typename eT> inline
	uword gmm_full<eT>::n_gaus() const { return means.n_cols; }

	template<typename eT> inline
	bool gmm_full<eT>::load(const std::string name) {
		arma_extra_debug_sigprint();
		...
		const Mat<eT>& storage_means = storage(count);
		++count;
		const Mat<eT>& storage_hefts = storage(count);
		++count;
		...
		for(uword g = 0; g < N_gaus; ++g) {
			const Mat<eT>& storage_fcov = storage(count);
			++count;
	...
	template<typename eT> inline
	bool gmm_full<eT>::save(const std::string name) const {
		arma_extra_debug_sigprint();
	...
	template<typename eT> inline
	Col<eT> gmm_full<eT>::generate() const {
		arma_extra_debug_sigprint();
	...
	template<typename eT> inline
	Mat<eT> gmm_full<eT>::generate(const uword N_vec) const {
		arma_extra_debug_sigprint();
	...
	template<typename eT> template<typename T1> inline
	eT gmm_full<eT>::log_p(const T1& expr, const gmm_empty_arg& junk1, typename enable_if<((is_arma_type<T1>::value) && (resolves_to_colvector<T1>::value == true))>::result* junk2) const {
		arma_extra_debug_sigprint();
		arma_ignore(junk1);
		arma_ignore(junk2);
	...
	template<typename eT> template<typename T1> inline
	eT gmm_full<eT>::log_p(const T1& expr, const uword gaus_id, typename enable_if<((is_arma_type<T1>::value) && (resolves_to_colvector<T1>::value == true))>::result* junk2) const {
		arma_extra_debug_sigprint();
		arma_ignore(junk2);
	...
	template<typename eT> template<typename T1> inline
	Row<eT> gmm_full<eT>::log_p(const T1& expr, const gmm_empty_arg& junk1, typename enable_if<((is_arma_type<T1>::value) && (resolves_to_colvector<T1>::value == false))>::result* junk2) const {
		arma_extra_debug_sigprint();
		arma_ignore(junk1);
		arma_ignore(junk2);
	...
	template<typename eT> template<typename T1> inline
	Row<eT> gmm_full<eT>::log_p(const T1& expr, const uword gaus_id, typename enable_if<((is_arma_type<T1>::value) && (resolves_to_colvector<T1>::value == false))>::result* junk2) const {
		arma_extra_debug_sigprint();
		arma_ignore(junk2);
	...
	template<typename eT> template<typename T1> inline
	eT gmm_full<eT>::sum_log_p(const Base<eT, T1>& expr) const {
		arma_extra_debug_sigprint();
	...
	template<typename eT> template<typename T1> inline
	eT gmm_full<eT>::sum_log_p(const Base<eT, T1>& expr, const uword gaus_id) const {
		arma_extra_debug_sigprint();
	...
	template<typename eT> template<typename T1> inline
	eT gmm_full<eT>::avg_log_p(const Base<eT, T1>& expr) const {
		arma_extra_debug_sigprint();
	...
	template<typename eT> template<typename T1> inline
	eT gmm_full<eT>::avg_log_p(const Base<eT, T1>& expr, const uword gaus_id) const {
		arma_extra_debug_sigprint();
	...
	template<typename eT> template<typename T1> inline
	uword gmm_full<eT>::assign(const T1& expr, const gmm_dist_mode& dist, typename enable_if<((is_arma_type<T1>::value) && (resolves_to_colvector<T1>::value == true))>::result* junk) const {
		arma_extra_debug_sigprint();
		arma_ignore(junk);
	...
	template<typename eT> template<typename T1> inline
	urowvec gmm_full<eT>::assign(const T1& expr, const gmm_dist_mode& dist, typename enable_if<((is_arma_type<T1>::value) && (resolves_to_colvector<T1>::value == false))>::result* junk) const {
		arma_extra_debug_sigprint();
		arma_ignore(junk);
	...
	template<typename eT> template<typename T1> inline
	urowvec gmm_full<eT>::raw_hist(const Base<eT, T1>& expr, const gmm_dist_mode& dist_mode) const {
		arma_extra_debug_sigprint();
	...
	template<typename eT> template<typename T1> inline
	Row<eT> gmm_full<eT>::norm_hist(const Base<eT, T1>& expr, const gmm_dist_mode& dist_mode) const {
		arma_extra_debug_sigprint();
	...
	(
		...
		if(seed_mode == keep_existing) {
			...
			if(print_mode) {
				get_cout_stream() << "gmm_full::learn(): generating initial means\n";
		...
		if(seed_mode != keep_existing) {
			if(print_mode) {
				get_cout_stream() << "gmm_full::learn(): generating initial covariances\n";
				get_cout_stream().flush();
			}
	...
	template<typename eT> inline
	void gmm_full<eT>::init(const gmm_full<eT>& x) {
		arma_extra_debug_sigprint();
	...
	template<typename eT> inline
	void gmm_full<eT>::init(const gmm_diag<eT>& x) {
		arma_extra_debug_sigprint();
		...
		for(uword g = 0; g < N_gaus; ++g) {
			...
			for(uword d = 0; d < N_dims; ++d) { fcov.at(d, d) = dcov_mem[d]; }
		}
	...
	template<typename eT> inline
	void gmm_full<eT>::init(const uword in_n_dims, const uword in_n_gaus) {
		arma_extra_debug_sigprint();
		...
		for(uword g = 0; g < in_n_gaus; ++g) { access::rw(fcovs).slice(g).diag().ones(); }
	...
	template<typename eT> inline
	void gmm_full<eT>::init_constants(const bool calc_chol) {
		arma_extra_debug_sigprint();
	...
	template<typename eT> inline
	umat gmm_full<eT>::internal_gen_boundaries(const uword N) const {
		arma_extra_debug_sigprint();
	...
	template<typename eT> inline
	eT gmm_full<eT>::internal_scalar_log_p(const eT* x) const {
		arma_extra_debug_sigprint();
	...
	template<typename eT> inline
	eT gmm_full<eT>::internal_scalar_log_p(const eT* x, const uword g) const {
		arma_extra_debug_sigprint();
		...
		for(uword i = 0; i < N_dims; ++i) {
			...
			for(uword j = 0; j < N_dims; ++j) { inner_acc += (x[j] - mean_mem[j]) * inv_fcov_coldata[j]; }
	...
	template<typename eT> template<typename T1> inline
	Row<eT> gmm_full<eT>::internal_vec_log_p(const T1& X) const {
		arma_extra_debug_sigprint();
        ...
        for(uword i=start_index; i <= end_index; ++i)
          ...
          out_mem[i] = internal_scalar_log_p( X.colptr(i) );
...
#else
			{
				...
				for(uword i = 0; i < N_samples; ++i) { out_mem[i] = internal_scalar_log_p(X.colptr(i)); }
			}
#endif
		}
	...
	template<typename eT> template<typename T1> inline
	Row<eT> gmm_full<eT>::internal_vec_log_p(const T1& X, const uword gaus_id) const {
		arma_extra_debug_sigprint();
        ...
        for(uword i=start_index; i <= end_index; ++i)
          ...
          out_mem[i] = internal_scalar_log_p( X.colptr(i), gaus_id );
...
#else
			{
				...
				for(uword i = 0; i < N_samples; ++i) { out_mem[i] = internal_scalar_log_p(X.colptr(i), gaus_id); }
			}
#endif
		}
	...
	template<typename eT> template<typename T1> inline
	eT gmm_full<eT>::internal_sum_log_p(const T1& X) const {
		arma_extra_debug_sigprint();
	...
	template<typename eT> template<typename T1> inline
	eT gmm_full<eT>::internal_sum_log_p(const T1& X, const uword gaus_id) const {
		arma_extra_debug_sigprint();
	...
	template<typename eT> template<typename T1> inline
	eT gmm_full<eT>::internal_avg_log_p(const T1& X) const {
		arma_extra_debug_sigprint();
    ...
    return avg;
...
#else
		{
			...
			for(uword i = 0; i < N_samples; ++i) { running_mean(internal_scalar_log_p(X.colptr(i))); }
	...
	template<typename eT> template<typename T1> inline
	eT gmm_full<eT>::internal_avg_log_p(const T1& X, const uword gaus_id) const {
		arma_extra_debug_sigprint();
    ...
    return avg;
...
#else
		{
			...
			for(uword i = 0; i < N_samples; ++i) { running_mean(internal_scalar_log_p(X.colptr(i), gaus_id)); }
	...
	template<typename eT> template<typename T1> inline
	uword gmm_full<eT>::internal_scalar_assign(const T1& X, const gmm_dist_mode& dist_mode) const {
		arma_extra_debug_sigprint();
	...
	template<typename eT> template<typename T1> inline
	void gmm_full<eT>::internal_vec_assign(urowvec& out, const T1& X, const gmm_dist_mode& dist_mode) const {
		arma_extra_debug_sigprint();
	...
	template<typename eT> inline
	void gmm_full<eT>::internal_raw_hist(urowvec& hist, const Mat<eT>& X, const gmm_dist_mode& dist_mode) const {
		arma_extra_debug_sigprint();
	...
	void gmm_full<eT>::generate_initial_means(const Mat<eT>& X, const gmm_seed_mode& seed_mode) {
		arma_extra_debug_sigprint();
	...
	template<typename eT> template<uword dist_id> inline
	void gmm_full<eT>::generate_initial_params(const Mat<eT>& X, const eT var_floor) {
		arma_extra_debug_sigprint();
	...
	template<typename eT> template<uword dist_id> inline
	bool gmm_full<eT>::km_iterate(const Mat<eT>& X, const uword max_iter, const bool verbose) {
		arma_extra_debug_sigprint();
    ...
    field< Row<uword> > t_last_indx(n_threads);
		...
		if(verbose) {
			get_cout_stream() << "gmm_full::learn(): k-means: n_threads: " << n_threads << '\n';
			get_cout_stream().flush();
		}
      ...
      for(uword t=0; t < n_threads; ++t)
        ...
        if( t_acc_hefts(t)(g) >= 1 )  { last_indx(g) = t_last_indx(t)(g); }
			...
			if(dead_gs.n_elem > 0) {
				if(verbose) {
					get_cout_stream() << "gmm_full::learn(): k-means: recovering from dead means\n";
					get_cout_stream().flush();
				}
	...
	template<typename eT> inline
	bool gmm_full<eT>::em_iterate(const Mat<eT>& X, const uword max_iter, const eT var_floor, const bool verbose) {
		arma_extra_debug_sigprint();
		...
		if(verbose) { get_cout_stream() << "gmm_full::learn(): EM: n_threads: " << n_threads << '\n'; }
	...
	template<typename eT> inline
	void gmm_full<eT>::em_update_params
	(
		const Mat<eT>& X,
    ...
    for(uword t=0; t<n_threads; t++)
      ...
      em_generate_acc(X, boundaries.at(0,t), boundaries.at(1,t), acc_means, acc_fcovs, acc_norm_lhoods, gaus_log_lhoods, progress_log_lhood);
		...
		for(uword g = 0; g < N_gaus; ++g) {
			...
			for(uword d = 0; d < N_dims; ++d) { acc_mean_mem[d] /= acc_norm_lhood; }
			...
			if(log_det_ok && inv_ok) {
				...
				for(uword d = 0; d < N_dims; ++d) { mean_mem[d] = acc_mean_mem[d]; }
	...
	template<typename eT> inline
	void gmm_full<eT>::em_generate_acc
	(
		const Mat<eT>& X,
	...
	const {
		...
		for(uword i = start_index; i <= end_index; i++) {
			...
			for(uword g = 0; g < N_gaus; ++g) {
				...
				for(uword d = 0; d < N_dims; ++d) { acc_mean_mem[d] += x[d] * norm_lhood; }
	...
	template<typename eT> inline
	void gmm_full<eT>::em_fix_params(const eT var_floor) {
		arma_extra_debug_sigprint();
