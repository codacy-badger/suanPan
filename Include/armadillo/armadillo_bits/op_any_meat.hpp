...
bool op_any::any_vec_helper(const Base<typename T1::elem_type, T1>& X) {
...
bool op_any::any_vec_helper(const subview<eT>& X) {
	arma_extra_debug_sigprint();
	...
	if(X_n_rows == 1) { for(uword col = 0; col < X_n_cols; ++col) { if(X.at(0, col) != eT(0)) { return true; } } } else {
		for(uword col = 0; col < X_n_cols; ++col) {
			...
			for(uword row = 0; row < X_n_rows; ++row) { if(X_colmem[row] != eT(0)) { return true; } }
		}
	}
...
template<typename T1> inline
bool op_any::any_vec_helper(const Op<T1, op_vectorise_col>& X) {
	arma_extra_debug_sigprint();
...
template<typename T1> inline
void op_any::apply_helper(Mat<uword>& out, const Proxy<T1>& P, const uword dim) {
	arma_extra_debug_sigprint();
...
template<typename T1> inline
void op_any::apply(Mat<uword>& out, const mtOp<uword, T1, op_any>& X) {
	arma_extra_debug_sigprint();
