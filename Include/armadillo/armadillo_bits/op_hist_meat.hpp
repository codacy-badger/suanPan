...
void op_hist::apply_noalias(Mat<uword>& out, const Mat<eT>& A, const uword n_bins, const bool A_is_row) {
...
void op_hist::apply(Mat<uword>& out, const mtOp<uword, T1, op_hist>& X) {
	arma_extra_debug_sigprint();
	...
	if(U.is_alias(out)) {
		...
		out.steal_mem(tmp);
	} else { op_hist::apply_noalias(out, U.M, n_bins, (T1::is_row)); }
}
