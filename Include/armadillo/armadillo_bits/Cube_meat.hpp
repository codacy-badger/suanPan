...
template<typename eT> inline Cube<eT>::~Cube() {
	arma_extra_debug_sigprint_this(this);

	delete_mat();

	if((mem_state == 0) && (n_elem > Cube_prealloc::mem_n_elem)) { memory::release(access::rw(mem)); }
...
template<typename eT> inline Cube<eT>::Cube(const uword in_n_rows, const uword in_n_cols, const uword in_n_slices)
	: n_rows(in_n_rows)
	, n_cols(in_n_cols)
...
template<typename eT> inline Cube<eT>::Cube(const SizeCube& s)
	: n_rows(s.n_rows)
	, n_cols(s.n_cols)
...
template<typename eT> template<typename fill_type> inline Cube<eT>::Cube(const uword in_n_rows, const uword in_n_cols, const uword in_n_slices, const fill::fill_class<fill_type>&)
	: n_rows(in_n_rows)
	, n_cols(in_n_cols)
	, n_elem_slice(in_n_rows * in_n_cols)
	, n_slices(in_n_slices)
...
template<typename eT> template<typename fill_type> inline Cube<eT>::Cube(const SizeCube& s, const fill::fill_class<fill_type>&)
	: n_rows(s.n_rows)
	, n_cols(s.n_cols)
	, n_elem_slice(s.n_rows * s.n_cols)
	, n_slices(s.n_slices)
...
template<typename eT> inline Cube<eT>::Cube(Cube<eT>&& in_cube)
	: n_rows(0)
	, n_cols(0)
	, n_elem_slice(0)
	, n_slices(0)
...
Cube<eT>& Cube<eT>::operator=(Cube<eT>&& in_cube) {
	arma_extra_debug_sigprint(arma_str::format("this = %x   in_cube = %x") % this % &in_cube);
...
template<typename eT> inline
void Cube<eT>::init_warm(const uword in_n_rows, const uword in_n_cols, const uword in_n_slices) {
	...
	} else // condition: old_n_elem != new_n_elem
	{
		...
		if(new_n_elem < old_n_elem) // reuse existing memory if possible
		{
			...
			} else { arma_extra_debug_print("Cube::init(): reusing memory"); }
...
(
	...
	if(use_at == false) {
		...
		for(uword i = 0; i < N; ++i) { out_mem[i] = std::complex<T>(A[i], B[i]); }
	} else {
...
template<typename eT> inline
void Cube<eT>::delete_mat() {
	arma_extra_debug_sigprint();

	if((n_slices > 0) && (mat_ptrs != NULL)) {
		for(uword uslice = 0; uslice < n_slices; ++uslice) { if(mat_ptrs[uslice] != NULL) { delete access::rw(mat_ptrs[uslice]); } }

		if((mem_state <= 2) && (n_slices > Cube_prealloc::mat_ptrs_size)) { delete [] mat_ptrs; }
	}
}

template<typename eT> inline
void Cube<eT>::create_mat() {
	arma_extra_debug_sigprint();

	if(n_slices == 0) { access::rw(mat_ptrs) = NULL; } else {
		if(mem_state <= 2) {
			if(n_slices <= Cube_prealloc::mat_ptrs_size) { access::rw(mat_ptrs) = const_cast<const Mat<eT>**>(mat_ptrs_local); } else {
		...
		for(uword uslice = 0; uslice < n_slices; ++uslice) { mat_ptrs[uslice] = NULL; }
	}
}
...
template<typename eT> inline
Cube<eT>& Cube<eT>::operator=(const eT val) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline Cube<eT>::Cube(const Cube<eT>& x)
	: n_rows(x.n_rows)
	, n_cols(x.n_cols)
...
//! construct a cube from a given cube
template<typename eT> inline
Cube<eT>& Cube<eT>::operator=(const Cube<eT>& x) {
	arma_extra_debug_sigprint(arma_str::format("this = %x   in_cube = %x") % this % &x);
...
template<typename eT> inline Cube<eT>::Cube(eT* aux_mem, const uword aux_n_rows, const uword aux_n_cols, const uword aux_n_slices, const bool copy_aux_mem, const bool strict, const bool prealloc_mat)
	: n_rows(aux_n_rows)
	, n_cols(aux_n_cols)
...
//! the array is copied.
template<typename eT> inline Cube<eT>::Cube(const eT* aux_mem, const uword aux_n_rows, const uword aux_n_cols, const uword aux_n_slices)
	: n_rows(aux_n_rows)
	, n_cols(aux_n_cols)
	, n_elem_slice(aux_n_rows * aux_n_cols)
	, n_slices(aux_n_slices)
...
template<typename eT> inline
Cube<eT>& Cube<eT>::operator+=(const Cube<eT>& m) {
	arma_extra_debug_sigprint();
...
//! for constructing a complex cube out of two non-complex cubes
template<typename eT> template<typename T1, typename T2> inline Cube<eT>::Cube
(
...
template<typename eT> inline Cube<eT>::Cube(const subview_cube<eT>& X)
	: n_rows(X.n_rows)
	, n_cols(X.n_cols)
	, n_elem_slice(X.n_elem_slice)
	, n_slices(X.n_slices)
...
//! construct a cube from a subview_cube instance (e.g. construct a cube from a delayed subcube operation)
template<typename eT> inline
Cube<eT>& Cube<eT>::operator=(const subview_cube<eT>& X) {
	arma_extra_debug_sigprint();
...
template<typename eT> template<typename T1> inline Cube<eT>::Cube(const subview_cube_slices<eT, T1>& X)
	: n_rows(0)
	, n_cols(0)
	, n_elem_slice(0)
	, n_slices(0)
...
Cube<eT>& Cube<eT>::operator=(const subview_cube_slices<eT, T1>& X) {
	arma_extra_debug_sigprint();

	const bool alias = (this == &(X.m));

	if(alias == false) { subview_cube_slices<eT, T1>::extract(*this, X); } else {
		Cube<eT> tmp(X);
...
subview_cube<eT> Cube<eT>::row(const uword in_row) {
	arma_extra_debug_sigprint();
...
//! creation of subview_cube (subcube comprised of specified row)
template<typename eT> arma_inline
const subview_cube<eT> Cube<eT>::row(const uword in_row) const {
	arma_extra_debug_sigprint();
...
template<typename eT> arma_inline
subview_cube<eT> Cube<eT>::col(const uword in_col) {
	arma_extra_debug_sigprint();
...
//! creation of subview_cube (subcube comprised of specified column)
template<typename eT> arma_inline
const subview_cube<eT> Cube<eT>::col(const uword in_col) const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
Mat<eT>& Cube<eT>::slice(const uword in_slice) {
	arma_extra_debug_sigprint();
...
//! provide the reference to the matrix representing a single slice
template<typename eT> inline
const Mat<eT>& Cube<eT>::slice(const uword in_slice) const {
	arma_extra_debug_sigprint();
...
template<typename eT> arma_inline
subview_cube<eT> Cube<eT>::rows(const uword in_row1, const uword in_row2) {
	arma_extra_debug_sigprint();
...
//! creation of subview_cube (subcube comprised of specified rows)
template<typename eT> arma_inline
const subview_cube<eT> Cube<eT>::rows(const uword in_row1, const uword in_row2) const {
	arma_extra_debug_sigprint();
...
template<typename eT> arma_inline
subview_cube<eT> Cube<eT>::cols(const uword in_col1, const uword in_col2) {
	arma_extra_debug_sigprint();
...
//! creation of subview_cube (subcube comprised of specified columns)
template<typename eT> arma_inline
const subview_cube<eT> Cube<eT>::cols(const uword in_col1, const uword in_col2) const {
	arma_extra_debug_sigprint();
...
template<typename eT> arma_inline
subview_cube<eT> Cube<eT>::slices(const uword in_slice1, const uword in_slice2) {
	arma_extra_debug_sigprint();
...
//! creation of subview_cube (subcube comprised of specified slices)
template<typename eT> arma_inline
const subview_cube<eT> Cube<eT>::slices(const uword in_slice1, const uword in_slice2) const {
	arma_extra_debug_sigprint();
...
template<typename eT> arma_inline
subview_cube<eT> Cube<eT>::subcube(const uword in_row1, const uword in_col1, const uword in_slice1, const uword in_row2, const uword in_col2, const uword in_slice2) {
	arma_extra_debug_sigprint();
...
//! creation of subview_cube (generic subcube)
template<typename eT> arma_inline
const subview_cube<eT> Cube<eT>::subcube(const uword in_row1, const uword in_col1, const uword in_slice1, const uword in_row2, const uword in_col2, const uword in_slice2) const {
	arma_extra_debug_sigprint();
...
//! creation of subview_cube (generic subcube)
template<typename eT> inline
subview_cube<eT> Cube<eT>::subcube(const uword in_row1, const uword in_col1, const uword in_slice1, const SizeCube& s) {
	arma_extra_debug_sigprint();
...
//! creation of subview_cube (generic subcube)
template<typename eT> inline
const subview_cube<eT> Cube<eT>::subcube(const uword in_row1, const uword in_col1, const uword in_slice1, const SizeCube& s) const {
	arma_extra_debug_sigprint();
...
//! creation of subview_cube (generic subcube)
template<typename eT> inline
subview_cube<eT> Cube<eT>::subcube(const span& row_span, const span& col_span, const span& slice_span) {
	arma_extra_debug_sigprint();
...
//! creation of subview_cube (generic subcube)
template<typename eT> inline
const subview_cube<eT> Cube<eT>::subcube(const span& row_span, const span& col_span, const span& slice_span) const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
subview_cube<eT> Cube<eT>::operator()(const span& row_span, const span& col_span, const span& slice_span) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
const subview_cube<eT> Cube<eT>::operator()(const span& row_span, const span& col_span, const span& slice_span) const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
subview_cube<eT> Cube<eT>::operator()(const uword in_row1, const uword in_col1, const uword in_slice1, const SizeCube& s) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
const subview_cube<eT> Cube<eT>::operator()(const uword in_row1, const uword in_col1, const uword in_slice1, const SizeCube& s) const {
	arma_extra_debug_sigprint();
...
template<typename eT> arma_inline
subview_cube<eT> Cube<eT>::tube(const uword in_row1, const uword in_col1) {
	arma_extra_debug_sigprint();
...
template<typename eT> arma_inline
const subview_cube<eT> Cube<eT>::tube(const uword in_row1, const uword in_col1) const {
	arma_extra_debug_sigprint();
...
template<typename eT> arma_inline
subview_cube<eT> Cube<eT>::tube(const uword in_row1, const uword in_col1, const uword in_row2, const uword in_col2) {
	arma_extra_debug_sigprint();
...
template<typename eT> arma_inline
const subview_cube<eT> Cube<eT>::tube(const uword in_row1, const uword in_col1, const uword in_row2, const uword in_col2) const {
	arma_extra_debug_sigprint();
...
template<typename eT> arma_inline
subview_cube<eT> Cube<eT>::tube(const uword in_row1, const uword in_col1, const SizeMat& s) {
	arma_extra_debug_sigprint();
...
template<typename eT> arma_inline
const subview_cube<eT> Cube<eT>::tube(const uword in_row1, const uword in_col1, const SizeMat& s) const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
subview_cube<eT> Cube<eT>::tube(const span& row_span, const span& col_span) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
const subview_cube<eT> Cube<eT>::tube(const span& row_span, const span& col_span) const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
subview_cube<eT> Cube<eT>::head_slices(const uword N) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
const subview_cube<eT> Cube<eT>::head_slices(const uword N) const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
subview_cube<eT> Cube<eT>::tail_slices(const uword N) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
const subview_cube<eT> Cube<eT>::tail_slices(const uword N) const {
	arma_extra_debug_sigprint();
...
subview_elem1<eT, T1> Cube<eT>::elem(const Base<uword, T1>& a) {
	arma_extra_debug_sigprint();
...
template<typename eT> template<typename T1> arma_inline
const subview_elem1<eT, T1> Cube<eT>::elem(const Base<uword, T1>& a) const {
	arma_extra_debug_sigprint();
...
template<typename eT> template<typename T1> arma_inline
subview_elem1<eT, T1> Cube<eT>::operator()(const Base<uword, T1>& a) {
	arma_extra_debug_sigprint();
...
template<typename eT> template<typename T1> arma_inline
const subview_elem1<eT, T1> Cube<eT>::operator()(const Base<uword, T1>& a) const {
	arma_extra_debug_sigprint();
...
template<typename eT> arma_inline
subview_cube_each1<eT> Cube<eT>::each_slice() {
	arma_extra_debug_sigprint();
...
template<typename eT> arma_inline
const subview_cube_each1<eT> Cube<eT>::each_slice() const {
	arma_extra_debug_sigprint();
...
template<typename eT> template<typename T1> inline
subview_cube_each2<eT, T1> Cube<eT>::each_slice(const Base<uword, T1>& indices) {
	arma_extra_debug_sigprint();
...
template<typename eT> template<typename T1> inline
const subview_cube_each2<eT, T1> Cube<eT>::each_slice(const Base<uword, T1>& indices) const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
const Cube<eT>& Cube<eT>::each_slice(const std::function<void(Mat<eT>&)>& F) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
const Cube<eT>& Cube<eT>::each_slice(const std::function<void(const Mat<eT>&)>& F) const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
const Cube<eT>& Cube<eT>::each_slice(const std::function<void(Mat<eT>&)>& F, const bool use_mp) {
	arma_extra_debug_sigprint();

	if((use_mp == false) || (arma_config::openmp == false)) { return (*this).each_slice(F); }
...
template<typename eT> inline
const Cube<eT>& Cube<eT>::each_slice(const std::function<void(const Mat<eT>&)>& F, const bool use_mp) const {
	arma_extra_debug_sigprint();
...
template<typename eT> template<typename T1> inline
subview_cube_slices<eT, T1> Cube<eT>::slices(const Base<uword, T1>& indices) {
	arma_extra_debug_sigprint();
...
template<typename eT> template<typename T1> inline
const subview_cube_slices<eT, T1> Cube<eT>::slices(const Base<uword, T1>& indices) const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
void Cube<eT>::shed_row(const uword row_num) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
void Cube<eT>::shed_col(const uword col_num) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
void Cube<eT>::shed_slice(const uword slice_num) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
void Cube<eT>::shed_rows(const uword in_row1, const uword in_row2) {
	arma_extra_debug_sigprint();
	...
	Cube<eT> X(n_keep_front + n_keep_back, n_cols, n_slices);
...
template<typename eT> inline
void Cube<eT>::shed_cols(const uword in_col1, const uword in_col2) {
	arma_extra_debug_sigprint();
	...
	Cube<eT> X(n_rows, n_keep_front + n_keep_back, n_slices);
...
template<typename eT> inline
void Cube<eT>::shed_slices(const uword in_slice1, const uword in_slice2) {
	arma_extra_debug_sigprint();
	...
	if(n_keep_front > 0) { X.slices(0, (n_keep_front - 1)) = slices(0, (in_slice1 - 1)); }

	if(n_keep_back > 0) { X.slices(n_keep_front, (n_keep_front + n_keep_back - 1)) = slices((in_slice2 + 1), (n_slices - 1)); }

	steal_mem(X);
}

template<typename eT> inline
void Cube<eT>::insert_rows(const uword row_num, const uword N, const bool set_to_zero) {
	arma_extra_debug_sigprint();
	...
	if(N > 0) {
		Cube<eT> out(t_n_rows + N, n_cols, n_slices);
		...
		if(set_to_zero == true) { out.rows(row_num, row_num + N - 1).zeros(); }
...
template<typename eT> inline
void Cube<eT>::insert_cols(const uword col_num, const uword N, const bool set_to_zero) {
	arma_extra_debug_sigprint();
	...
	if(N > 0) {
		Cube<eT> out(n_rows, t_n_cols + N, n_slices);
		...
		if(set_to_zero == true) { out.cols(col_num, col_num + N - 1).zeros(); }
...
template<typename eT> inline
void Cube<eT>::insert_slices(const uword slice_num, const uword N, const bool set_to_zero) {
	arma_extra_debug_sigprint();
	...
	if(N > 0) {
		...
		if(A_n_slices > 0) { out.slices(0, A_n_slices - 1) = slices(0, A_n_slices - 1); }

		if(B_n_slices > 0) { out.slices(slice_num + N, t_n_slices + N - 1) = slices(slice_num, t_n_slices - 1); }

		if(set_to_zero == true) {
			//out.slices(slice_num, slice_num + N - 1).zeros();

			for(uword i = slice_num; i < (slice_num + N); ++i) { arrayops::fill_zeros(out.slice_memptr(i), out.n_elem_slice); }
		}
...
template<typename eT> template<typename T1> inline
void Cube<eT>::insert_rows(const uword row_num, const BaseCube<eT, T1>& X) {
	arma_extra_debug_sigprint();
...
template<typename eT> template<typename T1> inline
void Cube<eT>::insert_cols(const uword col_num, const BaseCube<eT, T1>& X) {
	arma_extra_debug_sigprint();
...
template<typename eT> template<typename T1> inline
void Cube<eT>::insert_slices(const uword slice_num, const BaseCube<eT, T1>& X) {
	arma_extra_debug_sigprint();
...
//! create a cube from GenCube, i.e. run the previously delayed element generation operations
template<typename eT> template<typename gen_type> inline Cube<eT>::Cube(const GenCube<eT, gen_type>& X)
	: n_rows(X.n_rows)
	, n_cols(X.n_cols)
...
template<typename eT> template<typename gen_type> inline
Cube<eT>& Cube<eT>::operator=(const GenCube<eT, gen_type>& X) {
	arma_extra_debug_sigprint();
...
template<typename eT> template<typename T1, typename op_type> inline Cube<eT>::Cube(const OpCube<T1, op_type>& X)
	: n_rows(0)
	, n_cols(0)
	, n_elem_slice(0)
	, n_slices(0)
...
Cube<eT>& Cube<eT>::operator=(const OpCube<T1, op_type>& X) {
	arma_extra_debug_sigprint();
...
template<typename eT> template<typename T1, typename eop_type> inline Cube<eT>::Cube(const eOpCube<T1, eop_type>& X)
	: n_rows(X.get_n_rows())
	, n_cols(X.get_n_cols())
...
Cube<eT>& Cube<eT>::operator=(const eOpCube<T1, eop_type>& X) {
	arma_extra_debug_sigprint();
...
template<typename eT> template<typename T1, typename op_type> inline Cube<eT>::Cube(const mtOpCube<eT, T1, op_type>& X)
	: n_rows(0)
	, n_cols(0)
	, n_elem_slice(0)
	, n_slices(0)
...
template<typename eT> template<typename T1, typename op_type> inline
Cube<eT>& Cube<eT>::operator=(const mtOpCube<eT, T1, op_type>& X) {
	arma_extra_debug_sigprint();
...
//! create a cube from GlueCube, i.e. run the previously delayed binary operations
template<typename eT> template<typename T1, typename T2, typename glue_type> inline Cube<eT>::Cube(const GlueCube<T1, T2, glue_type>& X)
	: n_rows(0)
	, n_cols(0)
	, n_elem_slice(0)
	, n_slices(0)
...
Cube<eT>& Cube<eT>::operator=(const GlueCube<T1, T2, glue_type>& X) {
	arma_extra_debug_sigprint();
...
//! create a cube from eGlueCube, i.e. run the previously delayed binary operations
template<typename eT> template<typename T1, typename T2, typename eglue_type> inline Cube<eT>::Cube(const eGlueCube<T1, T2, eglue_type>& X)
	: n_rows(X.get_n_rows())
	, n_cols(X.get_n_cols())
	, n_elem_slice(X.get_n_elem_slice())
	, n_slices(X.get_n_slices())
...
Cube<eT>& Cube<eT>::operator=(const eGlueCube<T1, T2, eglue_type>& X) {
	arma_extra_debug_sigprint();
...
template<typename eT> template<typename T1, typename T2, typename glue_type> inline Cube<eT>::Cube(const mtGlueCube<eT, T1, T2, glue_type>& X)
	: n_rows(0)
	, n_cols(0)
	, n_elem_slice(0)
	, n_slices(0)
...
template<typename eT> template<typename T1, typename T2, typename glue_type> inline
Cube<eT>& Cube<eT>::operator=(const mtGlueCube<eT, T1, T2, glue_type>& X) {
	arma_extra_debug_sigprint();
...
template<typename eT> arma_inline
arma_warn_unused
const eT& Cube<eT>::at_alt(const uword i) const {
...
template<typename eT> arma_inline
arma_warn_unused
eT& Cube<eT>::operator()(const uword i) {
...
template<typename eT> arma_inline
arma_warn_unused
const eT& Cube<eT>::operator()(const uword i) const {
	arma_debug_check((i >= n_elem), "Cube::operator(): index out of bounds");
	return mem[i];
}
...
template<typename eT> arma_inline
arma_warn_unused
eT& Cube<eT>::operator[](const uword i) { return access::rw(mem[i]); }

//! linear element accessor (treats the cube as a vector); no bounds check
template<typename eT> arma_inline
arma_warn_unused
const eT& Cube<eT>::operator[](const uword i) const { return mem[i]; }

//! linear element accessor (treats the cube as a vector); no bounds check.  
template<typename eT> arma_inline
arma_warn_unused
eT& Cube<eT>::at(const uword i) { return access::rw(mem[i]); }

//! linear element accessor (treats the cube as a vector); no bounds check
template<typename eT> arma_inline
arma_warn_unused
const eT& Cube<eT>::at(const uword i) const { return mem[i]; }

//! element accessor; bounds checking not done when ARMA_NO_DEBUG is defined
template<typename eT> arma_inline
arma_warn_unused
eT& Cube<eT>::operator()(const uword in_row, const uword in_col, const uword in_slice) {
	arma_debug_check
	(
		(in_row >= n_rows) ||
		(in_col >= n_cols) ||
...
template<typename eT> arma_inline
arma_warn_unused
const eT& Cube<eT>::operator()(const uword in_row, const uword in_col, const uword in_slice) const {
	arma_debug_check
	(
		(in_row >= n_rows) ||
		(in_col >= n_cols) ||
...
template<typename eT> arma_inline
arma_warn_unused
eT& Cube<eT>::at(const uword in_row, const uword in_col, const uword in_slice) { return access::rw(mem[in_slice * n_elem_slice + in_col * n_rows + in_row]); }

//! element accessor; no bounds check
template<typename eT> arma_inline
arma_warn_unused
const eT& Cube<eT>::at(const uword in_row, const uword in_col, const uword in_slice) const { return mem[in_slice * n_elem_slice + in_col * n_rows + in_row]; }

//! prefix ++
template<typename eT> arma_inline
const Cube<eT>& Cube<eT>::operator++() {
...
template<typename eT> arma_inline
void Cube<eT>::operator++(int) { Cube_aux::postfix_pp(*this); }
...
//! postfix --  (must not return the object by reference)
template<typename eT> arma_inline
void Cube<eT>::operator--(int) { Cube_aux::postfix_mm(*this); }

//! returns true if all of the elements are finite
template<typename eT> arma_inline
arma_warn_unused
bool Cube<eT>::is_finite() const { return arrayops::is_finite(memptr(), n_elem); }

//! returns true if the cube has no elements
template<typename eT> arma_inline
arma_warn_unused
bool Cube<eT>::is_empty() const { return (n_elem == 0); }

template<typename eT> inline
arma_warn_unused
bool Cube<eT>::has_inf() const {
	arma_extra_debug_sigprint();
...
template<typename eT> arma_inline
arma_warn_unused
bool Cube<eT>::in_range(const uword i) const { return (i < n_elem); }

//! returns true if the given start and end indices are currently in range
template<typename eT> arma_inline
arma_warn_unused
bool Cube<eT>::in_range(const span& x) const {
	arma_extra_debug_sigprint();

	if(x.whole == true) { return true; } else {
		const uword a = x.a;
		const uword b = x.b;
...
template<typename eT> arma_inline
arma_warn_unused
bool Cube<eT>::in_range(const uword in_row, const uword in_col, const uword in_slice) const { return ((in_row < n_rows) && (in_col < n_cols) && (in_slice < n_slices)); }

template<typename eT> inline
arma_warn_unused
bool Cube<eT>::in_range(const span& row_span, const span& col_span, const span& slice_span) const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
arma_warn_unused
bool Cube<eT>::in_range(const uword in_row, const uword in_col, const uword in_slice, const SizeCube& s) const {
	const uword l_n_rows = n_rows;
	const uword l_n_cols = n_cols;
	const uword l_n_slices = n_slices;
...
template<typename eT> arma_inline
arma_warn_unused
eT* Cube<eT>::memptr() { return const_cast<eT*>(mem); }

//! returns a pointer to array of eTs used by the cube
template<typename eT> arma_inline
arma_warn_unused
const eT* Cube<eT>::memptr() const { return mem; }

//! returns a pointer to array of eTs used by the specified slice in the cube
template<typename eT> arma_inline
arma_warn_unused
eT* Cube<eT>::slice_memptr(const uword uslice) { return const_cast<eT*>(&mem[uslice * n_elem_slice]); }

//! returns a pointer to array of eTs used by the specified slice in the cube
template<typename eT> arma_inline
arma_warn_unused
const eT* Cube<eT>::slice_memptr(const uword uslice) const { return &mem[uslice * n_elem_slice]; }

//! returns a pointer to array of eTs used by the specified slice in the cube
template<typename eT> arma_inline
arma_warn_unused
eT* Cube<eT>::slice_colptr(const uword uslice, const uword col) { return const_cast<eT*>(&mem[uslice * n_elem_slice + col * n_rows]); }

//! returns a pointer to array of eTs used by the specified slice in the cube
template<typename eT> arma_inline
arma_warn_unused
const eT* Cube<eT>::slice_colptr(const uword uslice, const uword col) const { return &mem[uslice * n_elem_slice + col * n_rows]; }
...
inline
void Cube<eT>::impl_print(const std::string& extra_text) const {
	arma_extra_debug_sigprint();

	if(extra_text.length() != 0) { get_cout_stream() << extra_text << '\n'; }
...
template<typename eT> arma_cold
inline
void Cube<eT>::impl_print(std::ostream& user_stream, const std::string& extra_text) const {
	arma_extra_debug_sigprint();

	if(extra_text.length() != 0) { user_stream << extra_text << '\n'; }
...
template<typename eT> arma_cold
inline
void Cube<eT>::impl_raw_print(const std::string& extra_text) const {
	arma_extra_debug_sigprint();
...
template<typename eT> arma_cold
inline
void Cube<eT>::impl_raw_print(std::ostream& user_stream, const std::string& extra_text) const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
void Cube<eT>::set_size(const uword in_n_rows, const uword in_n_cols, const uword in_n_slices) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
void Cube<eT>::reshape(const uword in_rows, const uword in_cols, const uword in_slices) {
	arma_extra_debug_sigprint();
...
inline
void Cube<eT>::reshape(const uword in_rows, const uword in_cols, const uword in_slices, const uword dim) {
	arma_extra_debug_sigprint();
...
//! change the cube to have user specified dimensions (data is preserved)
template<typename eT> inline
void Cube<eT>::resize(const uword in_rows, const uword in_cols, const uword in_slices) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
void Cube<eT>::set_size(const SizeCube& s) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
void Cube<eT>::reshape(const SizeCube& s) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
void Cube<eT>::resize(const SizeCube& s) {
	arma_extra_debug_sigprint();
...
template<typename eT> template<typename eT2> inline
void Cube<eT>::copy_size(const Cube<eT2>& m) {
	arma_extra_debug_sigprint();
...
const Cube<eT>& Cube<eT>::for_each(functor F) {
	arma_extra_debug_sigprint();
...
template<typename eT> template<typename functor> inline
const Cube<eT>& Cube<eT>::for_each(functor F) const {
	arma_extra_debug_sigprint();
...
template<typename eT> template<typename functor> inline
const Cube<eT>& Cube<eT>::transform(functor F) {
	arma_extra_debug_sigprint();
...
template<typename eT> template<typename functor> inline
const Cube<eT>& Cube<eT>::imbue(functor F) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
const Cube<eT>& Cube<eT>::replace(const eT old_val, const eT new_val) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
const Cube<eT>& Cube<eT>::fill(const eT val) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
const Cube<eT>& Cube<eT>::zeros(const uword in_rows, const uword in_cols, const uword in_slices) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
const Cube<eT>& Cube<eT>::zeros(const SizeCube& s) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
const Cube<eT>& Cube<eT>::ones(const uword in_rows, const uword in_cols, const uword in_slices) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
const Cube<eT>& Cube<eT>::ones(const SizeCube& s) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
const Cube<eT>& Cube<eT>::randu(const uword in_rows, const uword in_cols, const uword in_slices) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
const Cube<eT>& Cube<eT>::randu(const SizeCube& s) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
const Cube<eT>& Cube<eT>::randn(const uword in_rows, const uword in_cols, const uword in_slices) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
const Cube<eT>& Cube<eT>::randn(const SizeCube& s) {
	arma_extra_debug_sigprint();
...
template<typename eT> template<typename T1> inline
void Cube<eT>::set_real(const BaseCube<typename Cube<eT>::pod_type, T1>& X) {
	arma_extra_debug_sigprint();
...
template<typename eT> template<typename T1> inline
void Cube<eT>::set_imag(const BaseCube<typename Cube<eT>::pod_type, T1>& X) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
eT Cube<eT>::min(uword& index_of_min_val) const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
eT Cube<eT>::max(uword& index_of_max_val) const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
eT Cube<eT>::min(uword& row_of_min_val, uword& col_of_min_val, uword& slice_of_min_val) const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
eT Cube<eT>::max(uword& row_of_max_val, uword& col_of_max_val, uword& slice_of_max_val) const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
arma_cold
bool Cube<eT>::save(const std::string name, const file_type type, const bool print_status) const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
arma_cold
bool Cube<eT>::save(const hdf5_name& spec, const file_type type, const bool print_status) const {
	arma_extra_debug_sigprint();
	...
	if((print_status == true) && (save_okay == false)) { if(err_msg.length() > 0) { arma_debug_warn("Cube::save(): ", err_msg, spec.filename); } else { arma_debug_warn("Cube::save(): couldn't write to ", spec.filename); } }
...
template<typename eT> inline
arma_cold
bool Cube<eT>::save(std::ostream& os, const file_type type, const bool print_status) const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
arma_cold
bool Cube<eT>::load(const std::string name, const file_type type, const bool print_status) {
	arma_extra_debug_sigprint();
	...
	if((print_status == true) && (load_okay == false)) { if(err_msg.length() > 0) { arma_debug_warn("Cube::load(): ", err_msg, name); } else { arma_debug_warn("Cube::load(): couldn't read ", name); } }
...
template<typename eT> inline
arma_cold
bool Cube<eT>::load(const hdf5_name& spec, const file_type type, const bool print_status) {
	arma_extra_debug_sigprint();
	...
	if((print_status == true) && (load_okay == false)) { if(err_msg.length() > 0) { arma_debug_warn("Cube::load(): ", err_msg, spec.filename); } else { arma_debug_warn("Cube::load(): couldn't read ", spec.filename); } }
...
template<typename eT> inline
arma_cold
bool Cube<eT>::load(std::istream& is, const file_type type, const bool print_status) {
	arma_extra_debug_sigprint();
	...
	if((print_status == true) && (load_okay == false)) { if(err_msg.length() > 0) { arma_debug_warn("Cube::load(): ", err_msg, "the given stream"); } else { arma_debug_warn("Cube::load(): couldn't load from the given stream"); } }
...
template<typename eT> inline
arma_cold
bool Cube<eT>::quiet_save(const std::string name, const file_type type) const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
arma_cold
bool Cube<eT>::quiet_save(const hdf5_name& spec, const file_type type) const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
arma_cold
bool Cube<eT>::quiet_save(std::ostream& os, const file_type type) const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
arma_cold
bool Cube<eT>::quiet_load(const std::string name, const file_type type) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
arma_cold
bool Cube<eT>::quiet_load(const hdf5_name& spec, const file_type type) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
arma_cold
bool Cube<eT>::quiet_load(std::istream& is, const file_type type) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
typename Cube<eT>::iterator Cube<eT>::begin() {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
typename Cube<eT>::const_iterator Cube<eT>::begin() const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
typename Cube<eT>::const_iterator Cube<eT>::cbegin() const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
typename Cube<eT>::iterator Cube<eT>::end() {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
typename Cube<eT>::const_iterator Cube<eT>::end() const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
typename Cube<eT>::const_iterator Cube<eT>::cend() const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
typename Cube<eT>::slice_iterator Cube<eT>::begin_slice(const uword slice_num) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
typename Cube<eT>::const_slice_iterator Cube<eT>::begin_slice(const uword slice_num) const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
typename Cube<eT>::slice_iterator Cube<eT>::end_slice(const uword slice_num) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
typename Cube<eT>::const_slice_iterator Cube<eT>::end_slice(const uword slice_num) const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
void Cube<eT>::clear() { reset(); }

//! returns true if the cube has no elements
template<typename eT> inline
bool Cube<eT>::empty() const { return (n_elem == 0); }

//! returns the number of elements in this cube
template<typename eT> inline
uword Cube<eT>::size() const { return n_elem; }

template<typename eT> inline
void Cube<eT>::swap(Cube<eT>& B) {
	Cube<eT>& A = (*this);
	...
	if((A.mem_state == 0) && (B.mem_state == 0) && (A.n_elem > Cube_prealloc::mem_n_elem) && (B.n_elem > Cube_prealloc::mem_n_elem)) {
		...
		A.create_mat();
		B.create_mat();
	} else if((A.mem_state == 0) && (B.mem_state == 0) && (A.n_elem <= Cube_prealloc::mem_n_elem) && (B.n_elem <= Cube_prealloc::mem_n_elem)) {
		A.delete_mat();
		B.delete_mat();
...
template<typename eT> inline
void Cube<eT>::steal_mem(Cube<eT>& x) {
	arma_extra_debug_sigprint();
...
template<typename eT> template<uword fixed_n_rows, uword fixed_n_cols, uword fixed_n_slices> arma_inline
void Cube<eT>::fixed<fixed_n_rows, fixed_n_cols, fixed_n_slices>::mem_setup() {
	arma_extra_debug_sigprint();
...
template<typename eT> template<uword fixed_n_rows, uword fixed_n_cols, uword fixed_n_slices> inline Cube<eT>::fixed<fixed_n_rows, fixed_n_cols, fixed_n_slices>::fixed() {
	arma_extra_debug_sigprint_this(this);

	mem_setup();
}

template<typename eT> template<uword fixed_n_rows, uword fixed_n_cols, uword fixed_n_slices> inline Cube<eT>::fixed<fixed_n_rows, fixed_n_cols, fixed_n_slices>::fixed(const fixed<fixed_n_rows, fixed_n_cols, fixed_n_slices>& X) {
	arma_extra_debug_sigprint_this(this);
...
template<typename eT> template<uword fixed_n_rows, uword fixed_n_cols, uword fixed_n_slices> template<typename fill_type> inline Cube<eT>::fixed<fixed_n_rows, fixed_n_cols, fixed_n_slices>::fixed(const fill::fill_class<fill_type>&) {
	arma_extra_debug_sigprint_this(this);
...
template<typename eT> template<uword fixed_n_rows, uword fixed_n_cols, uword fixed_n_slices> template<typename T1> inline Cube<eT>::fixed<fixed_n_rows, fixed_n_cols, fixed_n_slices>::fixed(const BaseCube<eT, T1>& A) {
	arma_extra_debug_sigprint_this(this);
...
template<typename eT> template<uword fixed_n_rows, uword fixed_n_cols, uword fixed_n_slices> template<typename T1, typename T2> inline Cube<eT>::fixed<fixed_n_rows, fixed_n_cols, fixed_n_slices>::fixed(const BaseCube<pod_type, T1>& A, const BaseCube<pod_type, T2>& B) {
	arma_extra_debug_sigprint_this(this);
...
template<typename eT> template<uword fixed_n_rows, uword fixed_n_cols, uword fixed_n_slices> inline
Cube<eT>& Cube<eT>::fixed<fixed_n_rows, fixed_n_cols, fixed_n_slices>::operator=(const fixed<fixed_n_rows, fixed_n_cols, fixed_n_slices>& X) {
	arma_extra_debug_sigprint();
...
template<typename eT> template<uword fixed_n_rows, uword fixed_n_cols, uword fixed_n_slices> arma_inline
arma_warn_unused
eT& Cube<eT>::fixed<fixed_n_rows, fixed_n_cols, fixed_n_slices>::operator[](const uword i) { return (use_extra) ? mem_local_extra[i] : mem_local[i]; }

template<typename eT> template<uword fixed_n_rows, uword fixed_n_cols, uword fixed_n_slices> arma_inline
arma_warn_unused
const eT& Cube<eT>::fixed<fixed_n_rows, fixed_n_cols, fixed_n_slices>::operator[](const uword i) const { return (use_extra) ? mem_local_extra[i] : mem_local[i]; }

template<typename eT> template<uword fixed_n_rows, uword fixed_n_cols, uword fixed_n_slices> arma_inline
arma_warn_unused
eT& Cube<eT>::fixed<fixed_n_rows, fixed_n_cols, fixed_n_slices>::at(const uword i) { return (use_extra) ? mem_local_extra[i] : mem_local[i]; }

template<typename eT> template<uword fixed_n_rows, uword fixed_n_cols, uword fixed_n_slices> arma_inline
arma_warn_unused
const eT& Cube<eT>::fixed<fixed_n_rows, fixed_n_cols, fixed_n_slices>::at(const uword i) const { return (use_extra) ? mem_local_extra[i] : mem_local[i]; }

template<typename eT> template<uword fixed_n_rows, uword fixed_n_cols, uword fixed_n_slices> arma_inline
arma_warn_unused
eT& Cube<eT>::fixed<fixed_n_rows, fixed_n_cols, fixed_n_slices>::operator()(const uword i) {
...
template<typename eT> template<uword fixed_n_rows, uword fixed_n_cols, uword fixed_n_slices> arma_inline
arma_warn_unused
const eT& Cube<eT>::fixed<fixed_n_rows, fixed_n_cols, fixed_n_slices>::operator()(const uword i) const {
	arma_debug_check((i >= fixed_n_elem), "Cube::operator(): index out of bounds");
...
template<typename eT> template<uword fixed_n_rows, uword fixed_n_cols, uword fixed_n_slices> arma_inline
arma_warn_unused
eT& Cube<eT>::fixed<fixed_n_rows, fixed_n_cols, fixed_n_slices>::at(const uword in_row, const uword in_col, const uword in_slice) {
...
template<typename eT> template<uword fixed_n_rows, uword fixed_n_cols, uword fixed_n_slices> arma_inline
arma_warn_unused
const eT& Cube<eT>::fixed<fixed_n_rows, fixed_n_cols, fixed_n_slices>::at(const uword in_row, const uword in_col, const uword in_slice) const {
	const uword i = in_slice * fixed_n_elem_slice + in_col * fixed_n_rows + in_row;
...
template<typename eT> template<uword fixed_n_rows, uword fixed_n_cols, uword fixed_n_slices> arma_inline
arma_warn_unused
eT& Cube<eT>::fixed<fixed_n_rows, fixed_n_cols, fixed_n_slices>::operator()(const uword in_row, const uword in_col, const uword in_slice) {
	arma_debug_check
	(
		(in_row >= fixed_n_rows) ||
		(in_col >= fixed_n_cols) ||
...
template<typename eT> template<uword fixed_n_rows, uword fixed_n_cols, uword fixed_n_slices> arma_inline
arma_warn_unused
const eT& Cube<eT>::fixed<fixed_n_rows, fixed_n_cols, fixed_n_slices>::operator()(const uword in_row, const uword in_col, const uword in_slice) const {
	arma_debug_check
	(
		(in_row >= fixed_n_rows) ||
		(in_col >= fixed_n_cols) ||
...
//! prefix ++
template<typename eT> arma_inline
void Cube_aux::prefix_pp(Cube<eT>& x) {
	eT* memptr = x.memptr();
...
void Cube_aux::prefix_pp(Cube<std::complex<T>>& x) { x += T(1); }

//! postfix ++
template<typename eT> arma_inline
void Cube_aux::postfix_pp(Cube<eT>& x) {
	eT* memptr = x.memptr();
	const uword n_elem = x.n_elem;
...
template<typename T> arma_inline
void Cube_aux::postfix_pp(Cube<std::complex<T>>& x) { x += T(1); }

//! prefix --
template<typename eT> arma_inline
void Cube_aux::prefix_mm(Cube<eT>& x) {
	eT* memptr = x.memptr();
	const uword n_elem = x.n_elem;
...
//! prefix -- for complex numbers (work around for limitations of the std::complex class)
template<typename T> arma_inline
void Cube_aux::prefix_mm(Cube<std::complex<T>>& x) { x -= T(1); }

//! postfix --
template<typename eT> arma_inline
void Cube_aux::postfix_mm(Cube<eT>& x) {
	eT* memptr = x.memptr();
	const uword n_elem = x.n_elem;
...
//! postfix ++ for complex numbers (work around for limitations of the std::complex class)
template<typename T> arma_inline
void Cube_aux::postfix_mm(Cube<std::complex<T>>& x) { x -= T(1); }

template<typename eT, typename T1> inline
void Cube_aux::set_real(Cube<eT>& out, const BaseCube<eT, T1>& X) {
	arma_extra_debug_sigprint();
...
template<typename eT, typename T1> inline
void Cube_aux::set_imag(Cube<eT>&, const BaseCube<eT, T1>&) {
	arma_extra_debug_sigprint();
}

template<typename T, typename T1> inline
void Cube_aux::set_real(Cube<std::complex<T>>& out, const BaseCube<T, T1>& X) {
	arma_extra_debug_sigprint();
...
template<typename T, typename T1> inline
void Cube_aux::set_imag(Cube<std::complex<T>>& out, const BaseCube<T, T1>& X) {
	arma_extra_debug_sigprint();
