...
void op_cumprod::apply_noalias(Mat<eT>& out, const Mat<eT>& X, const uword dim) {
...
void op_cumprod::apply(Mat<typename T1::elem_type>& out, const Op<T1, op_cumprod>& in) {
	arma_extra_debug_sigprint();
	...
	if(U.is_alias(out)) {
		...
		out.steal_mem(tmp);
	} else { op_cumprod::apply_noalias(out, U.M, dim); }
}

template<typename T1> inline
void op_cumprod_default::apply(Mat<typename T1::elem_type>& out, const Op<T1, op_cumprod_default>& in) {
	arma_extra_debug_sigprint();
