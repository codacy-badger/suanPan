...
template<typename T1> arma_inline SpValProxy<T1>::SpValProxy(uword in_row, uword in_col, T1& in_parent, eT* in_val_ptr)
	: row(in_row)
	, col(in_col)
	, val_ptr(in_val_ptr)
	, parent(in_parent) {
...
SpValProxy<T1>& SpValProxy<T1>::operator=(const SpValProxy<T1>& rhs) { return (*this).operator=(eT(rhs)); }

template<typename T1> template<typename T2> arma_inline
SpValProxy<T1>& SpValProxy<T1>::operator=(const SpValProxy<T2>& rhs) { return (*this).operator=(eT(rhs)); }

template<typename T1> arma_inline
SpValProxy<T1>& SpValProxy<T1>::operator=(const eT rhs) {
...
template<typename T1> arma_inline
SpValProxy<T1>& SpValProxy<T1>::operator++() {
	if(val_ptr) {
		(*val_ptr) += eT(1);
		parent.invalidate_cache();
		check_zero();
	} else { val_ptr = &parent.insert_element(row, col, eT(1)); }
...
template<typename T1> arma_inline
typename T1::elem_type SpValProxy<T1>::operator++(const int) {
	if(val_ptr) {
		(*val_ptr) += eT(1);
		parent.invalidate_cache();
		check_zero();
...
template<typename T1> arma_inline SpValProxy<T1>::operator eT() const { return (val_ptr) ? eT(*val_ptr) : eT(0); }

template<typename T1> arma_inline
typename get_pod_type<typename SpValProxy<T1>::eT>::result SpValProxy<T1>::real() const {
...
template<typename T1> arma_inline
typename get_pod_type<typename SpValProxy<T1>::eT>::result SpValProxy<T1>::imag() const {
	typedef typename get_pod_type<eT>::result T;
