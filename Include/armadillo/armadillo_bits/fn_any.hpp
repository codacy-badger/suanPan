...
arma_inline
const mtOp<uword, T1, op_any> any
(
	const T1& X,
	const uword dim = 0,
...
template<typename T1> arma_warn_unused
inline
bool any(const mtOp<uword, T1, op_any>& in) {
	arma_extra_debug_sigprint();
...
template<typename T1> arma_warn_unused
arma_inline
const Op<mtOp<uword, T1, op_any>, op_any> any(const mtOp<uword, T1, op_any>& in, const uword dim) {
	arma_extra_debug_sigprint();
