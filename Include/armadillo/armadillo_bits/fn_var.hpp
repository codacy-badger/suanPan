...
inline
const mtOp<typename T1::pod_type, T1, op_var> var
(
	const T1& X,
...
arma_inline
const typename arma_scalar_only<T>::result var(const T&) { return T(0); }

template<typename T1> arma_warn_unused
inline
const mtSpOp<typename T1::pod_type, T1, spop_var> var
(
	const T1& X,
	const uword norm_type = 0,
	const uword dim = 0,
