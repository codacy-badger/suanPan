...
struct state_type {
	...
	arma_inline operator int() const {
		int out;
	...
	arma_inline
	void operator=(const int in_state) {
#if   defined(ARMA_USE_OPENMP)
...
enum file_type {
	...
	hdf5_binary,
	//!< Open binary format, not specific to Armadillo, which can store arbitrary data
	hdf5_binary_trans,
	//!< as per hdf5_binary, but save/load the data with columns transposed to rows
...
namespace hdf5_opts {
	...
	inline opts::opts(const flag_type in_flags)
		: flags(in_flags) {}

	inline
	const opts opts::operator+(const opts& rhs) const {
	...
	struct opts_trans : public opts {
		inline opts_trans()
			: opts(flag_trans) {}
	};

	struct opts_append : public opts {
		inline opts_append()
			: opts(flag_append) {}
	};

	struct opts_replace : public opts {
		inline opts_replace()
			: opts(flag_replace) {}
	};
...
struct hdf5_name {
	...
	inline hdf5_name(const std::string& in_filename)
		: filename(in_filename)
		, dsname(std::string())
		, opts(hdf5_opts::none) {}

	inline hdf5_name(const std::string& in_filename, const std::string& in_dsname, const hdf5_opts::opts& in_opts = hdf5_opts::none)
		: filename(in_filename)
		, dsname(in_dsname)
		, opts(in_opts) {}
};
...
namespace fill {
	...
	template<typename fill_type> struct fill_class {
		inline fill_class() {}
	};
...
struct spsolve_opts_base {
	...
	inline spsolve_opts_base(const unsigned int in_id)
		: id(in_id) {}
};

struct spsolve_opts_none : public spsolve_opts_base {
	inline spsolve_opts_none()
		: spsolve_opts_base(0) {}
};
