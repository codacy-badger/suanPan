...
bool auxlib::inv(Mat<eT>& out, const Mat<eT>& A) {
...
inline
bool auxlib::inv_tiny(Mat<eT>& out, const Mat<eT>& X) {
	arma_extra_debug_sigprint();
...
bool auxlib::inv_tr(Mat<eT>& out, const Base<eT, T1>& X, const uword layout) {
	arma_extra_debug_sigprint();
...
template<typename eT, typename T1> inline
bool auxlib::inv_sympd(Mat<eT>& out, const Base<eT, T1>& X) {
	arma_extra_debug_sigprint();
...
template<typename eT> arma_cold
inline
bool auxlib::inv_sympd_tiny(Mat<eT>& out, const Mat<eT>& X) {
	arma_extra_debug_sigprint();
...
template<typename eT> arma_cold
inline
eT auxlib::det_tinymat(const Mat<eT>& X, const uword N) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
eT auxlib::det_lapack(const Mat<eT>& X, const bool make_copy) {
	arma_extra_debug_sigprint();
...
#elif defined(ARMA_USE_LAPACK)
	{
		...
		// on output tmp appears to be L+U_alt, where U_alt is U with the main diagonal set to zero
		eT val = tmp.at(0, 0);
		for(uword i = 1; i < tmp.n_rows; ++i) { val *= tmp.at(i, i); }
...
template<typename eT, typename T1> inline
bool auxlib::log_det(eT& out_val, typename get_pod_type<eT>::result& out_sign, const Base<eT, T1>& X) {
	arma_extra_debug_sigprint();
...
template<typename eT, typename T1> inline
bool auxlib::lu(Mat<eT>& L, Mat<eT>& U, podarray<blas_int>& ipiv, const Base<eT, T1>& X) {
	arma_extra_debug_sigprint();
...
#if defined(ARMA_USE_ATLAS) || defined(ARMA_USE_LAPACK)
	{
      ...
      if(info < 0)  { return false; }
		...
		for(uword col = 0; col < U_n_cols; ++col) {
			for(uword row = 0; (row < col) && (row < U_n_rows); ++row) { L.at(row, col) = eT(0); }

			if(L.in_range(col, col) == true) { L.at(col, col) = eT(1); }
...
template<typename eT, typename T1> inline
bool auxlib::lu(Mat<eT>& L, Mat<eT>& U, Mat<eT>& P, const Base<eT, T1>& X) {
	arma_extra_debug_sigprint();
	...
	for(uword i = 0; i < P_rows; ++i) { ipiv2_mem[i] = blas_int(i); }

	for(uword i = 0; i < n; ++i) {
		const uword k = static_cast<uword>(ipiv1_mem[i]);

		if(ipiv2_mem[i] != ipiv2_mem[k]) { std::swap(ipiv2_mem[i], ipiv2_mem[k]); }
	}

	P.zeros(P_rows, P_rows);

	for(uword row = 0; row < P_rows; ++row) { P.at(row, static_cast<uword>(ipiv2_mem[row])) = eT(1); }

	if(L.n_cols > U.n_rows) { L.shed_cols(U.n_rows, L.n_cols - 1); }

	if(U.n_rows > L.n_cols) { U.shed_rows(L.n_cols, U.n_rows - 1); }

	return true;
}

template<typename eT, typename T1> inline
bool auxlib::lu(Mat<eT>& L, Mat<eT>& U, const Base<eT, T1>& X) {
	arma_extra_debug_sigprint();
		...
		if(vecs_on) {
			for(uword j = 0; j < X.n_rows; ++j) {
				if((j < (X.n_rows - 1)) && (vals_mem[j] == std::conj(vals_mem[j + 1]))) {
					...
					++j;
				} else { for(uword i = 0; i < X.n_rows; ++i) { vecs.at(i, j) = std::complex<T>(tmp.at(i, j), T(0)); } }
			}
		}
		...
		if(vecs_on) {
			for(uword j = 0; j < A.n_rows; ++j) {
				if((j < (A.n_rows - 1)) && (vals_mem[j] == std::conj(vals_mem[j + 1]))) {
					...
					++j;
				} else { for(uword i = 0; i < A.n_rows; ++i) { vecs.at(i, j) = std::complex<T>(tmp.at(i, j), T(0)); } }
			}
		}
...
template<typename eT, typename T1> inline
bool auxlib::eig_sym(Col<eT>& eigval, const Base<eT, T1>& X) {
	arma_extra_debug_sigprint();
...
bool auxlib::eig_sym(Col<T>& eigval, const Base<std::complex<T>, T1>& X) {
	arma_extra_debug_sigprint();
...
template<typename eT, typename T1> inline
bool auxlib::eig_sym(Col<eT>& eigval, Mat<eT>& eigvec, const Base<eT, T1>& X) {
	arma_extra_debug_sigprint();
...
template<typename T, typename T1> inline
bool auxlib::eig_sym(Col<T>& eigval, Mat<std::complex<T>>& eigvec, const Base<std::complex<T>, T1>& X) {
	arma_extra_debug_sigprint();
...
template<typename eT, typename T1> inline
bool auxlib::eig_sym_dc(Col<eT>& eigval, Mat<eT>& eigvec, const Base<eT, T1>& X) {
	arma_extra_debug_sigprint();
...
template<typename T, typename T1> inline
bool auxlib::eig_sym_dc(Col<T>& eigval, Mat<std::complex<T>>& eigvec, const Base<std::complex<T>, T1>& X) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
bool auxlib::chol(Mat<eT>& X, const uword layout) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
bool auxlib::chol_band(Mat<eT>& X, const uword KD, const uword layout) {
	arma_extra_debug_sigprint();
...
bool auxlib::chol_band(Mat<std::complex<T>>& X, const uword KD, const uword layout) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
bool auxlib::chol_band_common(Mat<eT>& X, const uword KD, const uword layout) {
	arma_extra_debug_sigprint();
...
template<typename eT, typename T1> inline
bool auxlib::hess(Mat<eT>& H, const Base<eT, T1>& X, Col<eT>& tao) {
	arma_extra_debug_sigprint();
...
template<typename eT, typename T1> inline
bool auxlib::qr(Mat<eT>& Q, Mat<eT>& R, const Base<eT, T1>& X) {
	arma_extra_debug_sigprint();
...
#if defined(ARMA_USE_LAPACK)
	{
		...
		for(uword col = 0; col < R_n_cols; ++col) { for(uword row = (col + 1); row < R_n_rows; ++row) { R.at(row, col) = eT(0); } }
		...
		} else if((is_supported_complex_float<eT>::value) || (is_supported_complex_double<eT>::value)) {
...
template<typename eT, typename T1> inline
bool auxlib::qr_econ(Mat<eT>& Q, Mat<eT>& R, const Base<eT, T1>& X) {
	arma_extra_debug_sigprint();
...
#if defined(ARMA_USE_LAPACK)
	{
		...
		for(uword col = 0; col < Q_n_cols; ++col) {
			for(uword row = 0; row <= col; ++row) { R.at(row, col) = Q.at(row, col); }

			for(uword row = (col + 1); row < Q_n_cols; ++row) { R.at(row, col) = eT(0); }
		}
...
template<typename eT, typename T1> inline
bool auxlib::svd(Col<eT>& S, const Base<eT, T1>& X, uword& X_n_rows, uword& X_n_cols) {
	arma_extra_debug_sigprint();
...
template<typename T, typename T1> inline
bool auxlib::svd(Col<T>& S, const Base<std::complex<T>, T1>& X, uword& X_n_rows, uword& X_n_cols) {
	arma_extra_debug_sigprint();
...
template<typename eT, typename T1> inline
bool auxlib::svd(Col<eT>& S, const Base<eT, T1>& X) {
	arma_extra_debug_sigprint();
...
template<typename T, typename T1> inline
bool auxlib::svd(Col<T>& S, const Base<std::complex<T>, T1>& X) {
	arma_extra_debug_sigprint();
...
template<typename eT, typename T1> inline
bool auxlib::svd(Mat<eT>& U, Col<eT>& S, Mat<eT>& V, const Base<eT, T1>& X) {
	arma_extra_debug_sigprint();
...
template<typename T, typename T1> inline
bool auxlib::svd(Mat<std::complex<T>>& U, Col<T>& S, Mat<std::complex<T>>& V, const Base<std::complex<T>, T1>& X) {
	arma_extra_debug_sigprint();
...
template<typename eT, typename T1> inline
bool auxlib::svd_econ(Mat<eT>& U, Col<eT>& S, Mat<eT>& V, const Base<eT, T1>& X, const char mode) {
	arma_extra_debug_sigprint();
...
template<typename T, typename T1> inline
bool auxlib::svd_econ(Mat<std::complex<T>>& U, Col<T>& S, Mat<std::complex<T>>& V, const Base<std::complex<T>, T1>& X, const char mode) {
	arma_extra_debug_sigprint();
...
template<typename eT, typename T1> inline
bool auxlib::svd_dc(Col<eT>& S, const Base<eT, T1>& X, uword& X_n_rows, uword& X_n_cols) {
	arma_extra_debug_sigprint();
...
template<typename T, typename T1> inline
bool auxlib::svd_dc(Col<T>& S, const Base<std::complex<T>, T1>& X, uword& X_n_rows, uword& X_n_cols) {
	arma_extra_debug_sigprint();
...
template<typename eT, typename T1> inline
bool auxlib::svd_dc(Col<eT>& S, const Base<eT, T1>& X) {
	arma_extra_debug_sigprint();
...
template<typename T, typename T1> inline
bool auxlib::svd_dc(Col<T>& S, const Base<std::complex<T>, T1>& X) {
	arma_extra_debug_sigprint();
...
template<typename eT, typename T1> inline
bool auxlib::svd_dc(Mat<eT>& U, Col<eT>& S, Mat<eT>& V, const Base<eT, T1>& X) {
	arma_extra_debug_sigprint();
...
template<typename T, typename T1> inline
bool auxlib::svd_dc(Mat<std::complex<T>>& U, Col<T>& S, Mat<std::complex<T>>& V, const Base<std::complex<T>, T1>& X) {
	arma_extra_debug_sigprint();
...
template<typename eT, typename T1> inline
bool auxlib::svd_dc_econ(Mat<eT>& U, Col<eT>& S, Mat<eT>& V, const Base<eT, T1>& X) {
	arma_extra_debug_sigprint();
...
template<typename T, typename T1> inline
bool auxlib::svd_dc_econ(Mat<std::complex<T>>& U, Col<T>& S, Mat<std::complex<T>>& V, const Base<std::complex<T>, T1>& X) {
	arma_extra_debug_sigprint();
...
inline
bool auxlib::solve_square_tiny(Mat<typename T1::elem_type>& out, const Mat<typename T1::elem_type>& A, const Base<typename T1::elem_type, T1>& B_expr) {
	arma_extra_debug_sigprint();
...
template<typename T1> inline
bool auxlib::solve_square_fast(Mat<typename T1::elem_type>& out, Mat<typename T1::elem_type>& A, const Base<typename T1::elem_type, T1>& B_expr) {
	arma_extra_debug_sigprint();
...
template<typename T1> inline
bool auxlib::solve_square_refine(Mat<typename T1::pod_type>& out, typename T1::pod_type& out_rcond, Mat<typename T1::pod_type>& A, const Base<typename T1::pod_type, T1>& B_expr, const bool equilibrate, const bool allow_ugly) {
	arma_extra_debug_sigprint();
...
template<typename T1> inline
bool auxlib::solve_square_refine(Mat<std::complex<typename T1::pod_type>>& out, typename T1::pod_type& out_rcond, Mat<std::complex<typename T1::pod_type>>& A, const Base<std::complex<typename T1::pod_type>, T1>& B_expr, const bool equilibrate, const bool allow_ugly) {
	arma_extra_debug_sigprint();
...
template<typename T1> inline
bool auxlib::solve_sympd_fast(Mat<typename T1::elem_type>& out, Mat<typename T1::elem_type>& A, const Base<typename T1::elem_type, T1>& B_expr) {
	arma_extra_debug_sigprint();
...
template<typename T1> inline
bool auxlib::solve_sympd_fast_common(Mat<typename T1::elem_type>& out, Mat<typename T1::elem_type>& A, const Base<typename T1::elem_type, T1>& B_expr) {
	arma_extra_debug_sigprint();
...
template<typename T1> inline
bool auxlib::solve_sympd_refine(Mat<typename T1::pod_type>& out, typename T1::pod_type& out_rcond, Mat<typename T1::pod_type>& A, const Base<typename T1::pod_type, T1>& B_expr, const bool equilibrate, const bool allow_ugly) {
	arma_extra_debug_sigprint();
...
template<typename T1> inline
bool auxlib::solve_sympd_refine(Mat<std::complex<typename T1::pod_type>>& out, typename T1::pod_type& out_rcond, Mat<std::complex<typename T1::pod_type>>& A, const Base<std::complex<typename T1::pod_type>, T1>& B_expr, const bool equilibrate, const bool allow_ugly) {
	arma_extra_debug_sigprint();
...
template<typename T1> inline
bool auxlib::solve_approx_fast(Mat<typename T1::elem_type>& out, Mat<typename T1::elem_type>& A, const Base<typename T1::elem_type, T1>& B_expr) {
	arma_extra_debug_sigprint();
...
#if defined(ARMA_USE_LAPACK)
	{
		...
		if(arma::size(tmp) == arma::size(B)) { tmp = B; } else {
			tmp.zeros();
		...
		if(tmp.n_rows == A.n_cols) { out.steal_mem(tmp); } else { out = tmp.head_rows(A.n_cols); }
...
template<typename T1> inline
bool auxlib::solve_approx_svd(Mat<typename T1::pod_type>& out, Mat<typename T1::pod_type>& A, const Base<typename T1::pod_type, T1>& B_expr) {
	arma_extra_debug_sigprint();
...
template<typename T1> inline
bool auxlib::solve_approx_svd(Mat<std::complex<typename T1::pod_type>>& out, Mat<std::complex<typename T1::pod_type>>& A, const Base<std::complex<typename T1::pod_type>, T1>& B_expr) {
	arma_extra_debug_sigprint();
...
template<typename T1> inline
bool auxlib::solve_tri(Mat<typename T1::elem_type>& out, const Mat<typename T1::elem_type>& A, const Base<typename T1::elem_type, T1>& B_expr, const uword layout) {
	arma_extra_debug_sigprint();
...
template<typename T1> inline
bool auxlib::solve_band_fast(Mat<typename T1::pod_type>& out, Mat<typename T1::pod_type>& A, const uword KL, const uword KU, const Base<typename T1::pod_type, T1>& B_expr) {
	arma_extra_debug_sigprint();
...
template<typename T1> inline
bool auxlib::solve_band_fast(Mat<std::complex<typename T1::pod_type>>& out, Mat<std::complex<typename T1::pod_type>>& A, const uword KL, const uword KU, const Base<std::complex<typename T1::pod_type>, T1>& B_expr) {
	arma_extra_debug_sigprint();
...
template<typename T1> inline
bool auxlib::solve_band_fast_common(Mat<typename T1::elem_type>& out, const Mat<typename T1::elem_type>& A, const uword KL, const uword KU, const Base<typename T1::elem_type, T1>& B_expr) {
	arma_extra_debug_sigprint();
...
template<typename T1> inline
bool auxlib::solve_band_refine(Mat<typename T1::pod_type>& out, typename T1::pod_type& out_rcond, Mat<typename T1::pod_type>& A, const uword KL, const uword KU, const Base<typename T1::pod_type, T1>& B_expr, const bool equilibrate, const bool allow_ugly) {
	arma_extra_debug_sigprint();
...
template<typename T1> inline
bool auxlib::solve_band_refine(Mat<std::complex<typename T1::pod_type>>& out, typename T1::pod_type& out_rcond, Mat<std::complex<typename T1::pod_type>>& A, const uword KL, const uword KU, const Base<std::complex<typename T1::pod_type>, T1>& B_expr, const bool equilibrate, const bool allow_ugly) {
	arma_extra_debug_sigprint();
...
template<typename T1> inline
bool auxlib::solve_tridiag_fast(Mat<typename T1::pod_type>& out, Mat<typename T1::pod_type>& A, const Base<typename T1::pod_type, T1>& B_expr) {
	arma_extra_debug_sigprint();
...
template<typename T1> inline
bool auxlib::solve_tridiag_fast(Mat<std::complex<typename T1::pod_type>>& out, Mat<std::complex<typename T1::pod_type>>& A, const Base<std::complex<typename T1::pod_type>, T1>& B_expr) {
	arma_extra_debug_sigprint();
...
template<typename T1> inline
bool auxlib::solve_tridiag_fast_common(Mat<typename T1::elem_type>& out, const Mat<typename T1::elem_type>& A, const Base<typename T1::elem_type, T1>& B_expr) {
	arma_extra_debug_sigprint();
...
template<typename T1> inline
bool auxlib::solve_tridiag_refine(Mat<typename T1::pod_type>& out, typename T1::pod_type& out_rcond, Mat<typename T1::pod_type>& A, const Base<typename T1::pod_type, T1>& B_expr, const bool allow_ugly) {
	arma_extra_debug_sigprint();
...
template<typename T1> inline
bool auxlib::solve_tridiag_refine(Mat<std::complex<typename T1::pod_type>>& out, typename T1::pod_type& out_rcond, Mat<std::complex<typename T1::pod_type>>& A, const Base<std::complex<typename T1::pod_type>, T1>& B_expr, const bool allow_ugly) {
	arma_extra_debug_sigprint();
...
template<typename eT, typename T1> inline
bool auxlib::schur(Mat<eT>& U, Mat<eT>& S, const Base<eT, T1>& X, const bool calc_U) {
	arma_extra_debug_sigprint();
...
template<typename T, typename T1> inline
bool auxlib::schur(Mat<std::complex<T>>& U, Mat<std::complex<T>>& S, const Base<std::complex<T>, T1>& X, const bool calc_U) {
	arma_extra_debug_sigprint();
...
template<typename T> inline
bool auxlib::schur(Mat<std::complex<T>>& U, Mat<std::complex<T>>& S, const bool calc_U) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
bool auxlib::syl(Mat<eT>& X, const Mat<eT>& A, const Mat<eT>& B, const Mat<eT>& C) {
	arma_extra_debug_sigprint();
...
bool auxlib::qz(Mat<T>& A, Mat<T>& B, Mat<T>& vsl, Mat<T>& vsr, const Base<T, T1>& X_expr, const Base<T, T2>& Y_expr, const char mode) {
	arma_extra_debug_sigprint();
...
#if defined(ARMA_USE_LAPACK)
	{
		...
		if(mode == 'l') {
			eigsort = 'S';
			selctg = qz_helper::ptr_cast(&(qz_helper::select_lhp<T>));
		} else if(mode == 'r') {
			eigsort = 'S';
			selctg = qz_helper::ptr_cast(&(qz_helper::select_rhp<T>));
		} else if(mode == 'i') {
			eigsort = 'S';
			selctg = qz_helper::ptr_cast(&(qz_helper::select_iuc<T>));
		} else if(mode == 'o') {
			eigsort = 'S';
			selctg = qz_helper::ptr_cast(&(qz_helper::select_ouc<T>));
		}
...
template<typename T, typename T1, typename T2> inline
bool auxlib::qz(Mat<std::complex<T>>& A, Mat<std::complex<T>>& B, Mat<std::complex<T>>& vsl, Mat<std::complex<T>>& vsr, const Base<std::complex<T>, T1>& X_expr, const Base<std::complex<T>, T2>& Y_expr, const char mode) {
	arma_extra_debug_sigprint();
...
#if defined(ARMA_USE_LAPACK)
	{
		...
		if(mode == 'l') {
			eigsort = 'S';
			selctg = qz_helper::ptr_cast(&(qz_helper::cx_select_lhp<T>));
		} else if(mode == 'r') {
			eigsort = 'S';
			selctg = qz_helper::ptr_cast(&(qz_helper::cx_select_rhp<T>));
		} else if(mode == 'i') {
			eigsort = 'S';
			selctg = qz_helper::ptr_cast(&(qz_helper::cx_select_iuc<T>));
		} else if(mode == 'o') {
			eigsort = 'S';
			selctg = qz_helper::ptr_cast(&(qz_helper::cx_select_ouc<T>));
		}
...
template<typename T> inline
T auxlib::rcond(Mat<std::complex<T>>& A) {
#if defined(ARMA_USE_LAPACK)
	{
		typedef typename std::complex<T> eT;
...
template<typename eT> inline
eT auxlib::rcond_sympd(Mat<eT>& A, bool& calc_ok) {
#if defined(ARMA_USE_LAPACK)
	{
		arma_debug_assert_blas_size(A);
...
template<typename T> inline
T auxlib::rcond_sympd(Mat<std::complex<T>>& A, bool& calc_ok) {
#if defined(ARMA_CRIPPLED_LAPACK)
    {
...
template<typename T1> inline
bool auxlib::crippled_lapack(const Base<typename T1::elem_type, T1>&) {
#if defined(ARMA_CRIPPLED_LAPACK)
    {
...
namespace qz_helper {
	...
	template<typename T> inline
	blas_int select_lhp(const T* x_ptr, const T* y_ptr, const T* z_ptr) {
		arma_extra_debug_sigprint();
	...
	template<typename T> inline
	blas_int select_rhp(const T* x_ptr, const T* y_ptr, const T* z_ptr) {
		arma_extra_debug_sigprint();
	...
	template<typename T> inline
	blas_int select_iuc(const T* x_ptr, const T* y_ptr, const T* z_ptr) {
		arma_extra_debug_sigprint();
	...
	template<typename T> inline
	blas_int select_ouc(const T* x_ptr, const T* y_ptr, const T* z_ptr) {
		arma_extra_debug_sigprint();
	...
	template<typename T> inline
	blas_int cx_select_lhp(const std::complex<T>* x_ptr, const std::complex<T>* y_ptr) {
		arma_extra_debug_sigprint();
	...
	template<typename T> inline
	blas_int cx_select_rhp(const std::complex<T>* x_ptr, const std::complex<T>* y_ptr) {
		arma_extra_debug_sigprint();
	...
	template<typename T> inline
	blas_int cx_select_iuc(const std::complex<T>* x_ptr, const std::complex<T>* y_ptr) {
		arma_extra_debug_sigprint();
	...
	template<typename T> inline
	blas_int cx_select_ouc(const std::complex<T>* x_ptr, const std::complex<T>* y_ptr) {
		arma_extra_debug_sigprint();
	...
	template<typename T> inline
	void_ptr ptr_cast(blas_int (*function)(const T*, const T*, const T*)) {
		union converter {
	...
	template<typename T> inline
	void_ptr ptr_cast(blas_int (*function)(const std::complex<T>*, const std::complex<T>*)) {
		union converter {
