...
inline
typename T1::elem_type trace(const Base<typename T1::elem_type, T1>& X) {
...
template<typename T1> arma_warn_unused
inline
typename T1::elem_type trace(const Op<T1, op_diagmat>& X) {
	arma_extra_debug_sigprint();
...
inline
typename T1::elem_type trace(const Glue<T1, T2, glue_times>& X) {
	arma_extra_debug_sigprint();
	...
	if((partial_unwrap<T1>::do_trans == false) && (partial_unwrap<T2>::do_trans == false)) {
		...
		for(uword k = 0; k < N; ++k) {
			...
			if(i < A_n_cols) { acc1 += A.at(k, i) * B_colptr[i]; }
		}
		...
		for(uword k = 0; k < N; ++k) {
			...
			for(uword i = 0; i < A_n_rows; ++i) { acc += A_colptr[i] * B.at(k, i); }
		}
	}
...
template<typename T1> arma_warn_unused
inline
typename T1::elem_type trace(const SpBase<typename T1::elem_type, T1>& expr) {
	arma_extra_debug_sigprint();
...
template<typename T1, typename T2> arma_warn_unused
inline
typename T1::elem_type trace(const SpGlue<T1, T2, spglue_plus>& expr) {
	arma_extra_debug_sigprint();
...
template<typename T1, typename T2> arma_warn_unused
inline
typename T1::elem_type trace(const SpGlue<T1, T2, spglue_minus>& expr) {
	arma_extra_debug_sigprint();
...
template<typename T1, typename T2> arma_warn_unused
inline
typename T1::elem_type trace(const SpGlue<T1, T2, spglue_schur>& expr) {
	arma_extra_debug_sigprint();
	...
	for(uword i = 0; i < N; ++i) { acc += A.at(i, i) * B.at(i, i); }
...
template<typename T1, typename T2> arma_warn_unused
inline
typename T1::elem_type trace(const SpGlue<T1, T2, spglue_times>& expr) {
	arma_extra_debug_sigprint();
...
template<typename T1, typename T2> arma_warn_unused
inline
typename enable_if2<is_cx<typename T1::elem_type>::no, typename T1::elem_type>::result trace(const SpGlue<SpOp<T1, spop_htrans>, T2, spglue_times>& expr) {
	arma_extra_debug_sigprint();
