...
void op_logmat::apply(Mat<std::complex<typename T1::elem_type>>& out, const mtOp<std::complex<typename T1::elem_type>, T1, op_logmat>& in) {
...
template<typename T1> inline
bool op_logmat::apply_direct(Mat<std::complex<typename T1::elem_type>>& out, const Op<T1, op_diagmat>& expr, const uword) {
	arma_extra_debug_sigprint();
...
template<typename T1> inline
bool op_logmat::apply_direct(Mat<std::complex<typename T1::elem_type>>& out, const Base<typename T1::elem_type, T1>& expr, const uword n_iters) {
	arma_extra_debug_sigprint();
	...
	for(uword i = 0; i < n_elem; ++i) { Smem[i] = std::complex<in_T>(Pea[i]); }

	return op_logmat_cx::apply_common(out, S, n_iters);
}

template<typename T1> inline
void op_logmat_cx::apply(Mat<typename T1::elem_type>& out, const Op<T1, op_logmat_cx>& in) {
	arma_extra_debug_sigprint();
...
template<typename T1> inline
bool op_logmat_cx::apply_direct(Mat<typename T1::elem_type>& out, const Op<T1, op_diagmat>& expr, const uword) {
	arma_extra_debug_sigprint();
...
template<typename T1> inline
bool op_logmat_cx::apply_direct_noalias(Mat<typename T1::elem_type>& out, const diagmat_proxy<T1>& P) {
	arma_extra_debug_sigprint();
	...
	for(uword i = 0; i < N; ++i) { out.at(i, i) = std::log(P[i]); }

	return true;
}

template<typename T1> inline
bool op_logmat_cx::apply_direct(Mat<typename T1::elem_type>& out, const Base<typename T1::elem_type, T1>& expr, const uword n_iters) {
	arma_extra_debug_sigprint();
...
bool op_logmat_cx::apply_common(Mat<std::complex<T>>& out, Mat<std::complex<T>>& S, const uword n_iters) {
	arma_extra_debug_sigprint();
...
bool op_logmat_cx::helper(Mat<eT>& A, const uword m) {
	arma_extra_debug_sigprint();
...
template<typename T1> inline
void op_logmat_sympd::apply(Mat<typename T1::elem_type>& out, const Op<T1, op_logmat_sympd>& in) {
	arma_extra_debug_sigprint();
...
template<typename T1> inline
bool op_logmat_sympd::apply_direct(Mat<typename T1::elem_type>& out, const Base<typename T1::elem_type, T1>& expr) {
	arma_extra_debug_sigprint();
