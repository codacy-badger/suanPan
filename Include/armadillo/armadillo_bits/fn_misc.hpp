...
template<typename out_type> arma_warn_unused
inline
typename
enable_if2
<
...
inline
vec linspace(const double start, const double end, const uword num = 100u) {
	arma_extra_debug_sigprint();
...
(
	...
	for(uword i = 0; i < n_elem; ++i) { x_mem[i] = std::pow(T(10), x_mem[i]); }
...
inline
vec logspace(const double A, const double B, const uword N = 50u) {
	arma_extra_debug_sigprint();
...
inline
typename arma_real_only<eT>::result log_add_exp(eT log_a, eT log_b) {
	if(log_a < log_b) { std::swap(log_a, log_b); }

	const eT negdelta = log_b - log_a;

	if((negdelta < Datum<eT>::log_min) || (arma_isfinite(negdelta) == false)) { return log_a; } else { return (log_a + arma_log1p(std::exp(negdelta))); }
}

// for compatibility with earlier versions
template<typename eT> arma_warn_unused
inline
typename arma_real_only<eT>::result log_add(eT log_a, eT log_b) { return log_add_exp(log_a, log_b); }

template<typename eT> arma_warn_unused
arma_inline
bool is_finite(const eT x, const typename arma_scalar_only<eT>::result* junk = 0) {
	arma_ignore(junk);
...
>::result is_finite(const T1& X) {
	...
	if(Proxy<T1>::use_at == false) {
		...
		if(i < n_elem) { if(arma_isfinite(Pea[i]) == false) { return false; } }
	} else {
...
template<typename T1> arma_warn_unused
inline
bool is_finite(const SpBase<typename T1::elem_type, T1>& X) {
	arma_extra_debug_sigprint();
...
template<typename T1> arma_warn_unused
inline
bool is_finite(const BaseCube<typename T1::elem_type, T1>& X) {
	arma_extra_debug_sigprint();
...
inline
const T1& sympd(const Base<typename T1::elem_type, T1>& X) {
	arma_extra_debug_sigprint();
...
inline
uvec ind2sub(const SizeMat& s, const uword i) {
	arma_extra_debug_sigprint();
...
template<typename T1> arma_warn_unused
inline
typename enable_if2<(is_arma_type<T1>::value && is_same_type<uword, typename T1::elem_type>::yes), umat>::result ind2sub(const SizeMat& s, const T1& indices) {
	arma_extra_debug_sigprint();
...
inline
uvec ind2sub(const SizeCube& s, const uword i) {
	arma_extra_debug_sigprint();
...
template<typename T1> arma_warn_unused
inline
typename enable_if2<(is_arma_type<T1>::value && is_same_type<uword, typename T1::elem_type>::yes), umat>::result ind2sub(const SizeCube& s, const T1& indices) {
	arma_extra_debug_sigprint();
...
arma_warn_unused
arma_inline
uword sub2ind(const SizeMat& s, const uword row, const uword col) {
	arma_extra_debug_sigprint();
...
template<typename T1> arma_warn_unused
inline
uvec sub2ind(const SizeMat& s, const Base<uword, T1>& subscripts) {
	arma_extra_debug_sigprint();
...
arma_warn_unused
arma_inline
uword sub2ind(const SizeCube& s, const uword row, const uword col, const uword slice) {
	arma_extra_debug_sigprint();
...
template<typename T1> arma_warn_unused
inline
uvec sub2ind(const SizeCube& s, const Base<uword, T1>& subscripts) {
	arma_extra_debug_sigprint();
