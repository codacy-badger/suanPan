...
template<typename eT> inline SpCol<eT>::SpCol()
	: SpMat<eT>(arma_vec_indicator(), 1) {
	arma_extra_debug_sigprint();
}

template<typename eT> inline SpCol<eT>::SpCol(const uword in_n_elem)
	: SpMat<eT>(arma_vec_indicator(), in_n_elem, 1, 1) {
	arma_extra_debug_sigprint();
}

template<typename eT> inline SpCol<eT>::SpCol(const uword in_n_rows, const uword in_n_cols)
	: SpMat<eT>(arma_vec_indicator(), in_n_rows, in_n_cols, 1) {
	arma_extra_debug_sigprint();
}

template<typename eT> inline SpCol<eT>::SpCol(const SizeMat& s)
...
template<typename eT> inline SpCol<eT>::SpCol(const char* text)
	: SpMat<eT>(arma_vec_indicator(), 1) {
	arma_extra_debug_sigprint();
...
SpCol<eT>& SpCol<eT>::operator=(const char* text) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline SpCol<eT>::SpCol(const std::string& text)
	: SpMat<eT>(arma_vec_indicator(), 1) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
SpCol<eT>& SpCol<eT>::operator=(const std::string& text) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
SpCol<eT>& SpCol<eT>::operator=(const eT val) {
	arma_extra_debug_sigprint();
...
SpCol<eT>& SpCol<eT>::operator=(const Base<eT, T1>& X) {
	arma_extra_debug_sigprint();
...
template<typename eT> template<typename T1> inline
SpCol<eT>& SpCol<eT>::operator=(const SpBase<eT, T1>& X) {
	arma_extra_debug_sigprint();
...
template<typename eT> template<typename T1, typename T2> inline SpCol<eT>::SpCol
(
...
template<typename eT> inline
void SpCol<eT>::shed_row(const uword row_num) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
void SpCol<eT>::shed_rows(const uword in_row1, const uword in_row2) {
	arma_extra_debug_sigprint();
	...
	if(!end_found) { end = SpMat<eT>::n_nonzero; }
...
template<typename eT> inline
typename SpCol<eT>::row_iterator SpCol<eT>::begin_row(const uword row_num) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
typename SpCol<eT>::const_row_iterator SpCol<eT>::begin_row(const uword row_num) const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
typename SpCol<eT>::row_iterator SpCol<eT>::end_row(const uword row_num) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
typename SpCol<eT>::const_row_iterator SpCol<eT>::end_row(const uword row_num) const {
	arma_extra_debug_sigprint();
