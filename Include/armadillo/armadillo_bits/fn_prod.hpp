...
template<typename T1> arma_warn_unused
inline
typename T1::elem_type prod(const Op<T1, op_prod>& in) {
	arma_extra_debug_sigprint();
...
arma_inline
const typename arma_scalar_only<T>::result& prod(const T& x) { return x; }
