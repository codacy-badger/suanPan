...
template<typename eT> inline MapMat<eT>::MapMat(const uword in_n_rows, const uword in_n_cols)
	: n_rows(in_n_rows)
	, n_cols(in_n_cols)
...
template<typename eT> inline MapMat<eT>::MapMat(const SizeMat& s)
	: n_rows(s.n_rows)
	, n_cols(s.n_cols)
...
template<typename eT> inline MapMat<eT>::MapMat(const MapMat<eT>& x)
	: n_rows(0)
	, n_cols(0)
	, n_elem(0)
	, map_ptr(NULL) {
...
void MapMat<eT>::operator=(const MapMat<eT>& x) {
...
template<typename eT> inline MapMat<eT>::MapMat(const SpMat<eT>& x)
	: n_rows(0)
	, n_cols(0)
	, n_elem(0)
	, map_ptr(NULL) {
...
template<typename eT> inline
void MapMat<eT>::operator=(const SpMat<eT>& x) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline MapMat<eT>::MapMat(MapMat<eT>&& x)
	: n_rows(x.n_rows)
	, n_cols(x.n_cols)
	, n_elem(x.n_elem)
	, map_ptr(x.map_ptr) {
...
template<typename eT> inline
void MapMat<eT>::operator=(MapMat<eT>&& x) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
void MapMat<eT>::set_size(const uword in_n_rows) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
void MapMat<eT>::set_size(const uword in_n_rows, const uword in_n_cols) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
void MapMat<eT>::set_size(const SizeMat& s) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
void MapMat<eT>::zeros(const uword in_n_rows) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
void MapMat<eT>::zeros(const uword in_n_rows, const uword in_n_cols) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
void MapMat<eT>::zeros(const SizeMat& s) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
void MapMat<eT>::eye(const uword in_n_rows, const uword in_n_cols) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
void MapMat<eT>::eye(const SizeMat& s) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
void MapMat<eT>::speye(const uword in_n_rows, const uword in_n_cols) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
void MapMat<eT>::speye(const SizeMat& s) {
	arma_extra_debug_sigprint();
...
arma_warn_unused
MapMat_val<eT> MapMat<eT>::operator[](const uword index) { return MapMat_val<eT>(*this, index); }
...
template<typename eT> arma_inline
arma_warn_unused
MapMat_val<eT> MapMat<eT>::at(const uword in_row, const uword in_col) {
...
template<typename eT> inline
arma_warn_unused
eT MapMat<eT>::at(const uword in_row, const uword in_col) const {
	const uword index = (n_rows * in_col) + in_row;
...
template<typename eT> inline
arma_warn_unused
bool MapMat<eT>::is_empty() const { return (n_elem == 0); }

template<typename eT> inline
arma_warn_unused
bool MapMat<eT>::is_vec() const { return ((n_rows == 1) || (n_cols == 1)); }

template<typename eT> inline
arma_warn_unused
bool MapMat<eT>::is_rowvec() const { return (n_rows == 1); }

//! returns true if the object can be interpreted as a column vector
template<typename eT> inline
arma_warn_unused
bool MapMat<eT>::is_colvec() const { return (n_cols == 1); }

template<typename eT> inline
arma_warn_unused
bool MapMat<eT>::is_square() const { return (n_rows == n_cols); }

// this function is for debugging purposes only
template<typename eT> inline
void MapMat<eT>::sprandu(const uword in_n_rows, const uword in_n_cols, const double density) {
	arma_extra_debug_sigprint();
...
// this function is for debugging purposes only
template<typename eT> inline
void MapMat<eT>::print(const std::string& extra_text) const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
uword MapMat<eT>::get_n_nonzero() const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
void MapMat<eT>::get_locval_format(umat& locs, Col<eT>& vals) const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
void MapMat<eT>::init_warm(const uword in_n_rows, const uword in_n_cols) {
	arma_extra_debug_sigprint();
...
template<typename eT> arma_inline
void MapMat<eT>::set_val(const uword index, const eT& in_val) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
void MapMat<eT>::erase_val(const uword index) {
	arma_extra_debug_sigprint();
...
template<typename eT> arma_inline MapMat_val<eT>::MapMat_val(MapMat<eT>& in_parent, const uword in_index)
	: parent(in_parent)
	, index(in_index) {
	arma_extra_debug_sigprint();
}
...
template<typename eT> arma_inline
typename get_pod_type<eT>::result MapMat_val<eT>::real() const {
	arma_extra_debug_sigprint();
...
template<typename eT> arma_inline
typename get_pod_type<eT>::result MapMat_val<eT>::imag() const {
	arma_extra_debug_sigprint();
...
template<typename eT> arma_inline
void MapMat_val<eT>::operator=(const MapMat_val<eT>& x) {
	arma_extra_debug_sigprint();
...
template<typename eT> arma_inline
void MapMat_val<eT>::operator=(const eT in_val) {
	arma_extra_debug_sigprint();
...
template<typename eT> arma_inline
void MapMat_val<eT>::operator++(int) {
	arma_extra_debug_sigprint();
...
template<typename eT> arma_inline SpMat_MapMat_val<eT>::SpMat_MapMat_val(SpMat<eT>& in_s_parent, MapMat<eT>& in_m_parent, const uword in_row, const uword in_col)
...
template<typename eT> inline SpMat_MapMat_val<eT>::operator eT() const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
typename get_pod_type<eT>::result SpMat_MapMat_val<eT>::real() const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
typename get_pod_type<eT>::result SpMat_MapMat_val<eT>::imag() const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
SpMat_MapMat_val<eT>& SpMat_MapMat_val<eT>::operator=(const SpMat_MapMat_val<eT>& x) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
SpMat_MapMat_val<eT>& SpMat_MapMat_val<eT>::operator=(const eT in_val) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
SpMat_MapMat_val<eT>& SpMat_MapMat_val<eT>::operator++() {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
eT SpMat_MapMat_val<eT>::operator++(int) {
	arma_extra_debug_sigprint();
...
template<typename eT> arma_inline SpSubview_MapMat_val<eT>::SpSubview_MapMat_val(SpSubview<eT>& in_v_parent, MapMat<eT>& in_m_parent, const uword in_row, const uword in_col)
...
template<typename eT> arma_inline
void SpSubview_MapMat_val<eT>::update_n_nonzeros() {
	arma_extra_debug_sigprint();
	...
	if(v_parent.m.n_nonzero > old_n_nonzero) { access::rw(v_parent.n_nonzero)++; } else if(v_parent.m.n_nonzero < old_n_nonzero) { access::rw(v_parent.n_nonzero)--; }
}

template<typename eT> inline SpSubview_MapMat_val<eT>::operator eT() const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
typename get_pod_type<eT>::result SpSubview_MapMat_val<eT>::real() const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
typename get_pod_type<eT>::result SpSubview_MapMat_val<eT>::imag() const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
SpSubview_MapMat_val<eT>& SpSubview_MapMat_val<eT>::operator=(const SpSubview_MapMat_val<eT>& x) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
SpSubview_MapMat_val<eT>& SpSubview_MapMat_val<eT>::operator=(const eT in_val) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
SpSubview_MapMat_val<eT>& SpSubview_MapMat_val<eT>::operator++() {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
eT SpSubview_MapMat_val<eT>::operator++(int) {
	arma_extra_debug_sigprint();
