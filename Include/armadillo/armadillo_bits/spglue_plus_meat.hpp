...
inline
void spglue_plus::apply(SpMat<typename T1::elem_type>& out, const SpGlue<T1, T2, spglue_plus>& X) {
	...
	if(is_alias == false) { spglue_plus::apply_noalias(out, pa, pb); } else {
		SpMat<eT> tmp;
...
inline
void spglue_plus::apply_noalias(SpMat<eT>& out, const SpProxy<T1>& pa, const SpProxy<T2>& pb) {
	arma_extra_debug_sigprint();
...
inline
void spglue_plus::apply_noalias(SpMat<eT>& out, const SpMat<eT>& A, const SpMat<eT>& B) {
	arma_extra_debug_sigprint();
...
void spglue_plus_mixed::sparse_plus_sparse(SpMat<typename promote_type<typename T1::elem_type, typename T2::elem_type>::result>& out, const T1& X, const T2& Y) {
	arma_extra_debug_sigprint();
...
template<typename T1, typename T2> inline
void spglue_plus_mixed::dense_plus_sparse(Mat<typename promote_type<typename T1::elem_type, typename T2::elem_type>::result>& out, const T1& X, const T2& Y) {
	arma_extra_debug_sigprint();
