...
void op_vectorise_col::apply(Mat<typename T1::elem_type>& out, const Op<T1, op_vectorise_col>& in) {
	arma_extra_debug_sigprint();

	typedef typename T1::elem_type eT;

	if(is_same_type<T1, subview<eT>>::yes) { op_vectorise_col::apply_subview(out, reinterpret_cast<const subview<eT>&>(in.m)); } else {
...
void op_vectorise_col::apply_subview(Mat<eT>& out, const subview<eT>& sv) {
	arma_extra_debug_sigprint();
...
template<typename T1> inline
void op_vectorise_col::apply_proxy(Mat<typename T1::elem_type>& out, const Proxy<T1>& P) {
	arma_extra_debug_sigprint();
...
template<typename T1> inline
void op_vectorise_row::apply(Mat<typename T1::elem_type>& out, const Op<T1, op_vectorise_row>& in) {
	arma_extra_debug_sigprint();
...
template<typename T1> inline
void op_vectorise_row::apply_proxy(Mat<typename T1::elem_type>& out, const Proxy<T1>& P) {
	arma_extra_debug_sigprint();
...
template<typename T1> inline
void op_vectorise_all::apply(Mat<typename T1::elem_type>& out, const Op<T1, op_vectorise_all>& in) {
	arma_extra_debug_sigprint();
	...
	if(dim == 0) { op_vectorise_col::apply_proxy(out, P); } else { op_vectorise_row::apply_proxy(out, P); }
}

//

template<typename T1> inline
void op_vectorise_cube_col::apply(Mat<typename T1::elem_type>& out, const BaseCube<typename T1::elem_type, T1>& in) {
	arma_extra_debug_sigprint();

	typedef typename T1::elem_type eT;

	if(is_same_type<T1, subview_cube<eT>>::yes) { op_vectorise_cube_col::apply_subview(out, reinterpret_cast<const subview_cube<eT>&>(in.get_ref())); } else {
...
template<typename eT> inline
void op_vectorise_cube_col::apply_subview(Mat<eT>& out, const subview_cube<eT>& sv) {
	arma_extra_debug_sigprint();
...
template<typename T1> inline
void op_vectorise_cube_col::apply_proxy(Mat<typename T1::elem_type>& out, const ProxyCube<T1>& P) {
	arma_extra_debug_sigprint();
