...
void op_mean::apply(Mat<typename T1::elem_type>& out, const Op<T1, op_mean>& in) {
	...
	if(P.is_alias(out) == false) { op_mean::apply_noalias(out, P, dim); } else {
		Mat<eT> tmp;
...
template<typename T1> inline
void op_mean::apply_noalias(Mat<typename T1::elem_type>& out, const Proxy<T1>& P, const uword dim) {
	arma_extra_debug_sigprint();

	if(is_Mat<typename Proxy<T1>::stored_type>::value) { op_mean::apply_noalias_unwrap(out, P, dim); } else { op_mean::apply_noalias_proxy(out, P, dim); }
}

template<typename T1> inline
void op_mean::apply_noalias_unwrap(Mat<typename T1::elem_type>& out, const Proxy<T1>& P, const uword dim) {
	arma_extra_debug_sigprint();
	...
	if(dim == 0) {
		...
		for(uword row = 0; row < X_n_rows; ++row) { if(arma_isfinite(out_mem[row]) == false) { out_mem[row] = op_mean::direct_mean_robust(X, row); } }
	}
}

template<typename T1> arma_hot
inline
void op_mean::apply_noalias_proxy(Mat<typename T1::elem_type>& out, const Proxy<T1>& P, const uword dim) {
	arma_extra_debug_sigprint();
...
template<typename T1> inline
void op_mean::apply(Cube<typename T1::elem_type>& out, const OpCube<T1, op_mean>& in) {
	arma_extra_debug_sigprint();
...
template<typename T1> inline
void op_mean::apply_noalias(Cube<typename T1::elem_type>& out, const ProxyCube<T1>& P, const uword dim) {
	arma_extra_debug_sigprint();

	if(is_Cube<typename ProxyCube<T1>::stored_type>::value) { op_mean::apply_noalias_unwrap(out, P, dim); } else { op_mean::apply_noalias_proxy(out, P, dim); }
}

template<typename T1> inline
void op_mean::apply_noalias_unwrap(Cube<typename T1::elem_type>& out, const ProxyCube<T1>& P, const uword dim) {
	arma_extra_debug_sigprint();
...
template<typename T1> arma_hot
inline
void op_mean::apply_noalias_proxy(Cube<typename T1::elem_type>& out, const ProxyCube<T1>& P, const uword dim) {
	arma_extra_debug_sigprint();
...
eT op_mean::direct_mean(const eT* const X, const uword n_elem) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
eT op_mean::direct_mean_robust(const eT* const X, const uword n_elem) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
eT op_mean::direct_mean(const Mat<eT>& X, const uword row) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
eT op_mean::direct_mean_robust(const Mat<eT>& X, const uword row) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
eT op_mean::mean_all(const subview<eT>& X) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
eT op_mean::mean_all_robust(const subview<eT>& X) {
	arma_extra_debug_sigprint();
	...
	if(X_n_rows == 1) {
		...
		for(uword col = start_col; col < end_col_p1; ++col) for(uword row = start_row; row < end_row_p1; ++row, ++i) { r_mean = r_mean + (A.at(row, col) - r_mean) / T(i + 1); }
	}
...
template<typename eT> inline
eT op_mean::mean_all(const diagview<eT>& X) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
eT op_mean::mean_all_robust(const diagview<eT>& X) {
	arma_extra_debug_sigprint();
...
template<typename T1> inline
typename T1::elem_type op_mean::mean_all(const Op<T1, op_vectorise_col>& X) {
	arma_extra_debug_sigprint();
...
template<typename T1> inline
typename T1::elem_type op_mean::mean_all(const Base<typename T1::elem_type, T1>& X) {
	arma_extra_debug_sigprint();
...
eT op_mean::robust_mean(const eT A, const eT B) { return A + (B - A) / eT(2); }

template<typename T> arma_inline
std::complex<T> op_mean::robust_mean(const std::complex<T>& A, const std::complex<T>& B) { return A + (B - A) / T(2); }
