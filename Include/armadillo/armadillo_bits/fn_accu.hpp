...
inline
typename T1::elem_type accu_proxy_linear(const Proxy<T1>& P) {
...
template<typename T1> arma_hot
inline
typename T1::elem_type accu_proxy_at_mp(const Proxy<T1>& P) {
	arma_extra_debug_sigprint();
...
typename T1::elem_type accu_proxy_at(const Proxy<T1>& P) {
	...
	if(arma_config::openmp && Proxy<T1>::use_mp && mp_gate<eT>::eval(P.get_n_elem())) { return accu_proxy_at_mp(P); }
...
arma_hot
inline
typename enable_if2<is_arma_type<T1>::value, typename T1::elem_type>::result accu(const T1& X) {
	arma_extra_debug_sigprint();
...
inline
typename T1::elem_type accu(const eGlue<T1, T2, eglue_schur>& expr) {
	arma_extra_debug_sigprint();
...
template<typename T1> arma_warn_unused
inline
uword accu(const mtOp<uword, T1, op_rel_noteq>& X) {
	arma_extra_debug_sigprint();
	...
	if(Proxy<T1>::use_at == false) {
		...
		if(P_n_rows == 1) { for(uword col = 0; col < P_n_cols; ++col) { n_nonzero += (P.at(0, col) != val) ? uword(1) : uword(0); } } else { for(uword col = 0; col < P_n_cols; ++col) for(uword row = 0; row < P_n_rows; ++row) { n_nonzero += (P.at(row, col) != val) ? uword(1) : uword(0); } }
	}
...
template<typename T1> arma_warn_unused
inline
uword accu(const mtOp<uword, T1, op_rel_eq>& X) {
	arma_extra_debug_sigprint();
...
arma_hot
inline
eT accu(const subview_col<eT>& X) {
	arma_extra_debug_sigprint();
...
template<typename T1> arma_hot
inline
typename T1::elem_type accu_cube_proxy_linear(const ProxyCube<T1>& P) {
	arma_extra_debug_sigprint();
...
template<typename T1> arma_hot
inline
typename T1::elem_type accu_cube_proxy_at_mp(const ProxyCube<T1>& P) {
	arma_extra_debug_sigprint();
...
template<typename T1> arma_hot
inline
typename T1::elem_type accu_cube_proxy_at(const ProxyCube<T1>& P) {
	arma_extra_debug_sigprint();

	typedef typename T1::elem_type eT;

	if(arma_config::openmp && ProxyCube<T1>::use_mp && mp_gate<eT>::eval(P.get_n_elem())) { return accu_cube_proxy_at_mp(P); }
...
arma_hot
inline
typename T1::elem_type accu(const BaseCube<typename T1::elem_type, T1>& X) {
	arma_extra_debug_sigprint();
...
template<typename T1, typename T2> arma_warn_unused
inline
typename T1::elem_type accu(const eGlueCube<T1, T2, eglue_schur>& expr) {
	arma_extra_debug_sigprint();
...
inline
const typename arma_scalar_only<T>::result& accu(const T& x) { return x; }

//! accumulate values in a sparse object
template<typename T1> arma_warn_unused
inline
typename T1::elem_type accu(const SpBase<typename T1::elem_type, T1>& expr) {
	arma_extra_debug_sigprint();
...
template<typename T1, typename T2> arma_warn_unused
inline
typename T1::elem_type accu(const SpGlue<T1, T2, spglue_plus>& expr) {
	arma_extra_debug_sigprint();
...
template<typename T1, typename T2> arma_warn_unused
inline
typename T1::elem_type accu(const SpGlue<T1, T2, spglue_minus>& expr) {
	arma_extra_debug_sigprint();
...
template<typename T1, typename T2> arma_warn_unused
inline
typename T1::elem_type accu(const SpGlue<T1, T2, spglue_schur>& expr) {
	arma_extra_debug_sigprint();
