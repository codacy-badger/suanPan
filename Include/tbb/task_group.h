...
namespace tbb {
	...
	template<typename F> class task_handle : internal::no_assign {
		...
		void mark_scheduled() {
			...
			if(my_state & scheduled) internal::throw_exception(internal::eid_invalid_multiple_scheduling);
			my_state |= scheduled;
		}
	...
	namespace internal {
		...
		template<typename F> class task_handle_task : public task {
		...
		class task_group_base : internal::no_copy {
			...
			public:
				ref_count_guard(task& t)
					: my_task(t) { my_task.increment_ref_count(); }
		...
		protected:
			...
			template<typename F> task_group_status internal_run_and_wait(F& f) {
				__TBB_TRY {
				...
				} __TBB_CATCH( ... ) { my_context.register_pending_exception(); }
				return wait();
			}

			template<typename Task, typename F> void internal_run(__TBB_FORWARDING_REF(F) f) { owner().spawn(*new(owner().allocate_additional_child_of(*my_root)) Task(internal::forward<F>(f))); }
		...
		public:
            ...
            bool stack_unwinding_in_progress = std::uncaught_exception();
					...
					if(!is_canceling()) cancel();
					__TBB_TRY { my_root->wait_for_all(); } __TBB_CATCH (...) {
						task::destroy(*my_root);
						__TBB_RETHROW();
					}
					task::destroy(*my_root);
					if(!stack_unwinding_in_progress) internal::throw_exception(internal::eid_missing_wait);
				} else { task::destroy(*my_root); }
			}

			template<typename F> void run(task_handle<F>& h) { internal_run<internal::task_handle_task<F>>(h); }
	...
	public:
		task_group()
			: task_group_base(task_group_context::concurrent_wait) {}
...
#if __TBB_CPP11_RVALUE_REF_PRESENT
		template<typename F> void run(F&& f) { internal_run<internal::function_task<typename internal::strip<F>::type>>(std::forward<F>(f)); }
#else
    template<typename F>
    void run(const F& f) {
		...
		template<typename F> task_group_status run_and_wait(const F& f) { return internal_run_and_wait<const F>(f); }

		// TODO: add task_handle rvalues support
		template<typename F> task_group_status run_and_wait(task_handle<F>& h) {
			h.mark_scheduled();
	...
	inline
	bool is_current_task_group_canceling() { return task::self().is_cancelled(); }

#if __TBB_CPP11_RVALUE_REF_PRESENT
	template<class F> task_handle<typename internal::strip<F>::type> make_task(F&& f) { return task_handle<typename internal::strip<F>::type>(std::forward<F>(f)); }
#else
template<class F>
