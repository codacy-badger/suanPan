...
namespace tbb {
	...
	namespace interface6 {
		...
		template<typename T, typename Allocator, ets_key_usage_type ETS_key_type> class enumerable_thread_specific;
		...
		namespace internal {
			...
			template<ets_key_usage_type ETS_key_type> class ets_base : tbb::internal::no_copy {
			protected:
        ...
        public:
				...
				struct array {
					...
					size_t start(size_t h) const { return h >> (8 * sizeof(size_t) - lg_size); }
				};
        ...
        protected:
			...
			template<ets_key_usage_type ETS_key_type> ets_base<ETS_key_type>::~ets_base() {
				__TBB_ASSERT(!my_root, NULL);
			}

			template<ets_key_usage_type ETS_key_type> void ets_base<ETS_key_type>::table_clear() {
			...
			template<ets_key_usage_type ETS_key_type> void* ets_base<ETS_key_type>::table_lookup(bool& exists) {
			...
			template<> class ets_base<ets_key_per_instance> : protected ets_base<ets_no_key> {
			...
			template<typename Container, typename Value> class enumerable_thread_specific_iterator
...
#endif /* defined(_WIN64) && defined(_MSC_VER) */
			{
				...
				template<typename C, typename T> friend enumerable_thread_specific_iterator<C, T> operator+(ptrdiff_t offset, const enumerable_thread_specific_iterator<C, T>& v);

				template<typename C, typename T, typename U> friend bool operator==(const enumerable_thread_specific_iterator<C, T>& i,
				                                                                    const enumerable_thread_specific_iterator<C, U>& j);

				template<typename C, typename T, typename U> friend bool operator<(const enumerable_thread_specific_iterator<C, T>& i,
				                                                                   const enumerable_thread_specific_iterator<C, U>& j);

				template<typename C, typename T, typename U> friend ptrdiff_t operator-(const enumerable_thread_specific_iterator<C, T>& i,
				                                                                        const enumerable_thread_specific_iterator<C, U>& j);

				template<typename C, typename U> friend class enumerable_thread_specific_iterator;
			...
			public:
				...
				template<typename U> enumerable_thread_specific_iterator(const enumerable_thread_specific_iterator<Container, U>& other)
					:
					my_container(other.my_container)
					, my_index(other.my_index)
					, my_value(const_cast<Value *>(other.my_value)) {}

				enumerable_thread_specific_iterator operator+(ptrdiff_t offset) const { return enumerable_thread_specific_iterator(*my_container, my_index + offset); }
				...
				Value& operator*() const {
					...
					if(!value) { value = my_value = (*my_container)[my_index].value(); }
					__TBB_ASSERT( value==(*my_container)[my_index].value(), "corrupt cache" );
					return *value;
				}

				Value& operator[](ptrdiff_t k) const { return (*my_container)[my_index + k].value; }
			...
			template<typename Container, typename T> enumerable_thread_specific_iterator<Container, T> operator+(ptrdiff_t offset, const enumerable_thread_specific_iterator<Container, T>& v) { return enumerable_thread_specific_iterator<Container, T>(v.my_container, v.my_index + offset); }

			template<typename Container, typename T, typename U> bool operator==(const enumerable_thread_specific_iterator<Container, T>& i,
			                                                                     const enumerable_thread_specific_iterator<Container, U>& j) { return i.my_index == j.my_index && i.my_container == j.my_container; }

			template<typename Container, typename T, typename U> bool operator!=(const enumerable_thread_specific_iterator<Container, T>& i,
			                                                                     const enumerable_thread_specific_iterator<Container, U>& j) { return !(i == j); }

			template<typename Container, typename T, typename U> bool operator<(const enumerable_thread_specific_iterator<Container, T>& i,
			                                                                    const enumerable_thread_specific_iterator<Container, U>& j) { return i.my_index < j.my_index; }

			template<typename Container, typename T, typename U> bool operator>(const enumerable_thread_specific_iterator<Container, T>& i,
			                                                                    const enumerable_thread_specific_iterator<Container, U>& j) { return j < i; }
			...
			template<typename Container, typename T, typename U> ptrdiff_t operator-(const enumerable_thread_specific_iterator<Container, T>& i,
			                                                                         const enumerable_thread_specific_iterator<Container, U>& j) { return i.my_index - j.my_index; }

			template<typename SegmentedContainer, typename Value> class segmented_iterator
#if defined(_WIN64) && defined(_MSC_VER)
				: public std::iterator<std::input_iterator_tag, Value>
#endif
			{
				template<typename C, typename T, typename U> friend bool operator==(const segmented_iterator<C, T>& i, const segmented_iterator<C, U>& j);

				template<typename C, typename T, typename U> friend bool operator!=(const segmented_iterator<C, T>& i, const segmented_iterator<C, U>& j);

				template<typename C, typename U> friend class segmented_iterator;
			...
			public:
				...
				// Copy Constructor
				template<typename U> segmented_iterator(const segmented_iterator<SegmentedContainer, U>& other)
					:
				...
				// assignment
				template<typename U> segmented_iterator& operator=(const segmented_iterator<SegmentedContainer, U>& other) {
					if(this != &other) {
						my_segcont = other.my_segcont;
						outer_iter = other.outer_iter;
				...
				bool operator!=(const outer_iterator& other_outer) const { return !operator==(other_outer); }
			...
			private:
				...
				void advance_me() {
					...
					++inner_iter;
					while(inner_iter == outer_iter->end() && ++outer_iter != my_segcont->end()) { inner_iter = outer_iter->begin(); }
				}
			}; // segmented_iterator

			template<typename SegmentedContainer, typename T, typename U> bool operator==(const segmented_iterator<SegmentedContainer, T>& i,
			...
			// !=
			template<typename SegmentedContainer, typename T, typename U> bool operator!=(const segmented_iterator<SegmentedContainer, T>& i,
			                                                                              const segmented_iterator<SegmentedContainer, U>& j) { return !(i == j); }

			template<typename T> struct construct_by_default : tbb::internal::no_assign {
				void construct(void* where) { new(where) T(); } // C++ note: the () in T() ensure zero initialization.
				construct_by_default(int) {}
			};

			template<typename T> struct construct_by_exemplar : tbb::internal::no_assign {
				const T exemplar;

				void construct(void* where) { new(where) T(exemplar); }

				construct_by_exemplar(const T& t)
					: exemplar(t) {}
#if __TBB_ETS_USE_CPP11
				construct_by_exemplar(T&& t)
					: exemplar(std::move(t)) {}
#endif
			};

			template<typename T, typename Finit> struct construct_by_finit : tbb::internal::no_assign {
				Finit f;

				void construct(void* where) { new(where) T(f()); }

				construct_by_finit(const Finit& f_)
					: f(f_) {}
#if __TBB_ETS_USE_CPP11
				construct_by_finit(Finit&& f_)
					: f(std::move(f_)) {}
#endif
			};

#if __TBB_ETS_USE_CPP11
			template<typename T, typename... P> struct construct_by_args : tbb::internal::no_assign {
				internal::stored_pack<P...> pack;

				void construct(void* where) { internal::call([where](const typename strip<P>::type&... args) { new(where) T(args...); }, pack); }

				construct_by_args(P&& ... args)
					: pack(std::forward<P>(args)...) {}
			};
#endif

			// storage for initialization function pointer
			// TODO: consider removing the template parameter T here and in callback_leaf
			template<typename T> class callback_base {
			public:
				// Clone *this
			...
			template<typename T, typename Constructor> class callback_leaf : public callback_base<T>, Constructor {
#if __TBB_ETS_USE_CPP11
				template<typename... P> callback_leaf(P&& ... params)
            ...
            template<typename X> callback_leaf( const X& x ) : Constructor(x) {}
				...
				void construct(void* where) __TBB_override { Constructor::construct(where); }

			public:
#if __TBB_ETS_USE_CPP11
				template<typename... P> static callback_base<T>* make(P&& ... params) {
			...
			*/
			template<typename U> struct ets_element {
			...
			template<typename T, typename ETS> struct is_compatible_ets {
				static const bool value = false;
			};

			template<typename T, typename U, typename A, ets_key_usage_type C> struct is_compatible_ets<T, enumerable_thread_specific<U, A, C>> {
			...
			template<typename T> class is_callable_no_args {
			private:
				typedef char yes[1];
				typedef char no [2];
		...
		template<typename T,
		         ...
		         ets_key_usage_type ETS_key_type = ets_no_key> class enumerable_thread_specific : internal::ets_base<ETS_key_type> {
		...
		public:
			...
			template<typename P1, typename... P,
			         ...
			         >::type> enumerable_thread_specific(P1&& arg1, P&& ... args)
				: my_construct_callback(
		...
		private:
			...
			template<typename A2, ets_key_usage_type C2> void internal_copy(const enumerable_thread_specific<T, A2, C2>& other) {
...
#if __TBB_ETS_USE_CPP11
			template<typename A2, ets_key_usage_type C2> void internal_move(enumerable_thread_specific<T, A2, C2>&& other) {
#if TBB_USE_ASSERT
				// this tests is_compatible_ets
				__TBB_STATIC_ASSERT( (internal::is_compatible_ets<T, typename internal::strip<decltype(other)>::type>::value), "is_compatible_ets fails" );
#endif
		...
		public:
			...
			enumerable_thread_specific(const enumerable_thread_specific& other)
				: internal::ets_base<ETS_key_type>() /* prevents GCC warnings with -Wextra */ { internal_copy(other); }

			template<typename Alloc, ets_key_usage_type Cachetype> enumerable_thread_specific(const enumerable_thread_specific<T, Alloc, Cachetype>& other) { internal_copy(other); }

#if __TBB_ETS_USE_CPP11
			enumerable_thread_specific(enumerable_thread_specific&& other)
				: my_construct_callback() { internal_swap(other); }
			...
			template<typename Alloc, ets_key_usage_type Cachetype> enumerable_thread_specific(enumerable_thread_specific<T, Alloc, Cachetype>&& other)
				: my_construct_callback() { internal_move(std::move(other)); }
#endif
			...
			enumerable_thread_specific& operator=(enumerable_thread_specific&& other) {
				if(this != &other) internal_swap(other);
				return *this;
			}
			...
			template<typename combine_func_t> T combine(combine_func_t f_combine) {
				if(begin() == end()) {
				...
				T my_result = *ci;
				while(++ci != end()) my_result = f_combine(my_result, *ci);
				return my_result;
			}

			// combine_func_t takes T by value or by [const] reference, and returns nothing
			template<typename combine_func_t> void combine_each(combine_func_t f_combine) { for(iterator ci = begin(); ci != end(); ++ci) { f_combine(*ci); } }
		...
		public:
			...
			size_type size() const {
				...
				for(typename Container::const_iterator i = my_begin; i != my_end; ++i) { tot_size += i->size(); }
				return tot_size;
			}
		...
		template<typename Container> flattened2d<Container> flatten2d(const Container& c, const typename Container::const_iterator b, const typename Container::const_iterator e) { return flattened2d<Container>(c, b, e); }

		template<typename Container> flattened2d<Container> flatten2d(const Container& c) { return flattened2d<Container>(c); }
