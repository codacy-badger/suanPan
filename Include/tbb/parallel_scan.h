...
namespace tbb {
	...
	namespace internal {
		...
		/** @ingroup algorithms */
		template<typename Range, typename Body> class final_sum : public task {
		public:
			Body my_body;
		private:
		...
		public:
			final_sum(Body& body_)
				:
				my_body(body_, split()) { poison_pointer(my_stuff_last); }
		...
		private:
			task* execute() __TBB_override {
				...
				if(my_stuff_last) my_stuff_last->assign(my_body);
				return NULL;
			}
		};
		...
		/** @ingroup algorithms */
		template<typename Range, typename Body> class sum_node : public task {
		...
		private:
			...
			task* execute() __TBB_override {
				if(my_body) {
					if(my_incoming) my_left_sum->my_body.reverse_join(my_incoming->my_body);
			...
			template<typename Range_, typename Body_, typename Partitioner_> friend class start_scan;

			template<typename Range_, typename Body_> friend class finish_scan;
		};

		//! Combine partial results
		/** @ingroup algorithms */
		template<typename Range, typename Body> class finish_scan : public task {
		...
		public:
			...
			task* execute() __TBB_override {
				...
				if(my_result.my_left) my_result.my_left_is_final = false;
				if(my_right_zombie && my_sum) ((*my_sum)->my_body).reverse_join(my_result.my_left_sum->my_body);
				__TBB_ASSERT( !my_return_slot, NULL );
				if(my_right_zombie || my_result.my_right) { my_return_slot = &my_result; } else { destroy(my_result); }
		...
		/** @ingroup algorithms */
		template<typename Range, typename Body, typename Partitioner=simple_partitioner> class start_scan : public task {
			typedef sum_node<Range, Body> sum_node_type;
			typedef final_sum<Range, Body> final_sum_type;
			final_sum_type* my_body;
		...
		template<typename Range, typename Body, typename Partitioner> task* start_scan<Range, Body, Partitioner>::execute() {
			...
			if((my_is_right_child && !treat_as_stolen) || !my_range.is_divisible() || my_partition.should_execute_range(*this)) {
				if(my_is_final) (my_body->my_body)(my_range, final_scan_tag());
				else if(my_sum) (my_body->my_body)(my_range, pre_scan_tag());
				if(my_sum) *my_sum = my_body;
				__TBB_ASSERT( !*my_return_slot, NULL );
			} else {
				sum_node_type* result;
				if(my_parent_sum) result = new(allocate_additional_child_of(*my_parent_sum)) sum_node_type(my_range,/*my_left_is_final=*/my_is_final);
				else result = new(task::allocate_root()) sum_node_type(my_range,/*my_left_is_final=*/my_is_final);
		...
		template<typename Range, typename Value, typename Scan, typename ReverseJoin> class lambda_scan_body : no_assign {
			Value my_sum;
		...
		public:
			...
			template<typename Tag> void operator()(const Range& r, Tag tag) { my_sum = my_scan(r, my_sum, tag); }

			void reverse_join(lambda_scan_body& a) { my_sum = my_reverse_join(a.my_sum, my_sum); }
	...
	**/
	...
	/** @ingroup algorithms **/
	template<typename Range, typename Body> void parallel_scan(const Range& range, Body& body) { internal::start_scan<Range, Body,__TBB_DEFAULT_PARTITIONER>::run(range, body,__TBB_DEFAULT_PARTITIONER()); }

	//! Parallel prefix with simple_partitioner
	/** @ingroup algorithms **/
	template<typename Range, typename Body> void parallel_scan(const Range& range, Body& body, const simple_partitioner& partitioner) { internal::start_scan<Range, Body, simple_partitioner>::run(range, body, partitioner); }

	//! Parallel prefix with auto_partitioner
	/** @ingroup algorithms **/
	template<typename Range, typename Body> void parallel_scan(const Range& range, Body& body, const auto_partitioner& partitioner) { internal::start_scan<Range, Body, auto_partitioner>::run(range, body, partitioner); }

	//! Parallel prefix with default partitioner
	/** @ingroup algorithms **/
	template<typename Range, typename Value, typename Scan, typename ReverseJoin> Value parallel_scan(const Range& range, const Value& identity, const Scan& scan, const ReverseJoin& reverse_join) {
	...
	//! Parallel prefix with simple_partitioner
	/** @ingroup algorithms **/
	template<typename Range, typename Value, typename Scan, typename ReverseJoin> Value parallel_scan(const Range& range, const Value& identity, const Scan& scan, const ReverseJoin& reverse_join, const simple_partitioner& partitioner) {
		internal::lambda_scan_body<Range, Value, Scan, ReverseJoin> body(identity, scan, reverse_join);
	...
	//! Parallel prefix with auto_partitioner
	/** @ingroup algorithms **/
	template<typename Range, typename Value, typename Scan, typename ReverseJoin> Value parallel_scan(const Range& range, const Value& identity, const Scan& scan, const ReverseJoin& reverse_join, const auto_partitioner& partitioner) {
		internal::lambda_scan_body<Range, Value, Scan, ReverseJoin> body(identity, scan, reverse_join);
		tbb::parallel_scan(range, body, partitioner);
		return body.result();
	}
