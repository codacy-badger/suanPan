	...
	namespace interface6 {
		...
		namespace internal {
			...
			public:
				...
				void* realloc(void* ptr, size_t size) { return rml::pool_realloc(my_pool, ptr, size); }
...
#pragma warning (disable: 4100)
		...
		template<typename T, typename P = internal::pool_base> class memory_pool_allocator {
		protected:
			typedef P pool_type;
			pool_type* my_pool;
			template<typename U, typename R> friend class memory_pool_allocator;
			template<typename V, typename U, typename R> friend bool operator==(const memory_pool_allocator<V, R>& a, const memory_pool_allocator<U, R>& b);
			template<typename V, typename U, typename R> friend bool operator!=(const memory_pool_allocator<V, R>& a, const memory_pool_allocator<U, R>& b);
		public:
			...
			pointer allocate(size_type n, const void* /*hint*/ = 0) {
				...
				if(!p) tbb::internal::throw_exception(std::bad_alloc());
				return p;
			}
...
#pragma warning (pop)
		...
		/** @ingroup memory_allocation */
		template<typename P> class memory_pool_allocator<void, P> {
		public:
			typedef P pool_type;
		...
		template<typename T, typename U, typename P> inline bool operator==(const memory_pool_allocator<T, P>& a, const memory_pool_allocator<U, P>& b) { return a.my_pool == b.my_pool; }

		template<typename T, typename U, typename P> inline bool operator!=(const memory_pool_allocator<T, P>& a, const memory_pool_allocator<U, P>& b) { return a.my_pool != b.my_pool; }

		//! Thread-safe growable pool allocator for variable-size requests
		template<typename Alloc> class memory_pool : public internal::pool_base {
		...
		template<typename Alloc> memory_pool<Alloc>::memory_pool(const Alloc& src)
			: my_alloc(src) {
			...
			if(res != rml::POOL_OK) tbb::internal::throw_exception(std::runtime_error("Can't create pool"));
		}

		template<typename Alloc> void* memory_pool<Alloc>::allocate_request(intptr_t pool_id, size_t& bytes) {
...
#endif
		template<typename Alloc> int memory_pool<Alloc>::deallocate_request(intptr_t pool_id, void* raw_ptr, size_t raw_bytes) {
			memory_pool<Alloc>& self = *reinterpret_cast<memory_pool<Alloc>*>(pool_id);
			const size_t unit_size = sizeof(typename Alloc::value_type);
