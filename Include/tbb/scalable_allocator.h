...
typedef enum {
	...
	/* value turns using huge pages on and off */
...
#pragma warning (disable: 4100)
	...
	namespace internal {
		...
		template<typename E> void throw_exception(const E& e) {
			__TBB_THROW(e);
		}
	...
	public:
		...
		pointer allocate(size_type n, const void* /*hint*/ = 0) {
			...
			if(!p) internal::throw_exception(std::bad_alloc());
			return p;
		}
...
#pragma warning (pop)
	...
	/** @ingroup memory_allocation */
	template<> class scalable_allocator<void> {
	public:
	...
	template<typename T, typename U> inline bool operator==(const scalable_allocator<T>&, const scalable_allocator<U>&) { return true; }

	template<typename T, typename U> inline bool operator!=(const scalable_allocator<T>&, const scalable_allocator<U>&) { return false; }
