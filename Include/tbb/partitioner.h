...
namespace tbb {
	...
	namespace interface9 {
        ...
        flag = true;
				...
				static bool is_peer_stolen(task& t) { return static_cast<flag_task*>(t.parent())->my_child_stolen; }
			};
			...
			template<typename T, depth_t MaxCapacity> class range_vector {
				depth_t my_head;
				depth_t my_tail;
				depth_t my_size;
			...
			public:
				...
				~range_vector() { while(!empty()) pop_back(); }
				...
				bool is_divisible(depth_t max_depth) { return back_depth() < max_depth && back().is_divisible(); }
			};

			//! Provides default methods for partition objects and common algorithm blocks.
			template<typename Partition> struct partition_type_base {
				...
				template<typename StartType, typename Range> void work_balance(StartType& start, Range& range) {
					start.run_body(range); // simple partitioner goes always here
				}

				template<typename StartType, typename Range> void execute(StartType& start, Range& range) {
			...
			*/
			template<typename Range> class is_splittable_in_proportion {
			private:
				typedef char yes[1];
				typedef char no[2];
			...
			template<typename Partition> struct adaptive_mode : partition_type_base<Partition> {
				...
				adaptive_mode()
					: my_divisor(tbb::internal::get_initial_auto_partitioner_divisor() / 4 * my_partition::factor) {}
				...
				size_t do_split(adaptive_mode& src, split) { return src.my_divisor /= 2u; }
			};

			//! Provides proportional splitting strategy for partition objects
			template<typename Partition> struct proportional_mode : adaptive_mode<Partition> {
				typedef Partition my_partition;
				using partition_type_base<Partition>::self; // CRTP helper to get access to derived classes

				proportional_mode()
					: adaptive_mode<Partition>() {}
...
#pragma warning( disable: 4127 )
#endif
				template<typename Range> proportional_split get_split() {
            ...
            size_t right = (size + 2) / 3;
					...
					} else { return proportional_split(1, 1); }
				}
#if _MSC_VER && !defined(__INTEL_COMPILER)
#pragma warning( pop )
			...
			static size_t get_initial_partition_head() {
				...
				if(current_index == tbb::task_arena::not_initialized) current_index = 0;
				return size_t(current_index);
			}

			//! Provides default linear indexing of partitioner's sequence
			template<typename Partition> struct linear_affinity_mode : proportional_mode<Partition> {
				size_t my_head;
				...
				void set_affinity(task& t) { if(self().my_divisor) t.set_affinity(affinity_id(my_head) + 1); }
			};

			/*! Determine work-balance phase implementing splitting & stealing actions */
			template<class Mode> struct dynamic_grainsize_mode : Mode {
				using Mode::self;
        ...
        , my_dst_tsc(0)
			...
			public:
				auto_partition_type(const auto_partitioner&)
					: dynamic_grainsize_mode<adaptive_mode<auto_partition_type>>() { my_divisor *= __TBB_INITIAL_CHUNKS; }
				...
				bool is_divisible() {
					...
					if(my_divisor && my_max_depth) {
						// can split the task. TODO: on-stack flag instead
			...
			public:
				...
				template<typename StartType, typename Range> void execute(StartType& start, Range& range) {
					...
					while(range.is_divisible()) start.offer_work(split_obj);
			...
			public:
				...
				void set_affinity(task& t) {
					if(my_divisor) {
						...
						else t.set_affinity(my_array[my_head]);
					}
				}

				void note_affinity(task::affinity_id id) { if(my_divisor) my_array[my_head] = id; }
			};
			...
			public:
				bool should_execute_range(const task& t) {
					if(num_chunks < VICTIM_CHUNKS && t.is_stolen_task()) num_chunks = VICTIM_CHUNKS;
					return num_chunks == 1;
				}
				...
				old_auto_partition_type(old_auto_partition_type& pt, split) { num_chunks = pt.num_chunks = (pt.num_chunks + 1u) / 2u; }
			};
