        ...
        return my_built_predecessors.edge_count();
			...
			protected:
				...
				task* try_put_task(const input_type&) __TBB_override {
					{
						spin_mutex::scoped_lock l(my_mutex);
						if(++my_current_count < my_predecessor_count) return SUCCESSFULLY_ENQUEUED;
						else my_current_count = 0;
					}
					task* res = execute();
...
#include "internal/_flow_graph_async_msg_impl.h"
			...
			template<typename C, typename N> graph_iterator<C, N>::graph_iterator(C* g, bool begin)
				: my_graph(g)
				, current_node(NULL) {
			...
			template<typename C, typename N> typename graph_iterator<C, N>::reference graph_iterator<C, N>::operator*() const {
				__TBB_ASSERT(current_node, "graph_iterator at end");
				return *operator->();
			}

			template<typename C, typename N> typename graph_iterator<C, N>::pointer graph_iterator<C, N>::operator->() const { return current_node; }

			template<typename C, typename N> void graph_iterator<C, N>::internal_forward() { if(current_node) current_node = current_node->next; }
			...
			inline void graph::reset(reset_flags f) {
				...
				for(task_list_type::iterator rti = my_reset_task_list.begin(); rti != my_reset_task_list.end(); ++rti) { internal::spawn_in_graph_arena(*this, *(*rti)); }
...
inline void graph::set_name(const char *name) {
    tbb::internal::fgt_graph_desc(this, name);
			...
			inline graph_node::graph_node(graph& g)
				: my_graph(g) { my_graph.register_node(this); }

			inline graph_node::~graph_node() { my_graph.remove_node(this); }

#include "internal/_flow_graph_node_impl.h"

			//! An executable node that acts as a source, i.e. it has no predecessors
			template<typename Output> class source_node : public graph_node, public sender<Output> {
			public:
    ...
    void copy_successors(successor_list_type &v) __TBB_override {
        ...
        my_successors.copy_successors(v);
				...
				bool try_release() __TBB_override {
					...
					my_reserved = false;
					if(!my_successors.empty()) spawn_put();
					return true;
				}
        ...
        if(my_has_cached_item) my_has_cached_item = false;
			...
			protected:
				...
				void reset_node(reset_flags f) __TBB_override {
					...
					if(my_active) internal::add_task_to_graph_reset_list(this->my_graph, create_put_task());
				}
			...
			private:
				...
				void spawn_put() { if(internal::is_graph_active(this->my_graph)) { internal::spawn_in_graph_arena(this->my_graph, *create_put_task()); } }
				...
				task* apply_body_bypass() {
					output_type v;
					if(!try_reserve_apply_body(v)) return NULL;

					task* last_task = my_successors.try_put_task(v);
					if(last_task) try_consume();
					else try_release();
					return last_task;
				}
			}; // class source_node
			...
			template<typename Input, typename Output = continue_msg, typename Policy = queueing, typename Allocator=cache_aligned_allocator<Input>> class function_node : public graph_node, public internal::function_input<Input, Output, Policy, Allocator>, public internal::function_output<Output> {
			public:
    ...
    typedef typename fOutput_type::successor_list_type successor_list_type;
				...
				template<typename Body> function_node(
					...
					: graph_node(g)
					, input_impl_type(g, concurrency, __TBB_FLOW_GRAPH_PRIORITY_ARG1(body, priority)) {
    ...
    void extract( ) __TBB_override {
        ...
        successors().built_successors().sender_extract(*this);
			...
			template<typename Input, typename Output, typename Policy = queueing, typename Allocator=cache_aligned_allocator<Input>> class multifunction_node :
				public graph_node,
			...
			private:
				...
				using input_impl_type::my_predecessors;
			public:
				template<typename Body> multifunction_node(
					graph& g, size_t concurrency,
					__TBB_FLOW_GRAPH_PRIORITY_ARG1( Body body, node_priority_t priority = tbb::flow::internal::no_priority )
				)
					: graph_node(g)
					, base_type(g, concurrency, __TBB_FLOW_GRAPH_PRIORITY_ARG1(body, priority)) {
    ...
    void extract( ) __TBB_override {
        ...
        base_type::extract();
			...
			template<typename TupleType, typename Allocator=cache_aligned_allocator<TupleType>> class split_node : public graph_node, public receiver<TupleType> {
    ...
    void set_name( const char *name ) __TBB_override {
        tbb::internal::fgt_multioutput_node_desc( this, name );
			...
			protected:
				...
				void reset_node(reset_flags f) __TBB_override {
					if(f & rf_clear_edges) internal::clear_element<N>::clear_this(my_output_ports);
...
private: //! split_node doesn't use this "predecessors" functionality; so, we have "dummies" here;
    ...
    built_predecessors_type my_predessors;
			...
			template<typename Output, typename Policy = internal::Policy<void>> class continue_node : public graph_node, public internal::continue_input<Output, Policy>,
			...
			public:
				...
				template<typename Body> continue_node(
					graph& g,
					__TBB_FLOW_GRAPH_PRIORITY_ARG1( Body body, node_priority_t priority = tbb::flow::internal::no_priority )
				)
					: graph_node(g)
					, input_impl_type(g, __TBB_FLOW_GRAPH_PRIORITY_ARG1(body, priority)) {
    ...
    void extract() __TBB_override {
        ...
        successors().built_successors().sender_extract(*this);
			...
			template<typename T> class broadcast_node : public graph_node, public receiver<T>, public sender<T> {
			public:
				typedef T input_type;
				typedef T output_type;
    ...
    void extract() __TBB_override {
        ...
        my_successors.built_successors().sender_extract(*this);
           ...
           my_built_predecessors.clear();
			...
			template<typename T, typename A=cache_aligned_allocator<T>> class buffer_node : public graph_node, public internal::reservable_item_buffer<T, A>, public receiver<T>, public sender<T> {
			public:
				typedef T input_type;
				typedef T output_type;
				typedef typename receiver<input_type>::predecessor_type predecessor_type;
    ...
    internal::edge_container<predecessor_type> my_built_predecessors;
        ...
        union {
            ...
            predecessor_list_type *pvec;
				...
				virtual void handle_operations(buffer_operation* op_list) { handle_operations_impl(op_list, this); }

				template<typename derived_type> void handle_operations_impl(buffer_operation* op_list, derived_type* derived) {
            ...
            case blt_pred_cpy: internal_copy_preds(tmp); break;
				...
				inline task* grab_forwarding_task(buffer_operation& op_data) { return op_data.ltask; }
    ...
    virtual void internal_copy_preds(buffer_operation *op) {
        ...
        __TBB_store_with_release(op->status, internal::SUCCEEDED);
			...
			protected:
				...
				virtual void internal_forward_task(buffer_operation* op) { internal_forward_task_impl(op, this); }

				template<typename derived_type> void internal_forward_task_impl(buffer_operation* op, derived_type* derived) {
					__TBB_ASSERT(static_cast<class_type*>(derived) == this, "'this' is not a base class for derived");
					...
					for(; counter > 0 && derived->is_item_valid(); --counter) derived->try_put_and_add_task(last_task);

					op->ltask = last_task; // return task
					if(last_task && !counter) { __TBB_store_with_release(op->status, internal::SUCCEEDED); } else {
						__TBB_store_with_release(op->status, internal::FAILED);
				...
				virtual void internal_pop(buffer_operation* op) { if(this->pop_back(*(op->elem))) { __TBB_store_with_release(op->status, internal::SUCCEEDED); } else { __TBB_store_with_release(op->status, internal::FAILED); } }

				virtual void internal_reserve(buffer_operation* op) { if(this->reserve_front(*(op->elem))) { __TBB_store_with_release(op->status, internal::SUCCEEDED); } else { __TBB_store_with_release(op->status, internal::FAILED); } }
			...
			protected:
				...
				task* try_put_task(const T& t) __TBB_override {
					...
					if(ft && op_data.status == internal::FAILED) {
						...
						internal::spawn_in_graph_arena(graph_reference(), *ft);
						ft = NULL;
					} else if(!ft && op_data.status == internal::SUCCEEDED) { ft = SUCCESSFULLY_ENQUEUED; }
					return ft;
				}
...
public:
    void extract() __TBB_override {
        ...
        my_successors.built_successors().sender_extract(*this);
			...
			template<typename T, typename A=cache_aligned_allocator<T>> class queue_node : public buffer_node<T, A> {
			protected:
			...
			protected:
				void internal_forward_task(queue_operation* op) __TBB_override { this->internal_forward_task_impl(op, this); }

				void internal_pop(queue_operation* op) __TBB_override {
					if(this->my_reserved || !this->my_item_valid(this->my_head)) { __TBB_store_with_release(op->status, internal::FAILED); } else {
    ...
    void set_name( const char *name ) __TBB_override {
        tbb::internal::fgt_node_desc( this, name );
			...
			protected:
				void reset_node(reset_flags f) __TBB_override { base_type::reset_node(f); }
			}; // queue_node

			//! Forwards messages in sequence order
			template<typename T, typename A=cache_aligned_allocator<T>> class sequencer_node : public queue_node<T, A> {
        ...
        tbb::internal::fgt_node_desc( this, name );
			...
			private:
				bool internal_push(sequencer_operation* op) __TBB_override {
					...
					if(this->size(new_tail) > this->capacity()) { this->grow_my_array(this->size(new_tail)); }
			...
			template<typename T, typename Compare = std::less<T>, typename A=cache_aligned_allocator<T>> class priority_queue_node : public buffer_node<T, A> {
			public:
				typedef T input_type;
				typedef T output_type;
				typedef buffer_node<T, A> base_type;
    ...
    void set_name( const char *name ) __TBB_override {
        tbb::internal::fgt_node_desc( this, name );
			...
			protected:
				...
				//! Tries to forward valid items to successors
				void internal_forward_task(prio_operation* op) __TBB_override { this->internal_forward_task_impl(op, this); }

				void handle_operations(prio_operation* op_list) __TBB_override { this->handle_operations_impl(op_list, this); }
			...
			private:
				...
				void prio_push(const T& src) {
					if(this->my_tail >= this->my_array_size) this->grow_my_array(this->my_tail + 1);
				...
				const T& prio() { return this->get_my_item(prio_use_tail() ? this->my_tail - 1 : 0); }
				...
				void heapify() {
					...
					if(!mark) mark = 1;
					for(; mark < this->my_tail; ++mark) {
						// for each unheaped element
						size_type cur_pos = mark;
						input_type to_place;
						this->fetch_item(mark, to_place);
						do {
							// push to_place up the heap
							size_type parent = (cur_pos - 1) >> 1;
							if(!compare(this->get_my_item(parent), to_place)) break;
			...
			/** This node forwards items until its threshold is reached.
			    ...
			    message is dropped. */
			template<typename T> class limiter_node : public graph_node, public receiver<T>, public sender<T> {
			public:
				typedef T input_type;
				typedef T output_type;
				typedef typename receiver<input_type>::predecessor_type predecessor_type;
        ...
        decrement.built_predecessors().receiver_extract(decrement);
			...
			protected:
				...
				task* try_put_task(const T& t) __TBB_override {
					{
						spin_mutex::scoped_lock lock(my_mutex);
						if(my_count + my_tries >= my_threshold) return NULL;
						else ++my_tries;
					}
...
#include "internal/_flow_graph_join_impl.h"
			...
			template<typename OutputTuple> class join_node<OutputTuple, reserving> : public internal::unfolded_join_node<tbb::flow::tuple_size<OutputTuple>::value, reserving_port, OutputTuple, reserving> {
			private:
    ...
    void set_name( const char *name ) __TBB_override {
        tbb::internal::fgt_node_desc( this, name );
			...
			template<typename OutputTuple> class join_node<OutputTuple, queueing> : public internal::unfolded_join_node<tbb::flow::tuple_size<OutputTuple>::value, queueing_port, OutputTuple, queueing> {
			private:
				static const int N = tbb::flow::tuple_size<OutputTuple>::value;
    ...
    void set_name( const char *name ) __TBB_override {
        tbb::internal::fgt_node_desc( this, name );
			...
			template<typename OutputTuple, typename K, typename KHash> class join_node<OutputTuple, key_matching<K, KHash>> : public internal::unfolded_join_node<tbb::flow::tuple_size<OutputTuple>::value,
    ...
    join_node(graph &g) : unfolded_type(g) {}
				...
				template<typename __TBB_B0, typename __TBB_B1> join_node(graph& g, __TBB_B0 b0, __TBB_B1 b1)
					: unfolded_type(g, b0, b1) {
					tbb::internal::fgt_multiinput_node<N>(tbb::internal::FLOW_JOIN_NODE_TAG_MATCHING, &this->my_graph,
					                                      this->input_ports(), static_cast<sender<output_type> *>(this));
				}

				template<typename __TBB_B0, typename __TBB_B1, typename __TBB_B2> join_node(graph& g, __TBB_B0 b0, __TBB_B1 b1, __TBB_B2 b2)
					: unfolded_type(g, b0, b1, b2) {
					tbb::internal::fgt_multiinput_node<N>(tbb::internal::FLOW_JOIN_NODE_TAG_MATCHING, &this->my_graph,
					                                      this->input_ports(), static_cast<sender<output_type> *>(this));
				}

				template<typename __TBB_B0, typename __TBB_B1, typename __TBB_B2, typename __TBB_B3> join_node(graph& g, __TBB_B0 b0, __TBB_B1 b1, __TBB_B2 b2, __TBB_B3 b3)
					: unfolded_type(g, b0, b1, b2, b3) {
					tbb::internal::fgt_multiinput_node<N>(tbb::internal::FLOW_JOIN_NODE_TAG_MATCHING, &this->my_graph,
					                                      this->input_ports(), static_cast<sender<output_type> *>(this));
				}

				template<typename __TBB_B0, typename __TBB_B1, typename __TBB_B2, typename __TBB_B3, typename __TBB_B4> join_node(graph& g, __TBB_B0 b0, __TBB_B1 b1, __TBB_B2 b2, __TBB_B3 b3, __TBB_B4 b4)
					:
				...
				template<typename __TBB_B0, typename __TBB_B1, typename __TBB_B2, typename __TBB_B3, typename __TBB_B4,
				         typename __TBB_B5> join_node(graph& g, __TBB_B0 b0, __TBB_B1 b1, __TBB_B2 b2, __TBB_B3 b3, __TBB_B4 b4, __TBB_B5 b5)
					:
				...
				template<typename __TBB_B0, typename __TBB_B1, typename __TBB_B2, typename __TBB_B3, typename __TBB_B4,
				         typename __TBB_B5, typename __TBB_B6> join_node(graph& g, __TBB_B0 b0, __TBB_B1 b1, __TBB_B2 b2, __TBB_B3 b3, __TBB_B4 b4, __TBB_B5 b5, __TBB_B6 b6)
					:
				...
				template<typename __TBB_B0, typename __TBB_B1, typename __TBB_B2, typename __TBB_B3, typename __TBB_B4,
				         typename __TBB_B5, typename __TBB_B6, typename __TBB_B7> join_node(graph& g, __TBB_B0 b0, __TBB_B1 b1, __TBB_B2 b2, __TBB_B3 b3, __TBB_B4 b4, __TBB_B5 b5, __TBB_B6 b6,
				                                                                            __TBB_B7 b7)
				...
				template<typename __TBB_B0, typename __TBB_B1, typename __TBB_B2, typename __TBB_B3, typename __TBB_B4,
				         typename __TBB_B5, typename __TBB_B6, typename __TBB_B7, typename __TBB_B8> join_node(graph& g, __TBB_B0 b0, __TBB_B1 b1, __TBB_B2 b2, __TBB_B3 b3, __TBB_B4 b4, __TBB_B5 b5, __TBB_B6 b6,
				                                                                                               __TBB_B7 b7, __TBB_B8 b8)
				...
				template<typename __TBB_B0, typename __TBB_B1, typename __TBB_B2, typename __TBB_B3, typename __TBB_B4,
				         typename __TBB_B5, typename __TBB_B6, typename __TBB_B7, typename __TBB_B8, typename __TBB_B9> join_node(graph& g, __TBB_B0 b0, __TBB_B1 b1, __TBB_B2 b2, __TBB_B3 b3, __TBB_B4 b4, __TBB_B5 b5, __TBB_B6 b6,
				                                                                                                                  __TBB_B7 b7, __TBB_B8 b8, __TBB_B9 b9)
...
#include "internal/_flow_graph_indexer_impl.h"
			...
			template<typename T0> class indexer_node<T0> : public internal::unfolded_indexer_node<tuple<T0>> {
			private:
     ...
     void set_name( const char *name ) __TBB_override {
        tbb::internal::fgt_node_desc( this, name );
			...
			template<typename T0, typename T1> class indexer_node<T0, T1> : public internal::unfolded_indexer_node<tuple<T0, T1>> {
			private:
     ...
     void set_name( const char *name ) __TBB_override {
        tbb::internal::fgt_node_desc( this, name );
			...
			template<typename T0, typename T1, typename T2> class indexer_node<T0, T1, T2> : public internal::unfolded_indexer_node<tuple<T0, T1, T2>> {
			private:
    ...
    void set_name( const char *name ) __TBB_override {
        tbb::internal::fgt_node_desc( this, name );
			...
			template<typename T0, typename T1, typename T2, typename T3> class indexer_node<T0, T1, T2, T3> : public internal::unfolded_indexer_node<tuple<T0, T1, T2, T3>> {
			private:
    ...
    void set_name( const char *name ) __TBB_override {
        tbb::internal::fgt_node_desc( this, name );
			...
			template<typename T0, typename T1, typename T2, typename T3, typename T4> class indexer_node<T0, T1, T2, T3, T4> : public internal::unfolded_indexer_node<tuple<T0, T1, T2, T3, T4>> {
			private:
...
#if __TBB_VARIADIC_MAX >= 6
			template<typename T0, typename T1, typename T2, typename T3, typename T4, typename T5> class indexer_node<T0, T1, T2, T3, T4, T5> : public internal::unfolded_indexer_node<tuple<T0, T1, T2, T3, T4, T5>> {
			private:
    ...
    void set_name( const char *name ) __TBB_override {
        tbb::internal::fgt_node_desc( this, name );
			...
			template<typename T0, typename T1, typename T2, typename T3, typename T4, typename T5,
			         typename T6> class indexer_node<T0, T1, T2, T3, T4, T5, T6> : public internal::unfolded_indexer_node<tuple<T0, T1, T2, T3, T4, T5, T6>> {
			private:
...
#if __TBB_VARIADIC_MAX >= 8
			template<typename T0, typename T1, typename T2, typename T3, typename T4, typename T5,
			         typename T6, typename T7> class indexer_node<T0, T1, T2, T3, T4, T5, T6, T7> : public internal::unfolded_indexer_node<tuple<T0, T1, T2, T3, T4, T5, T6, T7>> {
			private:
...
#if __TBB_VARIADIC_MAX >= 9
			template<typename T0, typename T1, typename T2, typename T3, typename T4, typename T5,
			         typename T6, typename T7, typename T8> class indexer_node<T0, T1, T2, T3, T4, T5, T6, T7, T8> : public internal::unfolded_indexer_node<tuple<T0, T1, T2, T3, T4, T5, T6, T7, T8>> {
			private:
...
#if __TBB_VARIADIC_MAX >= 10
			template<typename T0, typename T1, typename T2, typename T3, typename T4, typename T5,
			         typename T6, typename T7, typename T8, typename T9> class indexer_node/*default*/ : public internal::unfolded_indexer_node<tuple<T0, T1, T2, T3, T4, T5, T6, T7, T8, T9>> {
			private:
...
inline void internal_make_edge( internal::untyped_sender &p, internal::untyped_receiver &s ) {
#else
			template<typename T> inline void internal_make_edge(sender<T>& p, receiver<T>& s) {
#endif
#if TBB_DEPRECATED_FLOW_NODE_EXTRACTION
    ...
    p.internal_add_built_successor(s);
			...
			template<typename T> inline void make_edge(sender<T>& p, receiver<T>& s) { internal_make_edge(p, s); }
...
inline void make_edge( sender<typename T::async_msg_data_type> &p, receiver<T> &s ) {
    internal_make_edge( p, s );
			...
			template<typename T, typename V,
			         typename = typename T::output_ports_type, typename = typename V::input_ports_type> inline void make_edge(T& output, V& input) { make_edge(get<0>(output.output_ports()), get<0>(input.input_ports())); }

			//Makes an edge from port 0 of a multi-output predecessor to a receiver.
			template<typename T, typename R,
			         typename = typename T::output_ports_type> inline void make_edge(T& output, receiver<R>& input) { make_edge(get<0>(output.output_ports()), input); }

			//Makes an edge from a sender to port 0 of a multi-input successor.
			template<typename S, typename V,
			         typename = typename V::input_ports_type> inline void make_edge(sender<S>& output, V& input) { make_edge(output, get<0>(input.input_ports())); }
#endif

#if __TBB_PREVIEW_ASYNC_MSG
inline void internal_remove_edge( internal::untyped_sender &p, internal::untyped_receiver &s ) {
#else
			template<typename T> inline void internal_remove_edge(sender<T>& p, receiver<T>& s) {
#endif
    ...
    s.internal_delete_built_predecessor(p);
			...
			template<typename T> inline void remove_edge(sender<T>& p, receiver<T>& s) { internal_remove_edge(p, s); }
...
inline void remove_edge( sender<typename T::async_msg_data_type> &p, receiver<T> &s ) {
    internal_remove_edge( p, s );
			...
			template<typename T, typename V,
			         typename = typename T::output_ports_type, typename = typename V::input_ports_type> inline void remove_edge(T& output, V& input) { remove_edge(get<0>(output.output_ports()), get<0>(input.input_ports())); }

			//Removes an edge between port 0 of a multi-output predecessor and a receiver.
			template<typename T, typename R,
			         typename = typename T::output_ports_type> inline void remove_edge(T& output, receiver<R>& input) { remove_edge(get<0>(output.output_ports()), input); }

			//Removes an edge between a sender and port 0 of a multi-input successor.
			template<typename S, typename V,
			         typename = typename V::input_ports_type> inline void remove_edge(sender<S>& output, V& input) { remove_edge(output, get<0>(input.input_ports())); }
#endif
...
void internal::edge_container<C>::receiver_extract( R &r ) {
    ...
    for ( typename edge_list_type::iterator i = e.begin(); i != e.end(); ++i ) {
        remove_edge(**i, r);
			...
			template<typename Body, typename Node> Body copy_body(Node& n) { return n.template copy_function_object<Body>(); }
			...
			template<typename... InputTypes, typename... OutputTypes> class composite_node<tbb::flow::tuple<InputTypes...>, tbb::flow::tuple<OutputTypes...>> : public graph_node {
    ...
    composite_node( graph &g, const char *type_name = "composite_node" ) : graph_node(g) {
        ...
        tbb::internal::fgt_multiinput_multioutput_node_desc( this, type_name );
				...
				composite_node(graph& g)
					: graph_node(g) { tbb::internal::fgt_multiinput_multioutput_node(tbb::internal::FLOW_COMPOSITE_NODE, this, &this->my_graph); }
#endif

				template<typename T1, typename T2> void set_external_ports(T1&& input_ports_tuple, T2&& output_ports_tuple) {
				...
				template<typename... NodeTypes> void add_visible_nodes(const NodeTypes&... n) { internal::add_nodes_impl(this, true, n...); }

				template<typename... NodeTypes> void add_nodes(const NodeTypes&... n) { internal::add_nodes_impl(this, false, n...); }
    ...
    void extract() __TBB_override {
        __TBB_ASSERT(false, "Current composite_node implementation does not support extract");
			...
			template<typename... InputTypes> class composite_node<tbb::flow::tuple<InputTypes...>, tbb::flow::tuple<>> : public graph_node {
			public:
				typedef tbb::flow::tuple<receiver<InputTypes>&...> input_ports_type;
...
#else
				composite_node(graph& g)
					: graph_node(g) { tbb::internal::fgt_composite(this, &g); }
#endif

				template<typename T> void set_external_ports(T&& input_ports_tuple) {
					__TBB_STATIC_ASSERT(NUM_INPUTS == tbb::flow::tuple_size<input_ports_type>::value, "number of arguments does not match number of input ports");
    ...
    void extract() __TBB_override {
        __TBB_ASSERT(false, "Current composite_node implementation does not support extract");
			...
			template<typename... OutputTypes> class composite_node<tbb::flow::tuple<>, tbb::flow::tuple<OutputTypes...>> : public graph_node {
			public:
				typedef tbb::flow::tuple<sender<OutputTypes>&...> output_ports_type;
    ...
    composite_node( graph &g, const char *type_name = "composite_node") : graph_node(g) {
        ...
        tbb::internal::fgt_multiinput_multioutput_node_desc( this, type_name );
				...
				template<typename T> void set_external_ports(T&& output_ports_tuple) {
					__TBB_STATIC_ASSERT(NUM_OUTPUTS == tbb::flow::tuple_size<output_ports_type>::value, "number of arguments does not match number of output ports");
    ...
    void extract() __TBB_override {
        __TBB_ASSERT(false, "Current composite_node implementation does not support extract");
			...
			namespace internal {
				...
				template<typename Gateway> class async_body_base : tbb::internal::no_assign {
				public:
				...
				template<typename Input, typename Ports, typename Gateway, typename Body> class async_body : public async_body_base<Gateway> {
				public:
					...
					void operator()(const Input& v, Ports&) { my_body(v, *this->my_gateway); }
			...
			template<typename Input, typename Output,
			         ...
			         typename Allocator=cache_aligned_allocator<Input>> class async_node : public multifunction_node<Input, tuple<Output>, Policy, Allocator>, public sender<Output> {
			...
			private:
				...
				public:
					...
					bool try_put(const Output& i) __TBB_override { return my_node->try_put_impl(i); }
			...
			public:
				template<typename Body> async_node(
					graph& g, size_t concurrency,
					__TBB_FLOW_GRAPH_PRIORITY_ARG1( Body body, node_priority_t priority = tbb::flow::internal::no_priority )
				)
					: base_type(
						...
						(body, &my_gateway) __TBB_FLOW_GRAPH_PRIORITY_ARG0(priority) )
					, my_gateway(self()) {
    ...
    void set_name( const char *name ) __TBB_override {
        tbb::internal::fgt_multioutput_node_desc( this, name );
				...
				//! Add a new successor to this node
				bool register_successor(successor_type& r) __TBB_override { return internal::output_port<0>(*this).register_successor(r); }

				//! Removes a successor from this node
				bool remove_successor(successor_type& r) __TBB_override { return internal::output_port<0>(*this).remove_successor(r); }
...
#include "internal/_flow_graph_streaming_node.h"
		...
		namespace interface10a {
			...
			template<typename T> class overwrite_node : public graph_node, public receiver<T>, public sender<T> {
			public:
				typedef T input_type;
				typedef T output_type;
				typedef typename receiver<input_type>::predecessor_type predecessor_type;
			...
			protected:
				...
				struct register_predecessor_task : public graph_task {
					...
					tbb::task* execute() __TBB_override {
						if(!s.register_predecessor(o)) { o.register_successor(s); }
						return NULL;
					}
    ...
    internal::edge_container<predecessor_type> my_built_predecessors;
			...
			template<typename T> class write_once_node : public overwrite_node<T> {
			public:
				typedef T input_type;
				typedef T output_type;
