...
#define __TBB_DECL_ATOMIC_FIELD(t,f,a) alignas(a) t f;
    ...
    #error Do not know syntax for forcing alignment.
		...
		template<size_t S> struct atomic_rep; // Primary template declared, but never defined.

		template<> struct atomic_rep<1> {
			// Specialization
			typedef int8_t word;
		};

		template<> struct atomic_rep<2> {
			// Specialization
			typedef int16_t word;
		};

		template<> struct atomic_rep<4> {
			// Specialization
#if _MSC_VER && !_WIN64
    ...
    typedef intptr_t word;
		...
		template<> struct atomic_rep<8> {
			// Specialization
			typedef int64_t word;
		};
#endif

		template<typename value_type, size_t size> struct aligned_storage;
...
#define ATOMIC_STORAGE_PARTIAL_SPECIALIZATION(S)                  \
    ...
    struct aligned_storage<value_type,S> {                            \
		...
		template<typename value_type> struct aligned_storage<value_type, 1> {
			value_type my_value;
#if __TBB_ATOMIC_CTORS
			aligned_storage() = default ;
		...
		template<size_t Size, memory_semantics M> struct atomic_traits; // Primary template declared, but not defined.
...
#define __TBB_DECL_ATOMIC_PRIMITIVES(S)                                                                  \
    ...
    struct atomic_traits<S,M> {                                                                          \
		...
		template<memory_semantics M> struct atomic_load_store_traits; // Primary template declaration
...
#define __TBB_MINUS_ONE(T) (T(T(0)-T(1)))
		...
		private:
			...
			template<typename value_type> union converter {
			...
			template<typename value_t> static typename converter<value_t>::bits_type to_bits(value_t value) { return converter<value_t>(value).bits; }

			template<typename value_t> static value_t to_value(typename converter<value_t>::bits_type bits) {
				converter<value_t> u;
				u.bits = bits;
				return u.value;
			}

			template<typename value_t> union ptr_converter; //Primary template declared, but never defined.

			template<typename value_t> union ptr_converter<value_t *> {
				ptr_converter() {}
			...
			template<typename value_t> static typename converter<value_t>::bits_type& to_bits_ref(value_t& value) {
		...
		public:
			...
			constexpr atomic_impl(value_type value)
				: my_storage(value) {}
#endif
			template<memory_semantics M> value_type fetch_and_store(value_type value) {
				return to_value<value_type>(
					internal::atomic_traits<sizeof(value_type), M>::fetch_and_store(&my_storage.my_value, to_bits(value))
				);
			}

			value_type fetch_and_store(value_type value) { return fetch_and_store<full_fence>(value); }

			template<memory_semantics M> value_type compare_and_swap(value_type value, value_type comparand) {
				return to_value<value_type>(
					internal::atomic_traits<sizeof(value_type), M>::compare_and_swap(&my_storage.my_value, to_bits(value), to_bits(comparand))
				);
			}

			value_type compare_and_swap(value_type value, value_type comparand) { return compare_and_swap<full_fence>(value, comparand); }

			operator value_type() const volatile {
				// volatile qualifier here for backwards compatibility
				return to_value<value_type>(
					__TBB_load_with_acquire(to_bits_ref(my_storage.my_value))
				);
			}

			template<memory_semantics M> value_type load() const {
				return to_value<value_type>(
			...
			template<memory_semantics M> void store(value_type value) { internal::atomic_load_store_traits<M>::store(to_bits_ref(my_storage.my_value), to_bits(value)); }
		...
		template<typename I, typename D, typename StepType> struct atomic_impl_with_arithmetic : atomic_impl<I> {
		public:
			...
			constexpr atomic_impl_with_arithmetic(value_type value)
				: atomic_impl<I>(value) {}
#endif
			template<memory_semantics M> value_type fetch_and_add(D addend) { return value_type(internal::atomic_traits<sizeof(value_type), M>::fetch_and_add(&this->my_storage.my_value, addend * sizeof(StepType))); }

			value_type fetch_and_add(D addend) { return fetch_and_add<full_fence>(addend); }

			template<memory_semantics M> value_type fetch_and_increment() { return fetch_and_add<M>(1); }

			value_type fetch_and_increment() { return fetch_and_add(1); }

			template<memory_semantics M> value_type fetch_and_decrement() { return fetch_and_add<M>(__TBB_MINUS_ONE(D)); }

			value_type fetch_and_decrement() { return fetch_and_add(__TBB_MINUS_ONE(D)); }
		...
		public:
			...
			value_type operator--(int) { return fetch_and_add(__TBB_MINUS_ONE(D)); }
		};
	...
	template<typename T> struct atomic : internal::atomic_impl<T> {
#if __TBB_ATOMIC_CTORS
		atomic() = default;
...
__TBB_DECL_ATOMIC_ALT(int,ptrdiff_t)
	...
	template<memory_semantics M, typename T> T load(const atomic<T>& a) { return a.template load<M>(); }

	template<memory_semantics M, typename T> void store(atomic<T>& a, T value) { a.template store<M>(value); }

	namespace interface6 {
		//! Make an atomic for use in an initialization (list), as an alternative to zero-initialization or normal assignment.
		template<typename T> atomic<T> make_atomic(T t) {
			atomic<T> a;
			store<relaxed>(a, t);
			return a;
		}
	...
	namespace internal {
		template<memory_semantics M, typename T> void swap(atomic<T>& lhs, atomic<T>& rhs) {
			T tmp = load<M>(lhs);
		...
		template<typename T> inline atomic<T>& as_atomic(T& t) { return (atomic<T>&)t; }
