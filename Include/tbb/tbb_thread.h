...
namespace tbb {
    ...
    public:
		...
		public:
			...
			template<class charT, class traits> friend std::basic_ostream<charT, traits>& operator<<(std::basic_ostream<charT, traits>& out,
			                                                                                         tbb_thread_v3::id id) {
				out << id.my_id;
				return out;
			}
			...
			friend id atomic_compare_and_swap(id& location, const id& value, const id& comparand) { return as_atomic(location.my_id).compare_and_swap(value.my_id, comparand.my_id); }
		}; // tbb_thread_v3::id
		...
		inline bool operator==(tbb_thread_v3::id x, tbb_thread_v3::id y) __TBB_NOEXCEPT(true) { return x.my_id == y.my_id; }
	...
	inline void move(tbb_thread& t1, tbb_thread& t2) { internal::move_v3(t1, t2); }
	...
	namespace this_tbb_thread {
		...
		inline void sleep(const tick_count::interval_t& i) { internal::thread_sleep_v3(i); }
	} // namespace this_tbb_thread
