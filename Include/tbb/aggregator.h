...
namespace tbb {
	namespace interface6 {
		...
		namespace internal {
			...
			template<typename Body> class basic_operation : public basic_operation_base, no_assign {
				const Body& my_body;
		...
		template<typename handler_type> class aggregator_ext : tbb::internal::no_copy {
		public:
			aggregator_ext(const handler_type& h)
				: handler_busy(0)
				, handle_operations(h) { mailbox = NULL; }
		...
		public:
			aggregator()
				: aggregator_ext<internal::basic_handler>(internal::basic_handler()) {}
