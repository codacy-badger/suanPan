...
namespace tbb {
	...
	namespace interface7 {
		...
		namespace internal {
			...
			template<typename F> struct return_type_or_void {
    ...
    typedef void type;
			...
			template<typename F, typename R> class delegated_function : public delegate_base {
				F& my_func;
				tbb::aligned_space<R> my_return_storage;

				// The function should be called only once.
				void operator()() const __TBB_override { new(my_return_storage.begin()) R(my_func()); }
			...
			public:
				...
				R consume_result() const { return tbb::internal::move(*(my_return_storage.begin())); }

				~delegated_function() { my_return_storage.begin()->~R(); }
			};

			template<typename F> class delegated_function<F, void> : public delegate_base {
				F& my_func;
			...
			template<typename R, typename F> R isolate_impl(F& f) {
        ...
        my_initialized = true;
			...
			template<typename F> void enqueue_impl(__TBB_FORWARDING_REF(F) f
        ...
        internal_enqueue(*new(task::allocate_root()) internal::function_task< typename internal::strip<F>::type >(internal::forward<F>(f)), p);
			...
			template<typename R, typename F> R execute_impl(F& f) {
				initialize();
		...
		public:
			...
			template<typename F> void enqueue(F&& f) { enqueue_impl(std::forward<F>(f)); }
#else
    template<typename F>
    ...
    void enqueue( const F& f, priority_t p ) {
        ...
        enqueue_impl(f,p);
			...
			template<typename F> typename internal::return_type_or_void<F>::type execute(F& f) { return execute_impl<typename internal::return_type_or_void<F>::type>(f); }

			//! Joins the arena and executes a constant functor, then returns
			//! If not possible to join, wraps the functor into a task, enqueues it and waits for task completion
			//! Can decrement the arena demand for workers, causing a worker to leave and free a slot to the calling thread
			//! Since C++11, the method returns the value returned by functor (prior to C++11 it returns void).
			template<typename F> typename internal::return_type_or_void<F>::type execute(const F& f) { return execute_impl<typename internal::return_type_or_void<F>::type>(f); }
    ...
    void debug_wait_until_empty() {
		...
		namespace this_task_arena {
			...
			//! Since C++11, the method returns the value returned by functor (prior to C++11 it returns void).
			template<typename F> typename internal::return_type_or_void<F>::type isolate(F& f) { return internal::isolate_impl<typename internal::return_type_or_void<F>::type>(f); }

			//! Executes a constant functor in isolation within the current task arena.
			//! Since C++11, the method returns the value returned by functor (prior to C++11 it returns void).
			template<typename F> typename internal::return_type_or_void<F>::type isolate(const F& f) { return internal::isolate_impl<typename internal::return_type_or_void<F>::type>(f); }
		}
#endif /* __TBB_TASK_ISOLATION */
	...
	namespace this_task_arena {
		...
		//! Returns the maximal number of threads that can work inside the arena
		inline int max_concurrency() { return tbb::task_arena::internal_max_concurrency(NULL); }
	} // namespace this_task_arena
