...
namespace tbb {
	namespace internal {
		...
		*/
		...
		template<typename Value, unsigned int N, typename = make_index_sequence<N>> class blocked_rangeNd_impl;

		template<typename Value, unsigned int N, std::size_t... Is> class blocked_rangeNd_impl<Value, N, index_sequence<Is...>> {
		public:
			//! Type of a value.
		...
		private:
			...
			template<std::size_t> using dim_type_helper = tbb::blocked_range<value_type>;
		...
		public:
			...
			bool empty() const { return std::any_of(my_dims.begin(), my_dims.end(), [](const tbb::blocked_range<value_type>& d) { return d.empty(); }); }

			//! True if at least one dimension is divisible.
			bool is_divisible() const { return std::any_of(my_dims.begin(), my_dims.end(), [](const tbb::blocked_range<value_type>& d) { return d.is_divisible(); }); }
			...
			blocked_rangeNd_impl(blocked_rangeNd_impl& r, proportional_split proportion)
				: my_dims(r.my_dims) { do_split(r, proportion); }
#endif
		...
		private:
			...
			template<typename split_type> void do_split(blocked_rangeNd_impl& r, split_type proportion) {
				...
				auto my_it = std::max_element(my_dims.begin(), my_dims.end(), [](const tbb::blocked_range<value_type>& first, const tbb::blocked_range<value_type>& second) { return (first.size() * second.grainsize() < second.size() * first.grainsize()); });
	...
	template<typename Value, unsigned int N> using blocked_rangeNd = internal::blocked_rangeNd_impl<Value, N>;
