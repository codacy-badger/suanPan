...
namespace tbb {
	namespace interface6 {
		...
		template<typename key_type, typename value_type, typename value_functor_type = value_type (*)(key_type)> class concurrent_lru_cache : internal::no_assign {
		private:
		...
		public:
			concurrent_lru_cache(value_function_type f, std::size_t number_of_lru_history_items)
				: my_value_function(f)
				, my_number_of_lru_history_items(number_of_lru_history_items) { my_aggregator.initialize_handler(aggregator_function_type(this)); }
			...
			handle_object operator[](key_type k) {
				...
				} else { tbb::internal::spin_wait_while_eq(op.result().second.my_is_ready, false); }
    ...
    struct handle_move_t:no_assign{
        ...
        handle_move_t(concurrent_lru_cache & cache_ref, typename map_storage_type::reference value_ref):my_cache_ref(cache_ref),my_map_record_ref(value_ref) {};
			...
			public:
				...
				handle_object(concurrent_lru_cache& cache_ref, typename map_storage_type::reference value_ref)
					...
					, my_map_record_ptr(&value_ref) {}

				operator bool() const { return (my_cache_pointer && my_map_record_ptr); }
#if __TBB_CPP11_RVALUE_REF_PRESENT
				...
				handle_object& operator=(handle_object&& src) {
					__TBB_ASSERT((src.my_cache_pointer && src.my_map_record_ptr) || (!src.my_cache_pointer && !src.my_map_record_ptr), "invalid state of moving object?");
					if(my_cache_pointer) { my_cache_pointer->signal_end_of_usage(*my_map_record_ptr); }
					my_cache_pointer = src.my_cache_pointer;
					my_map_record_ptr = src.my_map_record_ptr;
					src.my_cache_pointer = NULL;
					src.my_map_record_ptr = NULL;
        ...
        operator handle_move_t(){
            return move(*this);
				...
				~handle_object() { if(my_cache_pointer) { my_cache_pointer->signal_end_of_usage(*my_map_record_ptr); } }

			private:
#if __TBB_CPP11_RVALUE_REF_PRESENT
				// For source compatibility with C++03
				friend handle_object&& move(handle_object& h) { return std::move(h); }
#else
    ...
    public:
		...
		private:
			...
			struct aggregator_operation : tbb::internal::aggregated_operation<aggregator_operation> {
				...
				void cast_and_handle(self_type& container) { if(my_operation_type == op_retive) { static_cast<retrieve_aggregator_operation*>(this)->handle(container); } else { static_cast<signal_end_of_usage_aggregator_operation*>(this)->handle(container); } }
			};
			...
			struct retrieve_aggregator_operation : aggregator_operation, private internal::no_assign {
				...
				void handle(self_type& container) { my_result_map_record_pointer = & container.retrieve_serial(my_key, my_is_new_value_needed); }
			...
			struct signal_end_of_usage_aggregator_operation : aggregator_operation, private internal::no_assign {
				...
				void handle(self_type& container) { container.signal_end_of_usage_serial(my_map_record_ref); }
			};
