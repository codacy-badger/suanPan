...
namespace internal {
	...
	template<typename Input, typename Policy, typename A, typename ImplType> class function_input_base : public receiver<Input>, tbb::internal::no_assign {
		enum op_type {
			reg_pred,
			rem_pred,
			try_fwd,
	...
	public:
		...
		virtual ~function_input_base() { if(my_queue) delete my_queue; }

		task* try_put_task(const input_type& t) __TBB_override { return try_put_task_impl(t, internal::has_policy<lightweight, Policy>()); }
	...
	protected:
		...
		graph& graph_reference() __TBB_override { return my_graph_ref; }
	...
	private:
		...
		void handle_operations(operation_type* op_list) {
			...
			while(op_list) {
				...
				case app_body_bypass: {
					...
					--my_concurrency;
					if(my_concurrency < my_max_concurrency) tmp->bypass_t = perform_queued_requests();
				...
				case occupy_concurrency:
					if(my_concurrency < my_max_concurrency) {
						++my_concurrency;
						__TBB_store_with_release(tmp->status, SUCCEEDED);
					} else { __TBB_store_with_release(tmp->status, FAILED); }
					break;
#if TBB_DEPRECATED_FLOW_NODE_EXTRACTION
                case add_blt_pred: {
		...
		void internal_forward(operation_type* op) {
			op->bypass_t = NULL;
			if(my_concurrency < my_max_concurrency || !my_max_concurrency) op->bypass_t = perform_queued_requests();
			if(op->bypass_t) __TBB_store_with_release(op->status, SUCCEEDED);
			else {
				forwarder_busy = false;
				__TBB_store_with_release(op->status, FAILED);
			}
		...
		task* internal_try_put_bypass(const input_type& t) {
			...
			my_aggregator.execute(&op_data);
			if(op_data.status == internal::SUCCEEDED) { return op_data.bypass_t; }
			return NULL;
		}

		task* try_put_task_impl(const input_type& t, /*lightweight=*/tbb::internal::true_type) {
			if(my_max_concurrency == 0) { return apply_body_bypass(t); } else {
				operation_type check_op(t, occupy_concurrency);
				my_aggregator.execute(&check_op);
				if(check_op.status == internal::SUCCEEDED) { return apply_body_bypass(t); }
				return internal_try_put_bypass(t);
			}
		}

		task* try_put_task_impl(const input_type& t, /*lightweight=*/tbb::internal::false_type) { if(my_max_concurrency == 0) { return create_body_task(t); } else { return internal_try_put_bypass(t); } }

		//! Applies the body to the provided input
		//  then decides if more work is available
		task* apply_body_bypass(const input_type& i) { return static_cast<ImplType *>(this)->apply_body_impl_bypass(i); }
		...
		inline void spawn_forward_task() {
			...
			if(tp) { internal::spawn_in_graph_arena(graph_reference(), *tp); }
		}
	}; // function_input_base

	//! Implements methods for a function node that takes a type Input as input and sends
	//  a type Output to its successors.
	template<typename Input, typename Output, typename Policy, typename A> class function_input : public function_input_base<Input, Policy, A, function_input<Input, Output, Policy, A>> {
	public:
		typedef Input input_type;
		...
		// constructor
		template<typename Body> function_input(
...
#pragma warning (pop)
	...
	template<int N> struct clear_element {
		...
		template<typename P> static bool this_empty(P& p) {
			if(tbb::flow::get<N - 1>(p).successors().empty()) return clear_element<N - 1>::this_empty(p);
			return false;
		}
	};

	template<> struct clear_element<1> {
		template<typename P> static void clear_this(P& p) { (void)tbb::flow::get<0>(p).successors().clear(); }

		template<typename P> static bool this_empty(P& p) { return tbb::flow::get<0>(p).successors().empty(); }
	};
	...
	template<typename Input, typename OutputPortSet, typename Policy, typename A> class multifunction_input : public function_input_base<Input, Policy, A, multifunction_input<Input, OutputPortSet, Policy, A>> {
	public:
		...
		// constructor
		template<typename Body> multifunction_input(graph& g, size_t max_concurrency,
		                                            __TBB_FLOW_GRAPH_PRIORITY_ARG1(Body& body, node_priority_t priority)
		)
			: base_type(g, __TBB_FLOW_GRAPH_PRIORITY_ARG1(max_concurrency, priority))
		...
		task* apply_body_impl_bypass(const input_type& i) {
			...
			tbb::internal::fgt_end_body(my_body);
			task* ttask = NULL;
			if(base_type::my_max_concurrency != 0) { ttask = base_type::try_get_postponed_task(i); }
	...
	template<size_t N, typename MOP> typename tbb::flow::tuple_element<N, typename MOP::output_ports_type>::type& output_port(MOP& op) { return tbb::flow::get<N>(op.output_ports()); }

	inline void check_task_and_spawn(graph& g, task* t) { if(t && t != SUCCESSFULLY_ENQUEUED) { internal::spawn_in_graph_arena(g, *t); } }

	// helper structs for split_node
	template<int N> struct emit_element {
		template<typename T, typename P> static task* emit_this(graph& g, const T& t, P& p) {
	...
	template<typename Output, typename Policy> class continue_input : public continue_receiver {
	public:
		...
		template<typename Body> continue_input(graph& g, __TBB_FLOW_GRAPH_PRIORITY_ARG1(Body& body, node_priority_t priority))
			: continue_receiver(__TBB_FLOW_GRAPH_PRIORITY_ARG1(/*number_of_predecessors=*/0, priority))
			, my_graph_ref(g)
			, my_body(new internal::function_body_leaf<input_type, output_type, Body>(body))
			, my_init_body(new internal::function_body_leaf<input_type, output_type, Body>(body)) { }

		template<typename Body> continue_input(graph& g, int number_of_predecessors,
		                                       __TBB_FLOW_GRAPH_PRIORITY_ARG1(Body& body, node_priority_t priority)
		)
	...
	protected:
		...
		task* execute() __TBB_override {
			if(!internal::is_graph_active(my_graph_ref)) { return NULL; }
#if _MSC_VER && !__INTEL_COMPILER
#pragma warning (push)
#pragma warning (disable: 4127)  /* suppress conditional expression is constant */
#endif
...
#pragma warning (pop)
	...
	public:
		...
		function_output(const function_output& /*other*/)
			: sender<output_type>() { my_successors.set_owner(this); }
	...
	public:
		...
		multifunction_output()
			: base_type() { my_successors.set_owner(this); }
	...
	template<typename CompositeType, typename NodeType1, typename... NodeTypes> void add_nodes_impl(CompositeType* c_node, bool visible, const NodeType1& n1, const NodeTypes&... n) {
