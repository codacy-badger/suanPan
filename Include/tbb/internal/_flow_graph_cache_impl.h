...
namespace internal {
	...
	template<typename T, typename M=spin_mutex> class node_cache {
	public:
...
class predecessor_cache : public node_cache< untyped_sender, M > {
		...
		bool get_item(output_type& v) {
			...
			do {
				...
				if(msg == false) {
					...
					if(my_owner) src->register_successor(*my_owner);
				} else {
	...
	template<typename T, typename M=spin_mutex> class reservable_predecessor_cache : public predecessor_cache<T, M> {
	public:
		typedef M mutex_type;
		typedef T output_type;
#if __TBB_PREVIEW_ASYNC_MSG
    ...
    typedef untyped_receiver successor_type;
		...
		bool try_reserve(output_type& v) {
			...
			do {
				{
					typename mutex_type::scoped_lock lock(this->my_mutex);
					if(reserved_src || this->internal_empty()) return false;
	...
	template<typename T, typename M=spin_rw_mutex> class successor_cache : tbb::internal::no_copy {
	protected:
	...
	template<> class successor_cache<continue_msg> : tbb::internal::no_copy {
	protected:
    ...
    size_t successor_count() {
		...
		void register_successor(successor_type& r) {
			mutex_type::scoped_lock l(my_mutex, true);
			my_successors.push_back(&r);
			if(my_owner && r.is_continue_receiver()) { r.register_predecessor(*my_owner); }
		}
		...
		void remove_successor(successor_type& r) {
			...
			for(successors_type::iterator i = my_successors.begin();
				...
				if(*i == & r) {
					...
					// removing from r.
					if(my_owner) r.remove_predecessor(*my_owner);
					my_successors.erase(i);
					break;
				}
			}
	...
	template<typename T, typename M=spin_rw_mutex> class broadcast_cache : public successor_cache<T, M> {
		typedef M mutex_type;
	...
	template<typename T, typename M=spin_rw_mutex> class round_robin_cache : public successor_cache<T, M> {
		typedef size_t size_type;
		typedef M mutex_type;
		typedef typename successor_cache<T, M>::successors_type successors_type;
    ...
    task * try_put_task( const X &t ) {
		...
		task* try_put_task(const T& t) __TBB_override {
			...
			while(i != this->my_successors.end()) {
				task* new_task = (*i)->try_put_task(t);
				if(new_task) { return new_task; } else {
					if((*i)->register_predecessor(*this->my_owner)) {
						if(!upgraded) {
							l.upgrade_to_writer();
							upgraded = true;
