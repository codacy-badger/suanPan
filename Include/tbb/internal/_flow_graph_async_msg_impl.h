...
namespace internal {
	...
	public:
		...
		async_storage()
			: my_graph(nullptr) { my_data_ready.store<tbb::relaxed>(false); }
		...
		template<typename C> async_storage(C&& data)
			: my_graph(nullptr)
		...
		template<typename C> bool set(C&& data) {
			using namespace tbb::internal;
			__TBB_STATIC_ASSERT((is_same_type<typename strip<C>::type, typename strip<T>::type>::value), "incoming type must be T");
			...
			for(typename subscriber_list_type::iterator it = my_clients.begin(); it != my_clients.end(); ++it) { (*it)->try_put(my_data); }
...
public:
	...
	async_msg(T&& t)
		: my_storage(std::make_shared<internal::async_storage<T>>(std::move(t))) {}
