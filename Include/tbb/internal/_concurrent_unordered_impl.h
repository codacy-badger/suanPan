...
namespace tbb {
	namespace interface5 {
		...
		namespace internal {
			...
			template<typename T, typename Allocator> class split_ordered_list;
			template<typename Traits> class concurrent_unordered_base;

			// Forward list iterators (without skipping dummy elements)
			template<class Solist, typename Value> class flist_iterator : public std::iterator<std::forward_iterator_tag, Value> {
				template<typename T, typename Allocator> friend class split_ordered_list;
				template<typename Traits> friend class concurrent_unordered_base;
				template<class M, typename V> friend class flist_iterator;
			...
			protected:
				...
				template<typename M, typename T, typename U> friend bool operator==(const flist_iterator<M, T>& i, const flist_iterator<M, U>& j);
				template<typename M, typename T, typename U> friend bool operator!=(const flist_iterator<M, T>& i, const flist_iterator<M, U>& j);
			};

			template<typename Solist, typename T, typename U> bool operator==(const flist_iterator<Solist, T>& i, const flist_iterator<Solist, U>& j) { return i.my_node_ptr == j.my_node_ptr; }

			template<typename Solist, typename T, typename U> bool operator!=(const flist_iterator<Solist, T>& i, const flist_iterator<Solist, U>& j) { return i.my_node_ptr != j.my_node_ptr; }

			// Split-order list iterators, needed to skip dummy elements
			template<class Solist, typename Value> class solist_iterator : public flist_iterator<Solist, Value> {
				typedef flist_iterator<Solist, Value> base_type;
				typedef typename Solist::nodeptr_t nodeptr_t;
				using base_type::get_node_ptr;
				template<typename T, typename Allocator> friend class split_ordered_list;
				template<class M, typename V> friend class solist_iterator;
				template<typename M, typename T, typename U> friend bool operator==(const solist_iterator<M, T>& i, const solist_iterator<M, U>& j);
				template<typename M, typename T, typename U> friend bool operator!=(const solist_iterator<M, T>& i, const solist_iterator<M, U>& j);
			...
			template<typename Solist, typename T, typename U> bool operator==(const solist_iterator<Solist, T>& i, const solist_iterator<Solist, U>& j) { return i.my_node_ptr == j.my_node_ptr && i.my_list_ptr == j.my_list_ptr; }
			...
			public:
				...
				template<typename Arg> nodeptr_t create_node(sokey_t order_key, __TBB_FORWARDING_REF(Arg) t,
				...
				template<typename Arg> nodeptr_t create_node(sokey_t, __TBB_FORWARDING_REF(Arg),
				...
				template<typename __TBB_PARAMETER_PACK Args> nodeptr_t create_node_v(__TBB_FORWARDING_REF(Args) __TBB_PARAMETER_PACK args) {
					nodeptr_t pnode = my_node_allocator.allocate(1);

					//TODO: use RAII scoped guard instead of explicit catch
					__TBB_TRY { new(static_cast<void*>(&pnode->my_element)) T(__TBB_PACK_EXPANSION(tbb::internal::forward<Args>(args))); } __TBB_CATCH(...) {
						my_node_allocator.deallocate(pnode, 1);
						__TBB_RETHROW();
					}
				...
				const_iterator cbegin() const { return (((const self_type *)this)->begin()); }

				const_iterator cend() const { return (((const self_type *)this)->end()); }
				...
				// Returns a first element in the SOL, which is always a dummy
				raw_const_iterator raw_begin() const { return raw_const_iterator(my_head); }

				raw_iterator raw_end() { return raw_iterator(0); }

				raw_const_iterator raw_end() const { return raw_const_iterator(0); }

				static sokey_t get_order_key(const raw_const_iterator& it) { return it.get_node_ptr()->get_order_key(); }
				...
				raw_iterator get_iterator(raw_const_iterator it) { return raw_iterator(it.get_node_ptr()); }

				// Returns a non-const version of the iterator
				static iterator get_iterator(const_iterator it) { return iterator(it.my_node_ptr, it.my_list_ptr); }
				...
				iterator first_real_iterator(raw_iterator it) {
					...
					while(it != raw_end() && it.get_node_ptr()->is_dummy()) ++it;
...
#pragma warning(disable: 4127) // warning C4127: conditional expression is constant
			...
			private:
				...
				struct call_internal_clear_on_exit {
					...
					~call_internal_clear_on_exit() { if(my_instance) { my_instance->internal_clear(); } }
				};
			...
			protected:
					...
					if(a == right.get_allocator()) {
						...
						if(! right.my_solist.empty()) {
							...
							for(raw_const_iterator it = ++(right.my_solist.raw_begin()), last = right.my_solist.raw_end(); it != last; ++it) {
								...
								} else { node = my_solist.create_node(pnode->get_order_key(), std::move(pnode->my_element)); }
				...
				concurrent_unordered_base& operator=(const concurrent_unordered_base& right) {
					if(this != &right) internal_copy(right);
					return (*this);
				}
			...
			public:
				...
				public:
					...
					const_range_type(const concurrent_unordered_base& a_table)
						...
						, my_end_node(a_table.my_solist.end()) { set_midpoint(); }
				...
				// Modifiers
				std::pair<iterator, bool> insert(const value_type& value) { return internal_insert</*AllowCreate=*/tbb::internal::true_type>(value); }
...
#if __TBB_CPP11_RVALUE_REF_PRESENT
				std::pair<iterator, bool> insert(value_type&& value) { return internal_insert</*AllowCreate=*/tbb::internal::true_type>(std::move(value)); }
				...
				template<class Iterator> void insert(Iterator first, Iterator last) { for(Iterator it = first; it != last; ++it) insert(*it); }
				...
				iterator unsafe_erase(const_iterator where) { return internal_erase(where); }

				iterator unsafe_erase(const_iterator first, const_iterator last) {
					while(first != last) unsafe_erase(first++);
				...
				const_iterator find(const key_type& key) const { return const_cast<self_type*>(this)->internal_find(key); }
				...
				size_type count(const key_type& key) const {
					if(allow_multimapping) {
						...
						return item_count;
					} else { return const_cast<self_type*>(this)->internal_find(key) == end() ? 0 : 1; }
				}

				std::pair<iterator, iterator> equal_range(const key_type& key) { return internal_equal_range(key); }

				std::pair<const_iterator, const_iterator> equal_range(const key_type& key) const { return const_cast<self_type*>(this)->internal_equal_range(key); }

				// Bucket interface - for debugging
				size_type unsafe_bucket_count() const { return my_number_of_buckets; }

				size_type unsafe_max_bucket_count() const { return segment_size(pointers_per_table - 1); }
				...
				size_type unsafe_bucket_size(size_type bucket) {
					...
					if(is_initialized(bucket)) {
						...
						++it;
						for(; it != my_solist.raw_end() && !it.get_node_ptr()->is_dummy(); ++it) ++item_count;
					}
					return item_count;
				}
				...
				local_iterator unsafe_begin(size_type bucket) {
					if(!is_initialized(bucket)) return end();
				...
				const_local_iterator unsafe_cbegin(size_type bucket) const { return ((const self_type *)this)->unsafe_begin(bucket); }

				const_local_iterator unsafe_cend(size_type bucket) const { return ((const self_type *)this)->unsafe_end(bucket); }

				// Hash policy
				float load_factor() const { return (float)size() / (float)unsafe_bucket_count(); }

				float max_load_factor() const { return my_maximum_bucket_size; }

				void max_load_factor(float newmax) {
					if(newmax != newmax || newmax < 0) tbb::internal::throw_exception(tbb::internal::eid_invalid_load_factor);
				...
				void rehash(size_type buckets) {
					...
					if(current_buckets >= buckets) return;
			...
			private:
				...
				void internal_clear() {
					for(size_type index = 0; index < pointers_per_table; ++index) {
						if(my_buckets[index] != NULL) {
							...
							for(size_type index2 = 0; index2 < sz; ++index2) my_allocator.destroy(&my_buckets[index][index2]);
				...
				static size_type internal_distance(const_iterator first, const_iterator last) {
					...
					for(const_iterator it = first; it != last; ++it) ++num;

					return num;
				}

				// Insert an element in the hash given its value
				template<typename AllowCreate, typename ValueType> std::pair<iterator, bool> internal_insert(__TBB_FORWARDING_REF(ValueType) value, nodeptr_t pnode = NULL) {
					...
					for(raw_iterator where = previous;;) {
							...
							!my_hash_compare(get_key(*where), *pkey)) // TODO: fix negation
						{
							...
							if(pnode) my_solist.destroy_node(pnode);
				...
				iterator internal_erase(const_iterator it) {
					...
					for(raw_iterator where = previous; ; previous = where) {
						++where;
						if(where == last) return end();
						else if(my_solist.get_iterator(where) == it) return my_solist.erase_node(previous, it);
					}
				}
				...
				void init_bucket(size_type bucket) {
					...
					if(!is_initialized(parent_bucket)) init_bucket(parent_bucket);
				...
				static size_type segment_index_of(size_type index) { return size_type(__TBB_Log2( uintptr_t(index|1) )); }

				//! @return the first array index of given segment
				static size_type segment_base(size_type k) { return (size_type(1) << k & ~size_type(1)); }

				//! @return segment size
				static size_type segment_size(size_type k) { return k ? size_type(1) << k : 2; }
				...
				raw_iterator prepare_bucket(sokey_t hash_key) {
					...
					if(my_buckets[segment] == NULL || my_buckets[segment][index].get_node_ptr() == NULL) init_bucket(bucket);
				...
				void set_bucket(size_type bucket, raw_iterator dummy_head) {
					...
					if(my_buckets[segment] == NULL) {
						...
						if(my_buckets[segment].compare_and_swap(new_segment, NULL) != NULL) my_allocator.deallocate(new_segment, sz);
					}
				...
				bool is_initialized(size_type bucket) const {
					...
					if(my_buckets[segment] == NULL) return false;
				...
				sokey_t split_order_key_regular(sokey_t order_key) const { return __TBB_ReverseBits(order_key) | 0x1; }

				// A dummy order key has its original hash value reversed and the last bit unset
				sokey_t split_order_key_dummy(sokey_t order_key) const { return __TBB_ReverseBits(order_key) & ~sokey_t(0x1); }
