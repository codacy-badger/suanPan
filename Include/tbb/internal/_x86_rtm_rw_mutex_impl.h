...
private:
			...
			private:
				...
				static x86_rtm_rw_mutex* internal_get_mutex(const spin_rw_mutex::scoped_lock& lock) { return static_cast<x86_rtm_rw_mutex*>(lock.mutex); }

				static void internal_set_mutex(spin_rw_mutex::scoped_lock& lock, spin_rw_mutex* mtx) { lock.mutex = mtx; }
...
private:
				...
				public:
					...
					scoped_lock(x86_rtm_rw_mutex& m, bool write = true)
						: my_scoped_lock()
						, transaction_state(RTM_not_in_mutex) { acquire(m, write); }

					//! Release lock (if lock is held).
					~scoped_lock() { if(transaction_state != RTM_not_in_mutex) release(); }
