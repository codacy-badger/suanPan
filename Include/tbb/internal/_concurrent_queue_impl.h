...
namespace tbb {
	...
	namespace strict_ppl {
		...
		namespace internal {
			...
			inline bool is_valid_page(const concurrent_queue_rep_base::page* p) { return uintptr_t(p) > 1; }
...
#pragma warning( disable: 4146 )
			...
			template<typename T> class micro_queue : no_copy {
			public:
			...
			private:
				...
				void copy_item(page& dst, size_t dindex, const void* src, item_constructor_t construct_item) { construct_item(&get_ref(dst, dindex), src); }
			...
			public:
				...
				static T& get_ref(page& p, size_t index) { return (&static_cast<padded_page*>(static_cast<void*>(&p))->last)[index]; }
			...
			template<typename T> void micro_queue<T>::spin_wait_until_my_turn(atomic<ticket>& counter, ticket k, concurrent_queue_rep_base& rb) const {
			...
			template<typename T> void micro_queue<T>::push(const void* item, ticket k, concurrent_queue_base_v3<T>& base,
			                                               item_constructor_t construct_item) {
			...
			template<typename T> bool micro_queue<T>::pop(void* dst, ticket k, concurrent_queue_base_v3<T>& base) {
				k &= -concurrent_queue_rep_base::n_queue;
				...
				bool success = false;
				{
					...
					} else { --base.my_rep->n_invalid_entries; }
				}
				return success;
			}

			template<typename T> micro_queue<T>& micro_queue<T>::assign(const micro_queue<T>& src, concurrent_queue_base_v3<T>& base,
			                                                            item_constructor_t construct_item) {
				...
				if(is_valid_page(srcp)) {
					...
					__TBB_TRY {
						...
						tail_page = cur_page;
					} __TBB_CATCH (...) { invalidate_page_and_rethrow(g_index); }
				} else { head_page = tail_page = NULL; }
				return *this;
			}

			template<typename T> void micro_queue<T>::invalidate_page_and_rethrow(ticket k) {
				...
				page* invalid_page = (page*)uintptr_t(1);
				{
					...
					page* q = tail_page;
					if(is_valid_page(q)) q->next = invalid_page;
					else head_page = invalid_page;
					tail_page = invalid_page;
				}
				__TBB_RETHROW();
			}

			template<typename T> concurrent_queue_rep_base::page* micro_queue<T>::make_copy(concurrent_queue_base_v3<T>& base,
				...
				for(; begin_in_page != end_in_page; ++begin_in_page, ++g_index)
					if(new_page->mask & uintptr_t(1) << begin_in_page) copy_item(*new_page, begin_in_page, *src_page, begin_in_page, construct_item);
				return new_page;
			}

			template<typename T> class micro_queue_pop_finalizer : no_copy {
				typedef concurrent_queue_rep_base::page page;
				ticket my_ticket;
			...
			template<typename T> micro_queue_pop_finalizer<T>::~micro_queue_pop_finalizer() {
				page* p = my_page;
				if(is_valid_page(p)) {
					spin_mutex::scoped_lock lock(my_queue.page_mutex);
					page* q = p->next;
					my_queue.head_page = q;
					if(!is_valid_page(q)) { my_queue.tail_page = NULL; }
				}
				itt_store_word_with_release(my_queue.head_counter, my_ticket);
				if(is_valid_page(p)) { allocator.deallocate_page(p); }
			}
...
#pragma warning( pop )
			 ...
			 */
			template<typename T> struct concurrent_queue_rep : public concurrent_queue_rep_base {
			 ...
			 */
			template<typename T> class concurrent_queue_base_v3 : public concurrent_queue_page_allocator {
			private:
			...
			protected:
				...
				void internal_swap(concurrent_queue_base_v3& src) { std::swap(my_rep, src.my_rep); }
#endif /* __TBB_CPP11_RVALUE_REF_PRESENT */
			};

			template<typename T> concurrent_queue_base_v3<T>::concurrent_queue_base_v3() {
				...
				my_rep->items_per_page = item_size <= 8 ? 32 : item_size <= 16 ? 16 : item_size <= 32 ? 8 : item_size <= 64 ? 4 : item_size <= 128 ? 2 : 1;
			}

			template<typename T> bool concurrent_queue_base_v3<T>::internal_try_pop(void* dst) {
				concurrent_queue_rep<T>& r = *my_rep;
				ticket k;
				do {
					k = r.head_counter;
    ...
    #pragma warning (pop)
			...
			template<typename T> size_t concurrent_queue_base_v3<T>::internal_size() const {
				concurrent_queue_rep<T>& r = *my_rep;
			...
			template<typename T> bool concurrent_queue_base_v3<T>::internal_empty() const {
				concurrent_queue_rep<T>& r = *my_rep;
				ticket tc = r.tail_counter;
				ticket hc = r.head_counter;
			...
			template<typename T> void concurrent_queue_base_v3<T>::internal_finish_clear() {
				concurrent_queue_rep<T>& r = *my_rep;
				size_t nq = r.n_queue;
				for(size_t i = 0; i < nq; ++i) {
			...
			template<typename T> void concurrent_queue_base_v3<T>::assign(const concurrent_queue_base_v3& src,
			                                                              item_constructor_t construct_item) {
				concurrent_queue_rep<T>& r = *my_rep;
				...
				for(size_t i = 0; i < r.n_queue; ++i) r.array[i].assign(src.my_rep->array[i], *this, construct_item);
			...
			template<typename T> class concurrent_queue_iterator_rep : no_assign {
				typedef typename micro_queue<T>::padded_page padded_page;
			public:
				ticket head_counter;
				...
				concurrent_queue_iterator_rep(const concurrent_queue_base_v3<T>& queue)
					...
					, my_queue(queue) { for(size_t k = 0; k < concurrent_queue_rep<T>::n_queue; ++k) array[k] = queue.my_rep->array[k].head_page; }
			...
			template<typename T> bool concurrent_queue_iterator_rep<T>::get_item(T*& item, size_t k) {
			...
			/** @ingroup containers */
			template<typename Value> class concurrent_queue_iterator_base_v3 : no_assign {
				...
				template<typename C, typename T, typename U> friend bool operator==(const concurrent_queue_iterator<C, T>& i, const concurrent_queue_iterator<C, U>& j);
        ...
        __TBB_compiler_fence();
			...
			template<typename Value> concurrent_queue_iterator_base_v3<Value>::concurrent_queue_iterator_base_v3(const concurrent_queue_base_v3<Value>& queue) {
			...
			template<typename Value> void concurrent_queue_iterator_base_v3<Value>::assign(const concurrent_queue_iterator_base_v3<Value>& other) {
			...
			template<typename Value> void concurrent_queue_iterator_base_v3<Value>::advance() {
			...
			template<typename T> struct tbb_remove_cv {
				typedef T type;
			};

			template<typename T> struct tbb_remove_cv<const T> {
				typedef T type;
			};

			template<typename T> struct tbb_remove_cv<volatile T> {
				typedef T type;
			};

			template<typename T> struct tbb_remove_cv<const volatile T> {
				typedef T type;
			};
			...
			/** Value is either the T or const T type of the container.
			    @ingroup containers */
			template<typename Container, typename Value> class concurrent_queue_iterator : public concurrent_queue_iterator_base_v3<typename tbb_remove_cv<Value>::type>,
			                                                                               public std::iterator<std::forward_iterator_tag, Value> {
#if !__TBB_TEMPLATE_FRIENDS_BROKEN
				template<typename T, class A> friend class ::tbb::strict_ppl::concurrent_queue;
#else
public:
#endif
				//! Construct iterator pointing to head of queue.
			...
			template<typename C, typename T, typename U> bool operator==(const concurrent_queue_iterator<C, T>& i, const concurrent_queue_iterator<C, U>& j) { return i.my_item == j.my_item; }
...
public:
#endif
			template<typename T> struct padded_page : page {
				//! Not defined anywhere - exists to quiet warnings.
				padded_page();
				//! Not defined anywhere - exists to quiet warnings.
				void operator=(const padded_page&);
		...
		/** Value is either the T or const T type of the container.
		    @ingroup containers */
		template<typename Container, typename Value> class concurrent_queue_iterator : public concurrent_queue_iterator_base,
		                                                                               public std::iterator<std::forward_iterator_tag, Value> {

#if !__TBB_TEMPLATE_FRIENDS_BROKEN
			template<typename T, class A> friend class ::tbb::concurrent_bounded_queue;
#else
public:
#endif
