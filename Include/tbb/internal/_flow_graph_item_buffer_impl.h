...
namespace internal {
	...
	template<typename T, typename A=cache_aligned_allocator<T>> class item_buffer {
	public:
		typedef T item_type;
	...
	protected:
		...
		void set_my_item(size_t i, const item_type& o) {
			if(item(i).second != no_item) { destroy_item(i); }
		...
		void grow_my_array(size_t minimum_size) {
			...
			while(new_size < minimum_size) new_size *= 2;
			...
			for(size_type i = my_head; i < my_tail; ++i) {
				if(my_item_valid(i)) {
					// sequencer_node may have empty slots
		...
		bool push_back(item_type& v) {
			if(buffer_full()) { grow_my_array(size() + 1); }
		...
		bool pop_back(item_type& v) {
			if(!my_item_valid(my_tail - 1)) { return false; }
			v = this->back();
			destroy_back();
			return true;
		}
		...
		bool pop_front(item_type& v) {
			if(!my_item_valid(my_head)) { return false; }
			v = this->front();
			destroy_front();
			return true;
		}
		...
		void clean_up_buffer(bool reset_pointers) {
			if(my_array) {
				for(size_type i = my_head; i < my_tail; ++i) { if(my_item_valid(i)) destroy_item(i); }
				allocator_type().deallocate(my_array, my_array_size);
			}
			my_array = NULL;
			if(reset_pointers) { my_head = my_tail = my_array_size = 0; }
		}
	...
	public:
		...
		item_buffer()
			...
			, my_array_size(0)
			, my_head(0)
			, my_tail(0) { grow_my_array(initial_buffer_size); }
	...
	template<typename T, typename A=cache_aligned_allocator<T>> class reservable_item_buffer : public item_buffer<T, A> {
	protected:
