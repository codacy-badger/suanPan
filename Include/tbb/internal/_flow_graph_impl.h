...
struct graph_task : public task {
    ...
    node_priority_t priority;
			...
			template<typename GraphContainerType, typename GraphNodeType> class graph_iterator {
				friend class graph;
			...
			public:
				...
				//! Equality
				bool operator==(const graph_iterator& other) const { return ((my_graph == other.my_graph) && (current_node == other.current_node)); }
			...
			enum reset_flags {
				rf_reset_protocol = 0,
				rf_reset_bodies = 1 << 0,
				// delete the current node body, reset to a copy of the initial node body.
...
private:
    graph_task_priority_queue_t& my_priority_queue;
			...
			class graph : tbb::internal::no_copy, public tbb::flow::graph_proxy {
				...
				template<typename Body> class run_task : public graph_task {
				public:
					run_task(Body& body
#if __TBB_PREVIEW_FLOW_GRAPH_PRIORITIES
				...
				template<typename Receiver, typename Body> class run_and_put_task : public graph_task {
				public:
    ...
    void set_name(const char *name);
				...
				// ITERATORS
				template<typename C, typename N> friend class graph_iterator;
    ...
    virtual void extract() = 0;
			...
			namespace internal {
				...
				inline void activate_graph(graph& g) { g.my_is_active = true; }

				inline void deactivate_graph(graph& g) { g.my_is_active = false; }

				inline bool is_graph_active(graph& g) { return g.my_is_active; }

				//! Executes custom functor inside graph arena
				template<typename F> inline void execute_in_graph_arena(graph& g, F& f) {
    ...
    if( t->priority != no_priority ) {
        ...
        g.my_priority_queue.push(t);
				...
				inline void add_task_to_graph_reset_list(graph& g, tbb::task* tp) { g.my_reset_task_list.push_back(tp); }
