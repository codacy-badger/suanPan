...
namespace tbb {
	...
	namespace internal {
		...
		enum exception_id {
			...
			eid_reserved,
			// free slot for backward compatibility, can be reused.
			eid_invalid_swap,
...
namespace tbb {
    ...
    ~tbb_exception() throw() { /* keep the semicolon! */ ; }
		...
		void operator delete(void* p) { internal::deallocate_via_handler_v3(p); }
	};
	...
	public:
		captured_exception(const captured_exception& src)
			: tbb_exception(src)
			, my_dynamic(false) { set(src.my_exception_name, src.my_exception_info); }

		captured_exception(const char* name_, const char* info)
			: my_dynamic(false) { set(name_, info); }
	...
	/** Code using TBB can instantiate this template with an arbitrary ExceptionData type
	    ...
	    \sa tbb::tbb_exception **/
	template<typename ExceptionData> class movable_exception : public tbb_exception {
