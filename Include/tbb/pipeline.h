...
#define __TBB_PIPELINE_VERSION(x) ((unsigned char)(x-2)<<1)
	...
	public:
		...
		bool is_ordered() const { return (my_filter_mode & (filter_is_out_of_order | filter_is_serial)) == filter_is_serial; }

		//! True if filter is thread-bound.
		bool is_bound() const { return (my_filter_mode & filter_is_bound) == filter_is_bound; }

		//! true if an input filter can emit null
		bool object_may_be_null() { return (my_filter_mode & filter_may_emit_null) == filter_may_emit_null; }
	...
	namespace interface6 {
		...
		namespace internal {
			...
			template<typename T> struct tbb_large_object {
				enum { value = sizeof(T) > sizeof(void *) };
			};

			// Obtain type properties in one or another way
#if   __TBB_CPP11_TYPE_PROPERTIES_PRESENT
			template<typename T> struct tbb_trivially_copyable {
...
template<> struct tbb_trivially_copyable <double> { enum { value = !tbb_large_object<double>::value }; };
			...
			template<typename T> struct is_large_object {
			...
			template<typename T> class token_helper<T, true> {
			public:
			...
			template<typename T> class token_helper<T*, false> {
			public:
				typedef T* pointer;
				typedef T* value_type;
			...
			template<typename T, typename U, typename Body> class concrete_filter : public tbb::filter {
				const Body& my_body;
			...
			// input
			template<typename U, typename Body> class concrete_filter<void, U, Body> : public filter {
				const Body& my_body;
				typedef token_helper<U, is_large_object<U>::value> u_helper;
				typedef typename u_helper::pointer u_pointer;
			...
			template<typename T, typename Body> class concrete_filter<T, void, Body> : public filter {
				const Body& my_body;
				typedef token_helper<T, is_large_object<T>::value> t_helper;
				typedef typename t_helper::pointer t_pointer;
			...
			template<typename Body> class concrete_filter<void, void, Body> : public filter {
				const Body& my_body;
			...
			public:
				...
				~pipeline_proxy() {
					while(filter* f = my_pipe.filter_list) delete f; // filter destructor removes it from the pipeline
				}
        ...
        --(__TBB_TEST_FILTER_NODE_COUNT);
			...
			template<typename T, typename U, typename Body> class filter_node_leaf : public filter_node {
		...
		template<typename T, typename U, typename Body> filter_t<T, U> make_filter(tbb::filter::mode mode, const Body& body) { return new internal::filter_node_leaf<T, U, Body>(mode, body); }
		...
		template<typename T, typename U> class filter_t {
			...
			filter_t(filter_node* root_)
				: root(root_) { root->add_ref(); }

			friend class internal::pipeline_proxy;
			template<typename T_, typename U_, typename Body> friend filter_t<T_, U_> make_filter(tbb::filter::mode, const Body&);
			template<typename T_, typename V_, typename U_> friend filter_t<T_, U_> operator&(const filter_t<T_, V_>&, const filter_t<V_, U_>&);
		public:
			...
			filter_t(const filter_t<T, U>& rhs)
				: root(rhs.root) { if(root) root->add_ref(); }

			template<typename Body> filter_t(tbb::filter::mode mode, const Body& body)
				:
				root(new internal::filter_node_leaf<T, U, Body>(mode, body)) { root->add_ref(); }
			...
			~filter_t() { if(root) root->remove_ref(); }
		...
		inline internal::pipeline_proxy::pipeline_proxy(const filter_t<void, void>& filter_chain)
			: my_pipe() {
