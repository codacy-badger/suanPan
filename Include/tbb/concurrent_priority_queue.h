...
namespace tbb {
	namespace interface5 {
		namespace internal {
#if __TBB_CPP11_IS_COPY_CONSTRUCTIBLE_PRESENT
			template<typename T, bool C = std::is_copy_constructible<T>::value> struct use_element_copy_constructor {
				typedef tbb::internal::true_type type;
			};

			template<typename T> struct use_element_copy_constructor<T, false> {
    ...
    struct use_element_copy_constructor {
		...
		template<typename T, typename Compare=std::less<T>, typename A=cache_aligned_allocator<T>> class concurrent_priority_queue {
		public:
			...
			explicit concurrent_priority_queue(const allocator_type& a = allocator_type())
				: mark(0)
				, my_size(0)
				, data(a) { my_aggregator.initialize_handler(my_functor_t(this)); }
			...
			concurrent_priority_queue(concurrent_priority_queue&& src)
				: mark(src.mark)
				, my_size(src.my_size)
				, data(std::move(src.data)) { my_aggregator.initialize_handler(my_functor_t(this)); }
    ...
    // It seems that the reason is absence of support of allocator_traits (stateful allocators).
		...
		private:
			...
			public:
				...
				void operator()(cpq_operation* op_list) { cpq->handle_operations(op_list); }
			};
			...
			void handle_operations(cpq_operation* op_list) {
				...
				while(op_list) {
					...
					if(tmp->type == POP_OP) {
						...
						__TBB_TRY {
							if(tmp->type == PUSH_OP) { push_back_helper(*(tmp->elem), typename internal::use_element_copy_constructor<value_type>::type()); } else { data.push_back(tbb::internal::move(*(tmp->elem))); }
							__TBB_store_with_release(my_size, my_size + 1);
							itt_store_word_with_release(tmp->status, uintptr_t(SUCCEEDED));
						} __TBB_CATCH(...) { itt_store_word_with_release(tmp->status, uintptr_t(FAILED)); }
					}
				}
				...
				while(pop_list) {
					...
					if(data.empty()) { itt_store_word_with_release(tmp->status, uintptr_t(FAILED)); } else {
						__TBB_ASSERT(mark<=data.size(), NULL);
						if(mark < data.size() &&
							compare(data[0], data[data.size() - 1])) {
			...
			void reheap() {
				...
				while(child < mark) {
					...
					if(child + 1 < mark && compare(data[child], data[child + 1])) ++target;
				...
				if(cur_pos != data.size() - 1) data[cur_pos] = tbb::internal::move(data[data.size() - 1]);
				data.pop_back();
				if(mark > data.size()) mark = data.size();
			}

			void push_back_helper(const T& t, tbb::internal::true_type) { data.push_back(t); }
