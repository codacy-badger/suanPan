...
namespace tbb {
	...
	public:
		...
		template<typename finit> explicit combinable(finit _finit)
			: my_ets(_finit) { }
		...
		template<typename combine_func_t> T combine(combine_func_t f_combine) { return my_ets.combine(f_combine); }

		// combine_func_t has signature void(T) or void(const T&)
		template<typename combine_func_t> void combine_each(combine_func_t f_combine) { my_ets.combine_each(f_combine); }
